...
__attribute__ ((visibility("default"))) int zend_parse_arg_long_cap_weak(zval *arg, zend_long *dest)
{
 if (__builtin_expect(!!(zval_get_type(&(*(arg))) == 5), 1)) {
  ...
  if (__builtin_expect(!!(!(!(((*(arg)).value.dval) >= (9223372036854775807L) || ((*(arg)).value.dval) < (-9223372036854775807L -1)))), 0)) {
   *dest = ((*(arg)).value.dval > 0) ? (9223372036854775807L) : (-9223372036854775807L -1);
  } else {
   *dest = zend_dval_to_lval((*(arg)).value.dval);
  }
 } else if (__builtin_expect(!!(zval_get_type(&(*(arg))) == 6), 1)) {
  ...
  if (__builtin_expect(!!((type = is_numeric_str_function((*(arg)).value.str, dest, &d)) != 4), 0)) {
   if (__builtin_expect(!!(type != 0), 1)) {
    ...
    if (__builtin_expect(!!(!(!((d) >= (9223372036854775807L) || (d) < (-9223372036854775807L -1)))), 0)) {
     *dest = (d > 0) ? (9223372036854775807L) : (-9223372036854775807L -1);
    } else {
     *dest = zend_dval_to_lval(d);
    }
   } else {
...
__attribute__ ((visibility("default"))) void zend_merge_properties(zval *obj, HashTable *properties)
{
 ...
 (executor_globals.scope) = ((*(obj)).value.obj->ce);
...
__attribute__ ((visibility("default"))) int zend_update_class_constants(zend_class_entry *class_type)
{
 if (!(class_type->ce_flags & 0x100000)) {
   ...
   *scope = class_type;
   do { Bucket *_p = (&class_type->constants_table)->arData; Bucket *_end = _p + (&class_type->constants_table)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; val = _z; {
    do { if (__builtin_expect(!!((zval_get_type(&(*(val))) == 10)), 0)) { (val) = &(*(val)).value.ref->val; } } while (0);
    if ((((*(val)).u1.v.type_flags & (1<<0)) != 0)) {
     if (__builtin_expect(!!(zval_update_constant_ex(val, 1, class_type) != SUCCESS), 0)) {
      return FAILURE;
     }
    }
   } } } while (0);
   ...
   while (ce) {
    do { Bucket *_p = (&ce->properties_info)->arData; Bucket *_end = _p + (&ce->properties_info)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; prop_info = (*(_z)).value.ptr; {
     if (prop_info->ce == ce) {
      ...
      if ((((*(val)).u1.v.type_flags & (1<<0)) != 0)) {
       *scope = ce;
       if (__builtin_expect(!!(zval_update_constant_ex(val, 1, ((void *)0)) != SUCCESS), 0)) {
        return FAILURE;
       }
      }
     }
 ...
 if (strict_class &&
  ...
  if ((check_flags & (1<<1)) == 0 &&
   ...
   if (fcc->function_handler->op_array.fn_flags & 0x400) {
    if (!zend_check_private(fcc->function_handler, fcc->object ? fcc->object->ce : (executor_globals.scope), lmname)) {
     retval = 0;
     fcc->function_handler = ((void *)0);
