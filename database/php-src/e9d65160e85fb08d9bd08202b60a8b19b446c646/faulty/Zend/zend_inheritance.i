...
# 807 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_inheritance.c"
 ...
 if ((&parent_ce->constants_table)->nNumOfElements) {
  ...
  do { Bucket *_p = (&parent_ce->constants_table)->arData; Bucket *_end = _p + (&parent_ce->constants_table)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; key = _p->key; zv = _z; {
...
__attribute__ ((visibility("default"))) void zend_do_implement_interface(zend_class_entry *ce, zend_class_entry *iface)
{
 ...
 if (ignore) {
  ...
  do { Bucket *_p = (&ce->constants_table)->arData; Bucket *_end = _p + (&ce->constants_table)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; key = _p->key; zv = _z; {
  ...
  do { Bucket *_p = (&iface->constants_table)->arData; Bucket *_end = _p + (&iface->constants_table)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; key = _p->key; zv = _z; {
...
# 1596 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_inheritance.c"
