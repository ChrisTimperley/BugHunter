...
void *zend_shared_alloc(size_t size)
{
 ...
 if (block_size > (smm_shared_globals->shared_free)) {
  do { zend_accel_error(2, "Not enough free shared space to allocate %pd bytes (%pd bytes free)", (zend_long)size, (zend_long)(smm_shared_globals->shared_free)); if (zend_shared_alloc_get_largest_free_block() < 64*1024) { (smm_shared_globals->memory_exhausted) = 1; } } while (0);
  return ((void *)0);
 }
 for (i = 0; i < (smm_shared_globals->shared_segments_count); i++) {
