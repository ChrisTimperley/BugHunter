...
void zend_compile_switch(zend_ast *ast)
{
 ...
 if (cases->children == 1 && cases->child[0]->child[0] == ((void *)0)) {
 ...
 jmpnz_opnums = _safe_emalloc((sizeof(uint32_t)), (cases->children), (0) );
...
void zend_compile_class_decl(zend_ast *ast)
{
 ...
 if (decl->flags & 0x100) {
  ...
  if (!zend_hash_exists((compiler_globals.class_table), lcname)) {
   zend_hash_add_ptr((compiler_globals.class_table), lcname, ce);
  } else {

   zval zv;
   do { (*(&zv)).value.ptr = (ce); (*(&zv)).u1.type_info = 17; } while (0);
