...
static size_t php_stream_memory_write(php_stream *stream, const char *buf, size_t count )
{
 ...
 if (ms->fpos + count > ms->fsize) {
  ...
  if (!ms->data) {
   tmp = _emalloc((ms->fpos + count) );
  } else {
   tmp = _erealloc((ms->data), (ms->fpos + count), 0 );
  }
  if (!tmp) {
...
static int php_stream_memory_seek(php_stream *stream, off_t offset, int whence, off_t *newoffs )
{
 php_stream_memory_data *ms = (php_stream_memory_data*)stream->abstract;
 ((void) (0));
   ...
   switch (value) {
    ...
    case 1:
     ...
     if (newsize <= ms->fsize) {
      ...
      ms->data = _erealloc((ms->data), (newsize), 0 );
...
static size_t php_stream_temp_write(php_stream *stream, const char *buf, size_t count )
{
 ...
 if (((ts->innerstream)->ops == &php_stream_memory_ops)) {
  ...
  if (memsize + count >= ts->smax) {
   php_stream *file = _php_stream_fopen_tmpfile(0 );
   if (file == ((void *)0)) {
...
static int php_stream_temp_seek(php_stream *stream, off_t offset, int whence, off_t *newoffs )
{
 php_stream_temp_data *ts = (php_stream_temp_data*)stream->abstract;
 int ret;
...
static int php_stream_temp_set_option(php_stream *stream, int option, int value, void *ptrparam )
{
 ...
 switch(option) {
  case 11:
   if (ts->meta) {
    zend_hash_copy((*(zval*)ptrparam).value.ht, (*ts->meta).value.ht, (copy_ctor_func_t) zval_add_ref, ((void *)0), sizeof(zval*));
   }
   return 0;
  default:
   if (ts->innerstream) {
...
static php_stream * php_stream_url_wrap_rfc2397(php_stream_wrapper *wrapper, char *path, char *mode, int options, char **opened_path, php_stream_context *context )
{
 php_stream *stream;
 php_stream_temp_data *ts;
 ...
 if (comma != path) {
  ...
  do { (meta) = (zval*)_emalloc((sizeof(zval_gc_info)) ); ((zval_gc_info*)(meta))->u.buffered = ((void *)0); } while (0); (meta)->refcount__gc = 1; (meta)->is_ref__gc = 0;;;
  _array_init((meta), 0 );
  if (!semi) {
   add_assoc_stringl_ex(meta, "mediatype", strlen("mediatype")+1, path, mlen, 1);
   mlen = 0;
  } else if (sep && sep < semi) {
   plen = semi - path;
   add_assoc_stringl_ex(meta, "mediatype", strlen("mediatype")+1, path, plen, 1);
   mlen -= plen;
   path += plen;
 ...
 add_assoc_bool_ex(meta, "base64", strlen("base64")+1, base64);
