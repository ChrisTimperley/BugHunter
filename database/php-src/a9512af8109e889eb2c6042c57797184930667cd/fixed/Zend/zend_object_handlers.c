...
static inline __attribute__((always_inline)) uint32_t zend_get_property_offset(zend_class_entry *ce, zend_string *member, int silent, void **cache_slot)
{
 ...
 if (__builtin_expect(!!((member)->val[0] == '\0' && (member)->len != 0), 0)) {
  if (!silent) {
...
zval *zend_std_read_property(zval *object, zval *member, int type, void **cache_slot, zval *rv)
{
 ...
 if (zobj->ce->__get) {
   ...
   *guard &= ~(1<<0);
   ...
   if (((*(member)).value.str)->val[0] == '\0' && ((*(member)).value.str)->len != 0) {
    zend_throw_error(((void *)0), "Cannot access property started with '\\0'");
    retval = &(executor_globals.uninitialized_zval);
    goto exit;
   }
...
# 1328 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_object_handlers.c"
