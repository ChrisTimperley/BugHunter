...
# 30 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/php_rand.h"
...
# 84 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
# 370 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
# 1029 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
void zif_array_walk(zend_execute_data *execute_data, zval *return_value)
{
 ...
 orig_array_walk_fci = (basic_globals.array_walk_fci);
 orig_array_walk_fci_cache = (basic_globals.array_walk_fci_cache);
# 1529 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
# 1733 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
    return 0;
   }
...
# 1821 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
 ...
 do { Bucket *_p = ((var_array).value.arr)->arData; Bucket *_end = _p + ((var_array).value.arr)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (1 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; num_key = _p->h; var_name = _p->key; entry = _z; {
  ...
  switch (extract_type) {
   ...
   case 0:
    ...
    if (var_exists && (var_name)->len == sizeof("this")-1 && !strcmp((var_name)->val, "this")) {
     zend_class_entry *scope = zend_get_executed_scope();
     if (scope && (scope->name)->len != 0) {
      break;
     }
    }
...
# 2128 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
void zif_range(zend_execute_data *execute_data, zval *return_value)
{
...
void zif_array_splice(zend_execute_data *execute_data, zval *return_value)
{
 ...
 php_splice((*(array)).value.arr, (int)offset, (int)length, repl_array ? (*(repl_array)).value.arr : ((void *)0), rem_hash);
}
  ...
  if (string_key) {
   if ((dest_entry = zend_hash_find(dest, string_key)) != ((void *)0)) {
    ...
    if (zval_get_type(&(*(dest_zval))) == 1) {
     ...
     if (__builtin_expect(!!((*(dest_zval)).value.arr->nNextFreeElement > (*(dest_zval)).value.arr->nNumUsed), 0)) {
...
void zif_array_multisort(zend_execute_data *execute_data, zval *return_value)
{
 ...
 for (i = 0; i < num_arrays; i++) {
  if (((*(arrays[i])).value.arr)->nNumOfElements != array_size) {
 ...
 do { Bucket *_p = ((*(input)).value.arr)->arData; Bucket *_end = _p + ((*(input)).value.arr)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; num_key = _p->h; string_key = _p->key; {
  ...
  if ((double) (randval / (2147483647 + 1.0)) < (double) num_req / (double) num_avail) {
   ...
   if (zval_get_type(&(*(return_value))) != 7) {
    if (string_key) {
     { do { zval *__z = (return_value); zend_string *__s = (string_key); (*(__z)).value.str = __s; if (((__s)->gc.u.v.flags & (1<<1))) { (*(__z)).u1.type_info = 6; } else { (__s)->gc.refcount++; (*(__z)).u1.type_info = (6 | (( (1<<2) | (1<<4)) << 8)); } } while (0); return; };
    } else {
