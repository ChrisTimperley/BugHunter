                                       ...
                                       zend_op_array *op_array)
{
 ...
 if (zval_get_type(&(op_array->literals[constant])) == 6 &&
     op_type == (1<<3)) {
  do { info[constant].flags = (0x2000 | ((1) ? 0x1000 : 0) | ((kind) | ((slots) << 4) | (related))); info[constant].u.num = (op_array->this_var); } while (0);
 } else {
    ...
    case 11:
     if (info[i].flags & 0x1000) {
      if (info[i].flags & 0x2000) {
       int key_len = 20 + sizeof("->") - 1 + ((op_array->literals[i]).value.str)->len;
       key = zend_string_alloc(key_len, 0);
       (key)->len = ap_php_snprintf((key)->val, (key)->len-1, "%d->%s", info[i].u.num, ((op_array->literals[i]).value.str)->val);
...
# 528 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/Optimizer/compact_literals.c"
 }
}
