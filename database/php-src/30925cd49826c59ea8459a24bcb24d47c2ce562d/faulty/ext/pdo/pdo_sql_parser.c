# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/pdo_sql_parser.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 24 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo_driver.h"
...
# 25 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo_driver.h" 2
 ...
 if (stmt->supports_placeholders == PDO_PLACEHOLDER_NONE) {
  ...
  for (plc = placeholders; plc; plc = plc->next) {
   ...
   if (stmt->dbh->methods->quoter) {
    if (param->param_type == PDO_PARAM_LOB && (*param->parameter).type == 7) {
     php_stream *stm;

     (stm) = (php_stream*)zend_fetch_resource((&param->parameter) , -1, "stream", ((void *)0), 2, php_file_le_stream(), php_file_le_pstream());
     if (stm) {
      size_t len;
      char *buf = ((void *)0);

      len = _php_stream_copy_to_mem((stm), (&buf), (((size_t)-1)), (0) );
     ...
     switch ((tmp_param).type) {
      ...
      case 1:
      case 2:
       if ((&tmp_param)->type != 6) { _convert_to_string((&tmp_param) ); };
    ...
    plc->quoted = (*param->parameter).value.str.val;
    plc->qlen = (*param->parameter).value.str.len;
   }
...
rewrite:
  ...
  *outquery_len = newbuffer - *outquery;
  ...
  if (stmt->bound_param_map == ((void *)0)) {
   ...
   _zend_hash_init((stmt->bound_param_map), (13), (((void *)0)), (0) );
  }
  ...
  for (plc = placeholders; plc; plc = plc->next) {
   ...
   if (!skip_map && stmt->named_rewrite_template) {
    ...
    _zend_hash_add_or_update(stmt->bound_param_map, name, plc->len + 1, idxbuf, plc->qlen + 1, ((void *)0), (1<<0) );
   }


   _zend_hash_index_update_or_next_insert(stmt->bound_param_map, plc->bindno, idxbuf, plc->qlen + 1, ((void *)0), (1<<0) );
  ...
  for (plc = placeholders; plc; plc = plc->next) {
   ...
   name = _estrndup((plc->pos), (plc->len) );
   _zend_hash_index_update_or_next_insert(stmt->bound_param_map, plc->bindno, name, plc->len + 1, ((void *)0), (1<<0) );
   _efree((name) );
   plc->quoted = "?";
   plc->qlen = 1;
  }
