...
# 165 "/home/ct584/git/bughunter/repositories/php-src/main/php.h"
...
void zif_parse_url(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (zend_parse_parameters((ht) , "s|l", &str, &str_len, &key) == FAILURE) {
  return;
 }
...
__attribute__ ((visibility("default"))) char *php_url_encode(char const *s, int len, int *new_length)
{
 ...
 while (from < end) {
   ...
   *to++ = '+';
   ...
   to[2] = hexchars[c & 15];
   to += 3;
# 519 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/url.c"
  } else {
   *to++ = c;
  }
 }
...
__attribute__ ((visibility("default"))) char *php_raw_url_encode(char const *s, int len, int *new_length)
{
 ...
 if (__builtin_expect(y > 2147483647, 0)) {
  _efree((str) );
...
void zif_get_headers(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (zend_parse_parameters((ht) , "s|l", &url, &url_len, &format) == FAILURE) {
  return;
 }
 ...
 if (zend_hash_find(((*stream->wrapperdata).type==4 ? (*stream->wrapperdata).value.ht : (((*stream->wrapperdata).type==5 ? (*stream->wrapperdata).value.obj.handlers->get_properties((stream->wrapperdata) ) : ((void *)0)))), "headers", sizeof("headers"), (void **)&h) != FAILURE && (**h).type == 4) {
...
no_name_header:
    ...
    *p = '\0';
    ...
    if (zend_hash_find(((*return_value).type==4 ? (*return_value).value.ht : (((*return_value).type==5 ? (*return_value).value.obj.handlers->get_properties((return_value) ) : ((void *)0)))), (**hdr).value.str.val, (p - (**hdr).value.str.val + 1), (void **) &prev_val) == FAILURE) {
