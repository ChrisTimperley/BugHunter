  ...
  case 6:
   if (((*(data)).value.str)->len) {
    ...
    if (numbytes != ((*(data)).value.str)->len) {
     php_error_docref0(((void *)0), (1<<1L), "Only %pl of %zd bytes written, possibly out of free disk space", numbytes, ((*(data)).value.str)->len);
     numbytes = -1;
    }
   }
   break;
   ...
   if ((*(data)).value.obj->handlers != ((void *)0)) {
    ...
    if (zend_std_cast_object_tostring(data, &out, 6) == SUCCESS) {
     ...
     if (numbytes != ((out).value.str)->len) {
      php_error_docref0(((void *)0), (1<<1L), "Only %pd of %zd bytes written, possibly out of free disk space", numbytes, ((out).value.str)->len);
      numbytes = -1;
     }
     _zval_dtor((&out) );
     break;
