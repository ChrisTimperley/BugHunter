...
void zif_spl_autoload_call(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if ((spl_globals.autoload_functions)) {
  ...
  do { Bucket *_p = ((spl_globals.autoload_functions))->arData; Bucket *_end = _p + ((spl_globals.autoload_functions))->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; func_name = _p->key; alfi = (*(_z)).value.ptr; {
...
# 455 "/home/ct584/git/bughunter/repositories/php-src/ext/spl/php_spl.c"
