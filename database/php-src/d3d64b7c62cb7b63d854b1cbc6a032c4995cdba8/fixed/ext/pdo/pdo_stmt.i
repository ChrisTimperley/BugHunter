...
# 30 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo.h"
extern zend_module_entry pdo_module_entry;
# 51 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo.h"
...
# 49 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo_driver.h"
enum pdo_param_type {
 PDO_PARAM_NULL,
...
# 110 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo_driver.h"
...
# 170 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo_driver.h"
...
static inline long pdo_attr_lval(zval *options, enum pdo_attribute_type option_name, long defval )
{
 ...
 if (options && SUCCESS == zend_hash_index_find((*options).value.ht, option_name, (void**)&v)) {
  if ((**v).type != 1) {
   zval tmp = **v;
   _zval_copy_ctor((&tmp) );
   convert_to_long(&tmp);
   return (tmp).value.lval;
  }
  return (**v).value.lval;
 }
 return defval;
...
# 245 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo_driver.h"
...
# 401 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/php_pdo_driver.h"
...
static inline int rewrite_name_to_position(pdo_stmt_t *stmt, struct pdo_bound_param_data *param )
{
 if (stmt->bound_param_map) {
  ...
  while (SUCCESS == zend_hash_get_current_data_ex(stmt->bound_param_map, (void**)&name, ((void *)0))) {
...
iterate:
 if (ht) {
  ...
  while (SUCCESS == zend_hash_get_current_data_ex(ht, (void**)&param, ((void *)0))) {
...
# 249 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/pdo_stmt.c"
...
static void zim_PDOStatement_execute(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "|a!", &input_params)) {
 ...
 if (input_params) {
  ...
  while (SUCCESS == zend_hash_get_current_data_ex((*input_params).value.ht, (void*)&tmp, ((void *)0))) {
 ...
 switch (type) {
  case PDO_PARAM_ZVAL:
   if (value && value_len == sizeof(zval)) {
    ...
    zval *zv = *(zval**)value;
    do { zval *__z = (dest); zval *__zv = (zv); do { (__z)->value = (__zv)->value; (*__z).type = (*__zv).type; } while (0); if (need_copy) { _zval_copy_ctor((__z) ); } if (1) { if (!need_copy) { { (*__zv).type = 0; }; } _zval_ptr_dtor((&__zv) ); } } while (0);
   } else {
    { (*dest).type = 0; };
   }
  ...
  case PDO_PARAM_LOB:
   if (value == ((void *)0)) {
    ...
    if (stmt->dbh->stringify || new_type == PDO_PARAM_STR) {
     ...
     if(buf == ((void *)0)) {
      do { zval *__z = (dest); (*__z).value.str.len = 0; (*__z).value.str.val = (compiler_globals.interned_empty_string)? (compiler_globals.interned_empty_string) : _estrndup((""), (sizeof("")-1) ); (*__z).type = 6; } while (0);
     } else {
 ...
 long offset, int do_bind )
{
 ...
 if (do_bind && stmt->bound_columns) {
  ...
  while (SUCCESS == zend_hash_get_current_data_ex(stmt->bound_columns, (void**)&param, ((void *)0))) {
   if (param->paramno >= 0) {
    if ((param->parameter)->type != 6) { _convert_to_string((param->parameter) ); };
  ...
  switch (how) {
   ...
   case PDO_FETCH_NAMED:
    if (!return_all) {
     ...
     _zend_hash_init((return_value->value.ht), (stmt->column_count), ((void (*)(void *)) _zval_ptr_dtor), (0) );
     (*return_value).type = 4;
    } else {
   ...
   case PDO_FETCH_CLASS:
    if (flags & 0x00040000) {
     ...
     if ((val).type != 0) {
      ...
      if (zend_lookup_class((val).value.str.val, (val).value.str.len, &cep ) == FAILURE) {
       stmt->fetch.cls.ce = zend_standard_class_def;
      } else {
    ...
    case PDO_FETCH_NAMED:

     {
      ...
      if (zend_hash_find((*return_value).value.ht, stmt->columns[i].name,
       ...
       zval *arr;
       if ((**curr_val).type != 4) {
# 1083 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/pdo_stmt.c"
        zval *cur;
...
# 1132 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/pdo_stmt.c"
      if (!ce->unserialize) {
       _zval_ptr_dtor((&val) );
       pdo_raise_impl_error(stmt->dbh, stmt, "HY000", "cannot unserialize class" );
       return 0;
      } else if (ce->unserialize(&return_value, ce, (unsigned char *)((*val).type == 6 ? (*val).value.str.val : ""), (*val).type == 6 ? (*val).value.str.len : 0, ((void *)0) ) == FAILURE) {
       _zval_ptr_dtor((&val) );
       pdo_raise_impl_error(stmt->dbh, stmt, "HY000", "cannot unserialize class" );
  ...
  if (return_all) {
   if ((flags & 0x00030000) == 0x00030000) {
    ...
    if (zend_symtable_find((*return_all).value.ht, (grp_val).value.str.val, (grp_val).value.str.len+1, (void**)&pgrp) == FAILURE) {
...
static void zim_PDOStatement_fetch(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "|lll", &how,
   &ori, &off)) {
  { do { zval *__z = (return_value); (*__z).value.lval = ((0) != 0); (*__z).type = 3; } while (0); return; };
 }
...
static void zim_PDOStatement_fetchObject(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "|s!a", &class_name, &class_name_len, &ctor_args)) {
  { do { zval *__z = (return_value); (*__z).value.lval = ((0) != 0); (*__z).type = 3; } while (0); return; };
 }
...
static void zim_PDOStatement_fetchColumn(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "|l", &col_n)) {
  { do { zval *__z = (return_value); (*__z).value.lval = ((0) != 0); (*__z).type = 3; } while (0); return; };
 }
...
static void zim_PDOStatement_fetchAll(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "|lzz", &how, &arg2, &ctor_args)) {
  { do { zval *__z = (return_value); (*__z).value.lval = ((0) != 0); (*__z).type = 3; } while (0); return; };
 }
...
static int register_bound_param(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used , pdo_stmt_t *stmt, int is_param)
{
 ...
 if (FAILURE == zend_parse_parameters_ex(1<<1, (ht) ,
   "lz|llz!", &param.paramno, &param.parameter, &param_type, &param.max_value_len,
   &param.driver_params)) {
  if (FAILURE == zend_parse_parameters((ht) , "sz|llz!", &param.name,
...
static void zim_PDOStatement_bindValue(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters_ex(1<<1, (ht) ,
  ...
  if (FAILURE == zend_parse_parameters((ht) , "sz/|l", &param.name,
...
static void zim_PDOStatement_setAttribute(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "lz!", &attr, &value)) {
  { do { zval *__z = (return_value); (*__z).value.lval = ((0) != 0); (*__z).type = 3; } while (0); return; };
 }
...
static void zim_PDOStatement_getAttribute(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "l", &attr)) {
  { do { zval *__z = (return_value); (*__z).value.lval = ((0) != 0); (*__z).type = 3; } while (0); return; };
 }
...
static void zim_PDOStatement_getColumnMeta(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (FAILURE == zend_parse_parameters((ht) , "l", &colno)) {
  { do { zval *__z = (return_value); (*__z).value.lval = ((0) != 0); (*__z).type = 3; } while (0); return; };
 }
 if(colno < 0) {
...
# 2024 "/home/ct584/git/bughunter/repositories/php-src/ext/pdo/pdo_stmt.c"
 strcpy(stmt->error_code, "00000");
...
static void zim_PDOStatement_setFetchMode(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 do { zval *__z = (return_value); (*__z).value.lval = ((pdo_stmt_setup_fetch_mode(ht, return_value, return_value_ptr, this_ptr, return_value_used , stmt, 0) == SUCCESS ? 1 : 0) != 0); (*__z).type = 3; } while (0)
...
static union _zend_function *dbstmt_method_get(zval **object_pp, char *method_name, int method_len, const zend_literal *key )
{
...
static void pdo_stmt_iter_get_key(zend_object_iterator *iter, zval *key )
{
 ...
 if (I->key == (ulong)-1) {
  ...
  { zval *__z = (key); (*__z).value.lval = I->key; (*__z).type = 1; };
 }
}
 ...
 if (stmt) {
  if ((*member).type == 1) {
   ...
   for (colno = 0; colno < stmt->column_count; colno++) {
    if (strcmp(stmt->columns[colno].name, (*member).value.str.val) == 0) {
     fetch_value(stmt, return_value, colno, ((void *)0) );
...
static int row_call_method(const char *method, int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 return FAILURE;
}
...
static int row_get_classname(const zval *object, const char **class_name, zend_uint *class_name_len, int parent )
{
 if (parent) {
  return FAILURE;
 } else {
