   ...
   if (zend_hash_exists(files, ce->info.user.filename)) {
    do { Bucket *_p = (&ce->function_table)->arData; Bucket *_end = _p + (&ce->function_table)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; func = (*(_z)).value.ptr; {
     if (func->type == 2) {
      ...
      if (by_function) {
       zend_string *fn_name = strpprintf((name)->len + (func->op_array.function_name)->len + 2, "%.*s::%.*s", (name)->len, (name)->val, (func->op_array.function_name)->len, (func->op_array.function_name)->val);
   ...
   if (by_function) {
    if (cur->function_name == ((void *)0)) {
     ...
     if (last_scope == ((void *)0)) {
      ...
      fn_name = strpprintf((last_function)->len + (last_scope->name)->len + 2, "%.*s::%.*s", (last_scope->name)->len, (last_scope->name)->val, (last_function)->len, (last_function)->val);
     }
...
static inline size_t php_sapi_phpdbg_ub_write(const char *message, size_t length)
{
 ...
 return phpdbg_print(P_STDOUT , (phpdbg_globals.io)[1].fd, ((void *)0), ((void *)0), "%.*s", length, message);
}
