...
__attribute__ ((visibility("default"))) php_url *php_url_parse_ex(char const *str, size_t length)
{
 ...
 if ((p = zend_memrchr(s, '@', (e-s)))) {
  ...
  pp = s;
  while (pp < p) {
   if (!((*__ctype_b_loc ())[(int) ((*pp))] & (unsigned short int) _ISalnum) && *pp != ':' && *pp != ';' && *pp != '=' && !(*pp >= '!' && *pp <= ',')) {
    if (ret->scheme) {
     _efree((ret->scheme) );
    }
    _efree((ret) );
...
# 496 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/url.c"
...
__attribute__ ((visibility("default"))) zend_string *php_url_encode(char const *s, size_t len)
{
 ...
 while (from < end) {
   ...
   *to++ = '+';
   ...
   to[2] = hexchars[c & 15];
   to += 3;
# 534 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/url.c"
  } else {
   *to++ = c;
  }
 }
