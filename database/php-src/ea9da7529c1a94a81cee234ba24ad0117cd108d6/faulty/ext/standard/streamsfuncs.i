...
static int stream_array_to_fd_set(zval *stream_array, fd_set *fds, php_socket_t *max_fd)
{
 ...
 do { Bucket *_p = ((*(stream_array)).value.arr)->arData; Bucket *_end = _p + ((*(stream_array)).value.arr)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; elem = _z; {
  ...
  do { if (__builtin_expect(!!((zval_get_type(&(*(elem))) == 10)), 0)) { (elem) = &(*(elem)).value.ref->val; } } while (0);
...
# 1393 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/streamsfuncs.c"
 t.tv_sec = seconds;
...
# 1528 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/streamsfuncs.c"
 do { if (((stream) = (php_stream*)zend_fetch_resource2_ex((zstream), "stream", php_file_le_stream(), php_file_le_pstream())) == ((void *)0)) { { do { (*(return_value)).u1.type_info = 2; } while (0); return; }; } } while (0);
...
# 1645 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/streamsfuncs.c"
void zif_stream_socket_shutdown(zend_execute_data *execute_data, zval *return_value)
{
 zend_long how;
 zval *zstream;
