...
void zim_reflection_property_getValue(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if ((ref->prop.flags & 0x01)) {
  ...
  do { zval *_z1 = (return_value); const zval *_z2 = (member_p); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & (((1<<2)|(1<<1)) << 8)) != 0) { if ((_t & (((1<<4)|(1<<1)) << 8)) != 0) { _zval_copy_ctor_func(_z1 ); } else { (_gc)->gc.refcount++; } } } while (0);
 }
}
...
void zm_info_reflection(zend_module_entry *zend_module)
{
 ...
 php_info_print_table_row(2, "Version", "$Id: 50d0ba002342d41efa753927261e5e09f295ecf9 $");
