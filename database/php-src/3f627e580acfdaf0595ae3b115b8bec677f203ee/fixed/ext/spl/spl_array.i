...
# 65 "/home/ct584/git/bughunter/repositories/php-src/ext/spl/spl_array.c"
...
static zval **spl_array_get_dimension_ptr_ptr(int check_inherited, zval *object, zval *offset, int type )
{
 ...
 case 6:
  if (zend_symtable_find(ht, (*offset).value.str.val, (*offset).value.str.len+1, (void **) &retval) == FAILURE) {
   switch (type) {
    case 0:
...
static void spl_array_unset_dimension_ex(int check_inherited, zval *object, zval *offset )
{
 ...
 case 6:
  ...
  if (ht == &(executor_globals.symbol_table)) {
   ...
   if (zend_symtable_del(ht, (*offset).value.str.val, (*offset).value.str.len+1) == FAILURE) {
    zend_error((1<<3L),"Undefined index: %s", (*offset).value.str.val);
   } else {
 ...
 case 1:
  ...
  if (zend_hash_del_key_or_index(ht, ((void *)0), 0, index, 1) == FAILURE) {
...
static int spl_array_has_dimension_ex(int check_inherited, zval *object, zval *offset, int check_empty )
{
 ...
 switch((*offset).type) {
  case 6:
   {
    HashTable *ht = spl_array_get_hash_table(intern, 0 );
    if (zend_symtable_find(ht, (*offset).value.str.val, (*offset).value.str.len+1, (void **) &tmp) != FAILURE) {
     switch (check_empty) {
      case 0:
       return (**tmp).type != 0;
      case 2:
...
static inline int spl_array_object_verify_pos_ex(spl_array_object *object, HashTable *ht, const char *msg_prefix )
{
 ...
 if (object->pos && (object->ar_flags & 0x01000000) && spl_hash_verify_pos_ex(object, ht ) == FAILURE) {
...
{
 zval *index;
 if (zend_parse_parameters((ht) , "z", &index) == FAILURE) {
  return;
 }
...
{
 zval *index, *value;
 if (zend_parse_parameters((ht) , "zz", &index, &value) == FAILURE) {
  return;
 }
...
static HashTable *spl_array_get_gc(zval *object, zval ***gc_data, int *gc_data_count )
{
 spl_array_object *intern = (spl_array_object*)zend_object_store_get_object(object );
...
static zval **spl_array_get_property_ptr_ptr(zval *object, zval *member, int type, const zend_literal *key )
{
 spl_array_object *intern = (spl_array_object*)zend_object_store_get_object(object );

 if ((intern->ar_flags & 0x00000002) != 0
 && !std_object_handlers.has_property(object, member, 2, key )) {
  return spl_array_get_dimension_ptr_ptr(1, object, member, type );
 }
...
void zim_spl_Array___construct(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (zend_parse_parameters((ht) , "Z|lC", &array, &ar_flags, &ce_get_iterator) == FAILURE) {
...
void zim_spl_Array_setIteratorClass(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (zend_parse_parameters((ht) , "C", &ce_get_iterator) == FAILURE) {
  return;
 }
...
void zim_spl_Array_setFlags(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (zend_parse_parameters((ht) , "l", &ar_flags) == FAILURE) {
  return;
 }
...
void zim_spl_Array_seek(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (zend_parse_parameters((ht) , "l", &position) == FAILURE) {
  return;
 }
...
int static spl_array_object_count_elements_helper(spl_array_object *intern, long *count )
{
  ...
  *count = 0;
  spl_array_rewind(intern );
  while(intern->pos && spl_array_next(intern ) == SUCCESS) {
   (*count)++;
  }
...
static void spl_array_method(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used , char *fname, int fname_len, int use_arg)
{
 ...
 if (!use_arg) {
  ...
  if (zend_parse_parameters_ex(1<<1, (ht) , "|z", &arg) == FAILURE) {
   (*tmp).type = 0;
   _zval_ptr_dtor((&tmp) );
  ...
  if ((ht) != 1 || zend_parse_parameters_ex(1<<1, (ht) , "z", &arg) == FAILURE) {
   (*tmp).type = 0;
   _zval_ptr_dtor((&tmp) );
...
# 1502 "/home/ct584/git/bughunter/repositories/php-src/ext/spl/spl_array.c"
...
void zim_spl_Array_valid(int ht, zval *return_value, zval **return_value_ptr, zval *this_ptr, int return_value_used )
{
 ...
 if (spl_array_object_verify_pos(intern, aht ) == FAILURE) {
  ...
  { do { zval *__z = (return_value); (*__z).value.lval = (((zend_hash_get_current_key_type_ex(aht, &intern->pos) == 3 ? FAILURE : SUCCESS) == SUCCESS) != 0); (*__z).type = 3; } while (0); return; };
 }
}
