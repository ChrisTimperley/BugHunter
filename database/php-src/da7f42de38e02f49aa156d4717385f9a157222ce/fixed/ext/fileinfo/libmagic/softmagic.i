 ...
 case 17: {
  ...
  if ((pce = pcre_get_compiled_regex_cache((pattern).value.str)) == ((void *)0)) {
   ...
   if ((retval).value.lval < 0) {
    ...
    if ((pzval = zend_hash_index_find(ht, 0)) != ((void *)0)) {




     zval *match, *offset;
     if ((match = zend_hash_index_find((*(pzval)).value.arr, 0)) &&
       (offset = zend_hash_index_find((*(pzval)).value.arr, 1))) {
      if (zval_get_type(&(*(match))) != 6 && zval_get_type(&(*(offset))) != 4) {
       goto error_out;
      }
      ms->search.s += (*(offset)).value.lval;
      ms->search.offset += (*(offset)).value.lval;
