...
# 141 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/zend_file_cache.c"
...
# 420 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/zend_file_cache.c"
...
# 698 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/zend_file_cache.c"
 return filename;
}
...
# 792 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/zend_file_cache.c"
 zend_string_release((zend_string*)(accel_globals.mem));
 _efree((mem) );
 if (flock(fd, 8) != 0) {
                                             ...
                                             dtor_func_t dtor)
{
 ...
 while (p < end) {
  if (zval_get_type(&(p->val)) != 0) {
   do { if (p->key) { if (((size_t)(p->key) & 1UL)) { (p->key) = (void*)zend_file_cache_unserialize_interned((zend_string*)(p->key), !script->corrupted); } else { do { if (__builtin_expect(!(((char*)(p->key) < (char*)script->size)), 0)) __builtin_unreachable(); } while (0); (p->key) = (void*)((char*)buf + (size_t)(p->key)); (p->key)->gc.u.v.flags |= (1<<1); (p->key)->gc.u.v.flags &= ~(1<<2); } } } while (0);
                                                 ...
                                                 void *buf)
{
 ...
 if (!(((char*)(op_array->opcodes) >= (char*)script->mem && (char*)(op_array->opcodes) < (char*)script->mem + script->size) || ((char*)(op_array->opcodes) >= (accel_shared_globals->interned_strings_start) && (char*)(op_array->opcodes) < (accel_shared_globals->interned_strings_end)))) {
  ...
  while (opline < end) {
# 972 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/zend_file_cache.c"
  ...
  if (op_array->arg_info) {
   ...
   while (p < end) {
    if (!(((char*)(p->name) >= (char*)script->mem && (char*)(p->name) < (char*)script->mem + script->size) || ((char*)(p->name) >= (accel_shared_globals->interned_strings_start) && (char*)(p->name) < (accel_shared_globals->interned_strings_end)))) {
     do { if (p->name) { if (((size_t)(p->name) & 1UL)) { (p->name) = (void*)zend_file_cache_unserialize_interned((zend_string*)(p->name), !script->corrupted); } else { do { if (__builtin_expect(!(((char*)(p->name) < (char*)script->size)), 0)) __builtin_unreachable(); } while (0); (p->name) = (void*)((char*)buf + (size_t)(p->name)); (p->name)->gc.u.v.flags |= (1<<1); (p->name)->gc.u.v.flags &= ~(1<<2); } } } while (0);
    }
  ...
  if (op_array->vars) {
   ...
   while (p < end) {
    if (!(((char*)(*p) >= (char*)script->mem && (char*)(*p) < (char*)script->mem + script->size) || ((char*)(*p) >= (accel_shared_globals->interned_strings_start) && (char*)(*p) < (accel_shared_globals->interned_strings_end)))) {
     do { if (*p) { if (((size_t)(*p) & 1UL)) { (*p) = (void*)zend_file_cache_unserialize_interned((zend_string*)(*p), !script->corrupted); } else { do { if (__builtin_expect(!(((char*)(*p) < (char*)script->size)), 0)) __builtin_unreachable(); } while (0); (*p) = (void*)((char*)buf + (size_t)(*p)); (*p)->gc.u.v.flags |= (1<<1); (*p)->gc.u.v.flags &= ~(1<<2); } } } while (0);
    }
    p++;
   }
  }

  do { if (op_array->function_name) { if (((size_t)(op_array->function_name) & 1UL)) { (op_array->function_name) = (void*)zend_file_cache_unserialize_interned((zend_string*)(op_array->function_name), !script->corrupted); } else { do { if (__builtin_expect(!(((char*)(op_array->function_name) < (char*)script->size)), 0)) __builtin_unreachable(); } while (0); (op_array->function_name) = (void*)((char*)buf + (size_t)(op_array->function_name)); (op_array->function_name)->gc.u.v.flags |= (1<<1); (op_array->function_name)->gc.u.v.flags &= ~(1<<2); } } } while (0);
  do { if (op_array->filename) { if (((size_t)(op_array->filename) & 1UL)) { (op_array->filename) = (void*)zend_file_cache_unserialize_interned((zend_string*)(op_array->filename), !script->corrupted); } else { do { if (__builtin_expect(!(((char*)(op_array->filename) < (char*)script->size)), 0)) __builtin_unreachable(); } while (0); (op_array->filename) = (void*)((char*)buf + (size_t)(op_array->filename)); (op_array->filename)->gc.u.v.flags |= (1<<1); (op_array->filename)->gc.u.v.flags &= ~(1<<2); } } } while (0);
 ...
 if (ce->trait_precedences) {
  ...
  while (*p) {
   ...
   if (q->exclude_from_classes) {
    ...
    while (*s) {
     do { if (*s) { if (((size_t)(*s) & 1UL)) { (*s) = (void*)zend_file_cache_unserialize_interned((zend_string*)(*s), !script->corrupted); } else { do { if (__builtin_expect(!(((char*)(*s) < (char*)script->size)), 0)) __builtin_unreachable(); } while (0); (*s) = (void*)((char*)buf + (size_t)(*s)); (*s)->gc.u.v.flags |= (1<<1); (*s)->gc.u.v.flags &= ~(1<<2); } } } while (0);
     s++;
    }
   }
   p++;
