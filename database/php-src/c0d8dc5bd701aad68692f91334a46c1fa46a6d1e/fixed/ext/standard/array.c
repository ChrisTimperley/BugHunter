...
static inline zval *array_column_fetch_prop(zval *data, zval *name, zval *rv)
{
 ...
 if (zval_get_type(&(*(data))) == 8) {
  zend_string *key = _zval_get_string((name));




  if (!((*(data))).value.obj->handlers->has_property
    || ((*(data))).value.obj->handlers->has_property(data, name, 2, ((void *)0))
    || ((*(data))).value.obj->handlers->has_property(data, name, 0, ((void *)0))) {
   prop = zend_read_property(((*(data)).value.obj->ce), data, (key)->val, (key)->len, 1, rv);
  }

  zend_string_release(key);
 } else if (zval_get_type(&(*(data))) == 7) {
  if (zval_get_type(&(*(name))) == 6) {
   prop = zend_hash_find((*(data)).value.arr, (*(name)).value.str);
