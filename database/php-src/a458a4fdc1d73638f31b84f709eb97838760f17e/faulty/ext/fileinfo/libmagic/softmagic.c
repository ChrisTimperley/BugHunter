 ...
 case 17: {
  ...
  if ((pce = pcre_get_compiled_regex_cache((*pattern).value.str.val, (*pattern).value.str.len )) == ((void *)0)) {
   ...
   if ((*retval).value.lval < 0) {
    ...
    if ((zend_hash_get_current_key_type_ex(ht, &outer_pos) == 3 ? FAILURE : SUCCESS) == SUCCESS &&
     ...
     if (zend_hash_get_current_data_ex(ht, (void**)&ppzval, &outer_pos) != FAILURE) {
      ...
      if ((zend_hash_get_current_key_type_ex(inner_ht, &inner_pos) == 3 ? FAILURE : SUCCESS) == SUCCESS &&
