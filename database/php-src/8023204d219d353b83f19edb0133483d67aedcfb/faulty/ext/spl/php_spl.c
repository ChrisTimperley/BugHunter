...
void zif_spl_autoload_call(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if ((spl_globals.autoload_functions)) {
  ...
  while (zend_hash_get_current_key_ex((spl_globals.autoload_functions), &func_name, &num_idx, &pos) == 1) {
   alfi = zend_hash_get_current_data_ptr_ex((spl_globals.autoload_functions), &pos);
...
# 460 "/home/ct584/git/bughunter/repositories/php-src/ext/spl/php_spl.c"
...
void zif_spl_autoload_unregister(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if ((spl_globals.autoload_functions)) {
  if ((lc_name)->len == sizeof("spl_autoload_call") - 1 && !strcmp((lc_name)->val, "spl_autoload_call")) {
   ...
   if (!(spl_globals.autoload_running)) {
    ...
    zend_hash_clean((spl_globals.autoload_functions));
   }
   success = SUCCESS;
  } else {
...
__attribute__ ((visibility("default"))) zend_string *php_spl_object_hash(zval *obj)
{
 ...
 hash_handlers = (spl_globals.hash_mask_handlers);
