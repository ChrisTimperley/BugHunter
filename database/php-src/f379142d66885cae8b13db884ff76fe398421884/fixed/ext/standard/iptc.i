# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/iptc.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/iptc.c"
# 35 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/iptc.c"
...
# 36 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/iptc.c" 2
...
# 37 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/iptc.c" 2
...
# 38 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/iptc.c" 2
# 78 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/iptc.c"
static int php_iptc_put1(FILE *fp, int spool, unsigned char c, unsigned char **spoolbuf)
{
 if (spool > 0)
  (php_output_write((const char *) &(c), 1), (c));
...
static int php_iptc_get1(FILE *fp, int spool, unsigned char **spoolbuf)
{
 ...
 if (spool > 0) {
  cc = c;
  (php_output_write((const char *) &(cc), 1), (cc));
 }
...
void zif_iptcembed(zend_execute_data *execute_data, zval *return_value)
{
 ...
 int inx;
 zend_string *spoolbuf = ((void *)0);
 unsigned char *poi = ((void *)0);
 zend_stat_t sb;
 zend_bool written = 0;

 if (zend_parse_parameters((execute_data)->This.u2.num_args, "sp|l", &iptcdata, &iptcdata_len, &jpeg_file, &jpeg_file_len, &spool) != SUCCESS) {
  return;
 }
 ...
 if (iptcdata_len >= (18446744073709551615UL) - sizeof(psheader) - 1025) {
  php_error_docref0(((void *)0) , (1<<1L), "IPTC data too large");
  { do { (*(return_value)).u1.type_info = 2; } while (0); return; };
 }
 ...
 if (spool < 2) {
  ...
  poi = (unsigned char*)(spoolbuf)->val;
 ...
 if (spool < 2) {
  spoolbuf = zend_string_truncate(spoolbuf, poi - (unsigned char*)(spoolbuf)->val, 0);
  { do { zval *__z = (return_value); zend_string *__s = (spoolbuf); (*(__z)).value.str = __s; (*(__z)).u1.type_info = (6 | (( (1<<2) | (1<<4)) << 8)); } while (0); return; };
 } else {
...
void zif_iptcparse(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if (zend_parse_parameters((execute_data)->This.u2.num_args, "s", &str, &str_len) != SUCCESS) {
  return;
 }
 ...
 while (inx < str_len) {
  ...
  if ((element = zend_hash_str_find((*(return_value)).value.arr, key, strlen(key))) == ((void *)0)) {
   _array_init((&values), 0 );

   element = _zend_hash_str_update((*(return_value)).value.arr, key, strlen(key), &values );
  }
