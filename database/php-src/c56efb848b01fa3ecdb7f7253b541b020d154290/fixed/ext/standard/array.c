...
# 88 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
# 374 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
# 1037 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
void zif_array_walk(zend_execute_data *execute_data, zval *return_value)
{
 ...
 orig_array_walk_fci = (basic_globals.array_walk_fci);
 orig_array_walk_fci_cache = (basic_globals.array_walk_fci_cache);
# 1544 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
# 1749 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
    return 0;
   }
...
# 2076 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
void zif_range(zend_execute_data *execute_data, zval *return_value)
{
...
double_str:
  ...
  if (low > high) {
   ...
   do { double __calc_size = ((low - high) / step) + 1; if (fabs(__calc_size) >= (double)0x80000000) { php_error_docref0(((void *)0), (1<<1L), "The supplied range exceeds the maximum array size: start=%0.0f end=%0.0f", low > high ? high : low, low > high ? low : high); { do { (*(return_value)).u1.type_info = 2; } while (0); return; }; } _array_init((return_value), ((uint32_t)fabs(__calc_size)) ); zend_hash_real_init((*(return_value)).value.arr, 1); } while (0);
   do { HashTable *__fill_ht = ((*(return_value)).value.arr); Bucket *__fill_bkt = __fill_ht->arData + __fill_ht->nNumUsed; uint32_t __fill_idx = __fill_ht->nNumUsed; do { if (__builtin_expect(!(__fill_ht->u.flags & (1<<2)), 0)) __builtin_unreachable(); } while (0); {
   for (value = low; value >= (high - 0.000000000000001); value = low - (++i * step)) {
    (tmp).value.dval = value;
    do { do { zval *_z1 = (&__fill_bkt->val); const zval *_z2 = (&tmp); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); } while (0); __fill_bkt->h = (__fill_idx); __fill_bkt->key = ((void *)0); __fill_bkt++; __fill_idx++; } while (0);
   }
   } __fill_ht->nNumUsed = __fill_idx; __fill_ht->nNumOfElements = __fill_idx; __fill_ht->nNextFreeElement = __fill_idx; __fill_ht->nInternalPointer = __fill_idx ? 0 : ((uint32_t) -1); } while (0);
  } else if (high > low) {
   ...
   do { double __calc_size = ((high - low) / step) + 1; if (fabs(__calc_size) >= (double)0x80000000) { php_error_docref0(((void *)0), (1<<1L), "The supplied range exceeds the maximum array size: start=%0.0f end=%0.0f", high > low ? low : high, high > low ? high : low); { do { (*(return_value)).u1.type_info = 2; } while (0); return; }; } _array_init((return_value), ((uint32_t)fabs(__calc_size)) ); zend_hash_real_init((*(return_value)).value.arr, 1); } while (0);
   do { HashTable *__fill_ht = ((*(return_value)).value.arr); Bucket *__fill_bkt = __fill_ht->arData + __fill_ht->nNumUsed; uint32_t __fill_idx = __fill_ht->nNumUsed; do { if (__builtin_expect(!(__fill_ht->u.flags & (1<<2)), 0)) __builtin_unreachable(); } while (0); {
    for (value = low; value <= (high + 0.000000000000001); value = low + (++i * step)) {
     (tmp).value.dval = value;
     do { do { zval *_z1 = (&__fill_bkt->val); const zval *_z2 = (&tmp); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); } while (0); __fill_bkt->h = (__fill_idx); __fill_bkt->key = ((void *)0); __fill_bkt++; __fill_idx++; } while (0);
