...
static size_t phar_dir_read(php_stream *stream, char *buf, size_t count )
{
 ...
 if (3 == zend_hash_get_current_key_ex(data, &key, &keylen, &unused, 0, ((void *)0))) {
  return 0;
 }
...
# 152 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.c"
...
static php_stream *phar_make_dirstream(char *dir, HashTable *manifest )
{
 ...
 _zend_hash_init((data), (64), (zend_get_hash_value), (((void *)0)), (0) );
 ...
 while (FAILURE != (zend_hash_get_current_key_type_ex(manifest, ((void *)0)) == 3 ? FAILURE : SUCCESS)) {
  keylen = 0;
  if (3 == zend_hash_get_current_key_ex(manifest, &key, &keylen, &unused, 0, ((void *)0))) {
   break;
  }
  ...
  while (FAILURE != (zend_hash_get_current_key_type_ex(&phar->manifest, ((void *)0)) == 3 ? FAILURE : SUCCESS)) {
   if (3 !=
     zend_hash_get_current_key_ex(
      &phar->manifest, &key, &keylen, &unused, 0, ((void *)0))) {
    str_key = key;;
...
int phar_wrapper_rmdir(php_stream_wrapper *wrapper, char *url, int options, php_stream_context *context )
{
 ...
 if (!entry->is_deleted) {
  ...
  3 != zend_hash_get_current_key_ex(&phar->manifest, &key, &key_len, &unused, 0, ((void *)0));
  ...
  for (zend_hash_internal_pointer_reset_ex(&phar->virtual_dirs, ((void *)0));
   3 != zend_hash_get_current_key_ex(&phar->virtual_dirs, &key, &key_len, &unused, 0, ((void *)0));
