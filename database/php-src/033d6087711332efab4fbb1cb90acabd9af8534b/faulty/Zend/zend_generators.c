...
static void zend_generator_cleanup_unfinished_execution(zend_generator *generator)
{
 ...
 if (generator->send_target) {
  do { if ((((*((generator->send_target))).u1.v.type_flags & (1<<2)) != 0)) { zval_delref_p((generator->send_target)); } } while (0);
 ...
 if (root->node.parent) {
  if (root->node.parent->execute_data == ((void *)0)) {
   if (__builtin_expect(!!((executor_globals.exception) == ((void *)0)), 1)) {
    ...
    if (yield_from->opcode == 142 && !(yield_from->result_type & (1<<5))) {
...
void zim_Generator_send(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if (root->send_target) {
  do { if ((((*((root->send_target))).u1.v.type_flags & (1<<2)) != 0)) { zval_delref_p((root->send_target)); } } while (0);
