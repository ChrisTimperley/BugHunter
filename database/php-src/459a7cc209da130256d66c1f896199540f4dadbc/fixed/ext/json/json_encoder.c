# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c"
# 26 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c"
...
# 27 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/main/php_ini.h" 1
# 28 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2
...
# 29 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2
...
# 30 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2
...
# 31 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2
...
# 32 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2
...
# 33 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2


# 1 "/usr/lib/gcc/x86_64-linux-gnu/4.8/include/float.h" 1 3 4
# 36 "/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c" 2
...
{
 int i;
 HashTable *myht = (*(val)).value.arr;

 i = myht ? (myht)->nNumOfElements : 0;
 if (i > 0) {
  zend_string *key;
  zend_ulong index, idx;

  idx = 0;
  do { Bucket *_p = (myht)->arData; Bucket *_end = _p + (myht)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; index = _p->h; key = _p->key; {
   if (key) {
    return 1;
   } else {
    if (index != idx) {
...
{
 return !(sizeof (d) == sizeof (float) ? __isinff (d) : sizeof (d) == sizeof (double) ? __isinf (d) : __isinfl (d)) && !(sizeof (d) == sizeof (float) ? __isnanf (d) : sizeof (d) == sizeof (double) ? __isnan (d) : __isnanl (d));
}
...
static inline void php_json_encode_double(smart_str *buf, double d, int options)
{
 size_t len;
 char num[(3 + 53 - (-1021))];
 php_gcvt(d, (int)(executor_globals.precision), '.', 'e', &num[0]);
 len = strlen(num);
 if (options & (1<<10) && strchr(num, '.') == ((void *)0) && len < (3 + 53 - (-1021)) - 2) {
  num[len++] = '.';
  num[len++] = '0';
  num[len] = '\0';
 }
 smart_str_appendl_ex((buf), (num), (len), 0);
}
...
static void php_json_encode_array(smart_str *buf, zval *val, int options)
{
 ...
 if (zval_get_type(&(*(val))) == 7) {
  myht = (*(val)).value.arr;
  r = (options & (1<<4)) ? 1 : php_json_determine_array_type(val);
 } else {
  myht = ((*(val))).value.obj->handlers->get_properties(&(*(val)));
  r = 1;
 }

 if (myht && (((myht)->u.flags & 0xff00) >> 8) > 1) {
 ...
 if (i > 0) {
  ...
  do { Bucket *_p = (myht)->arData; Bucket *_end = _p + (myht)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (1 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; index = _p->h; key = _p->key; data = _z; {
   ...
   tmp_ht = (zval_get_type(&(*(data)))==7 ? (*(data)).value.arr : ((zval_get_type(&(*(data)))==8 ? (*(data)).value.obj->handlers->get_properties((data)) : ((void *)0))));
   if (tmp_ht && ((tmp_ht)->u.flags & (1<<1))) {
    ((tmp_ht)->u.flags += (1 << 8));
   }
   ...
   { jmp_buf *__orig_bailout = (executor_globals.bailout); jmp_buf __bailout; (executor_globals.bailout) = &__bailout; if (_setjmp (__bailout)==0) { {
    if (r == 0) {
     ...
     if (key) {
      if ((key)->val[0] == '\0' && zval_get_type(&(*(val))) == 8) {
      ...
      php_json_escape_string(buf, (key)->val, (key)->len, options & ~(1<<5));
      smart_str_appendc_ex((buf), (':'), 0);
    ...
    _zend_bailout("/home/ct584/git/bughunter/repositories/php-src/ext/json/json_encoder.c", 220);
...
static void php_json_escape_string(smart_str *buf, char *s, size_t len, int options)
{
 ...
 if (options & (1<<5)) {
  ...
  if ((type = is_numeric_string(s, len, &p, &d, 0)) != 0) {
   if (type == 4) {
    smart_str_append_long_ex((buf), (p), 0);
    return;
   } else if (type == 5 && php_json_is_valid_double(d)) {
 ...
 if (options & (1<<8)) {
  ...
  if (php_json_utf8_to_utf16(((void *)0), s, len) < 0) {
 ...
 do {
  ...
  if (us >= 0x80 && !(options & (1<<8))) {
   ...
   if (us >= 0x10000) {
    ...
    smart_str_appendl_ex((buf), ("\\u"), (2), 0);
    smart_str_appendc_ex((buf), (digits[(us & 0xf000) >> 12]), 0);
    smart_str_appendc_ex((buf), (digits[(us & 0xf00) >> 8]), 0);
    smart_str_appendc_ex((buf), (digits[(us & 0xf0) >> 4]), 0);
    smart_str_appendc_ex((buf), (digits[(us & 0xf)]), 0);
    us = next_us;
   }
   smart_str_appendl_ex((buf), ("\\u"), (2), 0);
   smart_str_appendc_ex((buf), (digits[(us & 0xf000) >> 12]), 0);
   ...
   switch (us) {
    ...
    case '\b':
     smart_str_appendl_ex((buf), ("\\b"), (2), 0);
     break;

    case '\f':
     smart_str_appendl_ex((buf), ("\\f"), (2), 0);
     break;
    ...
    default:
     if (us >= ' ') {
      smart_str_appendc_ex((buf), ((unsigned char) us), 0);
     } else {
      smart_str_appendl_ex((buf), ("\\u00"), (sizeof("\\u00")-1), 0);
      smart_str_appendc_ex((buf), (digits[(us & 0xf0) >> 4]), 0);
      smart_str_appendc_ex((buf), (digits[(us & 0xf)]), 0);
     }
     break;
...
static void php_json_encode_serializable_object(smart_str *buf, zval *val, int options)
{
 ...
 do { const char *_s = ("jsonSerialize"); do { do { zval *__z = (&fname); zend_string *__s = (zend_string_init(_s, strlen(_s), 0)); (*(__z)).value.str = __s; (*(__z)).u1.type_info = (6 | (( (1<<2) | (1<<4)) << 8)); } while (0); } while (0); } while (0);

 origin_error_code = (json_globals.error_code);
 if (FAILURE == call_user_function_ex((executor_globals.function_table), val, &fname, &retval, 0, ((void *)0), 1, ((void *)0)) || zval_get_type(&(retval)) == 0) {
  zend_throw_exception_ex(((void *)0), 0, "Failed calling %s::jsonSerialize()", (ce->name)->val);
  smart_str_appendl_ex((buf), ("null"), (sizeof("null") - 1), 0);
  _zval_ptr_dtor((&fname) );
  return;
 }

 (json_globals.error_code) = origin_error_code;
 ...
 if ((zval_get_type(&(retval)) == 8) &&
  ((retval).value.obj == (*(val)).value.obj)) {
...
again:
 switch (zval_get_type(&(*(val))))
 {
  ...
  case 4:
   smart_str_append_long_ex((buf), ((*(val)).value.lval), 0);
   break;

  case 5:
   if (php_json_is_valid_double((*(val)).value.dval)) {
    php_json_encode_double(buf, (*(val)).value.dval, options);
   } else {
    (json_globals.error_code) = PHP_JSON_ERROR_INF_OR_NAN;
    smart_str_appendc_ex((buf), ('0'), 0);
   }
   break;

  case 6:
   php_json_escape_string(buf, ((*(val)).value.str)->val, ((*(val)).value.str)->len, options);
   break;

  case 8:
   if (instanceof_function(((*(val)).value.obj->ce), php_json_serializable_ce)) {
