...
# 120 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/php_array.h"
...
# 29 "/home/ct584/git/bughunter/repositories/php-src/main/php_variables.c" 2
...
static void php_build_argv(char *s, zval *track_vars_array )
{
 ...
 if ((sapi_globals.request_info).argc) {
  ...
  _zend_hash_add_or_update(&(executor_globals.symbol_table), "argc", sizeof("argc"), &argc, sizeof(zval *), ((void *)0), (1<<0) );
 }
 if (track_vars_array) {
  zval_addref_p(arr);
  zval_addref_p(argc);
...
static void check_http_proxy(HashTable *var_table)
{
 if (zend_hash_exists(var_table, "HTTP_PROXY", sizeof("HTTP_PROXY"))) {
  ...
  if (!local_proxy) {
   zend_hash_del_key_or_index(var_table, "HTTP_PROXY", sizeof("HTTP_PROXY"), 0, 0);
  } else {
   zval *local_zval;
   do { (local_zval) = (zval*)_emalloc((sizeof(zval_gc_info)) ); ((zval_gc_info*)(local_zval))->u.buffered = ((void *)0); } while (0); *local_zval = zval_used_for_init;;;
   do { const char *__s=(local_proxy); zval *__z = (local_zval); (*__z).value.str.len = strlen(__s); (*__z).value.str.val = (1?_estrndup((__s), ((*__z).value.str.len) ):(char*)__s); (*__z).type = 6; } while (0);
   _zend_hash_add_or_update(var_table, "HTTP_PROXY", sizeof("HTTP_PROXY"), &local_zval, sizeof(zval **), ((void *)0), (1<<0) );
  }
 }
}
...
static zend_bool php_auto_globals_create_server(const char *name, uint name_len )
{
 ...
 check_http_proxy((*(core_globals.http_globals)[3]).value.ht);
...
static zend_bool php_auto_globals_create_env(const char *name, uint name_len )
{
 ...
 check_http_proxy((*(core_globals.http_globals)[4]).value.ht);
