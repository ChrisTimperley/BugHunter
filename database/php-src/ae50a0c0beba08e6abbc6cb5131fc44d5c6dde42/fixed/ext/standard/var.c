...
static void php_var_serialize_class(smart_str *buf, zval *struc, zval *retval_ptr, php_serialize_data_t var_hash)
{
 ...
 smart_str_append_unsigned_ex((buf), (count), 0);
 smart_str_appendl_ex((buf), (":{"), (2), 0);
 ...
 if (count > 0) {
  ...
  do { Bucket *_p = (ht)->arData; Bucket *_end = _p + (ht)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; key = _p->key; val = _z; {
...
void zif_unserialize(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if (!php_var_unserialize_ex(return_value, &p, p + buf_len, &var_hash, class_hash)) {
  ...
  if (!(executor_globals.exception)) {
   php_error_docref0(((void *)0), (1<<3L), "Error at offset " "%" "l" "d" " of %d bytes", (zend_long)((char*)p - buf), buf_len);
  }
  { do { (*(return_value)).u1.type_info = 2; } while (0); return; };
 }
 do { if ((basic_globals.serialize_lock) || (basic_globals.unserialize).level == 1) { var_destroy(&(var_hash)); _efree(((var_hash)) ); } if (!(basic_globals.serialize_lock) && !--(basic_globals.unserialize).level) { (basic_globals.unserialize).data = ((void *)0); } } while (0);
