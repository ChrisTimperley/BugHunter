...
# 566 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
void zend_stop_lexing(void)
{
...
static inline void zend_begin_loop(zend_uchar free_opcode, const znode *loop_var)
{
 ...
 if (loop_var && (loop_var->op_type & ((1<<2)|(1<<1)))) {
  ...
  info.u.brk_cont_offset = (compiler_globals.context).current_brk_cont;
...
{
 zend_brk_cont_element *brk_cont_element
  = &(compiler_globals.active_op_array)->brk_cont_array[(compiler_globals.context).current_brk_cont];
 brk_cont_element->cont = cont_addr;
 brk_cont_element->brk = get_next_op_number((compiler_globals.active_op_array));
   ...
   while (opline >= (compiler_globals.active_op_array)->opcodes) {
    ...
    if (opline->result_type==(1<<2)
     ...
     if (opline->opcode == 68) {
      ...
      while (opline->opcode != 60 || opline->op1.num != (1 << 3)) {
       opline--;
      }
      opline->op1.num |= (1 << 4);
     }
...
__attribute__ ((visibility("default"))) int do_bind_function(const zend_op_array *op_array, const zend_op *opline, HashTable *function_table, zend_bool compile_time)
{
 ...
 if (compile_time) {
  ...
  op1 = ((zval*)(((char*)((op_array)->literals)) + (opline->op1).constant));
  op2 = ((zval*)(((char*)((op_array)->literals)) + (opline->op2).constant));
 }
...
__attribute__ ((visibility("default"))) zend_class_entry *do_bind_inherited_class(const zend_op_array *op_array, const zend_op *opline, HashTable *class_table, zend_class_entry *parent_ce, zend_bool compile_time)
{
 ...
 if (!ce) {
  if (!compile_time) {
   ...
   zend_error_noreturn((1<<6L), "Cannot declare %s %s, because the name is already in use", zend_get_object_type(((*(op2)).value.obj->ce)), ((*(op2)).value.str)->val);
  }
  return ((void *)0);
 }

 if (zend_hash_exists(class_table, (*(op2)).value.str)) {
  zend_error_noreturn((1<<6L), "Cannot declare %s %s, because the name is already in use", zend_get_object_type(ce), (ce->name)->val);
 }
...
static void zend_mark_function_as_generator()
{
 ...
 if ((compiler_globals.active_op_array)->fn_flags & 0x40000000) {
  ...
  if (!(compiler_globals.active_op_array)->arg_info[-1].class_name) {
   zend_error_noreturn((1<<6L), msg,
    zend_get_type_by_const((compiler_globals.active_op_array)->arg_info[-1].type_hint));
  }
  if (!(((compiler_globals.active_op_array)->arg_info[-1].class_name)->len == sizeof("Traversable")-1
    && zend_binary_strcasecmp(((compiler_globals.active_op_array)->arg_info[-1].class_name)->val, sizeof("Traversable")-1, "Traversable", sizeof("Traversable")-1) == 0) &&
   !(((compiler_globals.active_op_array)->arg_info[-1].class_name)->len == sizeof("Iterator")-1
    && zend_binary_strcasecmp(((compiler_globals.active_op_array)->arg_info[-1].class_name)->val, sizeof("Iterator")-1, "Iterator", sizeof("Iterator")-1) == 0) &&
   !(((compiler_globals.active_op_array)->arg_info[-1].class_name)->len == sizeof("Generator")-1
    && zend_binary_strcasecmp(((compiler_globals.active_op_array)->arg_info[-1].class_name)->val, sizeof("Generator")-1, "Generator", sizeof("Generator")-1) == 0)) {
   zend_error_noreturn((1<<6L), msg, ((compiler_globals.active_op_array)->arg_info[-1].class_name)->val);
  }
 }
...
static zend_bool zend_try_ct_eval_class_const(zval *zv, zend_string *class_name, zend_string *name)
{
 ...
 if (class_name_refers_to_active_ce(class_name, fetch_type)) {
  c = zend_hash_find(&(compiler_globals.active_class_entry)->constants_table, name);
...
__attribute__ ((visibility("default"))) void zend_initialize_class_data(zend_class_entry *ce, zend_bool nullify_handlers)
{
 ...
 if (ce->type == 1) {
# 1633 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
...
__attribute__ ((visibility("default"))) zend_string *zend_get_compiled_variable_name(const zend_op_array *op_array, uint32_t var)
{
 return op_array->vars[(((zval*)(((char*)(((void *)0))) + ((int)(var)))) - (((zval*)(((void *)0))) + (((int)(((((sizeof(zend_execute_data)) + 8 - 1L) & ~(8 - 1L)) + (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)) - 1) / (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)))) + ((int)(0)))))];
}
...
# 1755 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
 if (len == 0) {
 ...
 if (end < path) {
# 1787 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
  path[0] = '.';
  path[1] = '\0';
  return 1 + len_adjust;
...
static void zend_emit_return_type_check(znode *expr, zend_arg_info *return_info)
{
 if (return_info->type_hint != 0) {
  zend_op *opline = zend_emit_op(((void *)0), 124, expr, ((void *)0));
...
void zend_emit_final_return(zval *zv)
{
 ...
 if ((compiler_globals.active_op_array)->fn_flags & 0x40000000) {
  zend_emit_return_type_check(((void *)0), (compiler_globals.active_op_array)->arg_info - 1);
 }

 zn.op_type = (1<<0);
 if (zv) {
  do { zval *_z1 = (&zn.u.constant); const zval *_z2 = (zv); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); } while (0);
 } else {
...
static int zend_try_compile_cv(znode *result, zend_ast *ast)
{
 ...
 if (name_ast->kind == ZEND_AST_ZVAL) {
  ...
  name = (compiler_globals.active_op_array)->vars[(((zval*)(((char*)(((void *)0))) + ((int)(result->u.op.var)))) - (((zval*)(((void *)0))) + (((int)(((((sizeof(zend_execute_data)) + 8 - 1L) & ~(8 - 1L)) + (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)) - 1) / (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)))) + ((int)(0)))))];
...
zend_op *zend_compile_static_prop_common(znode *result, zend_ast *ast, uint32_t type, int delayed)
{
 ...
 if (zend_is_const_default_class_ref(class_ast)) {
  ...
  do { zval *__z = (&class_node.u.constant); zend_string *__s = (zend_resolve_class_name_ast(class_ast)); (*(__z)).value.str = __s; (*(__z)).u1.type_info = ((__s)->gc.u.v.flags & (1<<1)) ? 6 : (6 | (( (1<<2) | (1<<4)) << 8)); } while (0);
 } else {
 ...
 if (delayed) {
  opline = zend_delayed_emit_op(result, 80, &prop_node, ((void *)0));
 } else {
  ...
  if (zend_is_variable(arg)) {
   if (zend_is_call(arg)) {
    ...
    if (arg_node.op_type & ((1<<0)|(1<<1))) {
     ...
     opcode = 106;
     flags |= (1<<2);
     if (fbc && zend_check_arg_send_type(fbc, arg_num, 1|2)) {
      flags |= (1<<0);
      if (zend_check_arg_send_type(fbc, arg_num, 2)) {
       flags |= (1<<3);
      }
     }
    }
   ...
   if (arg_node.op_type == (1<<2)) {
    opcode = 106;
    if (fbc && zend_check_arg_send_type(fbc, arg_num, 1)) {
     flags |= (1<<0);
    }
   } else {
    if (fbc) {
 ...
 if (fbc) {
  if (fbc->type == 1) {
   if (!zend_execute_internal &&
       !fbc->common.scope &&
       !(fbc->common.fn_flags & (0x02|0x40000|0x10000000|0x4000000))) {
    return 129;
   }
  } else {
   if (zend_execute_ex == execute_ex &&
       !(fbc->common.fn_flags & 0x800000)) {
    return 130;
   }
  }
...
void zend_compile_call_common(znode *result, zend_ast *args_ast, zend_function *fbc)
{
 ...
 opline = zend_emit_op(result, zend_get_call_op(opline->opcode, fbc), ((void *)0), ((void *)0));
...
int zend_compile_func_strlen(znode *result, zend_ast_list *args)
{
 ...
 if (arg_node.op_type == (1<<0) && zval_get_type(&(arg_node.u.constant)) == 6) {
  ...
  _zval_dtor((&arg_node.u.constant) );
 } else {
...
static int zend_compile_assert(znode *result, zend_ast_list *args, zend_string *name, zend_function *fbc)
{
 if ((executor_globals.assertions) >= 0) {
  ...
  (compiler_globals.active_op_array)->opcodes[check_op_number].op2.opline_num = get_next_op_number((compiler_globals.active_op_array));
 } else {
  if (!fbc) {
   zend_string_release(name);
  }
...
int zend_try_compile_special_func(znode *result, zend_string *lcname, zend_ast_list *args, zend_function *fbc)
{
 ...
 } else if (((lcname)->len == sizeof("assert")-1 && !memcmp((lcname)->val, "assert", sizeof("assert") - 1))) {
...
void zend_compile_new(znode *result, zend_ast *ast)
{
 ...
 if (zend_is_const_default_class_ref(class_ast)) {
  ...
  opline->op1.opline_num = get_next_op_number((compiler_globals.active_op_array));
 } else {
  zend_compile_class_ref(&class_node, class_ast, 1);
 }
...
static int zend_handle_loops_and_finally_ex(zend_long depth)
{
 ...
 for (; loop_var >= base; loop_var--) {
  if (loop_var->opcode == 162) {
   ...
   do { if (__builtin_expect(!(loop_var->var_type == (1<<2) || loop_var->var_type == (1<<1)), 0)) __builtin_unreachable(); } while (0);
   opline = get_next_op((compiler_globals.active_op_array));
...
{
 return zend_handle_loops_and_finally_ex(zend_stack_count(&(compiler_globals.loop_var_stack)) + 1);
}
...
void zend_compile_return(zend_ast *ast)
{
 ...
 if ((compiler_globals.context).in_finally) {
  opline = zend_emit_op(((void *)0), 159, ((void *)0), ((void *)0));
  opline->op1_type = (1<<1);
 ...
 if (!((compiler_globals.active_op_array)->fn_flags & 0x800000) && (compiler_globals.active_op_array)->fn_flags & 0x40000000) {
  zend_emit_return_type_check(expr_ast ? &expr_node : ((void *)0), (compiler_globals.active_op_array)->arg_info - 1);
 }
...
void zend_compile_foreach(zend_ast *ast)
{
 ...
 zend_emit_op(((void *)0), 127, &reset_node, ((void *)0));
}
...
void zend_compile_switch(zend_ast *ast)
{
 ...
 zend_end_loop(get_next_op_number((compiler_globals.active_op_array)));

 if (expr_node.op_type == (1<<2) || expr_node.op_type == (1<<1)) {
  zend_emit_op(((void *)0), 70, &expr_node, ((void *)0));
 } else if (expr_node.op_type == (1<<0)) {
...
__attribute__ ((visibility("default"))) void zend_set_function_arg_flags(zend_function *func)
{
 ...
 if (func->common.arg_info) {
  ...
  i = 0;
  while (i < n) {
   do { *(uint32_t*)&(func)->type |= (((func->common.arg_info[i].pass_by_reference) << 6) << (i + 1) * 2); } while (0);
   i++;
  }
  if (__builtin_expect(!!(func->common.fn_flags & 0x1000000 && func->common.arg_info[i].pass_by_reference), 0)) {
   ...
   while (i < 12) {
    do { *(uint32_t*)&(func)->type |= (((pass_by_reference) << 6) << (i + 1) * 2); } while (0);
    i++;
   }
  }
 }
...
void zend_compile_params(zend_ast *ast, zend_ast *return_type_ast)
{
 ...
 for (i = 0; i < list->children; ++i) {
  ...
  if ((((zval*)(((char*)(((void *)0))) + ((int)(var_node.u.op.var)))) - (((zval*)(((void *)0))) + (((int)(((((sizeof(zend_execute_data)) + 8 - 1L) & ~(8 - 1L)) + (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)) - 1) / (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)))) + ((int)(0))))) != i) {
...
static void zend_begin_func_decl(znode *result, zend_op_array *op_array, zend_ast_decl *decl)
{
 ...
 if (((compiler_globals.file_context).imports_function)) {
  zend_string *import_name = zend_hash_find_ptr(((compiler_globals.file_context).imports_function), lcname);
 ...
 if (op_array->fn_flags & 0x100000) {
  ...
  opline->opcode = 141;
  opline->op2_type = (1<<0);
  do { zval _c; do { zval *__z = (&_c); zend_string *__s = (zend_string_copy(lcname)); (*(__z)).value.str = __s; (*(__z)).u1.type_info = ((__s)->gc.u.v.flags & (1<<1)) ? 6 : (6 | (( (1<<2) | (1<<4)) << 8)); } while (0); opline->op2.constant = zend_add_literal((compiler_globals.active_op_array), &_c); } while (0);
 }
...
void zend_compile_class_decl(zend_ast *ast)
{
 ...
 zend_ast *stmt_ast = decl->child[2];
 zend_string *name, *lcname, *import_name = ((void *)0);
 ...
 if (__builtin_expect(!!((decl->flags & 0x100) == 0), 1)) {
  ...
  if (((compiler_globals.file_context).imports)) {
   import_name = zend_hash_find_ptr(((compiler_globals.file_context).imports), lcname);
  }
...
void zend_compile_const_decl(zend_ast *ast)
{
 ...
 for (i = 0; i < list->children; ++i) {
  ...
  if (((compiler_globals.file_context).imports_const)
   && (import_name = zend_hash_find_ptr(((compiler_globals.file_context).imports_const), name))
  ) {
...
static inline void zend_ct_eval_unary_pm(zval *result, zend_ast_kind kind, zval *op)
{
 ...
 if (kind == ZEND_AST_UNARY_PLUS) {
  ...
  { zval *__z = (&left); (*(__z)).value.lval = -1; (*(__z)).u1.type_info = 4; };
...
void zend_compile_unary_pm(znode *result, zend_ast *ast)
{
 ...
 if (ast->kind == ZEND_AST_UNARY_PLUS) {
  ...
  zend_emit_op_tmp(result, 1, &zero_node, &expr_node);
 } else {



  znode neg_one_node;
  neg_one_node.op_type = (1<<0);
  { zval *__z = (&neg_one_node.u.constant); (*(__z)).value.lval = -1; (*(__z)).u1.type_info = 4; };

  zend_emit_op_tmp(result, 3, &neg_one_node, &expr_node);
 }
}
 ...
 if (left_node.op_type == (1<<0)) {
  if ((ast->kind == ZEND_AST_AND && !zend_is_true(&left_node.u.constant))
   ...
   if (right_node.op_type == (1<<0)) {
    ...
    zend_emit_op(result, 52, &right_node, ((void *)0));
   }
  }
...
void zend_compile_silence(znode *result, zend_ast *ast)
{
 ...
 begin_opline_num = get_next_op_number((compiler_globals.active_op_array));
 ...
 end_opline_num = get_next_op_number((compiler_globals.active_op_array));
...
void zend_compile_array(znode *result, zend_ast *ast)
{
 ...
 if (!packed) {
  do { if (__builtin_expect(!(opnum_init != -1), 0)) __builtin_unreachable(); } while (0);
  opline = &(compiler_globals.active_op_array)->opcodes[opnum_init];
  opline->extended_value |= (1<<1);
 }
}
...
void zend_compile_class_const(znode *result, zend_ast *ast)
{
 ...
 if (zend_is_const_default_class_ref(class_ast)) {
  class_node.op_type = (1<<0);
  do { zval *__z = (&class_node.u.constant); zend_string *__s = (resolved_name); (*(__z)).value.str = __s; (*(__z)).u1.type_info = ((__s)->gc.u.v.flags & (1<<1)) ? 6 : (6 | (( (1<<2) | (1<<4)) << 8)); } while (0);
 } else {
  if (class_ast->kind == ZEND_AST_ZVAL) {
   zend_string_release(resolved_name);
  }
...
static void zend_compile_encaps_list(znode *result, zend_ast *ast)
{
 ...
 if (j == 1) {
  ...
  uint32_t var;
  zend_brk_cont_element *info = get_next_brk_cont_element((compiler_globals.active_op_array));
  info->start = rope_init_lineno;
  info->parent = (compiler_globals.context).current_brk_cont;
  info->cont = info->brk = opline - (compiler_globals.active_op_array)->opcodes;
