...
__attribute__ ((visibility("default"))) void zend_initialize_class_data(zend_class_entry *ce, zend_bool nullify_handlers)
{
 ...
 if (ce->type == 1) {
# 1751 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
...
# 1873 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
 if (len == 0) {
 ...
 if (end < path) {
# 1905 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
  path[0] = '.';
  path[1] = '\0';
  return 1 + len_adjust;
...
static void zend_compile_list_assign(znode *result, zend_ast *ast, znode *expr_node)
{
 ...
 if (list->children > 0 && list->child[0] != ((void *)0) && list->child[0]->kind == ZEND_AST_ARRAY_ELEM) {
 ...
 if (fbc && init_op->opcode == 61) {
  if (fbc->type == 1) {
   ...
   if (zend_execute_ex == execute_ex) {
    if (!(fbc->common.fn_flags & 0x800000)) {
     return 130;
    } else {
     return 131;
    }
