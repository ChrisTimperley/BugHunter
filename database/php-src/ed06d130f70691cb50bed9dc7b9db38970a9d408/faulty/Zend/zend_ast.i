# 1 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c"
# 22 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c"
# 1 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.h" 1
# 26 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.h"
...
# 27 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.h" 2
...
# 23 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c" 2
...
# 24 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c" 2
...
int zendparse (void);
# 26 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c" 2
...
# 27 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c" 2
...
# 28 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c" 2

__attribute__ ((visibility("default"))) zend_ast_process_t zend_ast_process = ((void *)0);

static inline void *zend_ast_alloc(size_t size) {
 return zend_arena_alloc(&(compiler_globals.ast_arena), size);
}
...
__attribute__ ((visibility("default"))) zend_ast *zend_ast_create_znode(znode *node) {
 ...
 ast->attr = 0;
 ast->lineno = (compiler_globals.zend_lineno);
 ast->node = *node;
 return (zend_ast *) ast;
}
...
__attribute__ ((visibility("default"))) zend_ast *zend_ast_create_zval_ex(zval *zv, zend_ast_attr attr) {
 ...
 ast->attr = attr;
 do { zval *_z1 = (&ast->val); const zval *_z2 = (zv); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); } while (0);
 ast->val.u2.lineno = (compiler_globals.zend_lineno);
 return (zend_ast *) ast;
}
...
__attribute__ ((visibility("default"))) zend_ast *zend_ast_create_decl(
 ...
 ast->end_lineno = (compiler_globals.zend_lineno);
 ast->flags = flags;
 ast->lex_pos = (language_scanner_globals.yy_text);
...
static zend_ast *zend_ast_create_from_va_list(zend_ast_kind kind, zend_ast_attr attr, va_list va) {
 ...
 for (i = 0; i < children; ++i) {
  ast->child[i] = __builtin_va_arg(va,zend_ast *);
  if (ast->child[i] != ((void *)0)) {
 ...
 if (ast->lineno == (2147483647 * 2U + 1U)) {
  ast->lineno = (compiler_globals.zend_lineno);
 }
...
__attribute__ ((visibility("default"))) zend_ast *zend_ast_create_list(uint32_t init_children, zend_ast_kind kind, ...) {
 ...
 list->kind = kind;
 list->attr = 0;
 list->lineno = (compiler_globals.zend_lineno);
 list->children = 0;

 {
  ...
  for (i = 0; i < init_children; ++i) {
   ast = zend_ast_list_add(ast, __builtin_va_arg(va,zend_ast *));
  }
  __builtin_va_end(va);
 }
...
__attribute__ ((visibility("default"))) zend_ast *zend_ast_list_add(zend_ast *ast, zend_ast *op) {
...
static int zend_ast_add_array_element(zval *result, zval *offset, zval *expr)
{
 switch (zval_get_type(&(*(offset)))) {
  case 0:
   _zend_hash_next_index_insert((*(result)).value.arr, expr );
   break;
  case 6:
   ...
   break;
  case 1:
   zend_symtable_update((*(result)).value.arr, (compiler_globals.empty_string), expr);
   break;
  case 4:
   _zend_hash_index_update((*(result)).value.arr, (*(offset)).value.lval, expr );
   break;
  case 2:
   _zend_hash_index_update((*(result)).value.arr, 0, expr );
   break;
  case 3:
   _zend_hash_index_update((*(result)).value.arr, 1, expr );
   break;
  case 5:
   _zend_hash_index_update((*(result)).value.arr, zend_dval_to_lval((*(offset)).value.dval), expr );
   break;
  default:
   zend_throw_error(((void *)0), "Illegal offset type");
   return FAILURE;
  }
 return SUCCESS;
}
...
__attribute__ ((visibility("default"))) int zend_ast_evaluate(zval *result, zend_ast *ast, zend_class_entry *scope)
{
 ...
 switch (ast->kind) {
  case ZEND_AST_BINARY_OP:
   if (__builtin_expect(!!(zend_ast_evaluate(&op1, ast->child[0], scope) != SUCCESS), 0)) {
    ret = FAILURE;
   } else if (__builtin_expect(!!(zend_ast_evaluate(&op2, ast->child[1], scope) != SUCCESS), 0)) {
    _zval_dtor((&op1) );
    ret = FAILURE;
   } else {
  ...
  case ZEND_AST_ZVAL:
  {
   ...
   if (scope) {
    ...
    if ((((*(zv)).u1.type_info & ((1<<0) << 8)) != 0)) {
     if (__builtin_expect(!!(zval_update_constant_ex(zv, 1, scope) != SUCCESS), 0)) {
      ret = FAILURE;
      break;
     }
    }
    do { zval *_z1 = (result); const zval *_z2 = (zv); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & (((1<<2)|(1<<1)) << 8)) != 0) { if ((_t & (((1<<4)|(1<<1)) << 8)) != 0) { _zval_copy_ctor_func(_z1 ); } else { (_gc)->gc.refcount++; } } } while (0);
   } else {
    do { zval *_z1 = (result); const zval *_z2 = (zv); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & (((1<<2)|(1<<1)) << 8)) != 0) { if ((_t & (((1<<4)|(1<<1)) << 8)) != 0) { _zval_copy_ctor_func(_z1 ); } else { (_gc)->gc.refcount++; } } } while (0);
    if ((((*(result)).u1.type_info & ((1<<0) << 8)) != 0)) {
     if (__builtin_expect(!!(zval_update_constant_ex(result, 1, scope) != SUCCESS), 0)) {
      ret = FAILURE;
      break;
     }
    }
  ...
  case ZEND_AST_AND:
   ...
   if (zend_is_true(&op1)) {
    if (__builtin_expect(!!(zend_ast_evaluate(&op2, ast->child[1], scope) != SUCCESS), 0)) {
     _zval_dtor((&op1) );
     ret = FAILURE;
     break;
    }
    do { (*(result)).u1.type_info = (zend_is_true(&op2)) ? 3 : 2; } while (0);
    _zval_dtor((&op2) );
   } else {
  ...
  case ZEND_AST_CONDITIONAL:
   ...
   if (zend_is_true(&op1)) {
     ...
     *result = op1;
     ...
     if (__builtin_expect(!!(zend_ast_evaluate(result, ast->child[1], scope) != SUCCESS), 0)) {
      _zval_dtor((&op1) );
      ret = FAILURE;
      break;
     }
    ...
    if (__builtin_expect(!!(zend_ast_evaluate(result, ast->child[2], scope) != SUCCESS), 0)) {
     _zval_dtor((&op1) );
     ret = FAILURE;
     break;
    }
  ...
  case ZEND_AST_UNARY_PLUS:
   if (__builtin_expect(!!(zend_ast_evaluate(&op2, ast->child[0], scope) != SUCCESS), 0)) {
    ret = FAILURE;
   } else {
    { zval *__z = (&op1); (*(__z)).value.lval = 0; (*(__z)).u1.type_info = 4; };
  ...
  case ZEND_AST_ARRAY:
   _array_init((result), 0 );
   {
    ...
    for (i = 0; i < list->children; i++) {
     ...
     if (elem->child[1]) {
      if (__builtin_expect(!!(zend_ast_evaluate(&op1, elem->child[1], scope) != SUCCESS), 0)) {
       _zval_dtor((result) );
       return FAILURE;
      }
     } else {
      do { (*(&op1)).u1.type_info = 0; } while (0);
     }
     if (__builtin_expect(!!(zend_ast_evaluate(&op2, elem->child[0], scope) != SUCCESS), 0)) {
      _zval_dtor((&op1) );
      _zval_dtor((result) );
      return FAILURE;
     }
     if (__builtin_expect(!!(zend_ast_add_array_element(result, &op1, &op2) != SUCCESS), 0)) {
      _zval_dtor((&op1) );
      _zval_dtor((&op2) );
      _zval_dtor((result) );
      return FAILURE;
  ...
  case ZEND_AST_DIM:
   if (__builtin_expect(!!(zend_ast_evaluate(&op1, ast->child[0], scope) != SUCCESS), 0)) {
    ...
    if (__builtin_expect(!!((zval_get_type(&(tmp)) == 10)), 0)) {
     do { zval *_z1 = (result); const zval *_z2 = (&(tmp).value.ref->val); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & (((1<<2)|(1<<1)) << 8)) != 0) { if ((_t & (((1<<4)|(1<<1)) << 8)) != 0) { _zval_copy_ctor_func(_z1 ); } else { (_gc)->gc.refcount++; } } } while (0);
    } else {
     do { zval *_z1 = (result); const zval *_z2 = (&tmp); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & (((1<<2)|(1<<1)) << 8)) != 0) { if ((_t & (((1<<4)|(1<<1)) << 8)) != 0) { _zval_copy_ctor_func(_z1 ); } else { (_gc)->gc.refcount++; } } } while (0);
    }
    _zval_ptr_dtor((&tmp) );
    _zval_dtor((&op1) );
    _zval_dtor((&op2) );
   ...
   break;
  default:
   zend_throw_error(((void *)0), "Unsupported constant expression");
   ret = FAILURE;
 }
 return ret;
}
...
__attribute__ ((visibility("default"))) zend_ast *zend_ast_copy(zend_ast *ast)
{
 if (ast == ((void *)0)) {
  ...
  zend_ast_zval *new = (__builtin_constant_p((sizeof(zend_ast_zval)) ) ? (((sizeof(zend_ast_zval)) <= 8) ? _emalloc_8() : (((sizeof(zend_ast_zval)) <= 16) ? _emalloc_16() : (((sizeof(zend_ast_zval)) <= 24) ? _emalloc_24() : (((sizeof(zend_ast_zval)) <= 32) ? _emalloc_32() : (((sizeof(zend_ast_zval)) <= 40) ? _emalloc_40() : (((sizeof(zend_ast_zval)) <= 48) ? _emalloc_48() : (((sizeof(zend_ast_zval)) <= 56) ? _emalloc_56() : (((sizeof(zend_ast_zval)) <= 64) ? _emalloc_64() : (((sizeof(zend_ast_zval)) <= 80) ? _emalloc_80() : (((sizeof(zend_ast_zval)) <= 96) ? _emalloc_96() : (((sizeof(zend_ast_zval)) <= 112) ? _emalloc_112() : (((sizeof(zend_ast_zval)) <= 128) ? _emalloc_128() : (((sizeof(zend_ast_zval)) <= 160) ? _emalloc_160() : (((sizeof(zend_ast_zval)) <= 192) ? _emalloc_192() : (((sizeof(zend_ast_zval)) <= 224) ? _emalloc_224() : (((sizeof(zend_ast_zval)) <= 256) ? _emalloc_256() : (((sizeof(zend_ast_zval)) <= 320) ? _emalloc_320() : (((sizeof(zend_ast_zval)) <= 384) ? _emalloc_384() : (((sizeof(zend_ast_zval)) <= 448) ? _emalloc_448() : (((sizeof(zend_ast_zval)) <= 512) ? _emalloc_512() : (((sizeof(zend_ast_zval)) <= 640) ? _emalloc_640() : (((sizeof(zend_ast_zval)) <= 768) ? _emalloc_768() : (((sizeof(zend_ast_zval)) <= 896) ? _emalloc_896() : (((sizeof(zend_ast_zval)) <= 1024) ? _emalloc_1024() : (((sizeof(zend_ast_zval)) <= 1280) ? _emalloc_1280() : (((sizeof(zend_ast_zval)) <= 1536) ? _emalloc_1536() : (((sizeof(zend_ast_zval)) <= 1792) ? _emalloc_1792() : (((sizeof(zend_ast_zval)) <= 2048) ? _emalloc_2048() : (((sizeof(zend_ast_zval)) <= 2560) ? _emalloc_2560() : (((sizeof(zend_ast_zval)) <= 3072) ? _emalloc_3072() : (((sizeof(zend_ast_zval)) <= ((2 * 1024 * 1024) - ((4 * 1024) * (1)))) ? _emalloc_large((sizeof(zend_ast_zval))) : _emalloc_huge((sizeof(zend_ast_zval)))) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) : _emalloc((sizeof(zend_ast_zval)) ) );
  new->kind = ZEND_AST_ZVAL;
  new->attr = ast->attr;
  do { zval *_z1 = (&new->val); const zval *_z2 = (zend_ast_get_zval(ast)); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & ((1<<2) << 8)) != 0) { (_gc)->gc.refcount++; } } while (0);
  return (zend_ast *) new;
 } else if (zend_ast_is_list(ast)) {
  zend_ast_list *list = zend_ast_get_list(ast);
  zend_ast_list *new = (__builtin_constant_p((zend_ast_list_size(list->children)) ) ? (((zend_ast_list_size(list->children)) <= 8) ? _emalloc_8() : (((zend_ast_list_size(list->children)) <= 16) ? _emalloc_16() : (((zend_ast_list_size(list->children)) <= 24) ? _emalloc_24() : (((zend_ast_list_size(list->children)) <= 32) ? _emalloc_32() : (((zend_ast_list_size(list->children)) <= 40) ? _emalloc_40() : (((zend_ast_list_size(list->children)) <= 48) ? _emalloc_48() : (((zend_ast_list_size(list->children)) <= 56) ? _emalloc_56() : (((zend_ast_list_size(list->children)) <= 64) ? _emalloc_64() : (((zend_ast_list_size(list->children)) <= 80) ? _emalloc_80() : (((zend_ast_list_size(list->children)) <= 96) ? _emalloc_96() : (((zend_ast_list_size(list->children)) <= 112) ? _emalloc_112() : (((zend_ast_list_size(list->children)) <= 128) ? _emalloc_128() : (((zend_ast_list_size(list->children)) <= 160) ? _emalloc_160() : (((zend_ast_list_size(list->children)) <= 192) ? _emalloc_192() : (((zend_ast_list_size(list->children)) <= 224) ? _emalloc_224() : (((zend_ast_list_size(list->children)) <= 256) ? _emalloc_256() : (((zend_ast_list_size(list->children)) <= 320) ? _emalloc_320() : (((zend_ast_list_size(list->children)) <= 384) ? _emalloc_384() : (((zend_ast_list_size(list->children)) <= 448) ? _emalloc_448() : (((zend_ast_list_size(list->children)) <= 512) ? _emalloc_512() : (((zend_ast_list_size(list->children)) <= 640) ? _emalloc_640() : (((zend_ast_list_size(list->children)) <= 768) ? _emalloc_768() : (((zend_ast_list_size(list->children)) <= 896) ? _emalloc_896() : (((zend_ast_list_size(list->children)) <= 1024) ? _emalloc_1024() : (((zend_ast_list_size(list->children)) <= 1280) ? _emalloc_1280() : (((zend_ast_list_size(list->children)) <= 1536) ? _emalloc_1536() : (((zend_ast_list_size(list->children)) <= 1792) ? _emalloc_1792() : (((zend_ast_list_size(list->children)) <= 2048) ? _emalloc_2048() : (((zend_ast_list_size(list->children)) <= 2560) ? _emalloc_2560() : (((zend_ast_list_size(list->children)) <= 3072) ? _emalloc_3072() : (((zend_ast_list_size(list->children)) <= ((2 * 1024 * 1024) - ((4 * 1024) * (1)))) ? _emalloc_large((zend_ast_list_size(list->children))) : _emalloc_huge((zend_ast_list_size(list->children)))) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) : _emalloc((zend_ast_list_size(list->children)) ) );
  uint32_t i;
  new->kind = list->kind;
  new->attr = list->attr;
  ...
  zend_ast *new = (__builtin_constant_p((zend_ast_size(children)) ) ? (((zend_ast_size(children)) <= 8) ? _emalloc_8() : (((zend_ast_size(children)) <= 16) ? _emalloc_16() : (((zend_ast_size(children)) <= 24) ? _emalloc_24() : (((zend_ast_size(children)) <= 32) ? _emalloc_32() : (((zend_ast_size(children)) <= 40) ? _emalloc_40() : (((zend_ast_size(children)) <= 48) ? _emalloc_48() : (((zend_ast_size(children)) <= 56) ? _emalloc_56() : (((zend_ast_size(children)) <= 64) ? _emalloc_64() : (((zend_ast_size(children)) <= 80) ? _emalloc_80() : (((zend_ast_size(children)) <= 96) ? _emalloc_96() : (((zend_ast_size(children)) <= 112) ? _emalloc_112() : (((zend_ast_size(children)) <= 128) ? _emalloc_128() : (((zend_ast_size(children)) <= 160) ? _emalloc_160() : (((zend_ast_size(children)) <= 192) ? _emalloc_192() : (((zend_ast_size(children)) <= 224) ? _emalloc_224() : (((zend_ast_size(children)) <= 256) ? _emalloc_256() : (((zend_ast_size(children)) <= 320) ? _emalloc_320() : (((zend_ast_size(children)) <= 384) ? _emalloc_384() : (((zend_ast_size(children)) <= 448) ? _emalloc_448() : (((zend_ast_size(children)) <= 512) ? _emalloc_512() : (((zend_ast_size(children)) <= 640) ? _emalloc_640() : (((zend_ast_size(children)) <= 768) ? _emalloc_768() : (((zend_ast_size(children)) <= 896) ? _emalloc_896() : (((zend_ast_size(children)) <= 1024) ? _emalloc_1024() : (((zend_ast_size(children)) <= 1280) ? _emalloc_1280() : (((zend_ast_size(children)) <= 1536) ? _emalloc_1536() : (((zend_ast_size(children)) <= 1792) ? _emalloc_1792() : (((zend_ast_size(children)) <= 2048) ? _emalloc_2048() : (((zend_ast_size(children)) <= 2560) ? _emalloc_2560() : (((zend_ast_size(children)) <= 3072) ? _emalloc_3072() : (((zend_ast_size(children)) <= ((2 * 1024 * 1024) - ((4 * 1024) * (1)))) ? _emalloc_large((zend_ast_size(children))) : _emalloc_huge((zend_ast_size(children)))) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) : _emalloc((zend_ast_size(children)) ) );
  new->kind = ast->kind;
  new->attr = ast->attr;
  for (i = 0; i < children; i++) {
...
static void zend_ast_destroy_ex(zend_ast *ast, zend_bool free) {
 ...
 switch (ast->kind) {
  case ZEND_AST_ZVAL:
   ...
   _zval_ptr_dtor_nogc((zend_ast_get_zval(ast)) );
   break;
...
# 554 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c"
  ...
  if (c < ' ') {
   switch (c) {
    case '\n':
     ...
     break;
    case '\r':
     smart_str_appendl_ex(((str)), (("\\r")), (strlen("\\r")), 0);
     break;
    case '\t':
     smart_str_appendl_ex(((str)), (("\\t")), (strlen("\\t")), 0);
     break;
    case '\f':
     smart_str_appendl_ex(((str)), (("\\f")), (strlen("\\f")), 0);
     break;
    case '\v':
     smart_str_appendl_ex(((str)), (("\\v")), (strlen("\\v")), 0);
     break;
    ...
    case '\e':
     ...
     smart_str_appendl_ex(((str)), (("\\e")), (strlen("\\e")), 0);
     break;
    default:
     smart_str_appendl_ex(((str)), (("\\0")), (strlen("\\0")), 0);
     smart_str_appendc_ex((str), ('0' + (c / 8)), 0);
     smart_str_appendc_ex((str), ('0' + (c % 8)), 0);
     break;
   }
  } else {
...
static void zend_ast_export_ns_name(smart_str *str, zend_ast *ast, int priority, int indent)
{
 if (ast->kind == ZEND_AST_ZVAL) {
  ...
  if (zval_get_type(&(*(zv))) == 6) {
    ...
    smart_str_appendl_ex(((str)), (("namespace\\")), (strlen("namespace\\")), 0);
      }
   smart_str_append_ex((str), ((*(zv)).value.str), 0);
   return;
  }
...
static void zend_ast_export_var(smart_str *str, zend_ast *ast, int priority, int indent)
{
 if (ast->kind == ZEND_AST_ZVAL) {
  zval *zv = zend_ast_get_zval(ast);
  if (zval_get_type(&(*(zv))) == 6 &&
      zend_ast_valid_var_name(((*(zv)).value.str)->val, ((*(zv)).value.str)->len)) {
   smart_str_append_ex((str), ((*(zv)).value.str), 0);
   return;
  }
...
static void zend_ast_export_encaps_list(smart_str *str, char quote, zend_ast_list *list, int indent)
{
 ...
 while (i < list->children) {
  ...
  if (ast->kind == ZEND_AST_ZVAL) {
   ...
   do { if (__builtin_expect(!(zval_get_type(&(*(zv))) == 6), 0)) __builtin_unreachable(); } while (0);
   zend_ast_export_qstr(str, quote, (*(zv)).value.str);
  } else if (ast->kind == ZEND_AST_VAR &&
             ast->child[0]->kind == ZEND_AST_ZVAL &&
             (i + 1 == list->children ||
              list->child[i + 1]->kind != ZEND_AST_ZVAL ||
              !zend_ast_valid_var_char(
                  *((*(zend_ast_get_zval(list->child[i + 1]))).value.str)->val
                                                            ))) {
   zend_ast_export_ex(str, ast, 0, indent);
  } else {
   smart_str_appendc_ex((str), ('{'), 0);
...
tail_call:
 ...
 while (i < list->children) {
  ast = list->child[i];
  do { if (__builtin_expect(!(ast->kind == ZEND_AST_IF_ELEM), 0)) __builtin_unreachable(); } while (0);
  if (ast->child[0]) {
   if (i == 0) {
    smart_str_appendl_ex(((str)), (("if (")), (strlen("if (")), 0);
   } else {
    zend_ast_export_indent(str, indent);
    smart_str_appendl_ex(((str)), (("} elseif (")), (strlen("} elseif (")), 0);
   }
   ...
   zend_ast_export_indent(str, indent);
   smart_str_appendl_ex(((str)), (("} else ")), (strlen("} else ")), 0);
...
static void zend_ast_export_zval(smart_str *str, zval *zv, int priority, int indent)
{
 ...
 switch (zval_get_type(&(*(zv)))) {
  ...
  case 3:
   ...
   break;
  case 4:
   smart_str_append_long_ex((str), ((*(zv)).value.lval), 0);
   break;
  case 5:
   key = zend_strpprintf(0, "%.*G", (int) (executor_globals.precision), (*(zv)).value.dval);
   smart_str_appendl_ex((str), ((key)->val), ((key)->len), 0);
   zend_string_release(key);
   break;
  case 6:
   smart_str_appendc_ex((str), ('\''), 0);
   zend_ast_export_str(str, (*(zv)).value.str);
   smart_str_appendc_ex((str), ('\''), 0);
   break;
  case 7:
   smart_str_appendc_ex((str), ('['), 0);
   first = 1;
   do { Bucket *_p = ((*(zv)).value.arr)->arData; Bucket *_end = _p + ((*(zv)).value.arr)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; idx = _p->h; key = _p->key; val = _z; {
    if (first) {
     first = 0;
    } else {
     smart_str_appendl_ex(((str)), ((", ")), (strlen(", ")), 0);
    ...
    if (key) {
     ...
     smart_str_append_long_ex((str), (idx), 0);
     smart_str_appendl_ex(((str)), ((" => ")), (strlen(" => ")), 0);
    }
   ...
   break;
  case 11:
   smart_str_appendl_ex((str), (((*(zv)).value.str)->val), (((*(zv)).value.str)->len), 0);
   break;
  case 12:
   zend_ast_export_ex(str, (*(zv)).value.ast->ast, priority, indent);
   break;
  default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
 }
}
# 967 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_ast.c"
static void zend_ast_export_ex(smart_str *str, zend_ast *ast, int priority, int indent)
{
 zend_ast_decl *decl;
 int p, pl, pr;
...
tail_call:
 ...
 switch (ast->kind) {
  ...
  case ZEND_AST_METHOD:
   ...
   if (decl->flags & 0x100) {
    smart_str_appendl_ex(((str)), (("public ")), (strlen("public ")), 0);
   } else if (decl->flags & 0x200) {
    smart_str_appendl_ex(((str)), (("protected ")), (strlen("protected ")), 0);
   } else if (decl->flags & 0x400) {
    smart_str_appendl_ex(((str)), (("private ")), (strlen("private ")), 0);
   }
   if (decl->flags & 0x01) {
    smart_str_appendl_ex(((str)), (("static ")), (strlen("static ")), 0);
   }
   if (decl->flags & 0x02) {
    smart_str_appendl_ex(((str)), (("abstract ")), (strlen("abstract ")), 0);
   }
   if (decl->flags & 0x04) {
    smart_str_appendl_ex(((str)), (("final ")), (strlen("final ")), 0);
   }
   smart_str_appendl_ex(((str)), (("function ")), (strlen("function ")), 0);
   if (decl->flags & 0x4000000) {
    smart_str_appendc_ex((str), ('&'), 0);
   }
   if (ast->kind != ZEND_AST_CLOSURE) {
    smart_str_appendl_ex((str), ((decl->name)->val), ((decl->name)->len), 0);
   }
   smart_str_appendc_ex((str), ('('), 0);
  ...
  case ZEND_AST_CLASS:
   decl = (zend_ast_decl *) ast;
   if (decl->flags & 0x40) {
    smart_str_appendl_ex(((str)), (("interface ")), (strlen("interface ")), 0);
   } else if (decl->flags & 0x80) {
    smart_str_appendl_ex(((str)), (("trait ")), (strlen("trait ")), 0);
   } else {
    if (decl->flags & 0x20) {
     smart_str_appendl_ex(((str)), (("abstract ")), (strlen("abstract ")), 0);
    }
    ...
    smart_str_appendl_ex(((str)), (("class ")), (strlen("class ")), 0);
   }
   smart_str_appendl_ex((str), ((decl->name)->val), ((decl->name)->len), 0);
   if (decl->child[0]) {
    smart_str_appendl_ex(((str)), ((" extends ")), (strlen(" extends ")), 0);
    zend_ast_export_ns_name(str, decl->child[0], 0, indent);
   }
   if (decl->child[1]) {
    smart_str_appendl_ex(((str)), ((" implements ")), (strlen(" implements ")), 0);
    zend_ast_export_ex(str, decl->child[1], 0, indent);
   }
   smart_str_appendl_ex(((str)), ((" {\n")), (strlen(" {\n")), 0);
   zend_ast_export_stmt(str, decl->child[2], indent + 1);
...
simple_list:
   ...
   break;
  case ZEND_AST_LIST:
   smart_str_appendl_ex(((str)), (("list(")), (strlen("list(")), 0);
   zend_ast_export_list(str, (zend_ast_list*)ast, 1, 20, indent);
   smart_str_appendc_ex((str), (')'), 0);
   break;
  case ZEND_AST_ARRAY:
  ...
  case ZEND_AST_CLOSURE_USES:
   smart_str_appendl_ex(((str)), ((" use(")), (strlen(" use(")), 0);
  ...
  case ZEND_AST_PROP_DECL:
   if (ast->attr & 0x100) {
    smart_str_appendl_ex(((str)), (("public ")), (strlen("public ")), 0);
   } else if (ast->attr & 0x200) {
    smart_str_appendl_ex(((str)), (("protected ")), (strlen("protected ")), 0);
   } else if (ast->attr & 0x400) {
    smart_str_appendl_ex(((str)), (("private ")), (strlen("private ")), 0);
   }
   if (ast->attr & 0x01) {
    smart_str_appendl_ex(((str)), (("static ")), (strlen("static ")), 0);
  ...
  case ZEND_AST_CLASS_CONST_DECL:
   smart_str_appendl_ex(((str)), (("const ")), (strlen("const ")), 0);
   goto simple_list;
  ...
  case ZEND_AST_MAGIC_CONST:
   switch (ast->attr) {
    case 370: do { op = "__LINE__"; goto append_str; } while (0);
    case 371: do { op = "__FILE__"; goto append_str; } while (0);
    case 372: do { op = "__DIR__"; goto append_str; } while (0);
    case 374: do { op = "__TRAIT__"; goto append_str; } while (0);
    case 375: do { op = "__METHOD__"; goto append_str; } while (0);
    case 376: do { op = "__FUNCTION__"; goto append_str; } while (0);
    case 389: do { op = "__NAMESPACE__"; goto append_str; } while (0);
    case 373: do { op = "__CLASS__"; goto append_str; } while (0);
    default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
   }
   break;
  case ZEND_AST_TYPE:
   switch (ast->attr) {
    case 7: do { op = "array"; goto append_str; } while (0);
    case 14: do { op = "callable"; goto append_str; } while (0);
    default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
   }
   break;
  ...
  case ZEND_AST_UNPACK:
   ...
   ast = ast->child[0];
   goto tail_call;
  case ZEND_AST_UNARY_PLUS: do { op = "+"; p = 240; pl = 241; goto prefix_op; } while (0);
  case ZEND_AST_UNARY_MINUS: do { op = "-"; p = 240; pl = 241; goto prefix_op; } while (0);
  case ZEND_AST_CAST:
   switch (ast->attr) {
    case 1: do { op = "(unset)"; p = 240; pl = 241; goto prefix_op; } while (0);
    case 13: do { op = "(bool)"; p = 240; pl = 241; goto prefix_op; } while (0);
    case 4: do { op = "(int)"; p = 240; pl = 241; goto prefix_op; } while (0);
    case 5: do { op = "(double)"; p = 240; pl = 241; goto prefix_op; } while (0);
    case 6: do { op = "(string)"; p = 240; pl = 241; goto prefix_op; } while (0);
    case 7: do { op = "(array)"; p = 240; pl = 241; goto prefix_op; } while (0);
    case 8: do { op = "(object)"; p = 240; pl = 241; goto prefix_op; } while (0);
    default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
   }
   break;
  case ZEND_AST_EMPTY:
   do { op = "empty"; goto func_op; } while (0);
  case ZEND_AST_ISSET:
   do { op = "isset"; goto func_op; } while (0);
  case ZEND_AST_SILENCE:
   do { op = "@"; p = 240; pl = 241; goto prefix_op; } while (0);
  case ZEND_AST_SHELL_EXEC:
   ...
   break;
  case ZEND_AST_CLONE:
   do { op = "clone "; p = 270; pl = 271; goto prefix_op; } while (0);
  case ZEND_AST_EXIT:
   if (ast->child[0]) {
    do { op = "exit"; goto func_op; } while (0);
   } else {
    do { op = "exit"; goto append_str; } while (0);
   }
   break;
  case ZEND_AST_PRINT:
   do { op = "print "; p = 60; pl = 61; goto prefix_op; } while (0);
  case ZEND_AST_INCLUDE_OR_EVAL:
   switch (ast->attr) {
    case (1<<2): do { op = "include_once"; goto func_op; } while (0);
    case (1<<1): do { op = "include"; goto func_op; } while (0);
    case (1<<4): do { op = "require_once"; goto func_op; } while (0);
    case (1<<3): do { op = "require"; goto func_op; } while (0);
    case (1<<0): do { op = "eval"; goto func_op; } while (0);
    default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
   }
   break;
  case ZEND_AST_UNARY_OP:
   switch (ast->attr) {
    case 12: do { op = "~"; p = 240; pl = 241; goto prefix_op; } while (0);
    case 13: do { op = "!"; p = 240; pl = 241; goto prefix_op; } while (0);
    default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
   }
   break;
  ...
  case ZEND_AST_POST_INC:
   do { op = "++"; p = 240; pl = 241; goto postfix_op; } while (0);
  case ZEND_AST_POST_DEC:
   do { op = "--"; p = 240; pl = 241; goto postfix_op; } while (0);

  case ZEND_AST_GLOBAL:
   do { op = "global"; goto append_node_1; } while (0);
  case ZEND_AST_UNSET:
   do { op = "unset"; goto func_op; } while (0);
  case ZEND_AST_RETURN:
   do { op = "return"; goto append_node_1; } while (0);
  case ZEND_AST_LABEL:
  ...
  case ZEND_AST_HALT_COMPILER:
   do { op = "__HALT_COMPILER()"; goto append_str; } while (0);
  case ZEND_AST_ECHO:
   do { op = "echo"; goto append_node_1; } while (0);
  case ZEND_AST_THROW:
   do { op = "throw"; goto append_node_1; } while (0);
  case ZEND_AST_GOTO:
   smart_str_appendl_ex(((str)), (("goto ")), (strlen("goto ")), 0);
   zend_ast_export_name(str, ast->child[0], 0, indent);
   break;
  case ZEND_AST_BREAK:
   do { op = "break"; goto append_node_1; } while (0);
  case ZEND_AST_CONTINUE:
   do { op = "continue"; goto append_node_1; } while (0);
  ...
  case ZEND_AST_CLASS_CONST:
   ...
   break;
  case ZEND_AST_ASSIGN: do { op = " = "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
  case ZEND_AST_ASSIGN_REF: do { op = " =& "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
  case ZEND_AST_ASSIGN_OP:
   switch (ast->attr) {
    case 23: do { op = " += "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 24: do { op = " -= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 25: do { op = " *= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 26: do { op = " /= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 27: do { op = " %= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 28: do { op = " <<= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 29: do { op = " >>= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 30: do { op = " .= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 31: do { op = " |= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 32: do { op = " &= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 33: do { op = " ^= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    case 166: do { op = " **= "; p = 90; pl = 91; pr = 90; goto binary_op; } while (0);
    default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
   }
   break;
  case ZEND_AST_BINARY_OP:
   switch (ast->attr) {
    case 1: do { op = " + "; p = 200; pl = 200; pr = 201; goto binary_op; } while (0);
    case 2: do { op = " - "; p = 200; pl = 200; pr = 201; goto binary_op; } while (0);
    case 3: do { op = " * "; p = 210; pl = 210; pr = 211; goto binary_op; } while (0);
    case 4: do { op = " / "; p = 210; pl = 210; pr = 211; goto binary_op; } while (0);
    case 5: do { op = " % "; p = 210; pl = 210; pr = 211; goto binary_op; } while (0);
    case 6: do { op = " << "; p = 190; pl = 190; pr = 191; goto binary_op; } while (0);
    case 7: do { op = " >> "; p = 190; pl = 190; pr = 191; goto binary_op; } while (0);
    case 8: do { op = " . "; p = 200; pl = 200; pr = 201; goto binary_op; } while (0);
    case 9: do { op = " | "; p = 140; pl = 140; pr = 141; goto binary_op; } while (0);
    case 10: do { op = " & "; p = 160; pl = 160; pr = 161; goto binary_op; } while (0);
    case 11: do { op = " ^ "; p = 150; pl = 150; pr = 151; goto binary_op; } while (0);
    case 15: do { op = " === "; p = 170; pl = 171; pr = 171; goto binary_op; } while (0);
    case 16: do { op = " !== "; p = 170; pl = 171; pr = 171; goto binary_op; } while (0);
    case 17: do { op = " == "; p = 170; pl = 171; pr = 171; goto binary_op; } while (0);
    case 18: do { op = " != "; p = 170; pl = 171; pr = 171; goto binary_op; } while (0);
    case 19: do { op = " < "; p = 180; pl = 181; pr = 181; goto binary_op; } while (0);
    case 20: do { op = " <= "; p = 180; pl = 181; pr = 181; goto binary_op; } while (0);
    case 166: do { op = " ** "; p = 250; pl = 251; pr = 250; goto binary_op; } while (0);
    case 14: do { op = " xor "; p = 40; pl = 40; pr = 41; goto binary_op; } while (0);
    case 170: do { op = " <=> "; p = 180; pl = 181; pr = 181; goto binary_op; } while (0);
    default: do { if (__builtin_expect(!(0), 0)) __builtin_unreachable(); } while (0); break;;
   }
   break;
  case ZEND_AST_GREATER: do { op = " > "; p = 180; pl = 181; pr = 181; goto binary_op; } while (0);
  case ZEND_AST_GREATER_EQUAL: do { op = " >= "; p = 180; pl = 181; pr = 181; goto binary_op; } while (0);
  case ZEND_AST_AND: do { op = " && "; p = 130; pl = 130; pr = 131; goto binary_op; } while (0);
  case ZEND_AST_OR: do { op = " || "; p = 120; pl = 120; pr = 121; goto binary_op; } while (0);
  case ZEND_AST_ARRAY_ELEM:
   ...
   break;
  case ZEND_AST_NEW:
   smart_str_appendl_ex(((str)), (("new ")), (strlen("new ")), 0);
   zend_ast_export_ns_name(str, ast->child[0], 0, indent);
   smart_str_appendc_ex((str), ('('), 0);
   zend_ast_export_ex(str, ast->child[1], 0, indent);
   smart_str_appendc_ex((str), (')'), 0);
  ...
  case ZEND_AST_INSTANCEOF:
   zend_ast_export_ex(str, ast->child[0], 0, indent);
   smart_str_appendl_ex(((str)), ((" instanceof ")), (strlen(" instanceof ")), 0);
   zend_ast_export_ns_name(str, ast->child[1], 0, indent);
   break;
  case ZEND_AST_YIELD:
   if (priority > 70) smart_str_appendc_ex((str), ('('), 0);
   smart_str_appendl_ex(((str)), (("yield ")), (strlen("yield ")), 0);
   if (ast->child[0]) {
    if (ast->child[1]) {
  ...
  case ZEND_AST_YIELD_FROM:
   do { op = "yield from "; p = 85; pl = 86; goto prefix_op; } while (0);
  case ZEND_AST_COALESCE: do { op = " ?? "; p = 110; pl = 111; pr = 110; goto binary_op; } while (0);
  case ZEND_AST_STATIC:
   smart_str_appendl_ex(((str)), (("static $")), (strlen("static $")), 0);
   zend_ast_export_name(str, ast->child[0], 0, indent);
   do { p = 1; goto append_default_value; } while (0);
  case ZEND_AST_WHILE:
   smart_str_appendl_ex(((str)), (("while (")), (strlen("while (")), 0);
   zend_ast_export_ex(str, ast->child[0], 0, indent);
   smart_str_appendl_ex(((str)), ((") {\n")), (strlen(") {\n")), 0);
   zend_ast_export_stmt(str, ast->child[1], indent + 1);
   zend_ast_export_indent(str, indent);
  ...
  case ZEND_AST_IF_ELEM:
   ...
   break;
  case ZEND_AST_SWITCH:
   smart_str_appendl_ex(((str)), (("switch (")), (strlen("switch (")), 0);
   zend_ast_export_ex(str, ast->child[0], 0, indent);
   smart_str_appendl_ex(((str)), ((") {\n")), (strlen(") {\n")), 0);
  ...
  case ZEND_AST_SWITCH_CASE:
   zend_ast_export_indent(str, indent);
   if (ast->child[0]) {
    smart_str_appendl_ex(((str)), (("case ")), (strlen("case ")), 0);
    zend_ast_export_ex(str, ast->child[0], 0, indent);
    smart_str_appendl_ex(((str)), ((":\n")), (strlen(":\n")), 0);
   } else {
    smart_str_appendl_ex(((str)), (("default:\n")), (strlen("default:\n")), 0);
   }
   zend_ast_export_stmt(str, ast->child[1], indent + 1);
   break;
  case ZEND_AST_DECLARE:
   smart_str_appendl_ex(((str)), (("declare(")), (strlen("declare(")), 0);
   do { if (__builtin_expect(!(ast->child[0]->kind == ZEND_AST_CONST_DECL), 0)) __builtin_unreachable(); } while (0);
  ...
  case ZEND_AST_TRAIT_PRECEDENCE:
   zend_ast_export_ex(str, ast->child[0], 0, indent);
   smart_str_appendl_ex(((str)), ((" insteadof ")), (strlen(" insteadof ")), 0);
   zend_ast_export_ex(str, ast->child[1], 0, indent);
   break;
  ...
  case ZEND_AST_TRAIT_ALIAS:
   zend_ast_export_name(str, ast->child[0], 0, indent);
   if (ast->attr & 0x100) {
    smart_str_appendl_ex(((str)), ((" as public")), (strlen(" as public")), 0);
   } else if (ast->attr & 0x200) {
    smart_str_appendl_ex(((str)), ((" as protected")), (strlen(" as protected")), 0);
   } else if (ast->attr & 0x400) {
    smart_str_appendl_ex(((str)), ((" as private")), (strlen(" as private")), 0);
   } else if (ast->child[1]) {
    smart_str_appendl_ex(((str)), ((" as")), (strlen(" as")), 0);
   }
   if (ast->child[1]) {
    smart_str_appendc_ex((str), (' '), 0);
    zend_ast_export_name(str, ast->child[1], 0, indent);
  ...
  case ZEND_AST_STATIC_CALL:
   ...
   break;
  case ZEND_AST_CONDITIONAL:
   if (priority > 100) smart_str_appendc_ex((str), ('('), 0);
  ...
  case ZEND_AST_TRY:
   smart_str_appendl_ex(((str)), (("try {\n")), (strlen("try {\n")), 0);
   zend_ast_export_stmt(str, ast->child[0], indent + 1);
   zend_ast_export_indent(str, indent);
   zend_ast_export_ex(str, ast->child[1], 0, indent);
   if (ast->child[2]) {
    smart_str_appendl_ex(((str)), (("} finally {\n")), (strlen("} finally {\n")), 0);
   ...
   break;
  case ZEND_AST_CATCH:
   smart_str_appendl_ex(((str)), (("} catch (")), (strlen("} catch (")), 0);
   zend_ast_export_ns_name(str, ast->child[0], 0, indent);
   smart_str_appendl_ex(((str)), ((" $")), (strlen(" $")), 0);
   zend_ast_export_var(str, ast->child[1], 0, indent);
   smart_str_appendl_ex(((str)), ((") {\n")), (strlen(") {\n")), 0);
  ...
  case ZEND_AST_PARAM:
   ...
   smart_str_appendc_ex((str), ('$'), 0);
   zend_ast_export_name(str, ast->child[1], 0, indent);
   do { p = 2; goto append_default_value; } while (0);


  case ZEND_AST_FOR:
   smart_str_appendl_ex(((str)), (("for (")), (strlen("for (")), 0);
   zend_ast_export_ex(str, ast->child[0], 0, indent);
   smart_str_appendc_ex((str), (';'), 0);
   if (ast->child[1]) {
    smart_str_appendc_ex((str), (' '), 0);
   ...
   break;
  case ZEND_AST_FOREACH:
   smart_str_appendl_ex(((str)), (("foreach (")), (strlen("foreach (")), 0);
   zend_ast_export_ex(str, ast->child[0], 0, indent);
   smart_str_appendl_ex(((str)), ((" as ")), (strlen(" as ")), 0);
   if (ast->child[2]) {
    zend_ast_export_ex(str, ast->child[2], 0, indent);
...
binary_op:
 if (priority > p) smart_str_appendc_ex((str), ('('), 0);
 zend_ast_export_ex(str, ast->child[0], pl, indent);
 smart_str_appendl_ex(((str)), ((op)), (strlen(op)), 0);
...
__attribute__ ((visibility("default"))) zend_string *zend_ast_export(const char *prefix, zend_ast *ast, const char *suffix)
{
 ...
 smart_str_appendl_ex(((&str)), ((prefix)), (strlen(prefix)), 0);
 zend_ast_export_ex(&str, ast, 0, 0);
 smart_str_appendl_ex(((&str)), ((suffix)), (strlen(suffix)), 0);
 smart_str_0(&str);
 return str.s;
}
