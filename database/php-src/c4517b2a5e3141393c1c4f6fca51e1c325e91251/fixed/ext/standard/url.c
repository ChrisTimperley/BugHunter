...
# 483 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/url.c"
...
__attribute__ ((visibility("default"))) zend_string *php_url_encode(char const *s, size_t len)
{
 ...
 while (from < end) {
   ...
   *to++ = '+';
   ...
   to[2] = hexchars[c & 15];
   to += 3;
# 521 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/url.c"
  } else {
   *to++ = c;
  }
 }
...
void zif_get_headers(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if (zend_parse_parameters((execute_data)->This.u2.num_args, "s|l", &url, &url_len, &format) == FAILURE) {
  return;
 }
