...
# 91 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_hash.c"
...
# 116 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_hash.c"
 return 0x2 << (__builtin_clz(nSize - 1) ^ 0x1f);
# 126 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_hash.c"
}
...
__attribute__ ((visibility("default"))) HashPosition zend_hash_iterator_pos(uint32_t idx, HashTable *ht)
{
 ...
 if (iter->pos == ((uint32_t) -1)) {
  ...
  if (__builtin_expect(!!(iter->ht), 1) && __builtin_expect(!!(iter->ht->u.v.nIteratorsCount != 255), 1)) {
...
static __attribute__((noinline)) void _zend_hash_iterators_remove(HashTable *ht)
{
 ...
 idx = (executor_globals.ht_iterators_used);
 while (idx > 0 && (executor_globals.ht_iterators)[idx - 1].ht == ((void *)0)) {
  idx--;
 }
 (executor_globals.ht_iterators_used) = idx;
...
# 1487 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_hash.c"
__attribute__ ((visibility("default"))) void zend_hash_apply(HashTable *ht, apply_func_t apply_func)
{
 uint32_t idx;
 Bucket *p;
