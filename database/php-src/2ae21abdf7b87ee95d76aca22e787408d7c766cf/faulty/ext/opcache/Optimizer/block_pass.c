...
# 908 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.h"
...
# 977 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.h"

# 1037 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.h"
...
# 114 "ext/opcache/Optimizer/zend_cfg.h"
...
# 44 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/Optimizer/zend_dfg.h"
...
static void strip_leading_nops(zend_op_array *op_array, zend_basic_block *b)
{
 ...
 while (b->len > 0 && opcodes[b->start].opcode == 0) {
  b->start++;
  b->len--;
 }
}
  ...
  if ((opline->op1_type & ((1<<1)|(1<<2))) &&
   ...
   if (src &&
    ...
    if (zend_optimizer_update_op1_const(op_array, opline, &c)) {
     ...
     do { _zval_ptr_dtor_nogc((&(op_array)->literals[(src)->op1.constant]) ); do { (*(&(op_array)->literals[(src)->op1.constant])).u1.type_info = 1; } while (0); } while (0);
  ...
  if (opline->opcode == 40) {
   ...
   if (opline->op1_type == (1<<0)) {
    if (last_op && last_op->opcode == 40 &&
     ...
     _zval_ptr_dtor_nogc((&(op_array)->literals[(opline)->op1.constant]) );
...
# 528 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/Optimizer/block_pass.c"
   ...
   case 53:
    ...
    if (opline->op1_type == (1<<0) &&
     ...
     do { _zval_ptr_dtor_nogc((&(op_array)->literals[(opline)->op1.constant]) ); do { (*(&(op_array)->literals[(opline)->op1.constant])).u1.type_info = 1; } while (0); } while (0);
     opline->opcode = 21;
     ...
     do { _zval_ptr_dtor_nogc((&(op_array)->literals[(opline)->op2.constant]) ); do { (*(&(op_array)->literals[(opline)->op2.constant])).u1.type_info = 1; } while (0); } while (0);
     opline->opcode = 21;
     opline->extended_value = 6;
     opline->op2_type = (1<<3);
     opline->op2.var = 0;
...
optimize_constant_binary_op:
     binary_op = get_binary_op(opline->opcode);
      ...
      Tsource[((uint32_t)(((zval*)(((char*)(((void *)0))) + ((int)(opline->result.var)))) - (((zval*)(((void *)0))) + (((int)(((((sizeof(zend_execute_data)) + 8 - 1L) & ~(8 - 1L)) + (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)) - 1) / (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)))) + ((int)(0))))))] = opline;
                  opline++;
      continue;
     } else if (zend_binary_op_produces_numeric_string_error(opline->opcode, &(op_array)->literals[(opline)->op1.constant], &(op_array)->literals[(opline)->op2.constant])) {
      Tsource[((uint32_t)(((zval*)(((char*)(((void *)0))) + ((int)(opline->result.var)))) - (((zval*)(((void *)0))) + (((int)(((((sizeof(zend_execute_data)) + 8 - 1L) & ~(8 - 1L)) + (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)) - 1) / (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)))) + ((int)(0))))))] = opline;
                  opline++;
      continue;
     }
 ...
 for (b = blocks; b < end; b++) {
  ...
  if (b->flags & (1<<31)) {
   ...
   if (opline->opcode == 42) {
    ...
    while (next < end && !(next->flags & (1<<31))) {
     next++;
    }
   ...
   } else if (b->len == 1 && opline->opcode == 0) {
   ...
   for (; op < end; op++) {
    if ((op)->op1_type == (1<<0)) {
     do { _zval_ptr_dtor_nogc((&(op_array)->literals[(op)->op1.constant]) ); do { (*(&(op_array)->literals[(op)->op1.constant])).u1.type_info = 1; } while (0); } while (0);
    }
    if ((op)->op2_type == (1<<0)) {
     do { _zval_ptr_dtor_nogc((&(op_array)->literals[(op)->op2.constant]) ); do { (*(&(op_array)->literals[(op)->op2.constant])).u1.type_info = 1; } while (0); } while (0);
    }
   }
  }
 }
...
# 1111 "/home/ct584/git/bughunter/repositories/php-src/ext/opcache/Optimizer/block_pass.c"
    }
   }
   break;
  ...
  case 44:
   ...
   if ((last_op)->op1_type == (1<<0)) {
    ...
    do { _zval_ptr_dtor_nogc((&(op_array)->literals[(last_op)->op1.constant]) ); do { (*(&(op_array)->literals[(last_op)->op1.constant])).u1.type_info = 1; } while (0); } while (0);
...
next_target:
    ...
    if (target == target_end) {
     ...
     last_op->opcode += 3;
     do { last_op->result_type = target->result_type; last_op->result = target->result; } while (0);
     ;
     block->successors[0] = target_block->successors[1];
     ;
...
static void zend_t_usage(zend_cfg *cfg, zend_op_array *op_array, zend_bitset used_ext, zend_optimizer_ctx *ctx)
{
 ...
 for (n = cfg->blocks_count; n > 0;) {
  ...
  if (!(block->flags & (1<<31)) || block->len == 0) {
   continue;
  }
 ...
 for (i = 0; i < cfg->blocks_count; i++) {
  ...
  if (b->flags & (1<<31)) {
       ...
       prev->successors[0] == i && prev->successors[1] == -1)
   {
    ...
    if (prev->len != 0 && last_op->opcode == 42) {
     do { (last_op)->op1.num = 0; (last_op)->op2.num = 0; (last_op)->result.num = 0; (last_op)->opcode = 0; (last_op)->op1_type = (1<<3); (last_op)->op2_type = (1<<3); (last_op)->result_type = (1<<3); } while (0);
    }
