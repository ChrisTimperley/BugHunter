# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.c"
# 21 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.c"
...
# 22 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.c" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.h" 1
# 22 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.h"
...
static int phar_dir_seek( php_stream *stream, off_t offset, int whence, off_t *newoffset );




# 23 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.c" 2
...
static int phar_dir_seek(php_stream *stream, off_t offset, int whence, off_t *newoffset )
{
 ...
 if (whence == 0) {
  zend_hash_internal_pointer_reset_ex(data, ((void *)0));
 }
  ...
  *newoffset = 0;
  while (*newoffset < offset && zend_hash_move_forward_ex(data, ((void *)0)) == SUCCESS) {
   ++(*newoffset);
  }
  return 0;
 }
...
static size_t phar_dir_read(php_stream *stream, char *buf, size_t count )
{
 ...
 if (3 == zend_hash_get_current_key_ex(data, &str_key, &keylen, &unused, 0, ((void *)0))) {
  return 0;
 }

 zend_hash_move_forward_ex(data, ((void *)0));
 to_read = (((keylen)<(count))?(keylen):(count));
...
# 144 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/dirstream.c"
static int phar_add_empty(HashTable *ht, char *arKey, uint nKeyLength)
{
 void *dummy = (char *) 1;

 return _zend_hash_add_or_update(ht, arKey, nKeyLength, (void *) &dummy, sizeof(void *), ((void *)0), (1<<0) );
}
...
static php_stream *phar_make_dirstream(char *dir, HashTable *manifest )
{
 ...
 (data) = (HashTable *) _emalloc((sizeof(HashTable)) );
 _zend_hash_init((data), (64), (((void *)0)), (0) );

 if ((*dir == '/' && dirlen == 1 && (manifest->nNumOfElements == 0)) || (dirlen >= sizeof(".phar")-1 && !memcmp(dir, ".phar", sizeof(".phar")-1))) {


  _efree((dir) );
  return _php_stream_alloc((&phar_dir_ops), (data), (((void *)0)), ("r") );
 }

 zend_hash_internal_pointer_reset_ex(manifest, ((void *)0));

 while (FAILURE != (zend_hash_get_current_key_type_ex(manifest, ((void *)0)) == 3 ? FAILURE : SUCCESS)) {
  if (3 == zend_hash_get_current_key_ex(manifest, &str_key, &keylen, &unused, 0, ((void *)0))) {
   break;
  }

  if (keylen <= (uint)dirlen) {
   if (keylen < (uint)dirlen || !strncmp(str_key, dir, dirlen)) {
    if (SUCCESS != zend_hash_move_forward_ex(manifest, ((void *)0))) {
     break;
    }
    continue;
   }
  ...
  if (*dir == '/') {
   ...
   if (((void *)0) != (found = (char *) memchr(str_key, '/', keylen))) {

    entry = (char *) _safe_emalloc((found - str_key), (1), (1) );
    memcpy(entry, str_key, found - str_key);
    keylen = found - str_key;
    entry[keylen] = '\0';
   } else {
    entry = (char *) _safe_emalloc((keylen), (1), (1) );
  ...
  if (((void *)0) != (found = (char *) memchr(save, '/', keylen - dirlen - 1))) {
   ...
   save -= dirlen + 1;
   entry = (char *) _safe_emalloc((found - save + dirlen), (1), (1) );
   ...
   save -= dirlen + 1;
   entry = (char *) _safe_emalloc((keylen - dirlen), (1), (1) );
...
PHAR_ADD_ENTRY:
 ...
 if (FAILURE != (zend_hash_get_current_key_type_ex(data, ((void *)0)) == 3 ? FAILURE : SUCCESS)) {
  _efree((dir) );
...
php_stream *phar_wrapper_open_dir(php_stream_wrapper *wrapper, const char *path, const char *mode, int options, char **opened_path, php_stream_context *context )
{
 ...
 if (*internal_file == '\0') {
  ...
  internal_file = _estrndup((internal_file - 1), (1) );
 ...
 if (SUCCESS == zend_hash_find(&phar->manifest, internal_file, strlen(internal_file), (void**)&entry) && !entry->is_dir) {
  ...
  if (entry->is_mounted) {
   php_url_free(resource);
   return _php_stream_opendir((entry->tmp), (options), (context) );
  }
  internal_file = _estrdup((internal_file) );
  php_url_free(resource);
  ...
  zend_hash_internal_pointer_reset_ex(&phar->manifest, ((void *)0));
  while (FAILURE != (zend_hash_get_current_key_type_ex(&phar->manifest, ((void *)0)) == 3 ? FAILURE : SUCCESS)) {
   if (3 !=
     zend_hash_get_current_key_ex(
      &phar->manifest, &str_key, &keylen, &unused, 0, ((void *)0))) {
    if (keylen > (uint)i_len && 0 == memcmp(str_key, internal_file, i_len)) {

     internal_file = _estrndup((internal_file), (i_len) )
             ;
     php_url_free(resource);
     return phar_make_dirstream(internal_file, &phar->manifest );
    }
   ...
   if (SUCCESS != zend_hash_move_forward_ex(&phar->manifest, ((void *)0))) {
    break;
   }
  }
 }
...
int phar_wrapper_mkdir(php_stream_wrapper *wrapper, const char *url_from, int mode, int options, php_stream_context *context )
{
 ...
 entry.filename = _estrdup((resource->path + 1) );
 ...
 if (error) {
  ...
  zend_hash_del_key_or_index(&phar->manifest, entry.filename, entry.filename_len, 0, 0);
  _efree((error) );
  return 0;
 }
...
int phar_wrapper_rmdir(php_stream_wrapper *wrapper, const char *url, int options, php_stream_context *context )
{
 ...
 if ((resource = phar_parse_url(wrapper, url, "w", options )) == ((void *)0)) {
  return 0;
 }
 ...
 if (!entry->is_deleted) {
  for (zend_hash_internal_pointer_reset_ex(&phar->manifest, ((void *)0));
   3 != zend_hash_get_current_key_ex(&phar->manifest, &str_key, &key_len, &unused, 0, ((void *)0));
  ...
  for (zend_hash_internal_pointer_reset_ex(&phar->virtual_dirs, ((void *)0));
   3 != zend_hash_get_current_key_ex(&phar->virtual_dirs, &str_key, &key_len, &unused, 0, ((void *)0));
 ...
 if (entry->is_temp_dir) {
  zend_hash_del_key_or_index(&phar->virtual_dirs, resource->path+1, path_len, 0, 0);
  _efree((entry->filename) );
  _efree((entry) );
 } else {
  entry->is_deleted = 1;
