...
int phar_parse_tarfile(php_stream* fp, char *fname, int fname_len, char *alias, int alias_len, phar_archive_data** pphar, int is_data, php_uint32 compression, char **error )
{
 ...
 _zend_hash_init((&myphar->manifest), (2 + (totalsize >> 12)), (destroy_phar_manifest_entry), ((zend_bool)myphar->is_persistent) )
                                                                                     ;
 _zend_hash_init((&myphar->mounted_dirs), (5), (((void *)0)), ((zend_bool)myphar->is_persistent) )
                                                              ;
 _zend_hash_init((&myphar->virtual_dirs), (4 + (totalsize >> 11)), (((void *)0)), ((zend_bool)myphar->is_persistent) )
                                                              ;
 myphar->is_tar = 1;
...
# 296 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/tar.c"
  ...
  if (!last_was_longlink && hdr->typeflag == 'L') {
   ...
   if (entry.filename_len == (2147483647 * 2U + 1U)) {
    if (error) {
  ...
  if (entry.tar_type == '1') {
   ...
   entry.link = _estrdup((hdr->linkname) );
...
# 1212 "/home/ct584/git/bughunter/repositories/php-src/ext/phar/tar.c"
