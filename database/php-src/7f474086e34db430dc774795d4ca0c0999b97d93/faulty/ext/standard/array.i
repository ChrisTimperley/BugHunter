...
void zif_extract(zend_execute_data *execute_data, zval *return_value)
{
 ...
 do { const int _flags = (0); int _min_num_args = (1); int _max_num_args = (3); int _num_args = (execute_data)->This.u2.num_args; int _i; zval *_real_arg, *_arg = ((void *)0); zend_expected_type _expected_type = 0; char *_error = ((void *)0); zend_bool _dummy; zend_bool _optional = 0; int error_code = 0; ((void)_i); ((void)_real_arg); ((void)_arg); ((void)_expected_type); ((void)_error); ((void)_dummy); ((void)_optional); do { if (__builtin_expect(!!(_num_args < _min_num_args), 0) || (__builtin_expect(!!(_num_args > _max_num_args), 0) && __builtin_expect(!!(_max_num_args >= 0), 1))) { if (!(_flags & (1<<1))) { zend_wrong_paramers_count_error(_num_args, _min_num_args, _max_num_args); } error_code = 1; break; } _i = 0; _real_arg = (((zval*)(execute_data)) + (((int)(((((sizeof(zend_execute_data)) + 8 - 1L) & ~(8 - 1L)) + (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)) - 1) / (((sizeof(zval)) + 8 - 1L) & ~(8 - 1L)))) + ((int)(((int)(0)) - 1))));
  ++_i; do { if (__builtin_expect(!(_i <= _min_num_args || _optional==1), 0)) __builtin_unreachable(); } while (0); do { if (__builtin_expect(!(_i > _min_num_args || _optional==0), 0)) __builtin_unreachable(); } while (0); if (_optional) { if (__builtin_expect(!!(_i >_num_args), 0)) break; } _real_arg++; _arg = _real_arg; do { if (__builtin_expect(!!((zval_get_type(&(*(_arg))) == 10)), 0)) { (_arg) = &(*(_arg)).value.ref->val; } } while (0); if (0) { do { zval *_zv = (_arg); do { if (__builtin_expect(!(zval_get_type(&(*(_zv))) != 10), 0)) __builtin_unreachable(); } while (0); if ((((*(_zv)).u1.v.type_flags & (1<<4)) != 0) || (((*(_zv)).u1.v.type_flags & (1<<1)) != 0)) { if (zval_refcount_p(_zv) > 1) { if (!(((*(_zv)).u1.v.type_flags & (1<<1)) != 0)) { zval_delref_p(_zv); } _zval_copy_ctor_func(_zv ); } } } while (0); }; if (__builtin_expect(!!(!zend_parse_arg_array(_arg, &var_array, 0, 0)), 0)) { _expected_type = Z_EXPECTED_ARRAY; error_code = 4; break; }
  _optional = 1;
 ...
 if (extract_refs) {
  do { zval *_zv = (var_array); if ((((*(_zv)).u1.v.type_flags & (1<<2)) != 0) || (((*(_zv)).u1.v.type_flags & (1<<1)) != 0)) { if (zval_refcount_p(_zv) > 1) { if ((((*(_zv)).u1.v.type_flags & (1<<4)) != 0) || (((*(_zv)).u1.v.type_flags & (1<<1)) != 0)) { if (!(((*(_zv)).u1.v.type_flags & (1<<1)) != 0)) { zval_delref_p(_zv); } _zval_copy_ctor_func(_zv ); } else if ((zval_get_type(&(*(_zv))) == 10)) { zval_delref_p(_zv); do { zval *_z1 = (_zv); const zval *_z2 = (&(*(_zv)).value.ref->val); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & (((1<<2)|(1<<1)) << 8)) != 0) { if ((_t & (((1<<4)|(1<<1)) << 8)) != 0) { _zval_copy_ctor_func(_z1 ); } else { (_gc)->gc.refcount++; } } } while (0); } } } } while (0);
 }
 extract_type &= 0xff;
...
# 2086 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
void zif_range(zend_execute_data *execute_data, zval *return_value)
{
