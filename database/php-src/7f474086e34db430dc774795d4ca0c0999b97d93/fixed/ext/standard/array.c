...
static void php_usort(zend_execute_data *execute_data, zval *return_value, compare_func_t compare_func, zend_bool renumber)
{
 ...
 if (zend_parse_parameters((execute_data)->This.u2.num_args, "a/f", &array, &(basic_globals.user_compare_fci), &(basic_globals.user_compare_fci_cache)) == FAILURE) {
 ...
 zval_addref_p(array);
 arr = (*(array)).value.counted;

 retval = zend_hash_sort_ex((*(array)).value.arr, zend_sort, compare_func, renumber) != FAILURE;

 if (arr != (*(array)).value.counted) {
...
void zif_array_walk(zend_execute_data *execute_data, zval *return_value)
{
 ...
 orig_array_walk_fci = (basic_globals.array_walk_fci);
 orig_array_walk_fci_cache = (basic_globals.array_walk_fci_cache);
# 1542 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
# 1747 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
    return 0;
   }
...
# 1831 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
...
void zif_array_fill(zend_execute_data *execute_data, zval *return_value)
{
 ...
 if (zend_parse_parameters((execute_data)->This.u2.num_args, "llz", &start_key, &num, &val) == FAILURE) {
  return;
 }
 ...
 num--;
 _zend_hash_index_update((*(return_value)).value.arr, start_key, val );
 do { if ((((*((val))).u1.v.type_flags & (1<<2)) != 0)) { zval_addref_p((val)); } } while (0);

 while (num--) {
  if (_zend_hash_next_index_insert((*(return_value)).value.arr, val ) != ((void *)0)) {
   do { if ((((*((val))).u1.v.type_flags & (1<<2)) != 0)) { zval_addref_p((val)); } } while (0);
  } else {
...
# 2092 "/home/ct584/git/bughunter/repositories/php-src/ext/standard/array.c"
void zif_range(zend_execute_data *execute_data, zval *return_value)
{
