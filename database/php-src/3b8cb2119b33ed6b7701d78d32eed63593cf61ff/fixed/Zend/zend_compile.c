...
# 585 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
void zend_stop_lexing(void)
{
...
__attribute__ ((visibility("default"))) zend_class_entry *do_bind_inherited_class(const zend_op_array *op_array, const zend_op *opline, HashTable *class_table, zend_class_entry *parent_ce, zend_bool compile_time)
{
 ...
 if (!ce) {
  if (!compile_time) {
   ...
   zend_error_noreturn((1<<6L), "Cannot declare  %s, because the name is already in use", zend_get_object_type(((*(lcname)).value.obj->ce)));
  }
  return ((void *)0);
 }
...
__attribute__ ((visibility("default"))) void zend_initialize_class_data(zend_class_entry *ce, zend_bool nullify_handlers)
{
 ...
 if (ce->type == 1) {
# 1760 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
...
# 1882 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
 if (len == 0) {
 ...
 if (end < path) {
# 1914 "/home/ct584/git/bughunter/repositories/php-src/Zend/zend_compile.c"
  path[0] = '.';
  path[1] = '\0';
  return 1 + len_adjust;
...
static void zend_compile_simple_var(znode *result, zend_ast *ast, uint32_t type, int delayed)
{
 ...
 if (is_this_fetch(ast)) {
  opline = zend_emit_op(result, 184, ((void *)0), ((void *)0));
...
static void zend_verify_list_assign_target(zend_ast *var_ast, zend_bool old_style) {
 if (var_ast->kind == ZEND_AST_ARRAY) {
  if (var_ast->attr == 2) {
   zend_error_noreturn((1<<6L), "Cannot assign to array(), use [] instead");
  }
  if (old_style != var_ast->attr) {
   zend_error_noreturn((1<<6L), "Cannot mix [] and list()");
  }
 } else if (!zend_can_write_to_variable(var_ast)) {
  zend_error_noreturn((1<<6L), "Assignments can only happen to writable values");
 }
}
...
void zend_compile_assign_ref(znode *result, zend_ast *ast)
{
 ...
 if ((target_ast->kind != ZEND_AST_VAR
  ...
  zend_emit_op(&source_node, 51, &source_node, ((void *)0));
 }
...
__attribute__ ((visibility("default"))) zend_uchar zend_get_call_op(const zend_op *init_op, zend_function *fbc)
{
 if (fbc) {
  if (fbc->type == 1) {
   if (init_op->opcode == 61 && !zend_execute_internal) {
   ...
   if (zend_execute_ex == execute_ex && !(fbc->common.fn_flags & 0x02)) {
    return 130;
   }
  }
...
void zend_compile_global_var(zend_ast *ast)
{
 ...
 if (is_this_fetch(var_ast)) {
  zend_error_noreturn((1<<6L), "Cannot use $this as global variable");
...
static void zend_compile_static_var_common(zend_ast *var_ast, zval *value, zend_bool by_ref)
{
 ...
 if ((((var_node.u.constant).value.str)->len == sizeof("this")-1 && !memcmp(((var_node.u.constant).value.str)->val, "this", sizeof("this") - 1))) {
  zend_error_noreturn((1<<6L), "Cannot use $this as static variable");
 }
...
void zend_compile_unset(zend_ast *ast)
{
 ...
 switch (var_ast->kind) {
  case ZEND_AST_VAR:
   if (is_this_fetch(var_ast)) {
    zend_error_noreturn((1<<6L), "Cannot unset $this");
...
{
 return zend_has_finally_ex(zend_stack_count(&(compiler_globals.loop_var_stack)) + 1);
}
...
void zend_compile_return(zend_ast *ast)
{
 ...
 if (((compiler_globals.active_op_array)->fn_flags & 0x20000000)
  && (expr_node.op_type == (1<<4) || (by_ref && expr_node.op_type == (1<<2)))
  && zend_has_finally()) {

  if (by_ref) {
   zend_emit_op(&expr_node, 51, &expr_node, ((void *)0));
  } else {
   zend_emit_op_tmp(&expr_node, 22, &expr_node, ((void *)0));
  }
 }
...
void zend_compile_try(zend_ast *ast)
{
 ...
 for (i = 0; i < catches->children; ++i) {
  ...
  for (j = 0; j < classes->children; j++) {
   ...
   if ((((*(var_name)).value.str)->len == sizeof("this")-1 && !memcmp(((*(var_name)).value.str)->val, "this", sizeof("this") - 1))) {
    zend_error_noreturn((1<<6L), "Cannot re-assign $this");
   }
   ...
   if (type_ast->kind == ZEND_AST_TYPE) {
    ...
    if (default_ast && !has_null_default && !(((default_node.u.constant).u1.v.type_flags & (1<<0)) != 0)) {
     if (arg_info->class_name) {
      ...
      case 19:
       if (zval_get_type(&(default_node.u.constant)) != 7) {
        zend_error_noreturn((1<<6L), "Default value for parameters "
...
void zend_compile_isset_or_empty(znode *result, zend_ast *ast)
{
 ...
 switch (var_ast->kind) {
  case ZEND_AST_VAR:
   if (is_this_fetch(var_ast)) {
    opline = zend_emit_op(result, 186, ((void *)0), ((void *)0));
   } else if (zend_try_compile_cv(&var_node, var_ast) == SUCCESS) {
