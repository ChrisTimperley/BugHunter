# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c"
# 21 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c"
...
# 22 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c" 2
...
# 25 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c" 2
...
# 26 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c" 2
...
# 27 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c" 2
...
__attribute__ ((visibility("default"))) const char *phpdbg_get_param_type(const phpdbg_param_t *param)
{
 switch (param->type) {
  case STACK_PARAM:
   return "stack";
...
__attribute__ ((visibility("default"))) void phpdbg_clear_param(phpdbg_param_t *param)
{
 if (param) {
  switch (param->type) {
   case FILE_PARAM:
    _efree((param->file.name) );
    break;
   case METHOD_PARAM:
    _efree((param->method.class) );
    _efree((param->method.name) );
    break;
   case STR_PARAM:
    _efree((param->str) );
    break;
...
__attribute__ ((visibility("default"))) char* phpdbg_param_tostring(const phpdbg_param_t *param, char **pointer)
{
 switch (param->type) {
  case STR_PARAM:
   (({ __typeof__ (asprintf(pointer, "%s", param->str)) __x = (asprintf(pointer, "%s", param->str)); (void) __x; }));
  break;

  case ADDR_PARAM:
   (({ __typeof__ (asprintf(pointer, "%" "l" "u", param->addr)) __x = (asprintf(pointer, "%" "l" "u", param->addr)); (void) __x; }));
  break;

  case NUMERIC_PARAM:
   (({ __typeof__ (asprintf(pointer, "%li", param->num)) __x = (asprintf(pointer, "%li", param->num)); (void) __x; }));
  break;

  case METHOD_PARAM:
   (({ __typeof__ (asprintf(pointer, "%s::%s", param->method.class, param->method.name)) __x = (asprintf(pointer, "%s::%s", param->method.class, param->method.name)); (void) __x; }));
  break;

  case FILE_PARAM:
   if (param->num) {
    (({ __typeof__ (asprintf(pointer, "%s:%lu#%lu", param->file.name, param->file.line, param->num)) __x = (asprintf(pointer, "%s:%lu#%lu", param->file.name, param->file.line, param->num)); (void) __x; }));
   } else {
    (({ __typeof__ (asprintf(pointer, "%s:%lu", param->file.name, param->file.line)) __x = (asprintf(pointer, "%s:%lu", param->file.name, param->file.line)); (void) __x; }));
   }
  break;

  case NUMERIC_FUNCTION_PARAM:
   (({ __typeof__ (asprintf(pointer, "%s#%lu", param->str, param->num)) __x = (asprintf(pointer, "%s#%lu", param->str, param->num)); (void) __x; }));
  break;

  case NUMERIC_METHOD_PARAM:
   (({ __typeof__ (asprintf(pointer, "%s::%s#%lu", param->method.class, param->method.name, param->num)) __x = (asprintf(pointer, "%s::%s#%lu", param->method.class, param->method.name, param->num)); (void) __x; }));
  break;
...
__attribute__ ((visibility("default"))) void phpdbg_copy_param(const phpdbg_param_t* src, phpdbg_param_t* dest)
{
 switch ((dest->type = src->type)) {
  ...
  case STR_PARAM:
   dest->str = _estrndup((src->str), (src->len) );
   dest->len = src->len;
  break;
  ...
  case METHOD_PARAM:
   dest->method.class = _estrdup((src->method.class) );
   dest->method.name = _estrdup((src->method.name) );
  break;

  case NUMERIC_FILE_PARAM:
  case FILE_PARAM:
   dest->file.name = _estrdup((src->file.name) );
...
__attribute__ ((visibility("default"))) zend_ulong phpdbg_hash_param(const phpdbg_param_t *param)
{
...
__attribute__ ((visibility("default"))) zend_bool phpdbg_match_param(const phpdbg_param_t *l, const phpdbg_param_t *r)
{
 if (l && r) {
  if (l->type == r->type) {
   switch (l->type) {
...
__attribute__ ((visibility("default"))) void phpdbg_param_debug(const phpdbg_param_t *param, const char *msg) {
 if (param && param->type) {
  switch (param->type) {
   ...
   case ADDR_PARAM:
    fprintf(stderr, "%s ADDR_PARAM(" "%" "l" "u" ")\n", msg, param->addr);
   break;
   ...
   switch (remove->type) {
    ...
    case METHOD_PARAM:
     if (remove->method.class) {
      _efree((remove->method.class) );
     }
     if (remove->method.name) {
      _efree((remove->method.name) );
     }
    break;
...
__attribute__ ((visibility("default"))) int phpdbg_stack_verify(const phpdbg_command_t *command, phpdbg_param_t **stack) {
 if (command) {
  char buffer[128] = {0,};
  const phpdbg_param_t *top = (stack != ((void *)0)) ? *stack : ((void *)0);
  ...
  if (!(arg) || !(*arg)) {
   ...
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"toomanyargs\" command=\"%s\" expected=\"0\"", "The command \"%s\" expected no arguments", phpdbg_command_name(command, buffer))
                                         ;
   return FAILURE;
  }
  ...
  arg = command->args;
# 519 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c"
  while (arg && *arg) {
   if (top && top->type == STACK_PARAM) {
    break;
   }
   ...
   switch (*arg) {
    ...
    case 'i': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("raw input"), current); return FAILURE; } } else if ((top)->type != (STR_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("raw input"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 's': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("string"), current); return FAILURE; } } else if ((top)->type != (STR_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("string"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 'n': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("number"), current); return FAILURE; } } else if ((top)->type != (NUMERIC_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("number"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 'm': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("method"), current); return FAILURE; } } else if ((top)->type != (METHOD_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("method"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 'a': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("address"), current); return FAILURE; } } else if ((top)->type != (ADDR_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("address"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 'f': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("file:line"), current); return FAILURE; } } else if ((top)->type != (FILE_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("file:line"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 'c': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("condition"), current); return FAILURE; } } else if ((top)->type != (COND_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("condition"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 'o': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("opcode"), current); return FAILURE; } } else if ((top)->type != (OP_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("opcode"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
    case 'b': if (!(top)) { if (!optional) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"noarg\" command=\"%s\" expected=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got nothing at parameter %lu", phpdbg_command_name(command, buffer), ("boolean"), current); return FAILURE; } } else if ((top)->type != (NUMERIC_PARAM)) { phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"wrongarg\" command=\"%s\" expected=\"%s\" got=\"%s\" num=\"%lu\"", "The command \"%s\" expected %s and got %s at parameter %lu", phpdbg_command_name(command, buffer), ("boolean"), phpdbg_get_param_type((top)), current); return FAILURE; }; break;
  ...
  if ((received < least)) {
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"toofewargs\" command=\"%s\" expected=\"%d\" argtypes=\"%s\" got=\"%d\"", "The command \"%s\" expected at least %lu arguments (%s) and received %lu", phpdbg_command_name(command, buffer), least, command->args, received)
...
__attribute__ ((visibility("default"))) const phpdbg_command_t *phpdbg_stack_resolve(const phpdbg_command_t *commands, const phpdbg_command_t *parent, phpdbg_param_t **top) {
 ...
 const phpdbg_command_t *matched[3] = {((void *)0), ((void *)0), ((void *)0)};
 ulong matches = 0L;
 ...
 switch (matches) {
  case 0:
   if (parent) {
    phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"notfound\" command=\"%s\" subcommand=\"%s\"", "The command \"%s %s\" could not be found", parent->name, name->str);
   } else {
    phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"notfound\" command=\"%s\"", "The command \"%s\" could not be found", name->str);
   }
   return parent;
  ...
  default: {
   ...
   while (it < matches) {
    if (!list) {
     list = (__builtin_constant_p((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) ) ? (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 8) ? _emalloc_8() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 16) ? _emalloc_16() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 24) ? _emalloc_24() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 32) ? _emalloc_32() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 40) ? _emalloc_40() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 48) ? _emalloc_48() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 56) ? _emalloc_56() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 64) ? _emalloc_64() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 80) ? _emalloc_80() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 96) ? _emalloc_96() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 112) ? _emalloc_112() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 128) ? _emalloc_128() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 160) ? _emalloc_160() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 192) ? _emalloc_192() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 224) ? _emalloc_224() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 256) ? _emalloc_256() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 320) ? _emalloc_320() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 384) ? _emalloc_384() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 448) ? _emalloc_448() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 512) ? _emalloc_512() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 640) ? _emalloc_640() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 768) ? _emalloc_768() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 896) ? _emalloc_896() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 1024) ? _emalloc_1024() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 1280) ? _emalloc_1280() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 1536) ? _emalloc_1536() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 1792) ? _emalloc_1792() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 2048) ? _emalloc_2048() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 2560) ? _emalloc_2560() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= 3072) ? _emalloc_3072() : (((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) <= ((2 * 1024 * 1024) - ((4 * 1024) * (1)))) ? _emalloc_large((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0))) : _emalloc_huge((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)))) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) ) : _emalloc((matched[it]->name_len + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) ) );
    } else {
     list = _erealloc((list), ((pos + matched[it]->name_len) + 1 + (it + 1 < matches ? sizeof(", ") - 1 : 0)) );
    }
   ...
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"ambiguous\" command=\"%s\" matches=\"%lu\" matched=\"%s\"", "The command \"%s\" is ambigious, matching %lu commands (%s)", name->str, matches, list);
   _efree((list) );
...
static int phpdbg_internal_stack_execute(phpdbg_param_t *stack, zend_bool allow_async_unsafe) {
 const phpdbg_command_t *handler = ((void *)0);
 ...
 switch (top->type) {
  case EVAL_PARAM:
   ...
   phpdbg_free_err_buf();
   return phpdbg_do_ev(top);

  case RUN_PARAM:
   if (!allow_async_unsafe) {
    phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "signalsegv", "command=\"run\"", "run command is disallowed during hard interrupt");
   }
   phpdbg_activate_err_buf(0);
   phpdbg_free_err_buf();
   return phpdbg_do_run(top);

  case SHELL_PARAM:
   if (!allow_async_unsafe) {
    phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "signalsegv", "command=\"sh\"", "sh command is disallowed during hard interrupt");
    return FAILURE;
   }
   phpdbg_activate_err_buf(0);
   phpdbg_free_err_buf();
   return phpdbg_do_sh(top);

  case STR_PARAM: {
   handler = phpdbg_stack_resolve(phpdbg_prompt_commands, ((void *)0), &top);

   if (handler) {
    if (!allow_async_unsafe && !(handler->flags & 1)) {
     phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "signalsegv", "command=\"%s\"", "%s command is disallowed during hard interrupt", handler->name);
     return FAILURE;
    }
  ...
  default:
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"invalidcommand\"", "The first parameter makes no sense !");
   return FAILURE;
 }
...
__attribute__ ((visibility("default"))) int phpdbg_stack_execute(phpdbg_param_t *stack, zend_bool allow_async_unsafe) {
 ...
 if (stack->type != STACK_PARAM) {
  phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"nostack\"", "The passed argument was not a stack !");
  return FAILURE;
 }

 if (!stack->len) {
  phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "command", "type=\"emptystack\"", "The stack contains nothing !");
  return FAILURE;
 }
...
__attribute__ ((visibility("default"))) char *phpdbg_read_input(char *buffered)
{
 ...
 if (((phpdbg_globals.flags) & (((1ULL<<16) | (1ULL<<18)) | (1ULL<<19))) != ((1ULL<<16) | (1ULL<<18))) {
  if (((phpdbg_globals.flags) & (1ULL<<30)) && (buffered == ((void *)0)) && !phpdbg_active_sigsafe_mem()) {
  ...
  if (buffered == ((void *)0)) {
   ...
   {
    phpdbg_print(P_WRITE , (phpdbg_globals.io)[1].fd, "prompt", "", "%s", phpdbg_get_prompt());
    phpdbg_consume_stdin_line(cmd = buf);
   }
# 776 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_cmd.c"
  } else {
   cmd = buffered;
  }
 ...
 if (buffer && ((*__ctype_b_loc ())[(int) ((*buffer))] & (unsigned short int) _ISspace)) {
  char *trimmed = buffer;
  while (((*__ctype_b_loc ())[(int) ((*trimmed))] & (unsigned short int) _ISspace))
   trimmed++;

  trimmed = _estrdup((trimmed) );
  _efree((buffer) );
  buffer = trimmed;
 }
 ...
 if (buffer && strlen(buffer)) {
  ...
  (phpdbg_globals.buffer) = strdup(buffer);
 } else if ((phpdbg_globals.buffer)) {
  if (buffer) {
   _efree((buffer) );
  }
  buffer = _estrdup(((phpdbg_globals.buffer)) );
 }

 return buffer;
}

__attribute__ ((visibility("default"))) void phpdbg_destroy_input(char **input)
{
 _efree((*input) );
}
...
__attribute__ ((visibility("default"))) int phpdbg_ask_user_permission(const char *question) {
 if (!((phpdbg_globals.flags) & (1ULL<<32))) {
  char buf[500];
  phpdbg_out_internal((phpdbg_globals.io)[1].fd, "%s", question);
  phpdbg_out_internal((phpdbg_globals.io)[1].fd, " (type y or n): ");
  ...
  while (1) {
   ...
   phpdbg_out_internal((phpdbg_globals.io)[1].fd, "Please enter either y (yes) or n (no): ");
  }
 }
