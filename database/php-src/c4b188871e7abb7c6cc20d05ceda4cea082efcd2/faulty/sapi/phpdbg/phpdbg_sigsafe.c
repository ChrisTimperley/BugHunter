# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_sigsafe.c"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 7 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_sigsafe.h" 2
...
# 2 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_sigsafe.c" 2
...
static void* zend_mm_mem_alloc(zend_mm_storage *storage, size_t size, size_t alignment) {
 ...
 if (__builtin_expect(!!(size <= ((2 * 1024 * 1024) * 2) && !(phpdbg_globals.sigsafe_mem).allocated), 1)) {
  (phpdbg_globals.sigsafe_mem).allocated = 1;
  return (void *) (((size_t) (phpdbg_globals.sigsafe_mem).mem & ~(alignment - 1)) + alignment);
 }

 (({ __typeof__ (write((phpdbg_globals.io)[2].fd, ("Tried to allocate more than " "((2 * 1024 * 1024) * 2)" " bytes from stack memory in signal handler ... bailing out of signal handler\n"), (sizeof("Tried to allocate more than " "((2 * 1024 * 1024) * 2)" " bytes from stack memory in signal handler ... bailing out of signal handler\n")-1))) __x = (write((phpdbg_globals.io)[2].fd, ("Tried to allocate more than " "((2 * 1024 * 1024) * 2)" " bytes from stack memory in signal handler ... bailing out of signal handler\n"), (sizeof("Tried to allocate more than " "((2 * 1024 * 1024) * 2)" " bytes from stack memory in signal handler ... bailing out of signal handler\n")-1))); (void) __x; }));

 if (*(executor_globals.bailout)) {
  longjmp(*(executor_globals.bailout), FAILURE);
 }

 (({ __typeof__ (write((phpdbg_globals.io)[2].fd, ("Bailed out without a bailout address in signal handler!\n"), (sizeof("Bailed out without a bailout address in signal handler!\n")-1))) __x = (write((phpdbg_globals.io)[2].fd, ("Bailed out without a bailout address in signal handler!\n"), (sizeof("Bailed out without a bailout address in signal handler!\n")-1))); (void) __x; }));
...
void phpdbg_set_sigsafe_mem(char *buffer) {
 phpdbg_signal_safe_mem *mem = &(phpdbg_globals.sigsafe_mem);
 ...
 mem->heap = zend_mm_startup_ex(&phpdbg_handlers, ((void *)0), 0);

 mem->old_heap = zend_mm_set_heap(mem->heap);
}

zend_mm_heap *phpdbg_original_heap_sigsafe_mem(void) {
 return (phpdbg_globals.sigsafe_mem).old_heap;
}

void phpdbg_clear_sigsafe_mem(void) {
 zend_mm_set_heap(phpdbg_original_heap_sigsafe_mem());
 (phpdbg_globals.sigsafe_mem).mem = ((void *)0);
}

zend_bool phpdbg_active_sigsafe_mem(void) {
 return !!(phpdbg_globals.sigsafe_mem).mem;
}
