# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_set.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_set.c"
# 21 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_set.c"
...
# 22 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_set.c" 2
...
# 24 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_set.c" 2
...
# 27 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_set.c" 2
...
const phpdbg_command_t phpdbg_set_commands[] = {
 {"prompt", sizeof("prompt")-1, "usage: set prompt [<string>]", sizeof("usage: set prompt [<string>]")-1, 'p', phpdbg_do_set_prompt, ((void *)0), "|s", &phpdbg_prompt_commands[17], 0},
 {"pagination", sizeof("pagination")-1, "usage: set pagination [<on|off>]", sizeof("usage: set pagination [<on|off>]")-1, 'P', phpdbg_do_set_pagination, ((void *)0), "|b", &phpdbg_prompt_commands[17], 1},

 {"color", sizeof("color")-1, "usage: set color  <element> <color>", sizeof("usage: set color  <element> <color>")-1, 'c', phpdbg_do_set_color, ((void *)0), "ss", &phpdbg_prompt_commands[17], 1},
 {"colors", sizeof("colors")-1, "usage: set colors [<on|off>]", sizeof("usage: set colors [<on|off>]")-1, 'C', phpdbg_do_set_colors, ((void *)0), "|b", &phpdbg_prompt_commands[17], 1},

 {"oplog", sizeof("oplog")-1, "usage: set oplog  [<output>]", sizeof("usage: set oplog  [<output>]")-1, 'O', phpdbg_do_set_oplog, ((void *)0), "|s", &phpdbg_prompt_commands[17], 0},
 {"break", sizeof("break")-1, "usage: set break id [<on|off>]", sizeof("usage: set break id [<on|off>]")-1, 'b', phpdbg_do_set_break, ((void *)0), "l|b", &phpdbg_prompt_commands[17], 1},
 {"breaks", sizeof("breaks")-1, "usage: set breaks [<on|off>]", sizeof("usage: set breaks [<on|off>]")-1, 'B', phpdbg_do_set_breaks, ((void *)0), "|b", &phpdbg_prompt_commands[17], 1},
 {"quiet", sizeof("quiet")-1, "usage: set quiet [<on|off>]", sizeof("usage: set quiet [<on|off>]")-1, 'q', phpdbg_do_set_quiet, ((void *)0), "|b", &phpdbg_prompt_commands[17], 1},
 {"stepping", sizeof("stepping")-1, "usage: set stepping [<line|op>]", sizeof("usage: set stepping [<line|op>]")-1, 's', phpdbg_do_set_stepping, ((void *)0), "|s", &phpdbg_prompt_commands[17], 1},
 {"refcount", sizeof("refcount")-1, "usage: set refcount [<on|off>]", sizeof("usage: set refcount [<on|off>]")-1, 'r', phpdbg_do_set_refcount, ((void *)0), "|b", &phpdbg_prompt_commands[17], 1},
 {"lines", sizeof("lines")-1, "usage: set lines [<number>]", sizeof("usage: set lines [<number>]")-1, 'l', phpdbg_do_set_lines, ((void *)0), "|l", &phpdbg_prompt_commands[17], 1},
...
int phpdbg_do_set_prompt(const phpdbg_param_t *param)
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setprompt", "str=\"%s\"", "Current prompt: %s", phpdbg_get_prompt());
 } else {
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setpagination", "active=\"%s\"", "Pagination %s", (phpdbg_globals.flags) & (1ULL<<36) ? "on" : "off");
 } else switch (param->type) {
  case NUMERIC_PARAM: {
   if (param->num) {
    (phpdbg_globals.flags) |= (1ULL<<36);
   } else {
    (phpdbg_globals.flags) &= ~(1ULL<<36);
   }
  } break;
  ...
  default:
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setpagination", "type=\"wrongargs\"", "set pagination used incorrectly: set pagination <on|off>");
 }
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setlines", "active=\"%s\"", "Lines %ld", (phpdbg_globals.lines));
 } else switch (param->type) {
  case NUMERIC_PARAM: {
   (phpdbg_globals.lines) = param->num;
  } break;

  default:
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setlines", "type=\"wrongargs\"", "set lines used incorrectly: set lines <number>");
 }
 ...
 switch (param->type) {
  case NUMERIC_PARAM: {
   if (param->next) {
    ...
    if (brake) {
     phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setbreak", "id=\"%ld\" active=\"%s\"", "Breakpoint #%ld %s", param->num, brake->disabled ? "off" : "on");
    } else {
     phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setbreak", "type=\"nobreak\" id=\"%ld\"", "Failed to find breakpoint #%ld", param->num);
    }
   }
  } break;

  default:
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setbreak", "type=\"wrongargs\"", "set break used incorrectly: set break [id] <on|off>");
 }
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setbreaks", "active=\"%s\"", "Breakpoints %s",(phpdbg_globals.flags) & (1ULL<<29) ? "on" : "off");
 } else switch (param->type) {
  case NUMERIC_PARAM: {
   if (param->num) {
  ...
  default:
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setbreaks", "type=\"wrongargs\"", "set breaks used incorrectly: set breaks <on|off>");
 }
...
int phpdbg_do_set_color(const phpdbg_param_t *param)
{
 ...
 if (!color) {
  phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setcolor", "type=\"nocolor\"", "Failed to find the requested color (%s)", param->next->str);
  return SUCCESS;
 }

 switch (phpdbg_get_element(param->str, param->len)) {
  case 0:
   phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "setcolor", "type=\"prompt\" color=\"%s\" code=\"%s\"", "setting prompt color to %s (%s)", color->name, color->code);
   if ((phpdbg_globals.prompt)[1]) {
    free((phpdbg_globals.prompt)[1]);
    (phpdbg_globals.prompt)[1]=((void *)0);
   }
   phpdbg_set_color(0, color);
  break;

  case 1:
   phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "setcolor", "type=\"error\" color=\"%s\" code=\"%s\"", "setting error color to %s (%s)", color->name, color->code);
   phpdbg_set_color(1, color);
  break;

  case 2:
   phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "setcolor", "type=\"notice\" color=\"%s\" code=\"%s\"", "setting notice color to %s (%s)", color->name, color->code);
   phpdbg_set_color(2, color);
  break;

  default:
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setcolor", "type=\"invalidtype\"", "Failed to find the requested element (%s)", param->str);
 }
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setcolors", "active=\"%s\"", "Colors %s", (phpdbg_globals.flags) & (1ULL<<17) ? "on" : "off");
 } else switch (param->type) {
  case NUMERIC_PARAM: {
   if (param->num) {
    (phpdbg_globals.flags) |= (1ULL<<17);
   } else {
    (phpdbg_globals.flags) &= ~(1ULL<<17);
   }
  } break;
  ...
  default:
   phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setcolors", "type=\"wrongargs\"", "set colors used incorrectly: set colors <on|off>");
 }
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "setoplog", "active=\"%s\"", "Oplog %s", (phpdbg_globals.oplog) ? "on" : "off");
 } else switch (param->type) {
  case STR_PARAM: {

   FILE *old = (phpdbg_globals.oplog);

   (phpdbg_globals.oplog) = fopen(param->str, "w+");
   if (!(phpdbg_globals.oplog)) {
    phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setoplog", "type=\"openfailure\" file=\"%s\"", "Failed to open %s for oplog", param->str);
    (phpdbg_globals.oplog) = old;
   } else {
    if (old) {
     phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "setoplog", "type=\"closingold\"", "Closing previously open oplog");
     fclose(old);
    }

    phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "setoplog", "file=\"%s\"", "Successfully opened oplog %s", param->str);
   }
  } break;
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setquiet", "active=\"%s\"", "Quietness %s", (phpdbg_globals.flags) & (1ULL<<15) ? "on" : "off");
 } else switch (param->type) {
  case NUMERIC_PARAM: {
   if (param->num) {
    (phpdbg_globals.flags) |= (1ULL<<15);
   } else {
    (phpdbg_globals.flags) &= ~(1ULL<<15);
   }
  } break;
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setstepping", "type=\"%s\"", "Stepping %s", (phpdbg_globals.flags) & (1ULL<<14) ? "opcode" : "line");
 } else switch (param->type) {
  case STR_PARAM: {
   if (param->len == sizeof("opcode") - 1 && !memcmp(param->str, "opcode", sizeof("opcode"))) {
    (phpdbg_globals.flags) |= (1ULL<<14);
   } else if (param->len == sizeof("line") - 1 && !memcmp(param->str, "line", sizeof("line"))) {
    (phpdbg_globals.flags) &= ~(1ULL<<14);
   } else {
    phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "setstepping", "type=\"wrongargs\"", "usage set stepping [<opcode|line>]");
   }
  } break;
...
{
 if (!param || param->type == EMPTY_PARAM) {
  phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "setrefcount", "active=\"%s\"", "Showing refcounts %s", (phpdbg_globals.flags) & (1ULL<<15) ? "on" : "off");
 } else switch (param->type) {
  case NUMERIC_PARAM: {
   if (param->num) {
    (phpdbg_globals.flags) |= (1ULL<<33);
   } else {
    (phpdbg_globals.flags) &= ~(1ULL<<33);
   }
  } break;
