# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c"
# 23 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c"
# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.h" 1
# 22 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.h"
...
# 23 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.h" 2
...
# 24 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c" 2
# 41 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c"
...
# 42 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c" 2
...
# 46 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c" 2
...
__attribute__ ((visibility("default"))) int phpdbg_consume_stdin_line(char *buf) {
 int bytes = (phpdbg_globals.input_buflen), len = 0;

 if ((phpdbg_globals.input_buflen)) {
  memcpy(buf, (phpdbg_globals.input_buffer), bytes);
 }
 ...
 do {
  ...
  for (i = len; i < len + bytes; i++) {
   ...
   if (buf[i] == '\n') {
    (phpdbg_globals.input_buflen) = len + bytes - 1 - i;
    if ((phpdbg_globals.input_buflen)) {
     memcpy((phpdbg_globals.input_buffer), buf + i + 1, (phpdbg_globals.input_buflen));
    }
    if (i != 500 - 1) {
     buf[i + 1] = 0;
    }
  ...
  len += bytes;
 } while ((bytes = phpdbg_mixed_read((phpdbg_globals.io)[0].fd, buf + len, 500 - len, -1)) > 0);

 if (bytes <= 0) {
  (phpdbg_globals.flags) |= (1ULL<<16) | (1ULL<<31);
  _zend_bailout("/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c", 92);
 }
...
__attribute__ ((visibility("default"))) int phpdbg_consume_bytes(int sock, char *ptr, int len, int tmo) {
 ...
 if (j == 0) {
# 126 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_io.c"
  return -1;
 }
...
recv_once:
 while(i > 0) {
  if (tmo < 0) {
   ...
   if (can_read == -1 && (*__errno_location ()) == 4) {
    continue;
   }
  ...
  } while (got_now == -1 && (*__errno_location ()) == 4);


  if (got_now == -1) {
   (({ __typeof__ (write((phpdbg_globals.io)[2].fd, ("Read operation timed out!\n"), (sizeof("Read operation timed out!\n")-1))) __x = (write((phpdbg_globals.io)[2].fd, ("Read operation timed out!\n"), (sizeof("Read operation timed out!\n")-1))); (void) __x; }));
   return -1;
  }
  i -= got_now;
  p += got_now;
...
static int phpdbg_output_pager(int sock, const char *ptr, int len) {
 ...
 while ((p = memchr(p, '\n', endp - p))) {
  ...
  if (count % (phpdbg_globals.lines) == 0) {
   bytes += write(sock, ptr + bytes, (p - ptr) - bytes);

   if (memchr(p, '\n', endp - p)) {
    char buf[500];
    write(sock, ("\r---Type <return> to continue or q <return> to quit---"), (sizeof("\r---Type <return> to continue or q <return> to quit---")-1));
 ...
 if (bytes && count % (phpdbg_globals.lines) != 0) {
...
__attribute__ ((visibility("default"))) int phpdbg_mixed_write(int sock, const char *ptr, int len) {
 ...
 if (((phpdbg_globals.flags) & (1ULL<<36))
  && !((phpdbg_globals.flags) & (1ULL<<32))
  && (phpdbg_globals.io)[1].fd == sock
  && (phpdbg_globals.lines) > 0) {
...
__attribute__ ((visibility("default"))) int phpdbg_create_listenable_socket(const char *addr, unsigned short port, struct addrinfo *addr_res) {
 int sock = -1, rc;
 int reuse = 1;
 ...
 do {
  ...
  rc = inet_pton(2, addr, &serveraddr);
  if (1 == rc) {
   hints.ai_family = 2;
   if (!any_addr) {
   ...
   rc = inet_pton(10, addr, &serveraddr);
   if (1 == rc) {
    hints.ai_family = 10;
    if (!any_addr) {
     hints.ai_flags |= 0x0004;
  ...
  ap_php_snprintf(port_buf, 7, "%u", port);
  if (!any_addr) {
   rc = getaddrinfo(addr, port_buf, &hints, &res);
  } else {
   rc = getaddrinfo(((void *)0), port_buf, &hints, &res);
  }
  ...
  if (0 != rc) {
   ...
   if (rc == -11) {
    ...
    wrote = ap_php_snprintf(buf, 128, "Could not translate address '%s'", addr);
    buf[wrote] = '\0';
    (({ __typeof__ (write((phpdbg_globals.io)[2].fd, buf, strlen(buf))) __x = (write((phpdbg_globals.io)[2].fd, buf, strlen(buf))); (void) __x; }));
    ...
    wrote = ap_php_snprintf(buf, 256, "Host '%s' not found. %s", addr, _estrdup((gai_strerror(rc)) ));
    buf[wrote] = '\0';
    (({ __typeof__ (write((phpdbg_globals.io)[2].fd, buf, strlen(buf))) __x = (write((phpdbg_globals.io)[2].fd, buf, strlen(buf))); (void) __x; }));
  ...
  if((sock = socket(res->ai_family, res->ai_socktype, res->ai_protocol)) == -1) {
   ...
   wrote = php_sprintf(buf, "Unable to create socket");
   buf[wrote] = '\0';
   (({ __typeof__ (write((phpdbg_globals.io)[2].fd, buf, strlen(buf))) __x = (write((phpdbg_globals.io)[2].fd, buf, strlen(buf))); (void) __x; }));
  ...
  if (setsockopt(sock, 1, 2, (char*) &reuse, sizeof(reuse)) == -1) {
