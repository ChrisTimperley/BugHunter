# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_frame.c"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "/usr/include/stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_frame.c"
# 21 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_frame.c"
...
# 22 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_frame.c" 2
...
# 23 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_frame.c" 2
...
# 25 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_frame.c" 2
...
# 26 "/home/ct584/git/bughunter/repositories/php-src/sapi/phpdbg/phpdbg_frame.c" 2

extern zend_phpdbg_globals phpdbg_globals;;

void phpdbg_restore_frame(void)
{
 if (((phpdbg_globals.frame).num) == 0) {
  return;
 }
 ...
 (executor_globals.current_execute_data) = ((phpdbg_globals.frame).execute_data);

 (executor_globals.scope) = ((executor_globals.current_execute_data)->func)->op_array.scope;
}

void phpdbg_switch_frame(int frame)
{
 zend_execute_data *execute_data = ((phpdbg_globals.frame).num)?((phpdbg_globals.frame).execute_data):(executor_globals.current_execute_data);
 int i = 0;

 if (((phpdbg_globals.frame).num) == frame) {
  phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "frame", "id=\"%d\"", "Already in frame #%d", frame);
  return;
 }
 ...
 { jmp_buf *__orig_bailout = (phpdbg_globals.sigsegv_bailout); jmp_buf __bailout; (phpdbg_globals.sigsegv_bailout) = &__bailout; if (_setjmp (__bailout) == 0) { {
  while (execute_data) {
   ...
   } while (execute_data && execute_data->opline == ((void *)0));
  }
 } } else { (phpdbg_globals.sigsegv_bailout) = __orig_bailout; {
  phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "signalsegv", "", "Couldn't switch frames, invalid data source");
  return;
 } } (phpdbg_globals.sigsegv_bailout) = __orig_bailout; };

 if (execute_data == ((void *)0)) {
  phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "frame", "type=\"maxnum\" id=\"%d\"", "No frame #%d", frame);
  return;
 }
 ...
 if (frame > 0) {
  ((phpdbg_globals.frame).num) = frame;


  ((phpdbg_globals.frame).execute_data) = (executor_globals.current_execute_data);
  (executor_globals.current_execute_data) = execute_data;

  (executor_globals.scope) = ((executor_globals.current_execute_data)->func)->op_array.scope;
 }

 phpdbg_print(P_NOTICE , (phpdbg_globals.io)[1].fd, "frame", "id=\"%d\"", "Switched to frame #%d", frame);
...
static void phpdbg_dump_prototype(zval *tmp)
{
 ...
 funcname = zend_hash_str_find((*(tmp)).value.arr, ("function"), (sizeof("function")-1));

 if ((class = zend_hash_str_find((*(tmp)).value.arr, ("object"), (sizeof("object")-1)))) {
  do { zval *__z = (&class_zv); zend_string *__s = (((*(class)).value.obj->ce)->name); (*(__z)).value.str = __s; (*(__z)).u1.type_info = (6 | (( (1<<2) | (1<<4)) << 8)); } while (0);
  class = &class_zv;
 } else {
  class = zend_hash_str_find((*(tmp)).value.arr, ("class"), (sizeof("class")-1));
 }

 if (class) {
  type = zend_hash_str_find((*(tmp)).value.arr, ("type"), (sizeof("type")-1));
 }

 args = zend_hash_str_find((*(tmp)).value.arr, ("args"), (sizeof("args")-1));

 phpdbg_xml_internal((phpdbg_globals.io)[1].fd, " symbol=\"%s%s%s\"", class ? ((*(class)).value.str)->val : "", class ? ((*(type)).value.str)->val : "", ((*(funcname)).value.str)->val);
 ...
 phpdbg_out_internal((phpdbg_globals.io)[1].fd, "%s%s%s(", class ? ((*(class)).value.str)->val : "", class ? ((*(type)).value.str)->val : "", ((*(funcname)).value.str)->val);

 if (args) {
  const zend_function *func = ((void *)0);
  const zend_arg_info *arginfo = ((void *)0);
  zend_bool is_variadic = 0;
  int j = 0, m;

  { jmp_buf *__orig_bailout = (phpdbg_globals.sigsegv_bailout); jmp_buf __bailout; (phpdbg_globals.sigsegv_bailout) = &__bailout; if (_setjmp (__bailout) == 0) { {

   if ((func = phpdbg_get_function(((*(funcname)).value.str)->val, class ? ((*(class)).value.str)->val : ((void *)0)))) {
  ...
  do { Bucket *_p = ((*(args)).value.arr)->arData; Bucket *_end = _p + ((*(args)).value.arr)->nNumUsed; for (; _p != _end; _p++) { zval *_z = &_p->val; if (0 && zval_get_type(&(*(_z))) == 15) { _z = (*(_z)).value.zv; } if (__builtin_expect(!!(zval_get_type(&(*(_z))) == 0), 0)) continue;; argstmp = _z; {
   if (j) {
    phpdbg_out_internal((phpdbg_globals.io)[1].fd, ", ");
   }
   phpdbg_xml_internal((phpdbg_globals.io)[1].fd, "<arg %r");
   if (m && j < m) {
    ...
    phpdbg_xml_internal((phpdbg_globals.io)[1].fd, " variadic=\"%s\" name=\"%s\">", is_variadic ? "variadic" : "", arg_name ? arg_name : "");
    phpdbg_out_internal((phpdbg_globals.io)[1].fd, "%s=%s", arg_name ? arg_name : "?", is_variadic ? "[": "");
   ...
   phpdbg_xml_internal((phpdbg_globals.io)[1].fd, "</arg>");
  } } } while (0);

  if (is_variadic) {
   phpdbg_out_internal((phpdbg_globals.io)[1].fd, "]");
  }
  phpdbg_xml_internal((phpdbg_globals.io)[1].fd, "</frame>");
 }
 phpdbg_out_internal((phpdbg_globals.io)[1].fd, ")");
}
...
void phpdbg_dump_backtrace(size_t num)
{
 ...
 if (limit < 0) {
  phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "backtrace", "type=\"minnum\"", "Invalid backtrace size %d", limit);
 ...
 { jmp_buf *__orig_bailout = (phpdbg_globals.sigsegv_bailout); jmp_buf __bailout; (phpdbg_globals.sigsegv_bailout) = &__bailout; if (_setjmp (__bailout) == 0) { {
  ...
  phpdbg_print(P_ERROR , (phpdbg_globals.io)[1].fd, "signalsegv", "", "Couldn't fetch backtrace, invalid data source");
  return;
 } } (phpdbg_globals.sigsegv_bailout) = __orig_bailout; };

 phpdbg_xml_internal((phpdbg_globals.io)[1].fd, "<backtrace %r>");

 (startline).value.lval = zend_get_executed_lineno();
 startfilename = zend_get_executed_filename();
 (startfile).value.str = zend_string_init(startfilename, strlen(startfilename), 0);

 zend_hash_internal_pointer_reset_ex((zbacktrace).value.arr, &position);
 tmp = zend_hash_get_current_data_ex((zbacktrace).value.arr, &position);
 while ((tmp = zend_hash_get_current_data_ex((zbacktrace).value.arr, &position))) {
  if (file) {
   phpdbg_out_internal((phpdbg_globals.io)[1].fd, "frame #%d: ", i);
   phpdbg_xml_internal((phpdbg_globals.io)[1].fd, "<frame %r id=\"%d\" file=\"%s\" line=\"" "%" "l" "d" "\"", i, ((*(file)).value.str)->val, (*(line)).value.lval);
   phpdbg_dump_prototype(tmp);
   phpdbg_out_internal((phpdbg_globals.io)[1].fd, " at %s:%ld\n", ((*(file)).value.str)->val, (*(line)).value.lval);
   i++;
  } else {
   phpdbg_out_internal((phpdbg_globals.io)[1].fd, " => ");
   phpdbg_xml_internal((phpdbg_globals.io)[1].fd, "<frame %r id=\"%d\" internal=\"internal\"", i);
   phpdbg_dump_prototype(tmp);
   phpdbg_out_internal((phpdbg_globals.io)[1].fd, " (internal function)\n");
  }

  file = zend_hash_str_find((*(tmp)).value.arr, ("file"), (sizeof("file")-1));
  line = zend_hash_str_find((*(tmp)).value.arr, ("line"), (sizeof("line")-1));
  zend_hash_move_forward_ex((zbacktrace).value.arr, &position);
 }

 phpdbg_print(P_WRITELN, (phpdbg_globals.io)[1].fd, "frame", "id=\"%d\" symbol=\"{main}\" file=\"%s\" line=\"%d\"", "frame #%d: {main} at %s:%ld", i, ((*(file)).value.str)->val, (*(line)).value.lval);
 phpdbg_xml_internal((phpdbg_globals.io)[1].fd, "</backtrace>");

 _zval_dtor((&zbacktrace) );
 zend_string_release((startfile).value.str);
