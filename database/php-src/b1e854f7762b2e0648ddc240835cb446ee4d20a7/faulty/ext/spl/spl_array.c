...
static inline HashTable *spl_array_get_hash_table(spl_array_object* intern) {
 ...
 } else if (zval_get_type(&(intern->array)) == 7) {
  return (intern->array).value.arr;
 } else {
  zend_object *obj = (intern->array).value.obj;
  if (!obj->properties) {
   rebuild_object_properties(obj);
  } else if ((obj->properties)->gc.refcount > 1) {
   if (__builtin_expect(!!(!((obj->properties)->gc.u.v.flags & (1<<1))), 1)) {
    (obj->properties)->gc.refcount--;
   }
 ...
 if (orig) {
  ...
  if (clone_orig) {
   if (other->ar_flags & 0x01000000) {
    ...
    do { zval *__z = (&intern->array); (*(__z)).value.arr = (zend_array_dup(spl_array_get_hash_table(other))); (*(__z)).u1.type_info = (7 | (( (1<<2) | (1<<3) | (1<<4)) << 8)); } while (0)
                                                     ;
   } else {
...
static zval *spl_array_get_dimension_ptr(int check_inherited, spl_array_object *intern, zval *offset, int type)
{
 ...
 if ((type == 1 || type == 2) && intern->nApplyCount > 0) {
...
static zval *spl_array_read_dimension_ex(int check_inherited, zval *object, zval *offset, int type, zval *rv)
{
 ...
 if (check_inherited &&
   (intern->fptr_offset_get || (type == 3 && intern->fptr_offset_has))) {
  if (type == 3) {
...
void zim_spl_Array_getArrayCopy(zend_execute_data *execute_data, zval *return_value)
{
 ...
 { do { zval *__z = (return_value); (*(__z)).value.arr = (zend_array_dup(spl_array_get_hash_table(intern))); (*(__z)).u1.type_info = (7 | (( (1<<2) | (1<<3) | (1<<4)) << 8)); } while (0); return; };
}
  ...
  if ((*(array)).value.obj->handlers == &spl_handler_ArrayObject || (*(array)).value.obj->handlers == &spl_handler_ArrayIterator) {
   ...
   if (handler != std_object_handlers.get_properties) {
    zend_throw_exception_ex(spl_ce_InvalidArgumentException, 0,
     ...
     (((*(array)).value.obj->ce)->name)->val, (intern->std.ce->name)->val);
    return;
   }
   _zval_ptr_dtor((&intern->array) );
   do { zval *_z1 = (&intern->array); const zval *_z2 = (array); zend_refcounted *_gc = (*(_z2)).value.counted; uint32_t _t = (*(_z2)).u1.type_info; do { (*(_z1)).value.counted = _gc; (*(_z1)).u1.type_info = _t; } while (0); if ((_t & ((1<<2) << 8)) != 0) { (_gc)->gc.refcount++; } } while (0);
...
void zim_spl_Array_exchangeArray(zend_execute_data *execute_data, zval *return_value)
{
 ...
 do { zval *__z = (return_value); (*(__z)).value.arr = (zend_array_dup(spl_array_get_hash_table(intern))); (*(__z)).u1.type_info = (7 | (( (1<<2) | (1<<3) | (1<<4)) << 8)); } while (0);
...
# 1488 "/home/ct584/git/bughunter/repositories/php-src/ext/spl/spl_array.c"
