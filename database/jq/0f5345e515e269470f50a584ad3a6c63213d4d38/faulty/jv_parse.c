...
static void push(struct jv_parser* p, jv v) {
  ((p->stackpos <= p->stacklen) ? (void) (0) : __assert_fail ("p->stackpos <= p->stacklen", "jv_parse.c", 133, __PRETTY_FUNCTION__));
  if (p->stackpos == p->stacklen) {
    p->stacklen = p->stacklen * 2 + 10;
    p->stack = jv_mem_realloc(p->stack, p->stacklen * sizeof(jv));
  }
  ((p->stackpos < p->stacklen) ? (void) (0) : __assert_fail ("p->stackpos < p->stacklen", "jv_parse.c", 138, __PRETTY_FUNCTION__));
...
static __attribute__((warn_unused_result)) presult parse_token(struct jv_parser* p, char ch) {
  ...
  case ',':
    ...
    if (jv_get_kind(p->stack[p->stackpos-1]) == JV_KIND_ARRAY) {
      ...
      ((p->stackpos > 1 && jv_get_kind(p->stack[p->stackpos-2]) == JV_KIND_OBJECT) ? (void) (0) : __assert_fail ("p->stackpos > 1 && jv_get_kind(p->stack[p->stackpos-2]) == JV_KIND_OBJECT", "jv_parse.c", 174, __PRETTY_FUNCTION__));
  ...
  case '}':
    ...
    if (jv_is_valid(p->next)) {
      if (jv_get_kind(p->stack[p->stackpos-1]) != JV_KIND_STRING)
        return "Objects must consist of key:value pairs";
      ((p->stackpos > 1 && jv_get_kind(p->stack[p->stackpos-2]) == JV_KIND_OBJECT) ? (void) (0) : __assert_fail ("p->stackpos > 1 && jv_get_kind(p->stack[p->stackpos-2]) == JV_KIND_OBJECT", "jv_parse.c", 207, __PRETTY_FUNCTION__));
      p->stack[p->stackpos-2] = jv_object_set(p->stack[p->stackpos-2],
                                              p->stack[p->stackpos-1], p->next);
      p->stackpos--;
      p->next = jv_invalid();
...
static __attribute__((warn_unused_result)) presult stream_token(struct jv_parser* p, char ch) {
  ...
  case ',':
    ...
    if (k == JV_KIND_NUMBER) {
      ...
      ((k == JV_KIND_NULL) ? (void) (0) : __assert_fail ("k == JV_KIND_NULL", "jv_parse.c", 285, __PRETTY_FUNCTION__));
  ...
  case ']':
    ...
    if (p->last_seen == JV_LAST_OPEN_ARRAY)
      ((!jv_is_valid(p->next)) ? (void) (0) : __assert_fail ("!jv_is_valid(p->next)", "jv_parse.c", 300, __PRETTY_FUNCTION__));
  ...
  case '}':
    ...
    if (p->last_seen == JV_LAST_OPEN_OBJECT)
      ((!jv_is_valid(p->next)) ? (void) (0) : __assert_fail ("!jv_is_valid(p->next)", "jv_parse.c", 335, __PRETTY_FUNCTION__));
...
static void tokenadd(struct jv_parser* p, char c) {
  ((p->tokenpos <= p->tokenlen) ? (void) (0) : __assert_fail ("p->tokenpos <= p->tokenlen", "jv_parse.c", 380, __PRETTY_FUNCTION__));
  if (p->tokenpos == p->tokenlen) {
    p->tokenlen = p->tokenlen*2 + 256;
    p->tokenbuf = jv_mem_realloc(p->tokenbuf, p->tokenlen);
  }
  ((p->tokenpos < p->tokenlen) ? (void) (0) : __assert_fail ("p->tokenpos < p->tokenlen", "jv_parse.c", 385, __PRETTY_FUNCTION__));
...
static __attribute__((warn_unused_result)) presult scan(struct jv_parser* p, char ch, jv* out) {
  ...
  if (ch == '\036' ) {
    ...
    ((!jv_is_valid(*out)) ? (void) (0) : __assert_fail ("!jv_is_valid(*out)", "jv_parse.c", 602, __PRETTY_FUNCTION__));
    parser_reset(p);
    jv_free(*out);
    *out = jv_invalid();
    return OK;
...
void jv_parser_set_buf(struct jv_parser* p, const char* buf, int length, int is_partial) {
  (((p->curr_buf == 0 || p->curr_buf_pos == p->curr_buf_length) && "previous buffer not exhausted") ? (void) (0) : __assert_fail ("(p->curr_buf == 0 || p->curr_buf_pos == p->curr_buf_length) && \"previous buffer not exhausted\"",
 "jv_parse.c"
# 672 "jv_parse.c" 3 4
  ,
 673
...
jv jv_parser_next(struct jv_parser* p) {
  ...
  if (msg == OK) {
    ...
    ((p->curr_buf_pos == p->curr_buf_length) ? (void) (0) : __assert_fail ("p->curr_buf_pos == p->curr_buf_length", "jv_parse.c", 743, __PRETTY_FUNCTION__));

    return jv_invalid();
  } else {

    ((p->curr_buf_pos == p->curr_buf_length) ? (void) (0) : __assert_fail ("p->curr_buf_pos == p->curr_buf_length", "jv_parse.c", 748, __PRETTY_FUNCTION__));
    jv_free(value);
