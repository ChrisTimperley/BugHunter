...
jv expand_path(jv path) {
  ((jv_get_kind(path) == JV_KIND_STRING) ? (void) (0) : __assert_fail ("jv_get_kind(path) == JV_KIND_STRING", "util.c", 46, __PRETTY_FUNCTION__));
...
jq_util_input_state jq_util_input_init(jq_msg_cb err_cb, void *err_cb_data) {
  ...
  new_state->parser = ((void *)0);
  new_state->current_input = ((void *)0);
...
void jq_util_input_set_parser(jq_util_input_state state, jv_parser *parser, int slurp) {
  ((!jv_is_valid(state->slurped)) ? (void) (0) : __assert_fail ("!jv_is_valid(state->slurped)", "util.c", 190, __PRETTY_FUNCTION__));
  state->parser = parser;
...
static int jq_util_input_read_more(jq_util_input_state state) {
  ...
  if (state->current_input) {
    ...
    while (!(res = fgets(state->buf, sizeof(state->buf), state->current_input)) &&
           ferror(state->current_input) && (*__errno_location ()) == 4)
...
jv jq_util_input_get_position(jq_state *jq) {
  ...
  ((cb == jq_util_input_next_input_cb) ? (void) (0) : __assert_fail ("cb == jq_util_input_next_input_cb", "util.c", 329, __PRETTY_FUNCTION__));
