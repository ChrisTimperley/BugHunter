...
int loadAppendOnlyFile(char *filename) {
    ...
    while(1) {
        ...
        ((fakeClient->bufpos == 0 && ((fakeClient->reply)->len) == 0)?(void)0 : (_redisAssert("fakeClient->bufpos == 0 && listLength(fakeClient->reply) == 0","aof.c",620),_exit(1)));

        (((fakeClient->flags & (1<<4)) == 0)?(void)0 : (_redisAssert("(fakeClient->flags & REDIS_BLOCKED) == 0","aof.c",622),_exit(1)));
...
int rioWriteBulkObject(rio *r, robj *obj) {
    ...
    if (obj->encoding == 1) {
        ...
        _redisPanic("\"Unknown string encoding\"","aof.c",669),_exit(1);
    }
}
...
int rewriteListObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","aof.c",725),_exit(1);
    }
    return 1;
}
...
int rewriteSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 6) {
        ...
        _redisPanic("\"Unknown set encoding\"","aof.c",772),_exit(1);
    }
    return 1;
}
...
int rewriteSortedSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 5) {
        ...
        ((eptr != ((void *)0))?(void)0 : (_redisAssert("eptr != NULL","aof.c",791),_exit(1)));
        sptr = ziplistNext(zl,eptr);
        ((sptr != ((void *)0))?(void)0 : (_redisAssert("sptr != NULL","aof.c",793),_exit(1)));

        while (eptr != ((void *)0)) {
            ((ziplistGet(eptr,&vstr,&vlen,&vll))?(void)0 : (_redisAssert("ziplistGet(eptr,&vstr,&vlen,&vll)","aof.c",796),_exit(1)));
        ...
        _redisPanic("\"Unknown sorted zset encoding\"","aof.c",841),_exit(1);
    }
    return 1;
}
...
static int rioWriteHashIteratorCursor(rio *r, hashTypeIterator *hi, int what) {
    ...
    _redisPanic("\"Unknown hash encoding\"","aof.c",872),_exit(1);
    return 0;
}
...
int rewriteAppendOnlyFile(char *filename) {
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (o->type == 0) {
                ...
                _redisPanic("\"Unknown object type\"","aof.c",979),_exit(1);
            }
    ...
    if (fdatasync(fileno(fp)) == -1) goto werr;
