...
int connectWithMaster(void) {
    ...
    if (aeCreateFileEvent(server.el,fd,2,syncWithMaster,((void *)0)) ==
            -1)
    {
        close(fd);
        redisLog(3,"Can't create readable event for SYNC");
...
void replicationCron(void) {
    ...
    if (server.masterhost && server.replstate == 3 &&
        (time(((void *)0))-server.repl_transfer_lastio) > 60)
    {
    ...
    if (server.masterhost && server.replstate == 4 &&
        (time(((void *)0))-server.master->lastinteraction) > 60)
    {
