...
void listTypePush(robj *subject, robj *value, int where) {
    ...
    if (subject->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",73),_exit(1);
    }
}
...
robj *listTypePop(robj *subject, int where) {
    ...
    if (subject->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",109),_exit(1);
    }
    return value;
}
...
unsigned long listTypeLength(robj *subject) {
    if (subject->encoding == 5) {
        ...
        return (((list*)subject->ptr)->len);
    } else {
        _redisPanic("\"Unknown list encoding\"","t_list.c",120),_exit(1);
    }
}
...
listTypeIterator *listTypeInitIterator(robj *subject, long index, unsigned char direction) {
    ...
    } else if (li->encoding == 4) {
        li->ln = listIndex(subject->ptr,index);
    } else {
        _redisPanic("\"Unknown list encoding\"","t_list.c",135),_exit(1);
    }
    return li;
}
...
int listTypeNext(listTypeIterator *li, listTypeEntry *entry) {
    ...
    ((li->subject->encoding == li->encoding)?(void)0 : (_redisAssert("li->subject->encoding == li->encoding","t_list.c",150),_exit(1)));
    ...
    if (li->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",172),_exit(1);
    }
    return 0;
}
...
robj *listTypeGet(listTypeEntry *entry) {
    ...
    if (li->encoding == 5) {
        ...
        unsigned int vlen;
        long long vlong;
        ((entry->zi != ((void *)0))?(void)0 : (_redisAssert("entry->zi != NULL","t_list.c",185),_exit(1)));
        ...
        ((entry->ln != ((void *)0))?(void)0 : (_redisAssert("entry->ln != NULL","t_list.c",194),_exit(1)));
        value = ((entry->ln)->value);
        incrRefCount(value);
    } else {
        _redisPanic("\"Unknown list encoding\"","t_list.c",198),_exit(1);
    }
    return value;
}
...
void listTypeInsert(listTypeEntry *entry, robj *value, int where) {
    ...
    if (entry->li->encoding == 5) {
        ...
        if (where == 1) {
            ...
            if (next == ((void *)0)) {
                subject->ptr = ziplistPush(subject->ptr,value->ptr,sdslen(value->ptr),1);
            } else {
        ...
        decrRefCount(value);
    } else if (entry->li->encoding == 4) {
        if (where == 1) {
            listInsertNode(subject->ptr,entry->ln,value,1);
        } else {
            listInsertNode(subject->ptr,entry->ln,value,0);
        }
        incrRefCount(value);
    } else {
        _redisPanic("\"Unknown list encoding\"","t_list.c",229),_exit(1);
    }
}
...
int listTypeEqual(listTypeEntry *entry, robj *o) {
    listTypeIterator *li = entry->li;
    if (li->encoding == 5) {
        ((o->encoding == 0)?(void)0 : (_redisAssertWithInfo(((void *)0),o,"o->encoding == REDIS_ENCODING_RAW","t_list.c",237),_exit(1)));
        return ziplistCompare(entry->zi,o->ptr,sdslen(o->ptr));
    } else if (li->encoding == 4) {
        return equalStringObjects(o,((entry->ln)->value));
    } else {
        _redisPanic("\"Unknown list encoding\"","t_list.c",242),_exit(1);
    }
}
...
void listTypeDelete(listTypeEntry *entry) {
    ...
    if (li->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",267),_exit(1);
    }
}
...
void listTypeConvert(robj *subject, int enc) {
    listTypeIterator *li;
    listTypeEntry entry;
    ((subject->type == 1)?(void)0 : (_redisAssertWithInfo(((void *)0),subject,"subject->type == REDIS_LIST","t_list.c",274),_exit(1)));
    ...
    if (enc == 4) {
        ...
        _redisPanic("\"Unsupported list conversion\"","t_list.c",289),_exit(1);
    }
}
...
void pushGenericCommand(redisClient *c, int where) {
    ...
    int may_have_waiting_clients = (lobj == ((void *)0));
...
void pushxGenericCommand(redisClient *c, robj *refval, robj *val, int where) {
    ...
    if ((subject = lookupKeyReadOrReply(c,c->argv[1],shared.czero)) == ((void *)0) ||
        checkType(c,subject,1)) return;

    if (refval != ((void *)0)) {


        ((refval->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,refval,"refval->encoding == REDIS_ENCODING_RAW","t_list.c",343),_exit(1)));
...
void lpushxCommand(redisClient *c) {
    ...
    pushxGenericCommand(c,((void *)0),c->argv[2],0);
}

void rpushxCommand(redisClient *c) {
    c->argv[2] = tryObjectEncoding(c->argv[2]);
    pushxGenericCommand(c,((void *)0),c->argv[2],1);
}
...
void lindexCommand(redisClient *c) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",446),_exit(1);
    }
}
...
void lsetCommand(redisClient *c) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",487),_exit(1);
    }
}
...
void lrangeCommand(redisClient *c) {
    ...
    if ((o = lookupKeyReadOrReply(c,c->argv[1],shared.emptymultibulk)) == ((void *)0)
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"List encoding is not LINKEDLIST nor ZIPLIST!\"","t_list.c",570),_exit(1);
    }
}
...
void ltrimCommand(redisClient *c) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",619),_exit(1);
    }
    if (listTypeLength(o) == 0) dbDelete(c->db,c->argv[1]);
    signalModifiedKey(c->db,c->argv[1]);
    server.dirty++;
...
void rpoplpushCommand(redisClient *c) {
    robj *sobj, *value;
    if ((sobj = lookupKeyWriteOrReply(c,c->argv[1],shared.nullbulk)) == ((void *)0) ||
        checkType(c,sobj,1)) return;
...
void blockForKeys(redisClient *c, robj **keys, int numkeys, time_t timeout, robj *target) {
    ...
    for (j = 0; j < numkeys; j++) {
        ...
        if (dictAdd(c->bpop.keys,keys[j],((void *)0)) != 0) continue;
        incrRefCount(keys[j]);
        ...
        if (de == ((void *)0)) {
            ...
            incrRefCount(keys[j]);
            ((retval == 0)?(void)0 : (_redisAssertWithInfo(c,keys[j],"retval == DICT_OK","t_list.c",779),_exit(1)));
        } else {
            l = ((de)->v.val);
        }
        listAddNodeTail(l,c);
...
void unblockClientWaitingData(redisClient *c) {
    ...
    ((((c->bpop.keys)->ht[0].used+(c->bpop.keys)->ht[1].used) != 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"dictSize(c->bpop.keys) != 0","t_list.c",797),_exit(1)));
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        ((l != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,key,"l != NULL","t_list.c",805),_exit(1)));
...
void signalListAsReady(redisClient *c, robj *key) {
    ...
    if (dictFind(c->db->blocking_keys,key) == ((void *)0)) return;


    if (dictFind(c->db->ready_keys,key) != ((void *)0)) return;
    ...
    incrRefCount(key);
    ((dictAdd(c->db->ready_keys,key,((void *)0)) == 0)?(void)0 : (_redisAssert("dictAdd(c->db->ready_keys,key,NULL) == DICT_OK","t_list.c",852),_exit(1)));
}
# 874 "t_list.c"
...
void blockingPopGenericCommand(redisClient *c, int where) {
        ...
        if (o != ((void *)0)) {
            if (o->type != 1) {
                ...
                if (listTypeLength(o) != 0) {
                    ...
                    robj *value = listTypePop(o,where);
                    ((value != ((void *)0))?(void)0 : (_redisAssert("value != NULL","t_list.c",1050),_exit(1)));
    ...
    blockForKeys(c, c->argv + 1, c->argc - 2, timeout, ((void *)0));
}
...
void brpoplpushCommand(redisClient *c) {
    ...
    if (key == ((void *)0)) {
        ...
        if (key->type != 1) {
            ...
            ((listTypeLength(key) > 0)?(void)0 : (_redisAssertWithInfo(c,key,"listTypeLength(key) > 0","t_list.c",1112),_exit(1)));
            rpoplpushCommand(c);
        }
    }
}
