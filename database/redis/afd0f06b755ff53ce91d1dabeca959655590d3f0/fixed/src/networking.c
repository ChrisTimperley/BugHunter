...
robj *dupLastObjectIfNeeded(list *reply) {
    robj *new, *cur;
    listNode *ln;
    ((((reply)->len) > 0)?(void)0 : (_redisAssert("listLength(reply) > 0","networking.c",84),_exit(1)));
    ln = ((reply)->tail);
    cur = ((ln)->value);
    if (cur->refcount > 1) {
...
void freeClient(redisClient *c) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",497),_exit(1)));
    listDelNode(server.clients,ln);


    if (c->flags & 256) {
        ln = listSearchKey(server.unblocked_clients,c);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",503),_exit(1)));
    ...
    if (c->flags & 1) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",514),_exit(1)));
        listDelNode(l,ln);
    }
...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argc == 0","networking.c",708),_exit(1)));
        ...
        ((c->querybuf[0] == '*')?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->querybuf[0] == '*'","networking.c",721),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->multibulklen > 0","networking.c",742),_exit(1)));
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",848),_exit(1);
        }
...
void readQueryFromClient(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (sdslen(c->querybuf) > server.client_max_querybuf_len) {
        ...
        redisLog(3,"Closing client that reached max query buffer length: %s", ci);
        sdsfree(ci);
        freeClient(c);
        return;
    }
...
void rewriteClientCommandVector(redisClient *c, int argc, ...) {
    ...
    ((c->cmd != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",1048),_exit(1)));
    __builtin_va_end(ap);
}
...
void rewriteClientCommandArgument(redisClient *c, int i, robj *newval) {
    ...
    ((i < c->argc)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"i < c->argc","networking.c",1057),_exit(1)));
    oldval = c->argv[i];
    c->argv[i] = newval;
    incrRefCount(newval);
    decrRefCount(oldval);
    ...
    if (i == 0) {
        c->cmd = lookupCommand(c->argv[0]->ptr);
        ((c->cmd != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",1066),_exit(1)));
    }
}
