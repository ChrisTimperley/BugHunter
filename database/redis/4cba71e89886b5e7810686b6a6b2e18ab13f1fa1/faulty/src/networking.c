...
robj *dupLastObjectIfNeeded(list *reply) {
    robj *new, *cur;
    listNode *ln;
    ((((reply)->len) > 0)?(void)0 : (_redisAssert("listLength(reply) > 0","networking.c",109),_exit(1)));
    ln = ((reply)->tail);
    cur = ((ln)->value);
    if (cur->refcount > 1) {
...
# 249 "networking.c"
    if (obj->encoding == 0) {
        ...
        _redisPanic("\"Wrong obj->encoding in addReply()\"","networking.c",271),_exit(1);
    }
}
...
void freeClient(redisClient *c) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",592),_exit(1)));
    listDelNode(server.clients,ln);


    if (c->flags & 256) {
        ln = listSearchKey(server.unblocked_clients,c);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",598),_exit(1)));
    ...
    if (c->flags & 1) {
        if (c->replstate == 5 && c->repldbfd != -1)
            close(c->repldbfd);
        list *l = (c->flags & 4) ? server.monitors : server.slaves;
        ln = listSearchKey(l,c);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",609),_exit(1)));
        listDelNode(l,ln);
    }
    ...
    if (c->flags & 2048) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",630),_exit(1)));
...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argc == 0","networking.c",815),_exit(1)));
        ...
        ((c->querybuf[0] == '*')?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->querybuf[0] == '*'","networking.c",833),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->multibulklen > 0","networking.c",854),_exit(1)));
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",965),_exit(1);
        }
...
sds getClientInfoString(redisClient *client) {
    ...
    if (client->flags & 2) *p++ = 'M';
    if (client->flags & 8) *p++ = 'x';
    if (client->flags & 16) *p++ = 'b';
    if (client->flags & 64) *p++ = 'd';
    if (client->flags & 128) *p++ = 'c';
    if (client->flags & 256) *p++ = 'u';
    if (client->flags & 2048) *p++ = 'A';
    if (p == flags) *p++ = 'N';
    *p++ = '\0';
...
void rewriteClientCommandVector(redisClient *c, int argc, ...) {
    ...
    ((c->cmd != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",1184),_exit(1)));
    __builtin_va_end(ap);
}
...
void rewriteClientCommandArgument(redisClient *c, int i, robj *newval) {
    ...
    ((i < c->argc)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"i < c->argc","networking.c",1193),_exit(1)));
    oldval = c->argv[i];
    c->argv[i] = newval;
    incrRefCount(newval);
    decrRefCount(oldval);
    ...
    if (i == 0) {
        c->cmd = lookupCommand(c->argv[0]->ptr);
        ((c->cmd != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",1202),_exit(1)));
    }
}
# 1219 "networking.c"
unsigned long getClientOutputBufferMemoryUsage(redisClient *c) {
...
# 1303 "networking.c"
void asyncCloseClientOnOutputBufferLimitReached(redisClient *c) {
    if (c->reply_bytes == 0 || c->flags & 2048) return;
