...
static int getnum (const char **fmt, int df) {
  if (!((*__ctype_b_loc ())[(int) ((**fmt))] & (unsigned short int) _ISdigit))
    return df;
  else {
    int a = 0;
    do {
      a = a*10 + *((*fmt)++) - '0';
    } while (((*__ctype_b_loc ())[(int) ((**fmt))] & (unsigned short int) _ISdigit));
    return a;
  }
}
...
static void controloptions (lua_State *L, int opt, const char **fmt,
  ...
  switch (opt) {
    case ' ': return;
    case '>': h->endian = 0; return;
    case '<': h->endian = 1; return;
    case '!': {
      int a = getnum(fmt, ((sizeof(struct cD) - sizeof(double)) > sizeof(int) ? (sizeof(struct cD) - sizeof(double)) : sizeof(int)));
      if (!((a) > 0 && ((a) & ((a) - 1)) == 0))
...
static int b_pack (lua_State *L) {
  ...
  int arg = 2;
  size_t totalsize = 0;
  ((&h)->endian = native.endian, (&h)->align = 1);
  lua_pushnil(L);
  luaL_buffinit(L, &b);
  while (*fmt != '\0') {
    int opt = *fmt++;
    ...
    totalsize += toalign;
    while (toalign-- > 0) ((void)((&b)->p < ((&b)->buffer+8192) || luaL_prepbuffer(&b)), (*(&b)->p++ = (char)('\0')));
    switch (opt) {
      ...
      case 'x': {
        ((void)((&b)->p < ((&b)->buffer+8192) || luaL_prepbuffer(&b)), (*(&b)->p++ = (char)('\0')));
        break;
      }
      case 'f': {
      ...
      case 'c': case 's': {
        ...
        if (size == 0) size = l;
        ((void)((l >= (size_t)size) || luaL_argerror(L, (arg), ("string too short"))));
...
static int b_unpack (lua_State *L) {
  ...
  while (*fmt) {
    ...
    ((void)((pos+size <= ld) || luaL_argerror(L, (2), ("data string too short"))));
    luaL_checkstack(L, 1, "too many results");
    switch (opt) {
      case 'b': case 'B': case 'h': case 'H':
      case 'l': case 'L': case 'T': case 'i': case 'I': {
        int issigned = ((*__ctype_b_loc ())[(int) ((opt))] & (unsigned short int) _ISlower);
...
static int b_size (lua_State *L) {
  ...
  while (*fmt) {
    ...
    if (!((*__ctype_b_loc ())[(int) ((opt))] & (unsigned short int) _ISalnum))
