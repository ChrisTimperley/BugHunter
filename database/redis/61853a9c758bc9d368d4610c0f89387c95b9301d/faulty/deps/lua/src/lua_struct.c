...
static int gettoalign (size_t len, Header *h, int opt, size_t size) {
  ...
  if (size > (size_t)h->align) size = h->align;
...
static void commoncases (lua_State *L, int opt, const char **fmt, Header *h) {
  switch (opt) {
    ...
    default: ((0) ? (void) (0) : __assert_fail ("0", "lua_struct.c", 130, __PRETTY_FUNCTION__));
  }
}
...
static void putinteger (lua_State *L, luaL_Buffer *b, int arg, int endian,
  ...
  unsigned long value;
  if (n < (lua_Number)9223372036854775807L)
    value = (long)n;
  else
  ...
  if (endian == 1) {
    int i;
    for (i = 0; i < size; i++)
      ((void)((b)->p < ((b)->buffer+8192) || luaL_prepbuffer(b)), (*(b)->p++ = (char)((value >> 8*i) & 0xff)));
  }
  else {
    int i;
    for (i = size - 1; i >= 0; i--)
