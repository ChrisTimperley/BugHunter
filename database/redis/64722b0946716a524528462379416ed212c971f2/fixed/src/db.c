...
void scanCommand(redisClient *c) {
    ...
    ((c->argc >= 2)?(void)0 : (_redisAssert("c->argc >= 2","db.c",340),_exit(1)));
    ...
    while (i < c->argc) {
        ...
        if (!strcasecmp(c->argv[i]->ptr, "count") && j >= 2) {
            if (getLongFromObjectOrReply(c, c->argv[i+1], &count, ((void *)0)) != 0) {
                goto cleanup;
            }
    ...
    ((rv < sizeof(buf))?(void)0 : (_redisAssert("rv < sizeof(buf)","db.c",403),_exit(1)));
    addReplyBulkCBuffer(c, buf, rv);

    addReplyMultiBulkLen(c, ((keys)->len));
    while ((ln = ((keys)->head)) != ((void *)0)) {
        robj *kobj = ((ln)->value);
        addReplyBulk(c, kobj);
        decrRefCount(kobj);
        listDelNode(keys, ln);
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",579),_exit(1)));
...
void setExpire(redisDb *db, robj *key, long long when) {
    ...
    ((kde != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"kde != NULL","db.c",588),_exit(1)));
...
long long getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",604),_exit(1)));
    return ((de)->v.s64);
}
# 616 "db.c"
void propagateExpire(redisDb *db, robj *key) {
...
void expireGenericCommand(redisClient *c, long long basetime, int unit) {
    ...
    if (when <= mstime() && !server.loading && !server.masterhost) {
        ...
        ((dbDelete(c->db,key))?(void)0 : (_redisAssertWithInfo(c,key,"dbDelete(c->db,key)","db.c",698),_exit(1)));
        server.dirty++;
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","db.c",797),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
