...
static void __redisSetErrorFromErrno(redisContext *c, int type, const char *prefix) {
    ...
    strerror_r((*__errno_location ()),buf+len,sizeof(buf)-len);
...
int redisContextSetTimeout(redisContext *c, const struct timeval tv) {
    if (setsockopt(c->fd,1,20,&tv,sizeof(tv)) == -1) {
        __redisSetErrorFromErrno(c,1,"setsockopt(SO_RCVTIMEO)");
        return -1;
    }
    if (setsockopt(c->fd,1,21,&tv,sizeof(tv)) == -1) {
    ...
    for (p = servinfo; p != ((void *)0); p = p->ai_next) {
        ...
        if (connect(s,p->ai_addr,p->ai_addrlen) == -1) {
            if ((*__errno_location ()) == 113) {
                redisContextCloseFd(c);
                continue;
            } else if ((*__errno_location ()) == 115 && !blocking) {

            } else {
                if (redisContextWaitReady(c,timeout) != 0)
                    goto error;
            }
        }
