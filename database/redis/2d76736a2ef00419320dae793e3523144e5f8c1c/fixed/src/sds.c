...
void sdsIncrLen(sds s, int incr) {
    ...
    ((sh->free >= incr) ? (void) (0) : __assert_fail ("sh->free >= incr", "sds.c", 203, __PRETTY_FUNCTION__));
    sh->len += incr;
    sh->free -= incr;
    ((sh->free >= 0) ? (void) (0) : __assert_fail ("sh->free >= 0", "sds.c", 206, __PRETTY_FUNCTION__));
    s[sh->len] = '\0';
}
...
sds sdstrim(sds s, const char *cset) {
    ...
    while(ep > start && (__extension__ (__builtin_constant_p (*ep) && !__builtin_constant_p (cset) && (*ep) == '\0' ? (char *) __rawmemchr (cset, *ep) : __builtin_strchr (cset, *ep)))) ep--;
    len = (sp > ep) ? 0 : ((ep-sp)+1);
