...
void sdsIncrLen(sds s, int incr) {
    ...
    switch(flags&7) {
        case 0: {
            ...
            (((incr > 0 && oldlen+incr < 32) || (incr < 0 && oldlen >= (unsigned int)(-incr))) ? (void) (0) : __assert_fail ("(incr > 0 && oldlen+incr < 32) || (incr < 0 && oldlen >= (unsigned int)(-incr))", "sds.c", 307, __PRETTY_FUNCTION__));
            *fp = 0 | ((oldlen+incr) << 3);
            len = oldlen+incr;
            break;
        }
        case 1: {
            struct sdshdr8 *sh = (void*)((s)-(sizeof(struct sdshdr8)));;
            (((incr >= 0 && sh->alloc-sh->len >= incr) || (incr < 0 && sh->len >= (unsigned int)(-incr))) ? (void) (0) : __assert_fail ("(incr >= 0 && sh->alloc-sh->len >= incr) || (incr < 0 && sh->len >= (unsigned int)(-incr))", "sds.c", 314, __PRETTY_FUNCTION__));
            len = (sh->len += incr);
            break;
        }
        case 2: {
            struct sdshdr16 *sh = (void*)((s)-(sizeof(struct sdshdr16)));;
            (((incr >= 0 && sh->alloc-sh->len >= incr) || (incr < 0 && sh->len >= (unsigned int)(-incr))) ? (void) (0) : __assert_fail ("(incr >= 0 && sh->alloc-sh->len >= incr) || (incr < 0 && sh->len >= (unsigned int)(-incr))", "sds.c", 320, __PRETTY_FUNCTION__));
            len = (sh->len += incr);
            break;
        }
        case 3: {
            struct sdshdr32 *sh = (void*)((s)-(sizeof(struct sdshdr32)));;
            (((incr >= 0 && sh->alloc-sh->len >= (unsigned int)incr) || (incr < 0 && sh->len >= (unsigned int)(-incr))) ? (void) (0) : __assert_fail ("(incr >= 0 && sh->alloc-sh->len >= (unsigned int)incr) || (incr < 0 && sh->len >= (unsigned int)(-incr))", "sds.c", 326, __PRETTY_FUNCTION__));
            len = (sh->len += incr);
            break;
        }
        case 4: {
            struct sdshdr64 *sh = (void*)((s)-(sizeof(struct sdshdr64)));;
            (((incr >= 0 && sh->alloc-sh->len >= (uint64_t)incr) || (incr < 0 && sh->len >= (uint64_t)(-incr))) ? (void) (0) : __assert_fail ("(incr >= 0 && sh->alloc-sh->len >= (uint64_t)incr) || (incr < 0 && sh->len >= (uint64_t)(-incr))", "sds.c", 332, __PRETTY_FUNCTION__));
            len = (sh->len += incr);
            break;
        }
    }
...
sds sdscatfmt(sds s, char const *fmt, ...) {
    ...
    while(*f) {
        ...
        case '%':
            ...
            case 'U':
                if (next == 'u')
                    unum = __builtin_va_arg(ap,unsigned int);
                else
                    unum = __builtin_va_arg(ap,unsigned long long);
                {
                    char buf[21];
...
sds sdstrim(sds s, const char *cset) {
    ...
    sp = start = s;
    ep = end = s+sdslen(s)-1;
    while(sp <= end && (__extension__ (__builtin_constant_p (*sp) && !__builtin_constant_p (cset) && (*sp) == '\0' ? (char *) __rawmemchr (cset, *sp) : __builtin_strchr (cset, *sp)))) sp++;
    while(ep > sp && (__extension__ (__builtin_constant_p (*ep) && !__builtin_constant_p (cset) && (*ep) == '\0' ? (char *) __rawmemchr (cset, *ep) : __builtin_strchr (cset, *ep)))) ep--;
    len = (sp > ep) ? 0 : ((ep-sp)+1);
...
void sdstolower(sds s) {
    ...
    for (j = 0; j < len; j++) s[j] = (__extension__ ({ int __res; if (sizeof (s[j]) > 1) { if (__builtin_constant_p (s[j])) { int __c = (s[j]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower (s[j]); } else __res = (*__ctype_tolower_loc ())[(int) (s[j])]; __res; }));
}
...
void sdstoupper(sds s) {
    ...
    for (j = 0; j < len; j++) s[j] = (__extension__ ({ int __res; if (sizeof (s[j]) > 1) { if (__builtin_constant_p (s[j])) { int __c = (s[j]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_toupper_loc ())[__c]; } else __res = toupper (s[j]); } else __res = (*__ctype_toupper_loc ())[(int) (s[j])]; __res; }));
}
# 754 "sds.c"
int sdscmp(const sds s1, const sds s2) {
    size_t l1, l2, minlen;
...
sds sdscatrepr(sds s, const char *p, size_t len) {
    ...
    while(len--) {
        ...
        default:
            if (((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISprint))
                s = sdscatprintf(s,"%c",*p);
            else
...
sds *sdssplitargs(const char *line, int *argc) {
    ...
    *argc = 0;
    while(1) {
        ...
        while(*p && ((*__ctype_b_loc ())[(int) ((*p))] & (unsigned short int) _ISspace)) p++;
        if (*p) {
            ...
            while(!done) {
                if (inq) {
                                             ...
                                             is_hex_digit(*(p+3)))
                    {
                        ...
                        if (*(p+1) && !((*__ctype_b_loc ())[(int) ((*(p+1)))] & (unsigned short int) _ISspace)) goto err;
                        done=1;
                    } else if (!*p) {
            ...
            if (vector == ((void *)0)) vector = zmalloc(sizeof(void*));
            return vector;
        }
    }
