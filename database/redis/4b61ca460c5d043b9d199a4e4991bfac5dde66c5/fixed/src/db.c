...
void lookupWaitBusyKey(redisDb *db, robj *key) {
    ...
    (((cacheScheduleIOGetFlags(db,key) & 8) == 0)?(void)0 : (_redisAssert("(cacheScheduleIOGetFlags(db,key) & REDIS_IO_SAVEINPROG) == 0","db.c",35),_exit(1)));
}
...
robj *lookupKey(redisDb *db, robj *key) {
    ...
    if (de) {
        ...
        if (server.ds_enabled &&
            cacheScheduleIOGetFlags(db,key) & 8)
        {

            redisLog(0,"Lookup found a key in SAVEINPROG state. Waiting. (Key was in the cache)");
        ...
        if (server.ds_enabled && cacheKeyMayExist(db,key)) {
            ...
            if (flags & 8) {
                redisLog(0,"Lookup found a key in SAVEINPROG state. Waiting (while force loading).");
                lookupWaitBusyKey(db,key);
            }

            redisLog(0,"Force loading key %s via lookup", key->ptr);
            val = dsGet(db,key,&expire);
            if (val) {
                int retval = dbAdd(db,key,val);
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","db.c",87),_exit(1)));
...
int dbAdd(redisDb *db, robj *key, robj *val) {
    ...
    if (dictFind(db->dict, key->ptr) != ((void *)0)) {
        return -1;
    } else {
...
int dbReplace(redisDb *db, robj *key, robj *val) {
    ...
    if ((oldval = dictFetchValue(db->dict,key->ptr)) == ((void *)0)) {
        sds copy = sdsdup(key->ptr);
        dictAdd(db->dict, copy, val);
        retval = 1;
    } else {
...
void delCommand(redisClient *c) {
    ...
    for (j = 1; j < c->argc; j++) {
        ...
        if (dbDelete(c->db,c->argv[j])) {
            ...
            {
                cacheScheduleIO(c->db,c->argv[j],2);
                deleted = 1;
            }
        }
    }
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",475),_exit(1)));
...
void setExpire(redisDb *db, robj *key, time_t when) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","db.c",484),_exit(1)));
...
time_t getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",499),_exit(1)));
...
void expireGenericCommand(redisClient *c, robj *key, robj *param, long offset) {
    ...
    if (getLongFromObjectOrReply(c, param, &seconds, ((void *)0)) != 0) return;
...
void expireatCommand(redisClient *c) {
    expireGenericCommand(c,c->argv[1],c->argv[2],time(((void *)0)));
}
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","db.c",636),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
