...
void processUnblockedClients(void) {
    ...
    while (((server.unblocked_clients)->len)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","blocked.c",113),_exit(1)));
        c = ln->value;
...
void unblockClient(redisClient *c) {
    if (c->btype == 1) {
        ...
        _redisPanic("\"Unknown btype in unblockClient().\"","blocked.c",133),_exit(1);
    }
...
void replyToBlockedClientTimedOut(redisClient *c) {
    if (c->btype == 1) {
        ...
        _redisPanic("\"Unknown btype in replyToBlockedClientTimedOut().\"","blocked.c",152),_exit(1);
    }
}
