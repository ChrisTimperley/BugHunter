...
int anetNonBlock(char *err, int fd)
{
    ...
    if ((flags = fcntl(fd, 3)) == -1) {
        anetSetError(err, "fcntl(F_GETFL): %s", strerror((*__errno_location ())));
        return -1;
    }
    if (fcntl(fd, 4, flags | 04000) == -1) {
        anetSetError(err, "fcntl(F_SETFL,O_NONBLOCK): %s", strerror((*__errno_location ())));
        return -1;
    }
    return 0;
}
...
int anetKeepAlive(char *err, int fd, int interval)
{
    ...
    {
        anetSetError(err, "setsockopt SO_KEEPALIVE: %s", strerror((*__errno_location ())));
        return -1;
    }
...
{
    if (setsockopt(fd, IPPROTO_TCP, 1, &val, sizeof(val)) == -1)
    {
        anetSetError(err, "setsockopt TCP_NODELAY: %s", strerror((*__errno_location ())));
        return -1;
    }
    return 0;
}
...
{
    if (setsockopt(fd, 1, 7, &buffsize, sizeof(buffsize)) == -1)
    {
        anetSetError(err, "setsockopt SO_SNDBUF: %s", strerror((*__errno_location ())));
        return -1;
    }
    return 0;
}
...
{
    int yes = 1;
    if (setsockopt(fd, 1, 9, &yes, sizeof(yes)) == -1) {
        anetSetError(err, "setsockopt SO_KEEPALIVE: %s", strerror((*__errno_location ())));
        return -1;
    }
    return 0;
...
int anetResolve(char *err, char *host, char *ipbuf, size_t ipbuf_len)
{
    ...
    if (info->ai_family == 2) {
        ...
        inet_ntop(2, &(sa->sin_addr), ipbuf, ipbuf_len);
    } else {
        struct sockaddr_in6 *sa = (struct sockaddr_in6 *)info->ai_addr;
        inet_ntop(10, &(sa->sin6_addr), ipbuf, ipbuf_len);
    }
...
static int anetSetReuseAddr(char *err, int fd) {
    ...
    if (setsockopt(fd, 1, 2, &yes, sizeof(yes)) == -1) {
        anetSetError(err, "setsockopt SO_REUSEADDR: %s", strerror((*__errno_location ())));
        return -1;
    }
    return 0;
}
...
static int anetCreateSocket(char *err, int domain) {
    ...
    if ((s = socket(domain, SOCK_STREAM, 0)) == -1) {
        anetSetError(err, "creating socket: %s", strerror((*__errno_location ())));
        return -1;
    }
...
static int anetTcpGenericConnect(char *err, char *addr, int port, int flags)
{
    ...
    for (p = servinfo; p != ((void *)0); p = p->ai_next) {
        ...
        if ((s = socket(p->ai_family,p->ai_socktype,p->ai_protocol)) == -1)
            continue;
        if (anetSetReuseAddr(err,s) == -1) goto error;
        if (flags & 1 && anetNonBlock(err,s) != 0)
            goto error;
        if (connect(s,p->ai_addr,p->ai_addrlen) == -1) {


            if ((*__errno_location ()) == 115 && flags & 1)
                goto end;
            close(s);
            s = -1;
            continue;
...
int anetTcpConnect(char *err, char *addr, int port)
{
    return anetTcpGenericConnect(err,addr,port,0);
}

int anetTcpNonBlockConnect(char *err, char *addr, int port)
{
    return anetTcpGenericConnect(err,addr,port,1);
}
...
int anetUnixGenericConnect(char *err, char *path, int flags)
{
    ...
    if ((s = anetCreateSocket(err,1)) == -1)
        return -1;

    sa.sun_family = 1;
    strncpy(sa.sun_path,path,sizeof(sa.sun_path)-1);
    if (flags & 1) {
        if (anetNonBlock(err,s) != 0)
            return -1;
    }
    if (connect(s,(struct sockaddr*)&sa,sizeof(sa)) == -1) {
        ...
        anetSetError(err, "connect: %s", strerror((*__errno_location ())));
        close(s);
        return -1;
    }
    return s;
...
int anetUnixConnect(char *err, char *path)
{
    return anetUnixGenericConnect(err,path,0);
}

int anetUnixNonBlockConnect(char *err, char *path)
{
    return anetUnixGenericConnect(err,path,1);
}
...
static int anetListen(char *err, int s, struct sockaddr *sa, socklen_t len) {
    if (bind(s,sa,len) == -1) {
        anetSetError(err, "bind: %s", strerror((*__errno_location ())));
        close(s);
        return -1;
    }
    ...
    if (listen(s, 511) == -1) {
        anetSetError(err, "listen: %s", strerror((*__errno_location ())));
        close(s);
        return -1;
    }
    return 0;
...
static int anetV6Only(char *err, int s) {
    int yes = 1;
    if (setsockopt(s,IPPROTO_IPV6,26,&yes,sizeof(yes)) == -1) {
        anetSetError(err, "setsockopt: %s", strerror((*__errno_location ())));
        close(s);
        return -1;
    }
    return 0;
...
static int _anetTcpServer(char *err, int port, char *bindaddr, int af)
{
    ...
    for (p = servinfo; p != ((void *)0); p = p->ai_next) {
        ...
        if (af == 10 && anetV6Only(err,s) == -1) goto error;
        if (anetSetReuseAddr(err,s) == -1) goto error;
        if (anetListen(err,s,p->ai_addr,p->ai_addrlen) == -1) goto error;
        goto end;
    }
    if (p == ((void *)0)) {
...
int anetTcpServer(char *err, int port, char *bindaddr)
{
    return _anetTcpServer(err, port, bindaddr, 2);
}

int anetTcp6Server(char *err, int port, char *bindaddr)
{
    return _anetTcpServer(err, port, bindaddr, 10);
}
...
int anetUnixServer(char *err, char *path, mode_t perm)
{
    ...
    sa.sun_family = 1;
    strncpy(sa.sun_path,path,sizeof(sa.sun_path)-1);
    if (anetListen(err,s,(struct sockaddr*)&sa,sizeof(sa)) == -1)
        return -1;
    if (perm)
    ...
    while(1) {
        ...
        if (fd == -1) {
            if ((*__errno_location ()) == 4)
                ...
                anetSetError(err, "accept: %s", strerror((*__errno_location ())));
                return -1;
            }
        }
        break;
...
int anetTcpAccept(char *err, int s, char *ip, size_t ip_len, int *port) {
    ...
    if (sa.ss_family == 2) {
        ...
        if (ip) inet_ntop(2,(void*)&(s->sin_addr),ip,ip_len);
        if (port) *port = (__extension__ ({ unsigned short int __v, __x = (unsigned short int) (s->sin_port); if (__builtin_constant_p (__x)) __v = ((unsigned short int) ((((__x) >> 8) & 0xff) | (((__x) & 0xff) << 8))); else __asm__ ("rorw $8, %w0" : "=r" (__v) : "0" (__x) : "cc"); __v; }));
    } else {
        struct sockaddr_in6 *s = (struct sockaddr_in6 *)&sa;
        if (ip) inet_ntop(10,(void*)&(s->sin6_addr),ip,ip_len);
        if (port) *port = (__extension__ ({ unsigned short int __v, __x = (unsigned short int) (s->sin6_port); if (__builtin_constant_p (__x)) __v = ((unsigned short int) ((((__x) >> 8) & 0xff) | (((__x) & 0xff) << 8))); else __asm__ ("rorw $8, %w0" : "=r" (__v) : "0" (__x) : "cc"); __v; }));
    }
    return fd;
}
