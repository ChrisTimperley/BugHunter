...
void *IOThreadEntryPoint(void *arg) {
    ...
    while(1) {
        ...
        if (j->type == 0) {
            ...
            if (j->val) {
                ((j->val->storage == 2)?(void)0 : (_redisAssert("j->val->storage == REDIS_DS_SAVING","dscache.c",388),_exit(1)));
        ...
        ((write(server.io_ready_pipe_write,"x",1) == 1)?(void)0 : (_redisAssert("write(server.io_ready_pipe_write,\"x\",1) == 1","dscache.c",404),_exit(1)));
    }
...
void cacheCron(void) {
    ...
    while((ln = ((server.cache_flush_queue)->head)) != ((void *)0)) {
        ...
        if ((now - dk->ctime) >= server.cache_flush_delay) {
            ...
            if (de) {
                val = ((de)->val);
                ((val->storage == 1)?(void)0 : (_redisAssert("val->storage == REDIS_DS_DIRTY","dscache.c",542),_exit(1)));
                val->storage = 2;
            } else {
...
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",619),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",646),_exit(1)));
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",742),_exit(1)));


    de = dictFind(c->db->io_keys,key);
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",746),_exit(1)));
    l = ((de)->val);
    ln = listSearchKey(l,c);
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",749),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
