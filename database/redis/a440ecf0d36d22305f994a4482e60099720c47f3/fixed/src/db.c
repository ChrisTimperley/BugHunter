...
robj *lookupKey(redisDb *db, robj *key) {
    ...
    if (de) {
        ...
        if (server.ds_enabled && val->storage == 2) {


            waitEmptyIOJobsQueue();
            processAllPendingIOJobs();
            ((val->storage != 2)?(void)0 : (_redisAssert("val->storage != REDIS_DS_SAVING","db.c",25),_exit(1)));
        }
...
# 40 "db.c"
        if (server.ds_enabled && cacheKeyMayExist(db,key)) {
            ...
            if (val) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","db.c",44),_exit(1)));
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",451),_exit(1)));
...
void setExpire(redisDb *db, robj *key, time_t when) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","db.c",460),_exit(1)));
