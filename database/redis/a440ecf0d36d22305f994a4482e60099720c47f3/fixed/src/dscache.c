...
void dsInit(void) {
    ...
    ((anetNonBlock(((void *)0),server.io_ready_pipe_read) != -1)?(void)0 : (_redisAssert("anetNonBlock(NULL,server.io_ready_pipe_read) != ANET_ERR","dscache.c",163),_exit(1)));
            ...
            int mask)
{
    ...
    while((retval = read(fd,buf,1)) == 1) {
        ...
        lockThreadedIO();
        ((((server.io_processed)->len) != 0)?(void)0 : (_redisAssert("listLength(server.io_processed) != 0","dscache.c",293),_exit(1)));
        if (toprocess == -1) {
        ...
        if (j->type == 0) {
            ...
            if (j->val) {
                ((j->val->storage == 2)?(void)0 : (_redisAssert("j->val->storage == REDIS_DS_SAVING","dscache.c",328),_exit(1)));
                j->val->storage = 0;
            }
            freeIOJob(j);
        }
...
void *IOThreadEntryPoint(void *arg) {
    ...
    while(1) {
        ...
        if (j->type == 0) {
            ...
            if (j->val) {
                ((j->val->storage == 2)?(void)0 : (_redisAssert("j->val->storage == REDIS_DS_SAVING","dscache.c",386),_exit(1)));
        ...
        ((write(server.io_ready_pipe_write,"x",1) == 1)?(void)0 : (_redisAssert("write(server.io_ready_pipe_write,\"x\",1) == 1","dscache.c",402),_exit(1)));
    }
...
void cacheScheduleForFlush(redisDb *db, robj *key) {
    ...
    redisLog(0,"Scheduling key %s for saving (%s)",key->ptr,
        de ? "key exists" : "key does not exist");
    dk = zmalloc(sizeof(*dk));
    dk->db = db;
    dk->key = key;
    incrRefCount(key);
    dk->ctime = time(((void *)0));
...
void cacheCron(void) {
    ...
    while((ln = ((server.cache_flush_queue)->head)) != ((void *)0)) {
        ...
        if ((now - dk->ctime) >= server.cache_flush_delay) {
            ...
            redisLog(0,"Creating IO Job to save key %s",dk->key->ptr);
            ...
            if (de) {
                val = ((de)->val);
                ((val->storage == 1)?(void)0 : (_redisAssert("val->storage == REDIS_DS_DIRTY","dscache.c",551),_exit(1)));
                val->storage = 2;
            } else {
            ...
            dsCreateIOJob(1,dk->db,dk->key,val);
...
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",628),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
# 642 "dscache.c"
    if (de == ((void *)0))
        dsCreateIOJob(0,c->db,key,((void *)0));
    return 1;
}
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",655),_exit(1)));
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",751),_exit(1)));


    de = dictFind(c->db->io_keys,key);
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",755),_exit(1)));
    l = ((de)->val);
    ln = listSearchKey(l,c);
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",758),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
