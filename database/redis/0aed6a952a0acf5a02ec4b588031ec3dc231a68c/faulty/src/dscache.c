...
void dsInit(void) {
    ...
    redisLog(2,"Opening Disk Store: %s", server.ds_path);

    if (dsOpen() != 0) {
        redisLog(3,"Fatal error opening disk store. Exiting.");
        exit(1);
    };
    ...
    pthread_mutex_init(&server.io_mutex,((void *)0));
    pthread_cond_init(&server.io_condvar,((void *)0));
    pthread_mutex_init(&server.bgsavethread_mutex,((void *)0));
    server.io_active_threads = 0;
    if (pipe(pipefds) == -1) {
        redisLog(3,"Unable to intialized DS: pipe(2): %s. Exiting."
            ,strerror((*__errno_location ())));
        exit(1);
    }
    server.io_ready_pipe_read = pipefds[0];
    server.io_ready_pipe_write = pipefds[1];
    ((anetNonBlock(((void *)0),server.io_ready_pipe_read) != -1)?(void)0 : (_redisAssert("anetNonBlock(NULL,server.io_ready_pipe_read) != ANET_ERR","dscache.c",144),_exit(1)));
    ...
    if (aeCreateFileEvent(server.el, server.io_ready_pipe_read, 1,
        vmThreadedIOCompletedJob, ((void *)0)) == -1)
...
int cacheFreeOneEntry(void) {
    int j, i;
    struct dictEntry *best = ((void *)0);
    double best_swappability = 0;
    redisDb *best_db = ((void *)0);
    robj *val;
    sds key;
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        for (i = 0; i < 5 && ((db->dict)->ht[0].used+(db->dict)->ht[1].used); i++) {
            dictEntry *de;
            double swappability;
            robj keyobj;
            sds keystr;
            ...
            keystr = ((de)->key);
            val = ((de)->val);
            do { keyobj.refcount = 1; keyobj.type = 0; keyobj.encoding = 0; keyobj.ptr = keystr; } while(0);;
    ...
    if (best == ((void *)0)) {
        ...
        if (((server.cache_io_queue)->len) > 0) {
            redisLog(0,"--- Busy waiting IO to reclaim memory");
            cacheScheduleIOPushJobs(2);
    ...
    redisLog(0,"Key selected for cache eviction: %s swappability:%f",
...
# 258 "dscache.c"
int cacheKeyMayExist(redisDb *db, robj *key) {
    return dictFind(db->io_negcache,key) == ((void *)0);
}
...
void cacheSetKeyDoesNotExist(redisDb *db, robj *key) {
    if (dictReplace(db->io_negcache,key,(void*)time(((void *)0)))) {
        incrRefCount(key);
    }
}
...
int negativeCacheEvictOneEntry(void) {
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        if (((db->io_negcache)->ht[0].used+(db->io_negcache)->ht[1].used) == 0) continue;
        for (i = 0; i < 3; i++) {
            de = dictGetRandomKey(db->io_negcache);
            time = (time_t) ((de)->val);

            if (best == ((void *)0) || time < best_time) {
                best = ((de)->key);
                best_db = db;
                best_time = time;
            }
            ...
            int mask)
{
    ...
    while((retval = read(fd,buf,1)) == 1) {
        ...
        lockThreadedIO();
        ((((server.io_processed)->len) != 0)?(void)0 : (_redisAssert("listLength(server.io_processed) != 0","dscache.c",345),_exit(1)));
        if (toprocess == -1) {
            toprocess = (((server.io_processed)->len)*1)/100;
        ...
        redisLog(0,"COMPLETED Job type %s, key: %s",
            (j->type == 0) ? "load" : "save",
        ...
        if (j->type == 0) {
            ...
            if (j->val != ((void *)0)) {
                ...
                if (dictFind(j->db->dict,j->key->ptr) == ((void *)0) &&
            ...
            cacheScheduleIODelFlag(j->db,j->key,4);
            handleClientsBlockedOnSwappedKey(j->db,j->key);
        } else if (j->type == 1) {
            cacheScheduleIODelFlag(j->db,j->key,8);
        }
        freeIOJob(j);
        processed++;
        if (privdata == ((void *)0)) cacheScheduleIOPushJobs(0);
        if (processed == toprocess) return;
    }
    if (retval < 0 && (*__errno_location ()) != 11) {
        redisLog(3,
...
void *IOThreadEntryPoint(void *arg) {
    ...
    while(1) {
        ...
        if (((server.io_newjobs)->len) == 0) {

            redisLog(0,"[T] wait for signal");
            pthread_cond_wait(&server.io_condvar,&server.io_mutex);
            redisLog(0,"[T] signal received");
            continue;
        }
        start = ustime();
        redisLog(0,"[T] %ld IO jobs to process",
            ((server.io_newjobs)->len));
        ...
        redisLog(0,"[T] %ld: new job type %s: %p about key '%s'",
            (long) pthread_self(),
            (j->type == 0) ? "load" : "save",
        ...
        redisLog(0,"[T] %ld completed the job: %p (key %s)",
            (long) pthread_self(), (void*)j, (char*)j->key->ptr);

        redisLog(0,"[T] lock IO");
        lockThreadedIO();
        redisLog(0,"[T] IO locked");
        listDelNode(server.io_processing,ln);
        listAddNodeTail(server.io_processed,j);


        ((write(server.io_ready_pipe_write,"x",1) == 1)?(void)0 : (_redisAssert("write(server.io_ready_pipe_write,\"x\",1) == 1","dscache.c",467),_exit(1)));
        redisLog(0,"TIME (%c): %lld\n", j->type == 0 ? 'L' : 'S', ustime()-start);
    }
...
void spawnIOThread(void) {
    ...
    sigaddset(&mask,1);
    sigaddset(&mask,13);
    pthread_sigmask(2, &mask, &omask);
    while ((err = pthread_create(&thread,&server.io_threads_attr,IOThreadEntryPoint,((void *)0))) != 0) {
        redisLog(3,"Unable to spawn an I/O thread: %s",
            strerror(err));
        usleep(1000000);
    }
    pthread_sigmask(2, &omask, ((void *)0));
...
int processActiveIOJobs(int max) {
    ...
    while(max == -1 || max > 0) {
        ...
        redisLog(0,"[P] lock IO");
        lockThreadedIO();
        redisLog(0,"Waiting IO jobs processing: new:%d proessing:%d processed:%d",((server.io_newjobs)->len),((server.io_processing)->len),((server.io_processed)->len));

        if (((server.io_newjobs)->len) == 0 &&
            ((server.io_processing)->len) == 0)
        {

            redisLog(0,"[P] Nothing to process, unlock IO, return");
            unlockThreadedIO();
            break;
        }
        ...
        redisLog(0,"[P] waitEmptyIOJobsQueue: new %d, processing %d, processed %d",
            ((server.io_newjobs)->len),
            ((server.io_processing)->len),
            ((server.io_processed)->len));

        if (((server.io_newjobs)->len)) {
            redisLog(0,"[P] There are new jobs, signal");
            pthread_cond_signal(&server.io_condvar);
        }



        io_processed_len = ((server.io_processed)->len);
        redisLog(0,"[P] Unblock IO");
        unlockThreadedIO();
        redisLog(0,"[P] Wait");
        usleep(10000);
        if (io_processed_len) {
            vmThreadedIOCompletedJob(((void *)0),server.io_ready_pipe_read,
                                                        (void*)0xdeadbeef,0);
            processed++;
            if (max != -1) max--;
        }
...
void cacheForcePointInTime(void) {
    redisLog(2,"Diskstore: synching on disk to reach point-in-time state.");
    while (((server.cache_io_queue)->len) != 0) {
        cacheScheduleIOPushJobs(2);
        processActiveIOJobs(1);
    }
...
void cacheScheduleIOAddFlag(redisDb *db, robj *key, long flag) {
    ...
    if (!de) {
        ...
        long flags = (long) ((de)->val);

        if (flags & flag) {
            redisLog(3,"Adding the same flag again: was: %ld, addede: %ld",flags,flag);
            ((!(flags & flag))?(void)0 : (_redisAssert("!(flags & flag)","dscache.c",673),_exit(1)));
        }
        flags |= flag;
        ((de)->val) = (void*) flags;
    }
...
void cacheScheduleIODelFlag(redisDb *db, robj *key, long flag) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",684),_exit(1)));
    flags = (long) ((de)->val);
    ((flags & flag)?(void)0 : (_redisAssert("flags & flag","dscache.c",686),_exit(1)));
    flags &= ~flag;
    if (flags == 0) {
...
int cacheScheduleIOGetFlags(redisDb *db, robj *key) {
    ...
    return (de == ((void *)0)) ? 0 : ((long) ((de)->val));
}
...
void cacheScheduleIO(redisDb *db, robj *key, int type) {
    ...
    redisLog(0,"Scheduling key %s for %s",
        key->ptr, type == 1 ? "loading" : "saving");
...
int cacheScheduleIOPushJobs(int flags) {
    ...
    lockThreadedIO();
    jobs = ((server.io_newjobs)->len);
    unlockThreadedIO();

    topush = 10 -jobs;
    if (topush < 0) topush = 0;
    if (topush > (signed)((server.cache_io_queue)->len))
        topush = ((server.cache_io_queue)->len);

    while((ln = ((server.cache_io_queue)->head)) != ((void *)0)) {
        ioop *op = ln->value;
        struct dictEntry *de;
        robj *val;
        ...
        if (op->type != 1 && flags & 1) break;


        if (op->type == 2 && !(flags & 2) &&
              (now - op->ctime) < server.cache_flush_delay) break;



        if (op->type == 2 &&
            cacheScheduleIOGetFlags(op->db,op->key) & 8)
        {



            if (((server.cache_io_queue)->len) > 1) {
        ...
        redisLog(0,"Creating IO %s Job for key %s",
            op->type == 1 ? "load" : "save", op->key->ptr);

        if (op->type == 1) {
            cacheCreateIOJob(0,op->db,op->key,((void *)0),0);
        } else {
            time_t expire = -1;
            ...
            cacheCreateIOJob(1,op->db,op->key,val,expire);
        }
...
void cacheCron(void) {
            ...
            server.cache_max_memory)
    {
        ...
        if (cacheFreeOneEntry() == 0) done++;
        if (negativeCacheEvictOneEntry() == 0) done++;
        if (done == 0) break;
    }
}
# 861 "dscache.c"
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",886),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",906),_exit(1)));
...
void execBlockClientOnSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (i = 0; i < c->mstate.count; i++) {
        ...
        if (mcmd->vm_preload_proc != ((void *)0)) {
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",1002),_exit(1)));


    de = dictFind(c->db->io_keys,key);
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",1006),_exit(1)));
    l = ((de)->val);
    ln = listSearchKey(l,c);
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",1009),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
        dictDelete(c->db->io_keys,key);

    decrRefCount(key);
    return ((c->io_keys)->len) == 0;
}
