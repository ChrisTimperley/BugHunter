...
int clusterSaveConfig(void) {
    ...
    if ((fd = open(server.cluster.configfile,01|0100,0644)) == -1)
        goto err;
...
void freeClusterNode(clusterNode *n) {
    ...
    ((dictDelete(server.cluster.nodes,nodename) == 0)?(void)0 : (_redisAssert("dictDelete(server.cluster.nodes,nodename) == DICT_OK","cluster.c",239),_exit(1)));
    sdsfree(nodename);
...
void clusterRenameNode(clusterNode *node, char *newname) {
    ...
    sdsfree(s);
    ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","cluster.c",278),_exit(1)));
...
void nodeIp2String(char *buf, clusterLink *link) {
    ...
    if (getpeername(link->fd, (struct sockaddr*) &sa, &salen) == -1)
        _redisPanic("\"getpeername() failed.\"","cluster.c",370),_exit(1);
...
void clusterCron(void) {
    ...
    while((de = dictNext(di)) != ((void *)0)) {
        ...
        if (node->link == ((void *)0)) {
            ...
            redisLog(2,"Connecting with Node %.40s at %s:%d\n", node->name, node->ip, node->port+10000);
        }
    }
    dictReleaseIterator(di);
...
int clusterAddSlot(clusterNode *n, int slot) {
    ((clusterNodeSetSlotBit(n,slot) == 0)?(void)0 : (_redisAssert("clusterNodeSetSlotBit(n,slot) == 0","cluster.c",876),_exit(1)));
    ...
    if (!strcasecmp(c->argv[1]->ptr,"meet") && c->argc == 4) {
        ...
        for (j = 0; j < 4096; j++) {
            if (slots[j]) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","cluster.c",1045),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
