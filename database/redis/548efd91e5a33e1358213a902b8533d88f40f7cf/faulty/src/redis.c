...
void beforeSleep(struct aeEventLoop *eventLoop) {
    ...
    while (((server.unblocked_clients)->len)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","redis.c",782),_exit(1)));
        c = ln->value;
...
void initServerConfig() {
    ...
    server.aof_rewrite_min_size = (1024*1024);
    ...
    server.client_obuf_limits[0].hard_limit_bytes = 0;
    server.client_obuf_limits[0].soft_limit_bytes = 0;
    server.client_obuf_limits[0].soft_limit_seconds = 0;
    server.client_obuf_limits[1].hard_limit_bytes = 1024*1024*256;
    server.client_obuf_limits[1].soft_limit_bytes = 1024*1024*64;
    server.client_obuf_limits[1].soft_limit_seconds = 60;
    server.client_obuf_limits[2].hard_limit_bytes = 1024*1024*32;
    server.client_obuf_limits[2].soft_limit_bytes = 1024*1024*8;
    server.client_obuf_limits[2].soft_limit_seconds = 60;
...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
        ...
        while(*f != '\0') {
            ...
            default: _redisPanic("\"Unsupported command flag\"","redis.c",1121),_exit(1); break;
            }
            f++;
        }

        retval = dictAdd(server.commands, sdsnew(c->name), c);
        ((retval == 0) ? (void) (0) : __assert_fail ("retval == 0", "redis.c", 1127, __PRETTY_FUNCTION__));
    }
}
...
int main(int argc, char **argv) {
    ...
    redisAsciiArt();
    redisLog(3,"Server started, Redis version " "2.9.3");
