...
void beforeSleep(struct aeEventLoop *eventLoop) {
    ...
    while (((server.unblocked_clients)->len)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","redis.c",785),_exit(1)));
        c = ln->value;
...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
        ...
        while(*f != '\0') {
            ...
            default: _redisPanic("\"Unsupported command flag\"","redis.c",1125),_exit(1); break;
            }
            f++;
        }

        retval = dictAdd(server.commands, sdsnew(c->name), c);
        ((retval == 0) ? (void) (0) : __assert_fail ("retval == 0", "redis.c", 1131, __PRETTY_FUNCTION__));
    }
}
