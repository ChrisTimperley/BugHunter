...
int create256dir(char *prefix) {
    ...
    for (j = 0; j < 256; j++) {
        ...
        if (mkdir(buf,0755) == -1) {
            redisLog(3,"Error creating dir %s for diskstore: %s",
                buf,strerror((*__errno_location ())));
            return -1;
        }
    }
    return 0;
...
int dsOpen(void) {
    ...
    if ((retval = stat(path,&sb) == -1) && (*__errno_location ()) != 2) {
        redisLog(3, "Error opening disk store at %s: %s",
                path, strerror((*__errno_location ())));
        return -1;
    }


    if (retval == 0 && ((((sb.st_mode)) & 0170000) == (0040000))) {
        redisLog(2,"Disk store %s exists", path);
        return 0;
    }


    if (retval == 0 && !((((sb.st_mode)) & 0170000) == (0040000))) {
        redisLog(3,"Disk store at %s is not a directory", path);
        return -1;
    }
    ...
    redisLog(2,"Disk store %s does not exist: creating", path);
    if (mkdir(path,0755) == -1) {
        redisLog(3,"Disk store init failed creating dir %s: %s",
            path, strerror((*__errno_location ())));
        return -1;
    }

    snprintf(buf,sizeof(buf),"%s/",path);
    if (create256dir(buf) == -1) return -1;
...
int dsSet(redisDb *db, robj *key, robj *val, time_t expire) {
    ...
    memcpy(buf2,buf,len);
    snprintf(buf2+len,sizeof(buf2)-len,"-%ld-%ld",(long)time(((void *)0)),(long)val);
    while ((fp = fopen(buf2,"w")) == ((void *)0)) {
        if ((*__errno_location ()) == 28) {
            redisLog(3,"Diskstore: No space left on device. Please make room and wait 30 seconds for Redis to continue.");
            sleep(30);
        } else {
            redisLog(3,"diskstore error opening %s: %s",
                buf2, strerror((*__errno_location ())));
            _redisPanic("\"Unrecoverable diskstore error. Exiting.\"","diskstore.c",201),_exit(1);
        }
    }
    if ((retval = rdbSaveKeyValuePair(fp,key,val,expire,time(((void *)0)))) == -1)
        return -1;
    fclose(fp);
    if (retval == 0) {
        ...
        if (rename(buf2,buf) == -1) {
            redisLog(3,"rename(2) returned an error: %s",
                strerror((*__errno_location ())));
            _redisPanic("\"Unrecoverable diskstore error. Exiting.\"","diskstore.c",216),_exit(1);
        }
    }
    return 0;
}
...
robj *dsGet(redisDb *db, robj *key, time_t *expire) {
    ...
    if (fp == ((void *)0)) {
        redisLog(3,"Disk store failed opening %s: %s",
            buf, strerror((*__errno_location ())));
        goto readerr;
    }
    ...
    if ((dskey = rdbLoadStringObject(fp)) == ((void *)0)) goto readerr;

    if ((val = rdbLoadObject(type,fp)) == ((void *)0)) goto readerr;
    fclose(fp);


    ((equalStringObjects(key,dskey))?(void)0 : (_redisAssert("equalStringObjects(key,dskey)","diskstore.c",252),_exit(1)));


    decrRefCount(dskey);
    if (expiretime != -1 && expiretime < time(((void *)0))) {
        decrRefCount(val);
        unlink(buf);
        return ((void *)0);
    }
...
readerr:
    redisLog(3,"Read error reading reading %s. Corrupted key?",
        buf);
    _redisPanic("\"Unrecoverable error reading from disk store\"","diskstore.c",269),_exit(1);
    return ((void *)0);
}
...
int dsDel(redisDb *db, robj *key) {
    ...
    if (unlink(buf) == -1) {
        if ((*__errno_location ()) == 2) {
            ...
            redisLog(3,"Disk store can't remove %s: %s",
                buf, strerror((*__errno_location ())));
            _redisPanic("\"Unrecoverable Disk store errore. Existing.\"","diskstore.c",283),_exit(1);
            return -1;
        }
    } else {
        return 0;
...
int dsGetDbidFromFilename(char *path) {
    ...
    ((p != ((void *)0) && len < 64)?(void)0 : (_redisAssert("p != NULL && len < 64","diskstore.c",303),_exit(1)));
    memcpy(id,path,len);
    id[len] = '\0';
    return atoi(id);
}
...
void dsFlushOneDir(char *path, int dbid) {
    ...
    dir = opendir(path);
    if (dir == ((void *)0)) {
        redisLog(3,"Disk store can't open dir %s: %s",
            path, strerror((*__errno_location ())));
        _redisPanic("\"Unrecoverable Disk store errore. Existing.\"","diskstore.c",317),_exit(1);
    }
    while(1) {
        char buf[1024];
        ...
        if (unlink(buf) == -1) {
            redisLog(3,
                "Can't unlink %s: %s", buf, strerror((*__errno_location ())));
            _redisPanic("\"Unrecoverable Disk store errore. Existing.\"","diskstore.c",335),_exit(1);
        }
    }
    closedir(dir);
}
...
void dsFlushDb(int dbid) {
    ...
    redisLog(2,"Flushing diskstore DB (%d)",dbid);
    for (j = 0; j < 256; j++) {
        for (i = 0; i < 256; i++) {
...
void *dsRdbSave_thread(void *arg) {
    ...
    redisLog(2,"Diskstore BGSAVE thread started");
    dsRdbSaveSetState(1);
    ...
    for (j = 0; j < 256; j++) {
        for (i = 0; i < 256; i++) {
            ...
            while(1) {
                ...
                if (strchr(dp->d_name,'-') != ((void *)0)) continue;
                ...
                if ((entryfp = fopen(buf,"r")) == ((void *)0)) {
                    redisLog(3,"Can't open %s: %s",
                        buf,strerror((*__errno_location ())));
                    closedir(dir);
                    goto werr;
                }
                while(1) {
                    ...
                    if (nread == 0) {
                        if (ferror(entryfp)) {
                            redisLog(3,"Error reading from file entry while performing BGSAVE for diskstore: %s", strerror((*__errno_location ())));
                            closedir(dir);
                            goto werr;
                        } else {
                            break;
    ...
    if (rename(tmpfile,filename) == -1) {
        redisLog(3,"Error moving temp DB file on the final destination: %s (diskstore)", strerror((*__errno_location ())));
        unlink(tmpfile);
        dsRdbSaveSetState(3);
        return ((void *)0);
    }
    redisLog(2,"DB saved on disk by diskstore thread");
    dsRdbSaveSetState(2);
    return ((void *)0);
...
int dsRdbSaveBackground(char *filename) {
    ...
    if (pthread_create(&thread,((void *)0),dsRdbSave_thread,zstrdup(filename)) != 0) {
        redisLog(3,"Can't create diskstore BGSAVE thread: %s",
            strerror((*__errno_location ())));
        return -1;
    } else {
...
int dsRdbSave(char *filename) {
    ...
    redisLog(2,"Starting a blocking SAVE (BGSAVE + blocking wait)");
    server.dirty_before_bgsave = server.dirty;
    if (dsRdbSaveBackground(filename) == -1) return -1;
    while(1) {
        usleep(1000);
        int state;
