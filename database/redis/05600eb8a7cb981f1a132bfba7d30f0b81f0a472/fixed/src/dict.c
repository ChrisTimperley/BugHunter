...
int dictRehash(dict *d, int n) {
    ...
    while(n--) {
        ...
        ((d->ht[0].size > (unsigned)d->rehashidx) ? (void) (0) : __assert_fail ("d->ht[0].size > (unsigned)d->rehashidx", "dict.c", 206, __PRETTY_FUNCTION__));
        while(d->ht[0].table[d->rehashidx] == ((void *)0)) d->rehashidx++;
...
int dictAdd(dict *d, void *key, void *val)
{
    ...
    do { if ((d)->type->valDup) entry->val = (d)->type->valDup((d)->privdata, val); else entry->val = (val); } while(0);
    return 0;
}
...
int dictReplace(dict *d, void *key, void *val)
{
    ...
    auxentry = *entry;
    do { if ((d)->type->valDup) entry->val = (d)->type->valDup((d)->privdata, val); else entry->val = (val); } while(0);
    if ((d)->type->valDestructor) (d)->type->valDestructor((d)->privdata, (&auxentry)->val);
    return 0;
}
        ...
        while(he) {
            if ((((d)->type->keyCompare) ? (d)->type->keyCompare((d)->privdata, key, he->key) : (key) == (he->key))) {
                ...
                if (!nofree) {
                    ...
                    if ((d)->type->valDestructor) (d)->type->valDestructor((d)->privdata, (he)->val);
                }
                zfree(he);
                d->ht[table].used--;
                return 0;
