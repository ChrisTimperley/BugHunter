...
void *IOThreadEntryPoint(void *arg) {
    ...
    while(1) {
        ...
        redisLog(0,"%ld IO jobs to process",
        ...
        redisLog(0,"Thread %ld: new job type %s: %p about key '%s'",
            (long) pthread_self(),
            (j->type == 0) ? "load" : "save",
        ...
        redisLog(0,"Thread %ld completed the job: %p (key %s)",
        ...
        ((write(server.io_ready_pipe_write,"x",1) == 1)?(void)0 : (_redisAssert("write(server.io_ready_pipe_write,\"x\",1) == 1","dscache.c",462),_exit(1)));
    }
...
void cacheScheduleIOAddFlag(redisDb *db, robj *key, long flag) {
    ...
    if (!de) {
        ...
        if (flags & flag) {
            ...
            ((!(flags & flag))?(void)0 : (_redisAssert("!(flags & flag)","dscache.c",659),_exit(1)));
        }
        flags |= flag;
        ((de)->val) = (void*) flags;
    }
...
void cacheScheduleIODelFlag(redisDb *db, robj *key, long flag) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",670),_exit(1)));
    flags = (long) ((de)->val);
    ((flags & flag)?(void)0 : (_redisAssert("flags & flag","dscache.c",672),_exit(1)));
    flags &= ~flag;
    if (flags == 0) {
...
int cacheScheduleIOPushJobs(int flags) {
    ...
    while((ln = ((server.cache_io_queue)->head)) != ((void *)0)) {
        ...
        if (op->type == 1) {
            cacheCreateIOJob(0,op->db,op->key,((void *)0));
        } else {
            ...
            cacheCreateIOJob(1,op->db,op->key,val);
        }
...
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",869),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",889),_exit(1)));
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",985),_exit(1)));


    de = dictFind(c->db->io_keys,key);
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",989),_exit(1)));
    l = ((de)->val);
    ln = listSearchKey(l,c);
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",992),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
