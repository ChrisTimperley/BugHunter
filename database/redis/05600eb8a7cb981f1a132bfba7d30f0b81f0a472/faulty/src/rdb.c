...
robj *rdbLoadIntegerObject(FILE *fp, int enctype, int encode) {
    ...
    if (enctype == 0) {
        ...
        val = 0;
        _redisPanic("\"Unknown RDB integer encoding type\"","rdb.c",606),_exit(1);
    }
    if (encode)
...
robj *rdbGenericLoadStringObject(FILE*fp, int encode) {
    ...
    if (isencoded) {
        ...
        default:
            _redisPanic("\"Unknown RDB encoding type\"","rdb.c",648),_exit(1);
        }
    }
...
robj *rdbLoadObject(int type, FILE *fp) {
    ...
    if (type == 0) {
        ...
        _redisPanic("\"Unknown object type\"","rdb.c",838),_exit(1);
    }
    return o;
}
