...
void memtest_fill_random(unsigned long *l, size_t bytes) {
    ...
    (((bytes & 4095) == 0) ? (void) (0) : __assert_fail ("(bytes & 4095) == 0", "memtest.c", 124, __PRETTY_FUNCTION__));
                        ...
                        unsigned long v2, char sym)
{
    ...
    (((bytes & 4095) == 0) ? (void) (0) : __assert_fail ("(bytes & 4095) == 0", "memtest.c", 156, __PRETTY_FUNCTION__));
    for (off = 0; off < step; off++) {
        l1 = l+off;
        l2 = l1+words;
        v = (off & 1) ? v2 : v1;
...
void memtest_compare(unsigned long *l, size_t bytes) {
    ...
    (((bytes & 4095) == 0) ? (void) (0) : __assert_fail ("(bytes & 4095) == 0", "memtest.c", 183, __PRETTY_FUNCTION__));
    l1 = l;
    l2 = l1+words;
    for (w = 0; w < words; w++) {
        if (*l1 != *l2) {
...
void memtest_test(size_t megabytes, int passes) {
    ...
    if (m == ((void *)0)) {
        fprintf(stderr,"Unable to allocate %zu megabytes: %s",
            megabytes, strerror((*__errno_location ())));
        exit(1);
    }
    while (pass != passes) {
        pass++;
        ...
        memtest_fill_value(m,bytes,0xaaaaaaaaaaaaaaaaUL,0x5555555555555555UL,'C');
        memtest_progress_end();
        memtest_compare_times(m,bytes,pass,4);
    }
}
