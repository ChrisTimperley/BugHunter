...
sds sdsnewlen(const void *init, size_t initlen) {
    ...
    if (initlen >= (32767 * 2 + 1)) {
        sh = zmalloc(sizeof(struct sdshdr)+initlen+1);
        sh->len = initlen;
        sh->_len = (32767 * 2 + 1);

            if (sh == ((void *)0)) sdsOomAbort();
...
static sds sdsMakeRoomFor(sds s, size_t addlen) {
    ...
    sh = (void*) (s-(sizeof(struct sdshdr)));
    newlen = (len+addlen);
    newfree = ((addlen*2) > (32767 * 2 + 1)) ? (32767 * 2 + 1) : (addlen*2);
    if (newlen+newfree >= (32767 * 2 + 1) || sh->_len == (32767 * 2 + 1)) {
        if (sh->_len == (32767 * 2 + 1)) {
            newsh = zrealloc(sh, sizeof(struct sdshdr)+newlen+1+newfree);
        } else {
            newsh = zmalloc(sizeof(struct sdshdr)+newlen+1+newfree);
            if (!newsh) return ((void *)0);
            memcpy(newsh->buf,sh->buf,len);
            newsh->buf[len] = '\0';
            zfree(((char*)sh)+sizeof(int));
        }

        if (newsh == ((void *)0)) sdsOomAbort();



        newsh->_len = (32767 * 2 + 1);
        newsh->free = newfree;
        newsh->len = newlen;
    } else {
...
sds *sdssplitlen(char *s, int len, char *sep, int seplen, int *count) {
    ...
    if (tokens == ((void *)0)) sdsOomAbort();

    if (seplen < 1 || len < 0 || tokens == ((void *)0)) return ((void *)0);
