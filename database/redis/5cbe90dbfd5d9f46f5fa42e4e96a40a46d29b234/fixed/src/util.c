...
int string2ll(char *s, size_t slen, long long *value) {
    ...
    while (plen < slen && p[0] >= '0' && p[0] <= '9') {
        if (v > ((9223372036854775807LL * 2ULL + 1) / 10))
            return 0;
        v *= 10;

        if (v > ((9223372036854775807LL * 2ULL + 1) - (p[0]-'0')))
            return 0;
        v += p[0]-'0';
    ...
    if (negative) {
        if (v > (-(unsigned long long)(-9223372036854775807LL -1)))
            return 0;
...
int isStringRepresentableAsLong(sds s, long *longval) {
    ...
    if (isStringRepresentableAsLongLong(s,&ll) == -1) return -1;
    if (ll < (-9223372036854775807L - 1L) || ll > 9223372036854775807L) return -1;
    *longval = (long)ll;
    return 0;
}

int isObjectRepresentableAsLongLong(robj *o, long long *llongval) {
    ((o->type == 0)?(void)0 : (_redisAssert("o->type == REDIS_STRING","util.c",336),_exit(1)));
    if (o->encoding == 1) {
