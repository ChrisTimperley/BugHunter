...
void rewriteClientCommandVector(redisClient *c, int argc, ...) {
    ...
    ((c->cmd != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",1067),_exit(1)));
    __builtin_va_end(ap);
}
...
void rewriteClientCommandArgument(redisClient *c, int i, robj *newval) {
    ...
    ((i < c->argc)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"i < c->argc","networking.c",1076),_exit(1)));
    oldval = c->argv[i];
    c->argv[i] = newval;
    incrRefCount(newval);
    decrRefCount(oldval);
    ...
    if (i == 0) {
        c->cmd = lookupCommand(c->argv[0]->ptr);
        ((c->cmd != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->cmd != NULL","networking.c",1085),_exit(1)));
    }
}
