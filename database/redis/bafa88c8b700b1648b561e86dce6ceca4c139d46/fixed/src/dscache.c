...
void dsInit(void) {
    ...
    ((anetNonBlock(((void *)0),server.io_ready_pipe_read) != -1)?(void)0 : (_redisAssert("anetNonBlock(NULL,server.io_ready_pipe_read) != ANET_ERR","dscache.c",131),_exit(1)));
            ...
            int mask)
{
    ...
    while((retval = read(fd,buf,1)) == 1) {
        ...
        lockThreadedIO();
        ((((server.io_processed)->len) != 0)?(void)0 : (_redisAssert("listLength(server.io_processed) != 0","dscache.c",327),_exit(1)));
        if (toprocess == -1) {
...
void *IOThreadEntryPoint(void *arg) {
    ...
    while(1) {
        ...
        ((write(server.io_ready_pipe_write,"x",1) == 1)?(void)0 : (_redisAssert("write(server.io_ready_pipe_write,\"x\",1) == 1","dscache.c",443),_exit(1)));
    }
...
void waitEmptyIOJobsQueue(void) {
    while(1) {
        ...
        redisLog(0,"waitEmptyIOJobsQueue: new %d, processing %d",
            ((server.io_newjobs)->len),
...
void cacheScheduleIOAddFlag(redisDb *db, robj *key, long flag) {
    ...
    if (!de) {
        ...
        if (flags & flag) {
            ...
            ((!(flags & flag))?(void)0 : (_redisAssert("!(flags & flag)","dscache.c",604),_exit(1)));
        }
        flags |= flag;
        ((de)->val) = (void*) flags;
    }
...
void cacheScheduleIODelFlag(redisDb *db, robj *key, long flag) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",615),_exit(1)));
    flags = (long) ((de)->val);
    ((flags & flag)?(void)0 : (_redisAssert("flags & flag","dscache.c",617),_exit(1)));
    flags &= ~flag;
    if (flags == 0) {
...
void cacheScheduleIOPushJobs(int onlyloads) {
    ...
    while((ln = ((server.cache_io_queue)->head)) != ((void *)0)) {
            ...
            (!onlyloads && (now - op->ctime) >= server.cache_flush_delay))
        {
            ...
            if (op->type == 1) {
                dsCreateIOJob(0,op->db,op->key,((void *)0));
            } else {
                ...
                dsCreateIOJob(1,op->db,op->key,val);
            }
...
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",804),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",824),_exit(1)));
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",920),_exit(1)));


    de = dictFind(c->db->io_keys,key);
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",924),_exit(1)));
    l = ((de)->val);
    ln = listSearchKey(l,c);
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",927),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
