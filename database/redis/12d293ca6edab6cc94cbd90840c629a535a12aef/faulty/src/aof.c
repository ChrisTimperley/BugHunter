...
int startAppendOnly(void) {
    ...
    if (rewriteAppendOnlyFileBackground() == -1) {
        server.appendonly = 0;
        close(server.appendfd);
        redisLog(3,"User tried turning on AOF with CONFIG SET but I can't trigger a background AOF rewrite operation. Check the above logs for more info about the error.");
        return -1;
    }
    return 0;
}
