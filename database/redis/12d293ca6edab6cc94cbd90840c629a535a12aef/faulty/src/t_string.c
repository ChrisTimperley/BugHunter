...
void setGenericCommand(redisClient *c, int nx, robj *key, robj *val, robj *expire) {
    ...
    if (expire) {
        if (getLongFromObjectOrReply(c, expire, &seconds, ((void *)0)) != 0)
            return;
        if (seconds <= 0) {
    ...
    setKey(c->db,key,val);
    server.dirty++;
    if (expire) setExpire(c->db,key,(time(((void *)0))+seconds)*1000);
    addReply(c, nx ? shared.cone : shared.ok);
}

void setCommand(redisClient *c) {
    c->argv[2] = tryObjectEncoding(c->argv[2]);
    setGenericCommand(c,0,c->argv[1],c->argv[2],((void *)0));
}

void setnxCommand(redisClient *c) {
    c->argv[2] = tryObjectEncoding(c->argv[2]);
    setGenericCommand(c,1,c->argv[1],c->argv[2],((void *)0));
}
