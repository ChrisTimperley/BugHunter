...
robj *dsGet(redisDb *db, robj *key, time_t *expire) {
    ...
    ((equalStringObjects(key,dskey))?(void)0 : (_redisAssert("equalStringObjects(key,dskey)","diskstore.c",228),_exit(1)));
...
readerr:
    redisLog(3,"Read error reading reading %s. Corrupted key?",
        buf);
    _redisPanic("\"Unrecoverable error reading from disk store\"","diskstore.c",245),_exit(1);
    return ((void *)0);
}
    ...
    if (unlink(buf) == -1) {
        if ((*__errno_location ()) == 2) {
            ...
            redisLog(3,"Disk store can't remove %s: %s",
                buf, strerror((*__errno_location ())));
            _redisPanic("\"Unrecoverable Disk store errore. Existing.\"","diskstore.c",259),_exit(1);
            return -1;
        }
    } else {
        return 0;
...
void dsFlushOneDir(char *path, int dbid) {
    ...
    if (dir == ((void *)0)) {
        redisLog(3,"Disk store can't open dir %s: %s",
            path, strerror((*__errno_location ())));
        _redisPanic("\"Unrecoverable Disk store errore. Existing.\"","diskstore.c",282),_exit(1);
    }
    while(1) {
        readdir_r(dir,&de,&dp);
        if (dp == ((void *)0)) break;
        ...
        if (dbid != -1) {
            ...
            ((p != ((void *)0) && len < 64)?(void)0 : (_redisAssert("p != NULL && len < 64","diskstore.c",296),_exit(1)));
            memcpy(id,dp->d_name,len);
            id[len] = '\0';
            if (atoi(id) != dbid) continue;
        }
        if (unlink(path) == -1) {
            redisLog(3,
                "Can't unlink %s: %s", path, strerror((*__errno_location ())));
            _redisPanic("\"Unrecoverable Disk store errore. Existing.\"","diskstore.c",304),_exit(1);
        }
    }
    closedir(dir);
}
