...
robj *lookupKey(redisDb *db, robj *key) {
    ...
    if (de) {
            ...
            cacheScheduleIOGetFlags(db,key) & 8)
        {
            ...
            ((!(cacheScheduleIOGetFlags(db,key) & 8))?(void)0 : (_redisAssert("!(cacheScheduleIOGetFlags(db,key) & REDIS_IO_SAVEINPROG)","db.c",27),_exit(1)));
        }
        ...
        if (server.ds_enabled && cacheKeyMayExist(db,key)) {
            if (cacheScheduleIOGetFlags(db,key) & 8) {


                waitEmptyIOJobsQueue();
                processAllPendingIOJobs();
                (((cacheScheduleIOGetFlags(db,key) & 8) == 0)?(void)0 : (_redisAssert("(cacheScheduleIOGetFlags(db,key) & REDIS_IO_SAVEINPROG) == 0","db.c",44),_exit(1)));
            }

            redisLog(0,"Force loading key %s via lookup",
                key->ptr);
            val = dsGet(db,key,&expire);
            if (val) {
                int retval = dbAdd(db,key,val);
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","db.c",52),_exit(1)));
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",459),_exit(1)));
...
void setExpire(redisDb *db, robj *key, time_t when) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","db.c",468),_exit(1)));
...
time_t getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",483),_exit(1)));
