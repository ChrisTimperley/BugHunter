...
void dsInit(void) {
    ...
    ((anetNonBlock(((void *)0),server.io_ready_pipe_read) != -1)?(void)0 : (_redisAssert("anetNonBlock(NULL,server.io_ready_pipe_read) != ANET_ERR","dscache.c",176),_exit(1)));
            ...
            int mask)
{
    ...
    while((retval = read(fd,buf,1)) == 1) {
        ...
        lockThreadedIO();
        ((((server.io_processed)->len) != 0)?(void)0 : (_redisAssert("listLength(server.io_processed) != 0","dscache.c",372),_exit(1)));
        if (toprocess == -1) {
...
void *IOThreadEntryPoint(void *arg) {
    ...
    while(1) {
        ...
        ((write(server.io_ready_pipe_write,"x",1) == 1)?(void)0 : (_redisAssert("write(server.io_ready_pipe_write,\"x\",1) == 1","dscache.c",474),_exit(1)));
    }
...
void cacheScheduleIOAddFlag(redisDb *db, robj *key, long flag) {
    ...
    if (!de) {
        ...
        if (flags & flag) {
            ...
            ((!(flags & flag))?(void)0 : (_redisAssert("!(flags & flag)","dscache.c",628),_exit(1)));
        }
        flags |= flag;
        ((de)->val) = (void*) flags;
    }
...
void cacheScheduleIODelFlag(redisDb *db, robj *key, long flag) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",639),_exit(1)));
    flags = (long) ((de)->val);
    ((flags & flag)?(void)0 : (_redisAssert("flags & flag","dscache.c",641),_exit(1)));
    flags &= ~flag;
    if (flags == 0) {
...
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",814),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",834),_exit(1)));
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",930),_exit(1)));


    de = dictFind(c->db->io_keys,key);
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",934),_exit(1)));
    l = ((de)->val);
    ln = listSearchKey(l,c);
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",937),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
