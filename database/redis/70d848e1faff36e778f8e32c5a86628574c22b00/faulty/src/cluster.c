        ...
        *p = '\0';
        ...
        while(p) {
            ...
            if (!strcasecmp(s,"myself")) {
                ((server.cluster.myself == ((void *)0))?(void)0 : (_redisAssert("server.cluster.myself == NULL","cluster.c",60),_exit(1)));
                ...
                _redisPanic("\"Unknown flag in redis cluster config file\"","cluster.c",78),_exit(1);
            }
            if (p) s = p+1;
        }
        ...
        for (j = 7; j < argc; j++) {
            ...
            if (argv[j][0] == '[') {
                ...
                p = strchr(argv[j],'-');
                ((p != ((void *)0))?(void)0 : (_redisAssert("p != NULL","cluster.c",110),_exit(1)));
                *p = '\0';
                direction = p[1];
                slot = atoi(argv[j]+1);
                p += 3;
...
void freeClusterNode(clusterNode *n) {
    ...
    ((dictDelete(server.cluster.nodes,nodename) == 0)?(void)0 : (_redisAssert("dictDelete(server.cluster.nodes,nodename) == DICT_OK","cluster.c",343),_exit(1)));
    sdsfree(nodename);
...
void clusterRenameNode(clusterNode *node, char *newname) {
    ...
    sdsfree(s);
    ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","cluster.c",382),_exit(1)));
...
void nodeIp2String(char *buf, clusterLink *link) {
    ...
    if (getpeername(link->fd, (struct sockaddr*) &sa, &salen) == -1)
        _redisPanic("\"getpeername() failed.\"","cluster.c",475),_exit(1);
...
int clusterDelSlot(int slot) {
    ...
    if (!n) return -1;
    ((clusterNodeClearSlotBit(n,slot) == 1)?(void)0 : (_redisAssert("clusterNodeClearSlotBit(n,slot) == 1","cluster.c",1100),_exit(1)));
               ...
               !strcasecmp(c->argv[1]->ptr,"delslots")) && c->argc >= 3)
    {
        ...
        for (j = 0; j < 4096; j++) {
            if (slots[j]) {
                ...
                ((retval == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"retval == REDIS_OK","cluster.c",1308),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
...
void createDumpPayload(rio *payload, robj *o) {
    ...
    ((rdbSaveObjectType(payload,o))?(void)0 : (_redisAssert("rdbSaveObjectType(payload,o)","cluster.c",1473),_exit(1)));
    ((rdbSaveObject(payload,o))?(void)0 : (_redisAssert("rdbSaveObject(payload,o)","cluster.c",1474),_exit(1)));
# 1484 "cluster.c"
    buf[0] = 4 & 0xff;
    buf[1] = (4 >> 8) & 0xff;
...
int verifyDumpPayload(unsigned char *p, size_t len) {
    ...
    if (rdbver != 4) return -1;


    SHA1Init(&ctx);
    SHA1Update(&ctx,p,len-8);
    SHA1Final(hash,&ctx);
    return (memcmp(hash,footer+2,8) == 0) ? 0 : -1;
}
...
void migrateCommand(redisClient *c) {
    ...
    ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",1625),_exit(1)));
    ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",1626),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,dbid)","cluster.c",1627),_exit(1)));

    ttl = getExpire(c->db,c->argv[3]);
    ((rioWriteBulkCount(&cmd,'*',4))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',4)","cluster.c",1630),_exit(1)));
    ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",1631),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argv[3]->encoding == REDIS_ENCODING_RAW","cluster.c",1632),_exit(1)));
    ((rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr))","cluster.c",1633),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl)","cluster.c",1634),_exit(1)));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,firstkey,"n != NULL","cluster.c",1782),_exit(1)));
            } else {
