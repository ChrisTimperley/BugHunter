...
int pubsubSubscribeChannel(redisClient *c, robj *channel) {
    ...
    addReplyLongLong(c,((c->pubsub_channels)->ht[0].used+(c->pubsub_channels)->ht[1].used)+((c->pubsub_patterns)->len));
    return retval;
}
...
int pubsubUnsubscribeChannel(redisClient *c, robj *channel, int notify) {
    ...
    if (dictDelete(c->pubsub_channels,channel) == 0) {
        ...
        de = dictFind(server.pubsub_channels,channel);
        ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","pubsub.c",66),_exit(1)));
        clients = ((de)->val);
        ln = listSearchKey(clients,c);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","pubsub.c",69),_exit(1)));
    ...
    if (notify) {
        ...
        addReplyBulk(c,channel);
        addReplyLongLong(c,((c->pubsub_channels)->ht[0].used+(c->pubsub_channels)->ht[1].used)+
