...
void replicationAbortSyncTransfer(void) {
    ((server.repl_state == 3)?(void)0 : (_redisAssert("server.repl_state == REDIS_REPL_TRANSFER","replication.c",268),_exit(1)));
...
void readSyncBulkPayload(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (server.repl_transfer_left == -1) {
        ...
        if (buf[0] == '-') {
            ...
            server.repl_transfer_lastio = time(((void *)0));
            return;
        } else if (buf[0] != '$') {
...
void syncWithMaster(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    while(maxtries--) {
        snprintf(tmpfile,256,
            "temp-%d.%ld.rdb",(int)time(((void *)0)),(long int)getpid());
...
void undoConnectWithMaster(void) {
    ...
    ((server.repl_state == 2)?(void)0 : (_redisAssert("server.repl_state == REDIS_REPL_CONNECTING","replication.c",499),_exit(1)));
    aeDeleteFileEvent(server.el,fd,1|2);
    close(fd);
