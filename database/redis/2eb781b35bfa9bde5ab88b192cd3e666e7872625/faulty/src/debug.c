...
void redisLogObjectDebugInfo(robj *o) {
    ...
    if (o->type == 0 && (o->encoding == 0 || o->encoding == 8)) {
        redisLog(3,"Object raw string len: %zu", sdslen(o->ptr));
        if (sdslen(o->ptr) < 4096) {
            sds repr = sdscatrepr(sdsempty(),o->ptr,sdslen(o->ptr));
            redisLog(3,"Object raw string content: %s", repr);
            sdsfree(repr);
        }
    } else if (o->type == 1) {
...
void logStackContent(void **sp) {
    ...
    for (i = 15; i >= 0; i--) {
        ...
        if (sizeof(long) == 4)
            redisLog(3, "(%08lx) -> %08lx", addr, val);
        else
            redisLog(3, "(%016lx) -> %016lx", addr, val);
    }
}
...
void logCurrentClient(void) {
    ...
    if (cc->argc >= 1) {
        ...
        if (de) {
            val = ((de)->v.val);
            redisLog(3,"key '%s' found in DB containing the following object:", (char*)key->ptr);
...
void sigsegvHandler(int sig, siginfo_t *info, void *secret) {
    ...
    bugReportStart();
    redisLog(3,
        "    Redis %s crashed by signal: %d", "2.9.11", sig);
    redisLog(3,
