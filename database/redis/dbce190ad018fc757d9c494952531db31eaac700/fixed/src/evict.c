...
unsigned long LFUDecrAndReturn(robj *o) {
    ...
    if (LFUTimeElapsed(ldt) >= 1 && counter) {
        if (counter > 5*2) {
            counter /= 2;
            if (counter < 5*2) counter = 5*2;
        } else {
            counter--;
        }
...
int freeMemoryIfNeeded(void) {
    ...
    while (mem_freed < mem_tofree) {
        ...
        if (server.maxmemory_policy & (1<<0)) {
            ...
            while(bestkey == ((void *)0)) {
                ...
                for (i = 0; i < server.dbnum; i++) {
                    db = server.db+i;
                    dict = (server.maxmemory_policy == ((4<<8)|(1<<0))) ?
                            db->dict : db->expires;
                ...
                for (k = 16 -1; k >= 0; k--) {
                    ...
                    if (server.maxmemory_policy == ((4<<8)|(1<<0))) {
        ...
        else if (server.maxmemory_policy == (6<<8) ||
                 server.maxmemory_policy == (3<<8))
        {
            ...
            for (i = 0; i < server.dbnum; i++) {
                ...
                db = server.db+j;
                dict = (server.maxmemory_policy == (6<<8)) ?
                        db->dict : db->expires;
...
# 447 "evict.c"
            for (i = 0; i < server.dbnum; i++) {
                ...
                if (((dict)->ht[0].used+(dict)->ht[1].used) != 0) {
                    for (k = 0; k < server.maxmemory_samples; k++) {
                        ...
                        de = dictGetRandomKey(dict);
                        thiskey = ((de)->key);
                        thisttl = (long) ((de)->v.val);



                        if (bestkey == ((void *)0) || thisttl < bestttl) {
                            bestkey = thiskey;
                            bestttl = thisttl;
                            bestdbid = j;
                        }
            ...
            for (i = 0; i < server.dbnum; i++) {
                j = (++next_db) % server.dbnum;
                db = server.db+j;
                dict = (server.maxmemory_policy == ((5<<8)|(1<<1))) ?
                        db->dict : db->expires;
                if (((dict)->ht[0].used+(dict)->ht[1].used) != 0) {
                    for (k = 0; k < server.maxmemory_samples; k++) {
                        sds thiskey;
                        ...
                        if (bestkey == ((void *)0) || thisfreq < bestfreq) {
                            bestkey = thiskey;
                            bestfreq = thisfreq;
                            bestdbid = j;
                        }
