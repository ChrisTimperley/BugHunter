...
robj *lookupKeyByPattern(redisDb *db, robj *pattern, robj *subst) {
    ...
    p = (__extension__ (__builtin_constant_p ('*') && !__builtin_constant_p (spat) && ('*') == '\0' ? (char *) __rawmemchr (spat, '*') : __builtin_strchr (spat, '*')));
    if (!p) {
        decrRefCount(subst);
        return ((void *)0);
    }
    ...
    do { keyobj.refcount = 1; keyobj.type = 0; keyobj.encoding = 0; keyobj.ptr = ((char*)&keyname)+(sizeof(struct sdshdr)); } while(0);;
    o = lookupKeyRead(db,&keyobj);
    if (o == ((void *)0)) return ((void *)0);

    if (fieldlen > 0) {
        if (o->type != 4 || fieldname.len < 1) return ((void *)0);



        do { fieldobj.refcount = 1; fieldobj.type = 0; fieldobj.encoding = 0; fieldobj.ptr = ((char*)&fieldname)+(sizeof(struct sdshdr)); } while(0);;
        o = hashTypeGetObject(o, &fieldobj);
    } else {
        if (o->type != 0) return ((void *)0);
...
void sortCommand(redisClient *c) {
    ...
    if (sortval && sortval->type != 2 && sortval->type != 1 &&
        sortval->type != 3)
    {
    ...
    while(j < c->argc) {
        ...
        if (!strcasecmp(c->argv[j]->ptr,"asc")) {
            ...
            if ((getLongFromObjectOrReply(c, c->argv[j+1], &limit_start, ((void *)0)) != 0) ||
                (getLongFromObjectOrReply(c, c->argv[j+2], &limit_count, ((void *)0)) != 0)) return;
            j+=2;
            ...
            if ((__extension__ (__builtin_constant_p ('*') && !__builtin_constant_p (c->argv[j+1]->ptr) && ('*') == '\0' ? (char *) __rawmemchr (c->argv[j+1]->ptr, '*') : __builtin_strchr (c->argv[j+1]->ptr, '*'))) == ((void *)0)) dontsort = 1;
            j++;
    ...
    default: vectorlen = 0; _redisPanic("\"Bad SORT type\"","sort.c",224),_exit(1);
    }
    ...
    if (sortval->type == 1) {
        ...
        _redisPanic("\"Unknown type\"","sort.c",262),_exit(1);
    }
    ((j == vectorlen)?(void)0 : (_redisAssertWithInfo(c,sortval,"j == vectorlen","sort.c",264),_exit(1)));
        ...
        for (j = 0; j < vectorlen; j++) {
            ...
            if (alpha) {
                ...
                if (byval->encoding == 0) {
                    ...
                    if (eptr[0] != '\0' || (*__errno_location ()) == 34 ||
                        (sizeof (vector[j].u.score) == sizeof (float) ? __isnanf (vector[j].u.score) : sizeof (vector[j].u.score) == sizeof (double) ? __isnan (vector[j].u.score) : __isnanl (vector[j].u.score)))
                    {
                    ...
                    ((1 != 1)?(void)0 : (_redisAssertWithInfo(c,sortval,"1 != 1","sort.c",297),_exit(1)));
                }
            }
        ...
        for (j = start; j <= end; j++) {
            ...
            while((ln = listNext(&li))) {
                ...
                if (sop->type == 0) {
                    ...
                    ((sop->type == 0)?(void)0 : (_redisAssertWithInfo(c,sortval,"sop->type == REDIS_SORT_GET","sort.c",358),_exit(1)));
                }
            }
        }
    } else {
            ...
            if (!getop) {
                ...
                while((ln = listNext(&li))) {
                    ...
                    if (sop->type == 0) {
                        ...
                        ((sop->type == 0)?(void)0 : (_redisAssertWithInfo(c,sortval,"sop->type == REDIS_SORT_GET","sort.c",389),_exit(1)));
                    }
                }
            }
        }
    ...
    if (sortval->type == 1 || sortval->type == 2)
        for (j = 0; j < vectorlen; j++)
