...
static pthread_mutex_t bio_mutex[1];
static pthread_cond_t bio_condvar[1];
static list *bio_jobs[1];
...
static unsigned long long bio_pending[1];
...
void *bioProcessBackgroundJobs(void *arg) {
    ...
    while(1) {
        ...
        if (type == 0) {
            ...
            _redisPanic("\"Wrong job type in bioProcessBackgroundJobs().\"","bio.c",133),_exit(1);
        }
        zfree(job);
