...
struct redisCommand redisCommandTable[] = {
    ...
    {"exec",execCommand,1,"s",0,((void *)0),0,0,0,0,0},
    ...
    {"sort",sortCommand,-2,"wmS",0,((void *)0),1,1,1,0,0},
    ...
    {"publish",publishCommand,3,"pf",0,((void *)0),0,0,0,0,0},
...
void beforeSleep(struct aeEventLoop *eventLoop) {
    ...
    while (((server.unblocked_clients)->len)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","redis.c",921),_exit(1)));
        c = ln->value;
...
void initServer() {
    ...
    if (server.arch_bits == 32 && server.maxmemory == 0) {
        redisLog(3,"Warning: 32 bit instance detected but no memory limit set. Setting 3.5 GB maxmemory limit with 'noeviction' policy now.");
        server.maxmemory = 3584LL*(1024*1024);
...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
        ...
        while(*f != '\0') {
            ...
            default: _redisPanic("\"Unsupported command flag\"","redis.c",1292),_exit(1); break;
            }
            f++;
        }

        retval = dictAdd(server.commands, sdsnew(c->name), c);
        ((retval == 0) ? (void) (0) : __assert_fail ("retval == 0", "redis.c", 1298, __PRETTY_FUNCTION__));
    }
}
...
void version() {
    printf("Redis server v=%s sha=%s:%d malloc=%s\n", "2.9.5",
        redisGitSHA1(), atoi(redisGitDirty()) > 0, "libc");
    exit(0);
}
...
int main(int argc, char **argv) {
    ...
    redisAsciiArt();
    redisLog(3,"Server started, Redis version " "2.9.5");
