...
void replicationAbortSyncTransfer(void) {
    ((server.repl_state == 3)?(void)0 : (_redisAssert("server.repl_state == REDIS_REPL_TRANSFER","replication.c",264),_exit(1)));
...
void readSyncBulkPayload(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (server.repl_transfer_left == -1) {
        ...
        server.repl_transfer_left = strtol(buf+1,((void *)0),10);
        redisLog(2,
    ...
    if (write(server.repl_transfer_fd,buf,nread) != nread) {
        redisLog(3,"Write error or short write writing to the DB dump file needed for MASTER <-> SLAVE synchrnonization: %s", strerror((*__errno_location ())));
        goto error;
    }
...
void syncWithMaster(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if(server.masterauth) {
        ...
        if (syncWrite(fd,authcmd,authlen,server.repl_syncio_timeout) == -1) {
            redisLog(3,"Unable to AUTH to MASTER: %s",
                strerror((*__errno_location ())));
            goto error;
        }

        if (syncReadLine(fd,buf,1024,server.repl_syncio_timeout) == -1) {
            redisLog(3,"I/O error reading auth result from MASTER: %s",
                strerror((*__errno_location ())));
            goto error;
        }
        if (buf[0] != '+') {
...
void undoConnectWithMaster(void) {
    ...
    ((server.repl_state == 2)?(void)0 : (_redisAssert("server.repl_state == REDIS_REPL_CONNECTING","replication.c",495),_exit(1)));
    aeDeleteFileEvent(server.el,fd,1|2);
    close(fd);
...
void replicationCron(void) {
    ...
    if (server.masterhost && server.repl_state == 2 &&
    ...
    if (server.masterhost && server.repl_state == 3 &&
        (time(((void *)0))-server.repl_transfer_lastio) > server.repl_timeout)
    {
