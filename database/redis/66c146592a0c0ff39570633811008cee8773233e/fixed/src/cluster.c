...
int clusterProcessPacket(clusterLink *link) {
    ...
    redisLog(0,"--- packet to process %lu bytes (%lu) ---",
    ...
    if (type == 0 || type == 2) {
        ...
        redisLog(2,"Received unknown packet type: %d", type);
    }
    return 1;
}
...
int clusterDelSlot(int slot) {
    ...
    if (!n) return -1;
    ((clusterNodeClearSlotBit(n,slot) == 1)?(void)0 : (_redisAssert("clusterNodeClearSlotBit(n,slot) == 1","cluster.c",1030),_exit(1)));
    ...
    if (!strcasecmp(c->argv[1]->ptr,"meet") && c->argc == 4) {
        ...
        for (j = 0; j < 4096; j++) {
            if (slots[j]) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","cluster.c",1235),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
...
void migrateCommand(redisClient *c) {
    ...
    ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_redisAssert("rioWriteBulkCount(&cmd,'*',2)","cluster.c",1464),_exit(1)));
    ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_redisAssert("rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",1465),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_redisAssert("rioWriteBulkLongLong(&cmd,dbid)","cluster.c",1466),_exit(1)));

    ttl = getExpire(c->db,c->argv[3]);
    ((rioWriteBulkCount(&cmd,'*',4))?(void)0 : (_redisAssert("rioWriteBulkCount(&cmd,'*',4)","cluster.c",1469),_exit(1)));
    ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_redisAssert("rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",1470),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssert("c->argv[3]->encoding == REDIS_ENCODING_RAW","cluster.c",1471),_exit(1)));
    ((rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr)))?(void)0 : (_redisAssert("rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr))","cluster.c",1472),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl))?(void)0 : (_redisAssert("rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl)","cluster.c",1473),_exit(1)));



    rioInitWithBuffer(&payload,sdsempty());
    ((rdbSaveObjectType(&payload,o))?(void)0 : (_redisAssert("rdbSaveObjectType(&payload,o)","cluster.c",1478),_exit(1)));
    ((rdbSaveObject(&payload,o) != -1)?(void)0 : (_redisAssert("rdbSaveObject(&payload,o) != -1","cluster.c",1479),_exit(1)));
    ((rioWriteBulkString(&cmd,payload.io.buffer.ptr,sdslen(payload.io.buffer.ptr)))?(void)0 : (_redisAssert("rioWriteBulkString(&cmd,payload.io.buffer.ptr,sdslen(payload.io.buffer.ptr))","cluster.c",1480),_exit(1)));
...
void dumpCommand(redisClient *c) {
    ...
    rioInitWithBuffer(&payload,sdsempty());
    ((rdbSaveObjectType(&payload,o))?(void)0 : (_redisAssert("rdbSaveObjectType(&payload,o)","cluster.c",1555),_exit(1)));
    ((rdbSaveObject(&payload,o))?(void)0 : (_redisAssert("rdbSaveObject(&payload,o)","cluster.c",1556),_exit(1)));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssert("n != NULL","cluster.c",1626),_exit(1)));
            } else {
