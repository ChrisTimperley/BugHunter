...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",461),_exit(1)));
...
void setExpire(redisDb *db, robj *key, long long when) {
    ...
    ((kde != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"kde != NULL","db.c",470),_exit(1)));
...
long long getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",486),_exit(1)));
    return ((de)->v.s64);
}
# 498 "db.c"
void propagateExpire(redisDb *db, robj *key) {
...
void expireGenericCommand(redisClient *c, long long basetime, int unit) {
    ...
    if (when <= mstime() && !server.loading && !server.masterhost) {
        ...
        ((dbDelete(c->db,key))?(void)0 : (_redisAssertWithInfo(c,key,"dbDelete(c->db,key)","db.c",579),_exit(1)));
        server.dirty++;
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","db.c",676),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
