...
void clusterGetRandomName(char *p) {
    ...
    if (fp == ((void *)0) || fread(p,40,1,fp) == 0) {
        for (j = 0; j < 40; j++)
            p[j] = rand();
    }
    for (j = 0; j < 40; j++)
...
int clusterLoadConfig(char *filename) {
    ...
    while(fgets(line,maxline,fp) != ((void *)0)) {
        ...
        if ((p = strchr(argv[1],':')) == ((void *)0)) goto fmterr;
        *p = '\0';
        ...
        while(p) {
            ...
            if (!strcasecmp(s,"myself")) {
                ((server.cluster.myself == ((void *)0))?(void)0 : (_redisAssert("server.cluster.myself == NULL","cluster.c",73),_exit(1)));
                ...
                _redisPanic("\"Unknown flag in redis cluster config file\"","cluster.c",91),_exit(1);
            }
            if (p) s = p+1;
        }
        ...
        for (j = 7; j < argc; j++) {
            ...
            if (argv[j][0] == '[') {
                ...
                p = strchr(argv[j],'-');
                ((p != ((void *)0))?(void)0 : (_redisAssert("p != NULL","cluster.c",123),_exit(1)));
                *p = '\0';
                direction = p[1];
                slot = atoi(argv[j]+1);
                p += 3;
    ...
    ((server.cluster.myself != ((void *)0))?(void)0 : (_redisAssert("server.cluster.myself != NULL","cluster.c",155),_exit(1)));
...
void freeClusterNode(clusterNode *n) {
    ...
    ((dictDelete(server.cluster.nodes,nodename) == 0)?(void)0 : (_redisAssert("dictDelete(server.cluster.nodes,nodename) == DICT_OK","cluster.c",356),_exit(1)));
    sdsfree(nodename);
...
void clusterRenameNode(clusterNode *node, char *newname) {
    ...
    sdsfree(s);
    ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","cluster.c",395),_exit(1)));
...
void nodeIp2String(char *buf, clusterLink *link) {
    ...
    if (getpeername(link->fd, (struct sockaddr*) &sa, &salen) == -1)
        _redisPanic("\"getpeername() failed.\"","cluster.c",488),_exit(1);
...
int clusterDelSlot(int slot) {
    ...
    if (!n) return -1;
    ((clusterNodeClearSlotBit(n,slot) == 1)?(void)0 : (_redisAssert("clusterNodeClearSlotBit(n,slot) == 1","cluster.c",1113),_exit(1)));
               ...
               !strcasecmp(c->argv[1]->ptr,"delslots")) && c->argc >= 3)
    {
        ...
        for (j = 0; j < 4096; j++) {
            if (slots[j]) {
                ...
                ((retval == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"retval == REDIS_OK","cluster.c",1321),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
...
void restoreCommand(redisClient *c) {
    ...
    if (ttl) setExpire(c->db,c->argv[1],time(((void *)0))+ttl);
    addReply(c,shared.ok);
    server.dirty++;
}
...
void migrateCommand(redisClient *c) {
    ...
    ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",1551),_exit(1)));
    ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",1552),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,dbid)","cluster.c",1553),_exit(1)));

    ttl = getExpire(c->db,c->argv[3]);
    ((rioWriteBulkCount(&cmd,'*',4))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',4)","cluster.c",1556),_exit(1)));
    ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",1557),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argv[3]->encoding == REDIS_ENCODING_RAW","cluster.c",1558),_exit(1)));
    ((rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr))","cluster.c",1559),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl)","cluster.c",1560),_exit(1)));



    rioInitWithBuffer(&payload,sdsempty());
    ((rdbSaveObjectType(&payload,o))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObjectType(&payload,o)","cluster.c",1565),_exit(1)));
    ((rdbSaveObject(&payload,o) != -1)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObject(&payload,o) != -1","cluster.c",1566),_exit(1)));
    ((rioWriteBulkString(&cmd,payload.io.buffer.ptr,sdslen(payload.io.buffer.ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,payload.io.buffer.ptr,sdslen(payload.io.buffer.ptr))","cluster.c",1567),_exit(1)));
...
socket_wr_err:
    redisLog(2,"Can't write to target node for MIGRATE: %s",
        strerror((*__errno_location ())));
...
socket_rd_err:
    redisLog(2,"Can't read from target node for MIGRATE: %s",
        strerror((*__errno_location ())));
...
void dumpCommand(redisClient *c) {
    ...
    rioInitWithBuffer(&payload,sdsempty());
    ((rdbSaveObjectType(&payload,o))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObjectType(&payload,o)","cluster.c",1650),_exit(1)));
    ((rdbSaveObject(&payload,o))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObject(&payload,o)","cluster.c",1651),_exit(1)));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,firstkey,"n != NULL","cluster.c",1721),_exit(1)));
            } else {
