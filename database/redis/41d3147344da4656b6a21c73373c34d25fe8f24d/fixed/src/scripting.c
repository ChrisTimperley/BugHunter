...
# 836 "scripting.c"
    ...
    if ((lua_type(lua, (-1)) == 0)) {
        ...
        if (luaCreateFunction(c,lua,funcname,c->argv[1]) == -1) {
            lua_settop(lua, -(1)-1);
        ...
        lua_getfield(lua, (-10002), (funcname));
        ((!(lua_type(lua, (-1)) == 0))?(void)0 : (_redisAssert("!lua_isnil(lua,-1)","scripting.c",887),_exit(1)));
    }
...
# 949 "scripting.c"
    if (evalsha) {
        if (!replicationScriptCacheExists(c->argv[1]->ptr)) {
            ...
            ((script != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"script != NULL","scripting.c",957),_exit(1)));
