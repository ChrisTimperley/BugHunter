...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",425),_exit(1)));
...
void setExpire(redisDb *db, robj *key, time_t when) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"de != NULL","db.c",434),_exit(1)));
...
time_t getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",449),_exit(1)));
...
void expireGenericCommand(redisClient *c, robj *key, robj *param, long offset) {
    ...
    if (seconds <= 0 && !server.loading && !server.masterhost) {
        ...
        ((dbDelete(c->db,key))?(void)0 : (_redisAssertWithInfo(c,key,"dbDelete(c->db,key)","db.c",531),_exit(1)));
        server.dirty++;
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","db.c",602),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
