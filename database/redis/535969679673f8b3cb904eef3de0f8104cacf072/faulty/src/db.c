...
void dbAdd(redisDb *db, robj *key, robj *val) {
    ...
    ((retval == 0)?(void)0 : (_serverAssertWithInfo(((void *)0),key,"retval == C_OK","db.c",119),_exit(1)));
...
# 231 "db.c"
robj *dbUnshareStringValue(redisDb *db, robj *key, robj *o) {
    ((o->type == 0)?(void)0 : (_serverAssert("o->type == OBJ_STRING","db.c",232),_exit(1)));
...
void scanCallback(void *privdata, const dictEntry *de) {
    ...
    if (o == ((void *)0)) {
        ...
        _serverPanic("\"Type not handled in SCAN callback.\"","db.c",492),_exit(1);
    }
...
# 586 "db.c"
    ...
    if (ht) {
        ...
        _serverPanic("\"Not handled encoding in SCAN.\"","db.c",640),_exit(1);
    }
    ...
    while (node) {
        ...
        if (!filter && use_pattern) {
            if ((kobj->encoding == 0 || kobj->encoding == 8)) {
                ...
                ((kobj->encoding == 1)?(void)0 : (_serverAssert("kobj->encoding == OBJ_ENCODING_INT","db.c",659),_exit(1)));
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",881),_exit(1)));
...
void setExpire(redisDb *db, robj *key, long long when) {
    ...
    ((kde != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"kde != NULL","db.c",890),_exit(1)));
...
long long getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",906),_exit(1)));
    return ((de)->v.s64);
}
# 918 "db.c"
void propagateExpire(redisDb *db, robj *key) {
...
void expireGenericCommand(client *c, long long basetime, int unit) {
    ...
    if (when <= mstime() && !server.loading && !server.masterhost) {
        ...
        ((dbDelete(c->db,key))?(void)0 : (_serverAssertWithInfo(c,key,"dbDelete(c->db,key)","db.c",1006),_exit(1)));
        server.dirty++;
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_serverAssert("j < argc","db.c",1107),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
