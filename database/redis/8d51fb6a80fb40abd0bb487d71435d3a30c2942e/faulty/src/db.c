...
robj *lookupKey(redisDb *db, robj *key) {
    ...
    if (de) {
        ...
        if (server.ds_enabled && val->storage == 2) {
            ...
            ((val->storage != 2)?(void)0 : (_redisAssert("val->storage != REDIS_DS_SAVING","db.c",24),_exit(1)));
        }
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",417),_exit(1)));
...
void setExpire(redisDb *db, robj *key, time_t when) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","db.c",426),_exit(1)));
...
time_t getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",441),_exit(1)));
