...
void dsInit(void) {
    ...
    ((anetNonBlock(((void *)0),server.io_ready_pipe_read) != -1)?(void)0 : (_redisAssert("anetNonBlock(NULL,server.io_ready_pipe_read) != ANET_ERR","dscache.c",147),_exit(1)));
            ...
            int mask)
{
    ...
    while((retval = read(fd,buf,1)) == 1) {
        ...
        lockThreadedIO();
        ((((server.io_processed)->len) != 0)?(void)0 : (_redisAssert("listLength(server.io_processed) != 0","dscache.c",277),_exit(1)));
        if (toprocess == -1) {
        ...
        if (j->type == 0) {
            ...
            ((j->val->storage == 2)?(void)0 : (_redisAssert("j->val->storage == REDIS_DS_SAVING","dscache.c",307),_exit(1)));
            j->val->storage = 0;
            freeIOJob(j);
        }
        processed++;
...
void *IOThreadEntryPoint(void *arg) {
    ...
    while(1) {
        ...
        if (j->type == 0) {
            ...
            ((j->val->storage == 2)?(void)0 : (_redisAssert("j->val->storage == REDIS_DS_SAVING","dscache.c",365),_exit(1)));
            if (j->val)
        ...
        ((write(server.io_ready_pipe_write,"x",1) == 1)?(void)0 : (_redisAssert("write(server.io_ready_pipe_write,\"x\",1) == 1","dscache.c",381),_exit(1)));
    }
...
void cacheCron(void) {
    ...
    while((ln = ((server.cache_flush_queue)->head)) != ((void *)0)) {
        ...
        if ((now - dk->ctime) >= server.cache_flush_delay) {
            ...
            if (de) {
                val = ((de)->val);
                ((val->storage == 1)?(void)0 : (_redisAssert("val->storage == REDIS_DS_DIRTY","dscache.c",506),_exit(1)));
                val->storage = 2;
            } else {
...
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",574),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",594),_exit(1)));
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",690),_exit(1)));
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",697),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
