...
# 732 "hyperloglog.c"
    ...
    if (is_zero || is_xzero) {
     ...
     if (deltalen > 0 && sdslen(o->ptr) > 3000) goto promote;
     if (deltalen && next) {
...
double hllSparseSum(uint8_t *sparse, int sparselen, double *PE, int *ezp) {
    ...
    ((idx == (1<<14))?(void)0 : (_redisAssert("idx == HLL_REGISTERS","hyperloglog.c",842),_exit(1)));
    *ezp = ez;
    return E;
}
# 856 "hyperloglog.c"
...
robj *createHLLObject(void) {
    ...
    (((p-(uint8_t*)s) == sparselen)?(void)0 : (_redisAssert("(p-(uint8_t*)s) == sparselen","hyperloglog.c",938),_exit(1)));
