...
static int cliReadInlineReply(int fd, int type) {
    ...
    if (reply == ((void *)0)) return 1;
    printf("%s\n", reply);
    if (type == 64) return 0;
    if (type == 4) return atoi(reply) < 0;
    if (type == 8) return reply[0] == '-';
    return 0;
}
...
static int cliSendCommand(int argc, char **argv) {
    ...
    if (rc->flags & 4) {
        retval = cliReadInlineReply(fd,4);
    } else if (rc->flags & 8) {
        retval = cliReadInlineReply(fd,8);
    } else if (rc->flags & 64) {
        retval = cliReadInlineReply(fd,64);
    } else if (rc->flags & 16) {
...
static int parseOptions(int argc, char **argv) {
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!strcmp(argv[i],"-h") && !lastarg) {
            char *ip = zmalloc(32);
            if (anetResolve(((void *)0),argv[i+1],ip) == -1) {
