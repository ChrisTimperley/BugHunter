...
void rdbCheckThenExit(char *reason, int where) {
    serverLog(3, "Corrupt RDB detected at rdb.c:%d (%s). "
...
ssize_t rdbSaveLongLongAsStringObject(rio *rdb, long long value) {
    ...
    if (enclen > 0) {
        ...
        ((enclen < 32)?(void)0 : (_serverAssert("enclen < 32","rdb.c",360),_exit(1)));
...
int rdbSaveStringObject(rio *rdb, robj *obj) {
    ...
    if (obj->encoding == 1) {
        ...
        (((obj->encoding == 0 || obj->encoding == 8))?(void)0 : (_serverAssertWithInfo(((void *)0),obj,"sdsEncodedObject(obj)","rdb.c",376),_exit(1)));
...
int rdbSaveObjectType(rio *rdb, robj *o) {
    ...
    case 1:
        if (o->encoding == 9)
            ...
            _serverPanic("\"Unknown list encoding\"","rdb.c",507),_exit(1);
    case 2:
        if (o->encoding == 6)
            ...
            _serverPanic("\"Unknown set encoding\"","rdb.c",514),_exit(1);
    case 3:
        if (o->encoding == 5)
            ...
            _serverPanic("\"Unknown sorted set encoding\"","rdb.c",521),_exit(1);
    case 4:
        if (o->encoding == 5)
            ...
            _serverPanic("\"Unknown hash encoding\"","rdb.c",528),_exit(1);
    default:
        _serverPanic("\"Unknown object type\"","rdb.c",530),_exit(1);
    }
    return -1;
}
...
ssize_t rdbSaveObject(rio *rdb, robj *o) {
    ...
    if (o->type == 0) {
        ...
        if (o->encoding == 9) {
            ...
            _serverPanic("\"Unknown list encoding\"","rdb.c",573),_exit(1);
        }
    } else if (o->type == 2) {
        ...
        if (o->encoding == 2) {
            ...
            _serverPanic("\"Unknown set encoding\"","rdb.c",597),_exit(1);
        }
    } else if (o->type == 3) {
        ...
        if (o->encoding == 5) {
            ...
            _serverPanic("\"Unknown sorted set encoding\"","rdb.c",625),_exit(1);
        }
    } else if (o->type == 4) {
        ...
        if (o->encoding == 5) {
            ...
            _serverPanic("\"Unknown hash encoding\"","rdb.c",654),_exit(1);
        }

    } else {
        _serverPanic("\"Unknown object type\"","rdb.c",658),_exit(1);
    }
    return nwritten;
}
...
size_t rdbSavedObjectLen(robj *o) {
    ...
    ((len != -1)?(void)0 : (_serverAssertWithInfo(((void *)0),o,"len != -1","rdb.c",669),_exit(1)));
    return len;
}
...
int rdbSave(char *filename) {
    ...
    fp = fopen(tmpfile,"w");
    if (!fp) {
        serverLog(3, "Failed opening .rdb for saving: %s",
    ...
    if (rename(tmpfile,filename) == -1) {
        serverLog(3,"Error moving temp DB file on the final destination: %s", strerror((*__errno_location ())));
        unlink(tmpfile);
        return -1;
    }
...
werr:
    serverLog(3,"Write error saving DB on disk: %s", strerror((*__errno_location ())));
    fclose(fp);
    unlink(tmpfile);
    return -1;
}
...
robj *rdbLoadObject(int rdbtype, rio *rdb) {
    ...
    if (rdbtype == 0) {
        ...
        while (o->encoding == 5 && len > 0) {
            ...
            (((field->encoding == 0 || field->encoding == 8))?(void)0 : (_serverAssert("sdsEncodedObject(field)","rdb.c",1054),_exit(1)));
            value = rdbLoadStringObject(rdb);
            if (value == ((void *)0)) return ((void *)0);
            (((value->encoding == 0 || value->encoding == 8))?(void)0 : (_serverAssert("sdsEncodedObject(value)","rdb.c",1057),_exit(1)));
        ...
        ((len == 0)?(void)0 : (_serverAssert("len == 0","rdb.c",1097),_exit(1)));
    } else if (rdbtype == 14) {
        if ((len = rdbLoadLen(rdb,((void *)0))) == (2147483647 * 2U + 1U)) return ((void *)0);
        o = createQuicklistObject();
        quicklistSetOptions(o->ptr, server.list_max_ziplist_size,
...
void backgroundSaveDoneHandler(int exitcode, int bysignal) {
    ...
    case 2:
        ...
        break;
    default:
        _serverPanic("\"Unknown RDB child type.\"","rdb.c",1532),_exit(1);
        break;
    }
}
...
# 1625 "rdb.c"
        ...
        serverLog(2,"Background RDB transfer started by pid %d",childpid);
        server.rdb_save_time_start = time(((void *)0));
        server.rdb_child_pid = childpid;
