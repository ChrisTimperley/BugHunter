...
static int aeApiAddEvent(aeEventLoop *eventLoop, int fd, int mask) {
    ...
    int op = eventLoop->events[fd].mask == 0 ?
            1 : 3;

    ee.events = 0;
    mask |= eventLoop->events[fd].mask;
    if (mask & 1) ee.events |= EPOLLIN;
    if (mask & 2) ee.events |= EPOLLOUT;
    ee.data.u64 = 0;
    ee.data.fd = fd;
...
static int aeApiPoll(aeEventLoop *eventLoop, struct timeval *tvp) {
    ...
    if (retval > 0) {
        ...
        for (j = 0; j < numevents; j++) {
            ...
            if (e->events & EPOLLIN) mask |= 1;
            if (e->events & EPOLLOUT) mask |= 2;
            if (e->events & EPOLLERR) mask |= 2;
            if (e->events & EPOLLHUP) mask |= 2;
        ...
        aeFileProc *proc, void *clientData)
{
    ...
    if (aeApiAddEvent(eventLoop, fd, mask) == -1)
        return -1;
    fe->mask |= mask;
    if (mask & 1) fe->rfileProc = proc;
    if (mask & 2) fe->wfileProc = proc;
...
void aeDeleteFileEvent(aeEventLoop *eventLoop, int fd, int mask)
{
    ...
    if (fd == eventLoop->maxfd && fe->mask == 0) {
        ...
        for (j = eventLoop->maxfd-1; j >= 0; j--)
            if (eventLoop->events[j].mask != 0) break;
        eventLoop->maxfd = j;
    }
...
int aeWait(int fd, int mask, long long milliseconds) {
    ...
    pfd.fd = fd;
    if (mask & 1) pfd.events |= 0x001;
    if (mask & 2) pfd.events |= 0x004;

    if ((retval = poll(&pfd, 1, milliseconds))== 1) {
        if (pfd.revents & 0x001) retmask |= 1;
        if (pfd.revents & 0x004) retmask |= 2;
 if (pfd.revents & 0x008) retmask |= 2;
        if (pfd.revents & 0x010) retmask |= 2;
        return retmask;
    } else {
        return retval;
    }
