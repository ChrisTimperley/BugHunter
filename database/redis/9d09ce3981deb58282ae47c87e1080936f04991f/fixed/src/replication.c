...
void replicationAbortSyncTransfer(void) {
    ((server.repl_state == 4)?(void)0 : (_redisAssert("server.repl_state == REDIS_REPL_TRANSFER","replication.c",337),_exit(1)));
...
void syncWithMaster(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (server.repl_state == 3) {
        ...
        if (buf[0] != '-' && buf[0] != '+') {
            redisLog(3,"Unexpected reply to PING from master.");
            goto error;
        } else {
            redisLog(2,
