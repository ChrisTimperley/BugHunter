...
int clusterLoadConfig(char *filename) {
        ...
        *p = '\0';
        ...
        while(p) {
            ...
            if (!strcasecmp(s,"myself")) {
                ((server.cluster.myself == ((void *)0))?(void)0 : (_redisAssert("server.cluster.myself == NULL","cluster.c",91),_exit(1)));
                ...
                _redisPanic("\"Unknown flag in redis cluster config file\"","cluster.c",109),_exit(1);
            }
            if (p) s = p+1;
        }
        ...
        for (j = 7; j < argc; j++) {
            ...
            if (argv[j][0] == '[') {
                ...
                ((p != ((void *)0))?(void)0 : (_redisAssert("p != NULL","cluster.c",141),_exit(1)));
                *p = '\0';
                direction = p[1];
                slot = atoi(argv[j]+1);
                p += 3;
    ...
    ((server.cluster.myself != ((void *)0))?(void)0 : (_redisAssert("server.cluster.myself != NULL","cluster.c",173),_exit(1)));
...
void clusterInit(void) {
    ...
    if (aeCreateFileEvent(server.el, server.cfd, 1,
        clusterAcceptHandler, ((void *)0)) == -1) _redisPanic("\"Unrecoverable error creating Redis Cluster file event.\"","cluster.c",243),_exit(1);
...
void freeClusterNode(clusterNode *n) {
    ...
    ((dictDelete(server.cluster.nodes,nodename) == 0)?(void)0 : (_redisAssert("dictDelete(server.cluster.nodes,nodename) == DICT_OK","cluster.c",374),_exit(1)));
    sdsfree(nodename);
...
void clusterRenameNode(clusterNode *node, char *newname) {
    ...
    sdsfree(s);
    ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","cluster.c",413),_exit(1)));
...
void nodeIp2String(char *buf, clusterLink *link) {
    ...
    if (getpeername(link->fd, (struct sockaddr*) &sa, &salen) == -1)
        _redisPanic("\"getpeername() failed.\"","cluster.c",506),_exit(1);
...
int clusterDelSlot(int slot) {
    ...
    if (!n) return -1;
    ((clusterNodeClearSlotBit(n,slot) == 1)?(void)0 : (_redisAssert("clusterNodeClearSlotBit(n,slot) == 1","cluster.c",1131),_exit(1)));
               ...
               !strcasecmp(c->argv[1]->ptr,"delslots")) && c->argc >= 3)
    {
        ...
        for (j = 0; j < 4096; j++) {
            if (slots[j]) {
                ...
                ((retval == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"retval == REDIS_OK","cluster.c",1339),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
...
void createDumpPayload(rio *payload, robj *o) {
    ...
    ((rdbSaveObjectType(payload,o))?(void)0 : (_redisAssert("rdbSaveObjectType(payload,o)","cluster.c",1504),_exit(1)));
    ((rdbSaveObject(payload,o))?(void)0 : (_redisAssert("rdbSaveObject(payload,o)","cluster.c",1505),_exit(1)));
# 1515 "cluster.c"
    buf[0] = 6 & 0xff;
    buf[1] = (6 >> 8) & 0xff;
...
try_again:
    ...
    ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",1791),_exit(1)));
    ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",1792),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,dbid)","cluster.c",1793),_exit(1)));
    ...
    ((rioWriteBulkCount(&cmd,'*',replace ? 5 : 4))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',replace ? 5 : 4)","cluster.c",1800),_exit(1)));
    ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",1801),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argv[3]->encoding == REDIS_ENCODING_RAW","cluster.c",1802),_exit(1)));
    ((rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr))","cluster.c",1803),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,ttl))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,ttl)","cluster.c",1804),_exit(1)));
...
# 1809 "cluster.c"
    ...
    if (replace)
        ((rioWriteBulkString(&cmd,"REPLACE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"REPLACE\",7)","cluster.c",1816),_exit(1)));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,firstkey,"n != NULL","cluster.c",1958),_exit(1)));
            } else {
