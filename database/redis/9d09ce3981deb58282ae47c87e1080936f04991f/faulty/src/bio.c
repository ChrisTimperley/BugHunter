...
static pthread_t bio_threads[2];
static pthread_mutex_t bio_mutex[2];
static pthread_cond_t bio_condvar[2];
static list *bio_jobs[2];
...
static unsigned long long bio_pending[2];
...
void bioInit(void) {
    ...
    for (j = 0; j < 2; j++) {
        ...
        pthread_cond_init(&bio_condvar[j],((void *)0));
    ...
    for (j = 0; j < 2; j++) {
        ...
        if (pthread_create(&thread,&attr,bioProcessBackgroundJobs,arg) != 0) {
            redisLog(3,"Fatal: Can't initialize Background Jobs.");
            exit(1);
        }
...
void *bioProcessBackgroundJobs(void *arg) {
    ...
    if (pthread_sigmask(0, &sigset, ((void *)0)))
        redisLog(3,
            "Warning: can't mask SIGALRM in bio.c thread: %s", strerror((*__errno_location ())));
    ...
    while(1) {
        ...
        if (type == 0) {
            ...
            _redisPanic("\"Wrong job type in bioProcessBackgroundJobs().\"","bio.c",180),_exit(1);
        }
        zfree(job);
