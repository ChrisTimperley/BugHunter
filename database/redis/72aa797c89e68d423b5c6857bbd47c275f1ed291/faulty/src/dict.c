...
int dictRehash(dict *d, int n) {
    ...
    while(n--) {
        ...
        ((d->ht[0].size > (unsigned)d->rehashidx)?(void)0 : (_redisAssert("d->ht[0].size > (unsigned)d->rehashidx","dict.c",254),_exit(1)));
...
void dictReleaseIterator(dictIterator *iter)
{
    if (!(iter->index == -1 && iter->table == 0)) {
        if (iter->safe)
            ...
            ((iter->fingerprint == dictFingerprint(iter->d))?(void)0 : (_redisAssert("iter->fingerprint == dictFingerprint(iter->d)","dict.c",602),_exit(1)));
    }
    zfree(iter);
}
...
unsigned int dictGetRandomKeys(dict *d, dictEntry **des, unsigned int count) {
    ...
    while(stored < count) {
        for (j = 0; j < 2; j++) {
            ...
            ((((d)->rehashidx != -1) != 0)?(void)0 : (_redisAssert("dictIsRehashing(d) != 0","dict.c",692),_exit(1)));
        }
    }
    return stored;
}
