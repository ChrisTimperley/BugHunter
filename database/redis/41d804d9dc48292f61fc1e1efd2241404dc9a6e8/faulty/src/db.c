...
# 78 "db.c"
        if (server.current_client &&
            ...
            server.current_client->cmd->flags & (1<<1))
        {
            return ((void *)0);
        }
    }
...
void dbAdd(redisDb *db, robj *key, robj *val) {
    ...
    ((retval == 0)?(void)0 : (_serverAssertWithInfo(((void *)0),key,"retval == DICT_OK","db.c",119),_exit(1)));
...
void dbOverwrite(redisDb *db, robj *key, robj *val) {
    ...
    ((de != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"de != NULL","db.c",132),_exit(1)));
...
# 230 "db.c"
robj *dbUnshareStringValue(redisDb *db, robj *key, robj *o) {
    ((o->type == 0)?(void)0 : (_serverAssert("o->type == OBJ_STRING","db.c",231),_exit(1)));
...
void scanCallback(void *privdata, const dictEntry *de) {
    ...
    if (o == ((void *)0)) {
        ...
        _serverPanic("\"Type not handled in SCAN callback.\"","db.c",491),_exit(1);
    }
...
# 585 "db.c"
    ...
    if (ht) {
        ...
        _serverPanic("\"Not handled encoding in SCAN.\"","db.c",639),_exit(1);
    }
    ...
    while (node) {
        ...
        if (!filter && use_pattern) {
            if ((kobj->encoding == 0 || kobj->encoding == 8)) {
                ...
                ((kobj->encoding == 1)?(void)0 : (_serverAssert("kobj->encoding == OBJ_ENCODING_INT","db.c",658),_exit(1)));
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",884),_exit(1)));
...
void setExpire(redisDb *db, robj *key, long long when) {
    ...
    ((kde != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"kde != NULL","db.c",893),_exit(1)));
...
long long getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",909),_exit(1)));
    return ((de)->v.s64);
}
# 921 "db.c"
void propagateExpire(redisDb *db, robj *key, int lazy) {
...
void expireGenericCommand(client *c, long long basetime, int unit) {
    ...
    if (when <= mstime() && !server.loading && !server.masterhost) {
        ...
        ((deleted)?(void)0 : (_serverAssertWithInfo(c,key,"deleted","db.c",1012),_exit(1)));
        server.dirty++;
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_serverAssert("j < argc","db.c",1112),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
...
int *getKeysFromCommand(struct redisCommand *cmd, robj **argv, int argc, int *numkeys) {
    ...
    } else if (!(cmd->flags & (1<<3)) && cmd->getkeys_proc) {
...
void slotToKeyDel(robj *key) {
    unsigned int hashslot = keyHashSlot(key->ptr,sdslen(key->ptr));
    zslDelete(server.cluster->slots_to_keys,hashslot,key->ptr,((void *)0));
}
