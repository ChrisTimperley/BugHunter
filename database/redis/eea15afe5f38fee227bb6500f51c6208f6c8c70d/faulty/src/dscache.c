...
void cacheCron(void) {
    ...
    while((ln = ((server.cache_flush_queue)->head)) != ((void *)0)) {
        ...
        if ((now - dk->ctime) >= server.cache_flush_delay) {
            ...
            if (de) {
                val = ((de)->val);
                ((val->storage == 1)?(void)0 : (_redisAssert("val->storage == REDIS_DS_DIRTY","dscache.c",496),_exit(1)));
                val->storage = 2;
            } else {
...
int waitForSwappedKey(redisClient *c, robj *key) {
    ...
    if (de == ((void *)0)) {
        ...
        incrRefCount(key);
        ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","dscache.c",562),_exit(1)));
    } else {
        l = ((de)->val);
    }
    listAddNodeTail(l,c);
...
void waitForMultipleSwappedKeys(redisClient *c, struct redisCommand *cmd, int argc, robj **argv) {
    ...
    for (j = cmd->vm_firstkey; j <= last; j += cmd->vm_keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","dscache.c",582),_exit(1)));
...
int dontWaitForSwappedKey(redisClient *c, robj *key) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",678),_exit(1)));


    de = dictFind(c->db->io_keys,key);
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","dscache.c",682),_exit(1)));
    l = ((de)->val);
    ln = listSearchKey(l,c);
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","dscache.c",685),_exit(1)));
    listDelNode(l,ln);
    if (((l)->len) == 0)
