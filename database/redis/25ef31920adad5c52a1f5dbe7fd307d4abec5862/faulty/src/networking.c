...
int _installWriteEvent(redisClient *c) {
    ...
    ((!(c->flags & 128))?(void)0 : (_redisAssert("!(c->flags & REDIS_CLOSE_AFTER_REPLY)","networking.c",64),_exit(1)));
...
robj *dupLastObjectIfNeeded(list *reply) {
    robj *new, *cur;
    listNode *ln;
    ((((reply)->len) > 0)?(void)0 : (_redisAssert("listLength(reply) > 0","networking.c",80),_exit(1)));
    ln = ((reply)->tail);
    cur = ((ln)->value);
    if (cur->refcount > 1) {
...
void freeClient(redisClient *c) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",457),_exit(1)));
    listDelNode(server.clients,ln);


    if (c->flags & 256) {
        ln = listSearchKey(server.unblocked_clients,c);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",463),_exit(1)));
        listDelNode(server.unblocked_clients,ln);
    }


    if (c->flags & 32) {
        ((server.ds_enabled)?(void)0 : (_redisAssert("server.ds_enabled","networking.c",469),_exit(1)));
        if (((c->io_keys)->len) == 0) {
            ln = listSearchKey(server.io_ready_clients,c);



            ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",475),_exit(1)));
    ...
    if (c->flags & 1) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",493),_exit(1)));
        listDelNode(l,ln);
    }
...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_redisAssert("c->argc == 0","networking.c",679),_exit(1)));
        ...
        ((c->querybuf[0] == '*')?(void)0 : (_redisAssert("c->querybuf[0] == '*'","networking.c",688),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssert("c->multibulklen > 0","networking.c",708),_exit(1)));
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",786),_exit(1);
        }
