...
void freeClient(redisClient *c) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",474),_exit(1)));
    listDelNode(server.clients,ln);


    if (c->flags & 256) {
        ln = listSearchKey(server.unblocked_clients,c);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",480),_exit(1)));
        listDelNode(server.unblocked_clients,ln);
    }


    if (c->flags & 32) {
        ((server.ds_enabled)?(void)0 : (_redisAssert("server.ds_enabled","networking.c",486),_exit(1)));
        if (((c->io_keys)->len) == 0) {
            ln = listSearchKey(server.io_ready_clients,c);



            ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",492),_exit(1)));
...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_redisAssert("c->argc == 0","networking.c",696),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssert("c->multibulklen > 0","networking.c",725),_exit(1)));
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",803),_exit(1);
        }
