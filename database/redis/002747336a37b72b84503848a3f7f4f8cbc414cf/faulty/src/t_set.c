...
int setTypeAdd(robj *subject, robj *value) {
    ...
    if (subject->encoding == 2) {
        ...
        if (isObjectRepresentableAsLongLong(value,&llval) == 0) {
            ...
            ((dictAdd(subject->ptr,value,((void *)0)) == 0)?(void)0 : (_redisAssertWithInfo(((void *)0),value,"dictAdd(subject->ptr,value,NULL) == DICT_OK","t_set.c",71),_exit(1)));
            incrRefCount(value);
            return 1;
        }
    } else {
        _redisPanic("\"Unknown set encoding\"","t_set.c",76),_exit(1);
    }
    return 0;
}
...
int setTypeRemove(robj *setobj, robj *value) {
    ...
    if (setobj->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",95),_exit(1);
    }
    return 0;
}
...
int setTypeIsMember(robj *subject, robj *value) {
    ...
    if (subject->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",109),_exit(1);
    }
    return 0;
}
...
setTypeIterator *setTypeInitIterator(robj *subject) {
    ...
    if (si->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",123),_exit(1);
    }
    return si;
}
...
robj *setTypeNextObject(setTypeIterator *si) {
    ...
    switch(encoding) {
        ...
        case 2:
            ...
            return objele;
        default:
            _redisPanic("\"Unsupported encoding\"","t_set.c",178),_exit(1);
    }
    return ((void *)0);
}
# 196 "t_set.c"
int setTypeRandomElement(robj *setobj, robj **objele, int64_t *llele) {
    if (setobj->encoding == 2) {
        ...
        *llele = intsetRandom(setobj->ptr);
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",203),_exit(1);
    }
...
unsigned long setTypeSize(robj *subject) {
    if (subject->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",214),_exit(1);
    }
}
...
# 223 "t_set.c"
    ...
    if (enc == 2) {
        ...
        while (setTypeNext(si,((void *)0),&intele) != -1) {
            ...
            ((dictAdd(d,element,((void *)0)) == 0)?(void)0 : (_redisAssertWithInfo(((void *)0),element,"dictAdd(d,element,NULL) == DICT_OK","t_set.c",238),_exit(1)));
        }
        ...
        _redisPanic("\"Unsupported set conversion\"","t_set.c",246),_exit(1);
    }
}
...
void srandmemberWithCountCommand(redisClient *c) {
    ...
    if (count >= size) {
        sunionDiffGenericCommand(c,c->argv,c->argc-1,((void *)0),0);
        return;
    }
...
# 488 "t_set.c"
    if (count*3 > size) {
        ...
        while((encoding = setTypeNext(si,&ele,&llele)) != -1) {
            ...
            } else if (ele->encoding == 0) {
                retval = dictAdd(d,dupStringObject(ele),((void *)0));
            } else if (ele->encoding == 1) {
                retval = dictAdd(d,
                    createStringObjectFromLongLong((long)ele->ptr),((void *)0));
            }
            ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","t_set.c",504),_exit(1)));
        }
        setTypeReleaseIterator(si);
        ((((d)->ht[0].used+(d)->ht[1].used) == size)?(void)0 : (_redisAssert("dictSize(d) == size","t_set.c",507),_exit(1)));
