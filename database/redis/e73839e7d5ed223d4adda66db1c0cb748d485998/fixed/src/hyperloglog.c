...
void hllSelftestCommand(redisClient *c) {
    ...
    for (j = 0; j < 1000; j++) {
        ...
        for (i = 0; i < 16384; i++) {
            ...
            bytecounters[i] = r;
            do { uint8_t *_p = (uint8_t*) bitcounters; int _byte = i*6/8; int _leftshift = i*6&7; int _rightshift = 8 - _leftshift; _p[_byte] &= ~(((1<<6)-1) >> _leftshift); _p[_byte] |= r >> _leftshift; _p[_byte+1] &= ~(((1<<6)-1) << _rightshift); _p[_byte+1] |= r << _rightshift; } while(0);
        }
