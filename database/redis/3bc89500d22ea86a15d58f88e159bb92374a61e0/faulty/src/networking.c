...
void _addReplyObjectToList(redisClient *c, robj *o) {
    ...
    if (((c->reply)->len) == 0) {
        ...
        if (tail->ptr != ((void *)0) &&
            sdslen(tail->ptr)+sdslen(o->ptr) <= (5*1500))
        {
...
void _addReplySdsToList(redisClient *c, sds s) {
    ...
    if (((c->reply)->len) == 0) {
        ...
        if (tail->ptr != ((void *)0) &&
            sdslen(tail->ptr)+sdslen(s) <= (5*1500))
        {
            tail = dupLastObjectIfNeeded(c->reply);
...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_redisAssert("c->argc == 0","networking.c",706),_exit(1)));
        ...
        ((c->querybuf[0] == '*')?(void)0 : (_redisAssert("c->querybuf[0] == '*'","networking.c",719),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssert("c->multibulklen > 0","networking.c",740),_exit(1)));
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",818),_exit(1);
        }
...
void rewriteClientCommandVector(redisClient *c, int argc, ...) {
    ...
    ((c->cmd != ((void *)0))?(void)0 : (_redisAssert("c->cmd != NULL","networking.c",973),_exit(1)));
    __builtin_va_end(ap);
}
...
void rewriteClientCommandArgument(redisClient *c, int i, robj *newval) {
    ...
    ((i < c->argc)?(void)0 : (_redisAssert("i < c->argc","networking.c",982),_exit(1)));
    oldval = c->argv[i];
    c->argv[i] = newval;
    incrRefCount(newval);
    decrRefCount(oldval);
    ...
    if (i == 0) {
        c->cmd = lookupCommand(c->argv[0]->ptr);
        ((c->cmd != ((void *)0))?(void)0 : (_redisAssert("c->cmd != NULL","networking.c",991),_exit(1)));
    }
}
