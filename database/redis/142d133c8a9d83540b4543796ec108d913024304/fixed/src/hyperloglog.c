...
int hllSparseAdd(robj *o, unsigned char *ele, size_t elesize) {
    ...
    while(p < end) {
        ...
        if ((((*p) & 0xc0) == 0)) span = (((*p) & 0x3f)+1);
        else if ((((*p) & 0xc0) == 0x40)) span = (((((*p) & 0x3f) << 8) | (*(p+1)))+1);
        else span = (((*p) & 0x3)+1);
...
# 750 "hyperloglog.c"
    ...
    if (is_zero || is_xzero) {
     ...
     if (deltalen > 0 && sdslen(o->ptr)+deltalen > 3000) goto promote;
...
double hllSparseSum(uint8_t *sparse, int sparselen, double *PE, int *ezp) {
    ...
    ((idx == (1<<14))?(void)0 : (_redisAssert("idx == HLL_REGISTERS","hyperloglog.c",858),_exit(1)));
    *ezp = ez;
    return E;
}
# 872 "hyperloglog.c"
uint64_t hllCount(struct hllhdr *hdr) {
    ...
    if (hdr->encoding == 0) {
        ...
        E = hllSparseSum(hdr->registers,sdslen((sds)hdr)-sizeof(struct hllhdr),PE,&ez);
    }
...
robj *createHLLObject(void) {
    ...
    (((p-(uint8_t*)s) == sparselen)?(void)0 : (_redisAssert("(p-(uint8_t*)s) == sparselen","hyperloglog.c",964),_exit(1)));
...
void pfmergeCommand(redisClient *c) {
    ...
    hdr = o->ptr;
    for (j = 0; j < (1<<14); j++) {
        do { uint8_t *_p = (uint8_t*) hdr->registers; unsigned long _byte = j*6/8; unsigned long _fb = j*6&7; unsigned long _fb8 = 8 - _fb; unsigned long _v = max[j]; _p[_byte] &= ~(((1<<6)-1) << _fb); _p[_byte] |= _v << _fb; _p[_byte+1] &= ~(((1<<6)-1) >> _fb8); _p[_byte+1] |= _v >> _fb8; } while(0);
    }
    (hdr)->card[0] |= (1<<7);
