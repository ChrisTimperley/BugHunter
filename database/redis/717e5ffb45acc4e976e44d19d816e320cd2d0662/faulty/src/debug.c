        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (o->type == 0) {
                ...
                if (o->encoding == 5) {
                    ...
                    ((eptr != ((void *)0))?(void)0 : (_redisAssert("eptr != NULL","debug.c",178),_exit(1)));
                    sptr = ziplistNext(zl,eptr);
                    ((sptr != ((void *)0))?(void)0 : (_redisAssert("sptr != NULL","debug.c",180),_exit(1)));

                    while (eptr != ((void *)0)) {
                        ((ziplistGet(eptr,&vstr,&vlen,&vll))?(void)0 : (_redisAssert("ziplistGet(eptr,&vstr,&vlen,&vll)","debug.c",183),_exit(1)));
                    ...
                    _redisPanic("\"Unknown sorted set encoding\"","debug.c",216),_exit(1);
                }
            } else if (o->type == 4) {
                hashTypeIterator *hi;
                robj *obj;
                ...
                _redisPanic("\"Unknown object type\"","debug.c",237),_exit(1);
            }
...
void debugCommand(redisClient *c) {
    if (!strcasecmp(c->argv[1]->ptr,"segfault")) {
        *((char*)-1) = 'x';
        ...
        void *ptr = zmalloc((9223372036854775807L * 2UL + 1UL));
        zfree(ptr);
        addReply(c,shared.ok);
    } else if (!strcasecmp(c->argv[1]->ptr,"assert")) {
        if (c->argc >= 3) c->argv[2] = tryObjectEncoding(c->argv[2]);
        ((1 == 2)?(void)0 : (_redisAssertWithInfo(c,c->argv[0],"1 == 2","debug.c",258),_exit(1)));
        ...
        for (j = 0; j < keys; j++) {
            ...
            if (lookupKeyRead(c->db,key) != ((void *)0)) {
                decrRefCount(key);
                continue;
            }
...
void _redisAssert(char *estr, char *file, int line) {
    ...
    redisLog(3,"(forcing SIGSEGV to print the bug report.)");
...
void _redisAssertPrintClientInfo(redisClient *c) {
    ...
    bugReportStart();
    redisLog(3,"=== ASSERTION FAILED CLIENT CONTEXT ===");
    redisLog(3,"client->flags = %d", c->flags);
    redisLog(3,"client->fd = %d", c->fd);
    redisLog(3,"client->argc = %d", c->argc);
    for (j=0; j < c->argc; j++) {
        char buf[128];
        char *arg;
        ...
        redisLog(3,"client->argv[%d] = \"%s\" (refcount: %d)",
            j, arg, c->argv[j]->refcount);
    }
}

void redisLogObjectDebugInfo(robj *o) {
    redisLog(3,"Object type: %d", o->type);
    redisLog(3,"Object encoding: %d", o->encoding);
    redisLog(3,"Object refcount: %d", o->refcount);
    if (o->type == 0 && o->encoding == 0) {
        redisLog(3,"Object raw string len: %d", sdslen(o->ptr));
        if (sdslen(o->ptr) < 4096)
            redisLog(3,"Object raw string content: \"%s\"", (char*)o->ptr);
    } else if (o->type == 1) {
        redisLog(3,"List length: %d", (int) listTypeLength(o));
    } else if (o->type == 2) {
        redisLog(3,"Set size: %d", (int) setTypeSize(o));
    } else if (o->type == 4) {
        redisLog(3,"Hash size: %d", (int) hashTypeLength(o));
    } else if (o->type == 3) {
        redisLog(3,"Sorted set size: %d", (int) zsetLength(o));
        if (o->encoding == 7)
            redisLog(3,"Skiplist level: %d", (int) ((zset*)o->ptr)->zsl->level);
    }
}

void _redisAssertPrintObject(robj *o) {
    bugReportStart();
    redisLog(3,"=== ASSERTION FAILED OBJECT CONTEXT ===");
...
void _redisPanic(char *msg, char *file, int line) {
    ...
    redisLog(3,"(forcing SIGSEGV in order to print the stack trace)");
...
void logStackContent(void **sp) {
    ...
    for (i = 15; i >= 0; i--) {
        if (sizeof(long) == 4)
            redisLog(3, "(%08lx) -> %08lx", sp+i, sp[i]);
        else
            redisLog(3, "(%016lx) -> %016lx", sp+i, sp[i]);
    }
}

void logRegisters(ucontext_t *uc) {
    redisLog(3, "--- REGISTERS");
# 571 "debug.c"
    redisLog(3,
    "\n"
    "RAX:%016lx RBX:%016lx\nRCX:%016lx RDX:%016lx\n"
...
void logCurrentClient(void) {
    ...
    redisLog(3, "--- CURRENT CLIENT INFO");
    client = getClientInfoString(cc);
    redisLog(3,"client: %s", client);
    sdsfree(client);
    for (j = 0; j < cc->argc; j++) {
        robj *decoded;

        decoded = getDecodedObject(cc->argv[j]);
        redisLog(3,"argv[%d]: '%s'", j, (char*)decoded->ptr);
    ...
    if (cc->argc >= 1) {
        ...
        if (de) {
            val = ((de)->v.val);
            redisLog(3,"key '%s' found in DB containing the following object:", key->ptr);
...
void sigsegvHandler(int sig, siginfo_t *info, void *secret) {
    ...
    bugReportStart();
    redisLog(3,
        "    Redis %s crashed by signal: %d", "2.9.7", sig);
    redisLog(3,
...
void redisLogHexDump(int level, char *descr, void *value, size_t len) {
    ...
    while(len) {
        ...
        v++;
        if (b-buf == 64 || len == 0) {
            redisLogRaw(level|(1<<10),buf);
            b = buf;
        }
    }
    redisLogRaw(level|(1<<10),"\n");
}
...
void enableWatchdog(int period) {
    ...
    if (server.watchdog_period == 0) {
        ...
        sigemptyset(&act.sa_mask);
        act.sa_flags = 0x40000000 | 0x08000000 | 4;
