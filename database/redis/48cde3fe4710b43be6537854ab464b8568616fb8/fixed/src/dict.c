...
int dictRehash(dict *d, int n) {
    ...
    while(n--) {
        ...
        ((d->ht[0].size > (unsigned)d->rehashidx) ? (void) (0) : __assert_fail ("d->ht[0].size > (unsigned)d->rehashidx", "dict.c", 260, __PRETTY_FUNCTION__));
...
void dictReleaseIterator(dictIterator *iter)
{
    if (!(iter->index == -1 && iter->table == 0)) {
        if (iter->safe)
            ...
            ((iter->fingerprint == dictFingerprint(iter->d)) ? (void) (0) : __assert_fail ("iter->fingerprint == dictFingerprint(iter->d)", "dict.c", 587, __PRETTY_FUNCTION__));
    }
    zfree(iter);
}
