...
int clusterAddSlot(clusterNode *n, int slot) {
    ((clusterNodeSetSlotBit(n,slot) == 0)?(void)0 : (_redisAssert("clusterNodeSetSlotBit(n,slot) == 0","cluster.c",1017),_exit(1)));
...
void clusterCommand(redisClient *c) {
    ...
    if (!strcasecmp(c->argv[1]->ptr,"meet") && c->argc == 4) {
        ...
        for (j = 2; j < c->argc; j++) {
            if (getLongLongFromObject(c->argv[j],&slot) != 0 ||
                slot < 0 || slot > 4096)
            {
        ...
        for (j = 0; j < 4096; j++) {
            if (slots[j]) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","cluster.c",1200),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
        ...
        if (getLongLongFromObjectOrReply(c,c->argv[2],&aux,((void *)0)) != 0)
            return;
        if (aux < 0 || aux >= 4096) {
...
void restoreCommand(redisClient *c) {
    ...
    fp = fopen(buf,"w+");
    if (!fp) {
        redisLog(3,"Can't open tmp file for RESTORE: %s",
            strerror((*__errno_location ())));
    ...
    if (fwrite(data+1,sdslen((sds)data)-1,1,fp) != 1) {
        redisLog(3,"Can't write against tmp file for RESTORE: %s",
            strerror((*__errno_location ())));
    ...
    if ((data[0] > 4 && data[0] < 9) ||
         data[0] > 11 ||
        (o = rdbLoadObject(data[0],fp)) == ((void *)0))
    {
...
void migrateCommand(redisClient *c) {
    ...
    fp = fopen(buf,"w+");
    if (!fp) {
        redisLog(3,"Can't open tmp file for MIGRATE: %s",
            strerror((*__errno_location ())));
...
file_wr_err:
    redisLog(3,"Can't write on tmp file for MIGRATE: %s",
        strerror((*__errno_location ())));
...
file_rd_err:
    redisLog(3,"Can't read from tmp file for MIGRATE: %s",
        strerror((*__errno_location ())));
...
void dumpCommand(redisClient *c) {
    ...
    if (fseeko(fp,0,0) == -1) goto file_rd_err;
    dump = sdsnewlen(((void *)0),payload_len+1);
    if (payload_len && fread(dump+1,payload_len,1,fp) != 1) goto file_rd_err;
    fclose(fp);
    type = o->type;
    if (type == 1 && o->encoding == 5)
        type = 10;
    else if (type == 4 && o->encoding == 3)
        type = 9;
    else if (type == 2 && o->encoding == 6)
        type = 11;
    else
        type = o->type;
    dump[0] = type;
    ...
    dumpobj = createObject(0,dump);
    addReplyBulk(c,dumpobj);
    decrRefCount(dumpobj);
    return;

file_wr_err:
    redisLog(3,"Can't write on tmp file for DUMP: %s",
        strerror((*__errno_location ())));
...
file_rd_err:
    redisLog(3,"Can't read from tmp file for DUMP: %s",
        strerror((*__errno_location ())));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssert("n != NULL","cluster.c",1663),_exit(1)));
            } else {
