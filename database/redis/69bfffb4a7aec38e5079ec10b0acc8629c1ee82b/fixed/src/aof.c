...
void stopAppendOnly(void) {
    ...
    if (server.bgsavechildpid != -1) {
        ...
        if (kill(server.bgsavechildpid,9) != -1)
...
int loadAppendOnlyFile(char *filename) {
    ...
    while(1) {
        ...
        ((fakeClient->bufpos == 0 && ((fakeClient->reply)->len) == 0)?(void)0 : (_redisAssert("fakeClient->bufpos == 0 && listLength(fakeClient->reply) == 0","aof.c",283),_exit(1)));
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (o->type == 0) {
                ...
                if (o->encoding == 5) {
                    ...
                    _redisPanic("\"Unknown list encoding\"","aof.c",401),_exit(1);
                }
            } else if (o->type == 2) {
                char cmd[]="*3\r\n$4\r\nSADD\r\n";
                ...
                if (o->encoding == 6) {
                    ...
                    _redisPanic("\"Unknown set encoding\"","aof.c",426),_exit(1);
                }
            } else if (o->type == 3) {
                ...
                _redisPanic("\"Unknown object type\"","aof.c",478),_exit(1);
            }
...
int rewriteAppendOnlyFileBackground(void) {
    ...
    if (server.ds_enabled != 0) {
        redisLog(3,"BGREWRITEAOF called with diskstore enabled: AOF is not supported when diskstore is enabled. Operation not performed.");
        return -1;
    }
    if ((childpid = fork()) == 0) {
...
void bgrewriteaofCommand(redisClient *c) {
    ...
    if (rewriteAppendOnlyFileBackground() == 0) {
...
void backgroundRewriteDoneHandler(int exitcode, int bysignal) {
    if (!bysignal && exitcode == 0) {
        ...
        fd = open(tmpfile,01|02000);
        if (fd == -1) {
            redisLog(3, "Not able to open the temp append only file produced by the child: %s", strerror((*__errno_location ())));
            goto cleanup;
        }

        if (write(fd,server.bgrewritebuf,sdslen(server.bgrewritebuf)) !=
                (signed) sdslen(server.bgrewritebuf)) {
            redisLog(3, "Error or short write trying to flush the parent diff of the append log file in the child temp file: %s", strerror((*__errno_location ())));
            close(fd);
            goto cleanup;
        }
        redisLog(2,"Parent diff flushed into the new append log file with success (%lu bytes)",sdslen(server.bgrewritebuf));


        if (rename(tmpfile,server.appendfilename) == -1) {
            redisLog(3,"Can't rename the temp append only file into the stable one: %s", strerror((*__errno_location ())));
            close(fd);
            goto cleanup;
        }
        ...
        if (server.appendfd != -1) {
            ...
            close(server.appendfd);
            server.appendfd = fd;
            if (server.appendfsync != 0) fdatasync(fd);
            server.appendseldb = -1;
