...
# 1 "../deps/lua/src/lauxlib.h" 1
# 12 "../deps/lua/src/lauxlib.h"
# 1 "/usr/lib/gcc/x86_64-linux-gnu/4.8/include/stddef.h" 1 3 4
# 13 "../deps/lua/src/lauxlib.h" 2


# 1 "../deps/lua/src/lua.h" 1
# 16 "../deps/lua/src/lauxlib.h" 2
# 35 "../deps/lua/src/lauxlib.h"
...
# 36 "scripting.c" 2
# 1 "../deps/lua/src/lualib.h" 1
# 19 "../deps/lua/src/lualib.h"
...
char *redisProtocolToLuaType_Int(lua_State *lua, char *reply) {
    char *p = (__extension__ (__builtin_constant_p ('\r') && !__builtin_constant_p (reply+1) && ('\r') == '\0' ? (char *) __rawmemchr (reply+1, '\r') : __builtin_strchr (reply+1, '\r')));
    long long value;
...
int luaRedisGenericCommand(lua_State *lua, int raise_error) {
    ...
    for (j = 0; j < argc; j++) {
        ...
        argv[j] = createStringObject((char*)lua_tolstring(lua, (j+1), ((void *)0)),
                                     lua_objlen(lua, (j+1)));
    }
    ...
    if (server.maxmemory && server.lua_write_dirty == 0 &&
        (cmd->flags & 4))
    {
        if (freeMemoryIfNeeded() == -1) {
            luaPushError(lua, shared.oomerr->ptr);
            goto cleanup;
        }
    }

    if (cmd->flags & 128) server.lua_random_dirty = 1;
    if (cmd->flags & 1) server.lua_write_dirty = 1;
...
void luaMaskCountHook(lua_State *lua, lua_Debug *ar) {
    ...
    if (elapsed >= server.lua_time_limit && server.lua_timedout == 0) {
        redisLog(3,"Lua slow script detected: still in execution after %lld milliseconds. You can try killing the script using the SCRIPT KILL command.",elapsed);
        server.lua_timedout = 1;
         ...
         aeDeleteFileEvent(server.el, server.lua_caller->fd, 1);
    }
    if (server.lua_timedout)
        aeProcessEvents(server.el, 1|4);
    if (server.lua_kill) {
        redisLog(3,"Lua script killed by user with SCRIPT KILL.");
...
void scriptingInit(void) {
    ...
    server.lua_scripts = dictCreate(&dbDictType,((void *)0));
...
int luaCreateFunction(redisClient *c, lua_State *lua, char *funcname, robj *body) {
...

    {
        ...
        ((retval == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"retval == DICT_OK","scripting.c",780),_exit(1)));
        incrRefCount(body);
    }
    return 0;
}
...
# 804 "scripting.c"
    ...
    if (!evalsha) {
        ...
        for (j = 0; j < 40; j++)
            funcname[j+2] = (__extension__ ({ int __res; if (sizeof (sha[j]) > 1) { if (__builtin_constant_p (sha[j])) { int __c = (sha[j]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower (sha[j]); } else __res = (*__ctype_tolower_loc ())[(int) (sha[j])]; __res; }));
        funcname[42] = '\0';
    }
    ...
    if ((lua_type(lua, (1)) == 0)) {
        ...
        if (luaCreateFunction(c,lua,funcname,c->argv[1]) == -1) return;

        lua_getfield(lua, (-10002), (funcname));
        ((!(lua_type(lua, (1)) == 0))?(void)0 : (_redisAssert("!lua_isnil(lua,1)","scripting.c",846),_exit(1)));
    }
    ...
    server.lua_kill = 0;
    if (server.lua_time_limit > 0 && server.masterhost == ((void *)0)) {
    ...
    if (lua_pcall(lua,0,1,0)) {
        if (delhook) lua_sethook(lua,luaMaskCountHook,0,0);
        if (server.lua_timedout) {
...
# 906 "scripting.c"
    if (evalsha) {
        ...
        ((script != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"script != NULL","scripting.c",909),_exit(1)));
...
void scriptCommand(redisClient *c) {
    if (c->argc == 2 && !strcasecmp(c->argv[1]->ptr,"flush")) {
        ...
        if (dictFind(server.lua_scripts,sha) == ((void *)0)) {
