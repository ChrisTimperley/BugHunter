...
void migrateCommand(redisClient *c) {
    ...
    ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",1623),_exit(1)));
    ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",1624),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,dbid)","cluster.c",1625),_exit(1)));

    ttl = getExpire(c->db,c->argv[3]);
    ((rioWriteBulkCount(&cmd,'*',4))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',4)","cluster.c",1628),_exit(1)));
    ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",1629),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argv[3]->encoding == REDIS_ENCODING_RAW","cluster.c",1630),_exit(1)));
    ((rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr))","cluster.c",1631),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl)","cluster.c",1632),_exit(1)));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,firstkey,"n != NULL","cluster.c",1774),_exit(1)));
            } else {
