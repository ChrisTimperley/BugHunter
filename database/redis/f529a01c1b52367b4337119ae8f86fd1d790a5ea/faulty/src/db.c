...
robj *lookupKey(redisDb *db, robj *key) {
    ...
    if (de) {
        ...
        if (server.rdb_child_pid == -1 && server.aof_child_pid == -1)
            val->lru = ((1000/server.hz <= 1000) ? server.lruclock : getLRUClock());
        return val;
    } else {
        return ((void *)0);
    }
...
robj *lookupKeyRead(redisDb *db, robj *key) {
    ...
    if (expireIfNeeded(db,key) == 1) {
        ...
        if (server.masterhost == ((void *)0)) return ((void *)0);
# 81 "db.c"
        if (server.current_client &&
            server.current_client != server.master &&
            server.current_client->cmd &&
            server.current_client->cmd->flags & 2)
        {
            return ((void *)0);
        }
    }
...
void dbAdd(redisDb *db, robj *key, robj *val) {
    ...
    ((retval == 0)?(void)0 : (_serverAssertWithInfo(((void *)0),key,"retval == C_OK","db.c",122),_exit(1)));
    if (val->type == 1) signalListAsReady(db, key);
...
void dbOverwrite(redisDb *db, robj *key, robj *val) {
    ...
    ((de != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"de != NULL","db.c",135),_exit(1)));
...
# 226 "db.c"
robj *dbUnshareStringValue(redisDb *db, robj *key, robj *o) {
    ((o->type == 0)?(void)0 : (_serverAssert("o->type == OBJ_STRING","db.c",227),_exit(1)));
...
void flushdbCommand(client *c) {
    server.dirty += ((c->db->dict)->ht[0].used+(c->db->dict)->ht[1].used);
    signalFlushedDb(c->db->id);
    dictEmpty(c->db->dict,((void *)0));
    dictEmpty(c->db->expires,((void *)0));
    if (server.cluster_enabled) slotToKeyFlush();
...
void scanCallback(void *privdata, const dictEntry *de) {
    ...
    if (o == ((void *)0)) {
        ...
        _serverPanic("\"Type not handled in SCAN callback.\"","db.c",415),_exit(1);
    }
...
int parseScanCursorOrReply(client *c, robj *o, unsigned long *cursor) {
    ...
    *cursor = strtoul(o->ptr, &eptr, 10);
    if (((*__ctype_b_loc ())[(int) ((((char*)o->ptr)[0]))] & (unsigned short int) _ISspace) || eptr[0] != '\0' || (*__errno_location ()) == 34)
    {
...
void scanGenericCommand(client *c, robj *o, unsigned long cursor) {
    ...
    ((o == ((void *)0) || o->type == 2 || o->type == 4 || o->type == 3)?(void)0 : (_serverAssert("o == NULL || o->type == OBJ_SET || o->type == OBJ_HASH || o->type == OBJ_ZSET",
 "db.c"
# 463 "db.c"
    ,
 464
...
# 509 "db.c"
    ...
    if (ht) {
        ...
        _serverPanic("\"Not handled encoding in SCAN.\"","db.c",563),_exit(1);
    }
    ...
    while (node) {
        ...
        if (!filter && use_pattern) {
            if ((kobj->encoding == 0 || kobj->encoding == 8)) {
                ...
                ((kobj->encoding == 1)?(void)0 : (_serverAssert("kobj->encoding == OBJ_ENCODING_INT","db.c",582),_exit(1)));
    ...
    addReplyMultiBulkLen(c, ((keys)->len));
...
void moveCommand(client *c) {
    ...
    if (getLongLongFromObject(c->argv[2],&dbid) == -1 ||
        dbid < (-2147483647 - 1) || dbid > 2147483647 ||
        selectDb(c,dbid) == -1)
    {
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",802),_exit(1)));
...
void setExpire(redisDb *db, robj *key, long long when) {
    ...
    ((kde != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"kde != NULL","db.c",811),_exit(1)));
    de = dictReplaceRaw(db->expires,((kde)->key));
...
long long getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_serverAssertWithInfo(((void *)0),key,"dictFind(db->dict,key->ptr) != NULL","db.c",827),_exit(1)));
    return ((de)->v.s64);
}
# 839 "db.c"
void propagateExpire(redisDb *db, robj *key) {
...
# 878 "db.c"
    if (server.masterhost != ((void *)0)) return now > when;
...
void expireGenericCommand(client *c, long long basetime, int unit) {
    ...
    if (when <= mstime() && !server.loading && !server.masterhost) {
        ...
        ((dbDelete(c->db,key))?(void)0 : (_serverAssertWithInfo(c,key,"dbDelete(c->db,key)","db.c",927),_exit(1)));
        server.dirty++;
...
void ttlGenericCommand(client *c, int output_ms) {
    ...
    if (lookupKeyRead(c->db,c->argv[1]) == ((void *)0)) {
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_serverAssert("j < argc","db.c",1028),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
