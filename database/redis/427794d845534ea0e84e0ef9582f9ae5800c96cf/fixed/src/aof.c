    ...
    while((ln = listNext(&li))) {
        ...
        if (block->used) {
            ...
            if (nwritten != (ssize_t)block->used) {
                if (nwritten == 0) (*__errno_location ()) = 5;
                return -1;
            }
            count += nwritten;
        }
...
void stopAppendOnly(void) {
    ((server.aof_state != 0)?(void)0 : (_redisAssert("server.aof_state != REDIS_AOF_OFF","aof.c",207),_exit(1)));
    ...
    if (server.aof_child_pid != -1) {
        ...
        redisLog(2,"Killing running AOF rewrite child: %ld",
...
int startAppendOnly(void) {
    ...
    server.aof_fd = open(server.aof_filename,01|02000|0100,0644);
    ((server.aof_state == 0)?(void)0 : (_redisAssert("server.aof_state == REDIS_AOF_OFF","aof.c",238),_exit(1)));
    if (server.aof_fd == -1) {
        redisLog(3,"Redis needs to enable the AOF but can't open the append only file: %s",strerror((*__errno_location ())));
        return -1;
    }
    if (rewriteAppendOnlyFileBackground() == -1) {
        close(server.aof_fd);
        redisLog(3,"Redis needs to enable the AOF but can't trigger a background AOF rewrite operation. Check the above logs for more info about the error.");
        return -1;
    }
...
void flushAppendOnlyFile(int force) {
    ...
    if (server.fdatasync == 2)
        sync_in_progress = bioPendingJobsOfType(1) != 0;

    if (server.fdatasync == 2 && !force) {
        ...
        if (sync_in_progress) {
            ...
            redisLog(2,"Asynchronous AOF fsync is taking too long (disk is busy?). Writing the AOF buffer without waiting for fsync to complete, this may slow down Redis.");
        }
    }
    ...
    if (nwritten != (signed)sdslen(server.aof_buf)) {
        ...
        if (nwritten == -1) {
            if (can_log) {
                redisLog(3,"Error writing to the AOF file: %s",
                    strerror((*__errno_location ())));
                server.aof_last_write_errno = (*__errno_location ());
            }
        } else {
            if (can_log) {
                redisLog(3,"Short write while writing to "
            ...
            if (ftruncate(server.aof_fd, server.aof_current_size) == -1) {
                if (can_log) {
                    redisLog(3, "Could not remove short write "
                             "from the append-only file.  Redis may refuse "
                             "to load the AOF the next time it starts.  "
                             "ftruncate: %s", strerror((*__errno_location ())));
                }
            } else {
        ...
        if (server.fdatasync == 1) {




            redisLog(3,"Can't recover from AOF write error when the AOF fsync policy is 'always'. Exiting...");
            exit(1);
        } else {
...
int loadAppendOnlyFile(char *filename) {
    ...
    if (fp == ((void *)0)) {
        redisLog(3,"Fatal error: can't open the append log file for reading: %s",strerror((*__errno_location ())));
        exit(1);
    }
    ...
    while(1) {
        ...
        if (!cmd) {
            redisLog(3,"Unknown command '%s' reading the append only file", (char*)argv[0]->ptr);
            exit(1);
        }
        ...
        ((fakeClient->bufpos == 0 && ((fakeClient->reply)->len) == 0)?(void)0 : (_redisAssert("fakeClient->bufpos == 0 && listLength(fakeClient->reply) == 0","aof.c",688),_exit(1)));

        (((fakeClient->flags & (1<<4)) == 0)?(void)0 : (_redisAssert("(fakeClient->flags & REDIS_BLOCKED) == 0","aof.c",690),_exit(1)));
    ...
    if (fakeClient->flags & (1<<3)) goto uxeof;
...
readerr:
    if (!feof(fp)) {
        redisLog(3,"Unrecoverable error reading the append only file: %s", strerror((*__errno_location ())));
        exit(1);
    }

uxeof:
    if (server.aof_load_truncated) {
        redisLog(3,"!!! Warning: short read while loading the AOF file !!!");
        redisLog(3,"!!! Truncating the AOF at offset %llu !!!",
            (unsigned long long) valid_up_to);
        if (valid_up_to == -1 || truncate(filename,valid_up_to) == -1) {
            if (valid_up_to == -1) {
                redisLog(3,"Last valid command offset is invalid");
            } else {
                redisLog(3,"Error truncating the AOF file: %s",
                    strerror((*__errno_location ())));
            }
        } else {
            ...
            if (server.aof_fd != -1 && lseek(server.aof_fd,0,2) == -1) {
                redisLog(3,"Can't seek the end of the AOF file: %s",
                    strerror((*__errno_location ())));
            } else {
                redisLog(3,
    ...
    redisLog(3,"Unexpected end of file reading the append only file. You can: 1) Make a backup of your AOF file, then use ./redis-check-aof --fix <filename>. 2) Alternatively you can set the 'aof-load-truncated' configuration option to yes and restart the server.");
    exit(1);
...
int rioWriteBulkObject(rio *r, robj *obj) {
    ...
    if (obj->encoding == 1) {
        ...
        _redisPanic("\"Unknown string encoding\"","aof.c",764),_exit(1);
    }
}
...
int rewriteListObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 9) {
        ...
        _redisPanic("\"Unknown list encoding\"","aof.c",797),_exit(1);
    }
    return 1;
}
...
int rewriteSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 6) {
        ...
        _redisPanic("\"Unknown set encoding\"","aof.c",844),_exit(1);
    }
    return 1;
}
...
int rewriteSortedSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 5) {
        ...
        ((eptr != ((void *)0))?(void)0 : (_redisAssert("eptr != NULL","aof.c",863),_exit(1)));
        sptr = ziplistNext(zl,eptr);
        ((sptr != ((void *)0))?(void)0 : (_redisAssert("sptr != NULL","aof.c",865),_exit(1)));

        while (eptr != ((void *)0)) {
            ((ziplistGet(eptr,&vstr,&vlen,&vll))?(void)0 : (_redisAssert("ziplistGet(eptr,&vstr,&vlen,&vll)","aof.c",868),_exit(1)));
        ...
        _redisPanic("\"Unknown sorted zset encoding\"","aof.c",913),_exit(1);
    }
    return 1;
}
...
static int rioWriteHashIteratorCursor(rio *r, hashTypeIterator *hi, int what) {
    if (hi->encoding == 5) {
        ...
        unsigned int vlen = (2147483647 * 2U + 1U);
        long long vll = 9223372036854775807LL;
    ...
    _redisPanic("\"Unknown hash encoding\"","aof.c",944),_exit(1);
    return 0;
}
...
int rewriteAppendOnlyFile(char *filename) {
    ...
    fp = fopen(tmpfile,"w");
    if (!fp) {
        redisLog(3, "Opening the temp file for AOF rewrite in rewriteAppendOnlyFile(): %s", strerror((*__errno_location ())));
        return -1;
    }
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (o->type == 0) {
                ...
                _redisPanic("\"Unknown object type\"","aof.c",1069),_exit(1);
            }
    ...
    if (syncRead(server.aof_pipe_read_ack_from_parent,&byte,1,5000) != 1 ||
        byte != '!') goto werr;
    redisLog(2,"Parent agreed to stop sending diffs. Finalizing AOF...");
    ...
    if (rename(tmpfile,filename) == -1) {
        redisLog(3,"Error moving temp append only file on the final destination: %s", strerror((*__errno_location ())));
        unlink(tmpfile);
        return -1;
    }
...
werr:
    redisLog(3,"Write error writing append only file on disk: %s", strerror((*__errno_location ())));
    fclose(fp);
    unlink(tmpfile);
...
void aofChildPipeReadable(aeEventLoop *el, int fd, void *privdata, int mask) {
    ...
    if (read(fd,&byte,1) == 1 && byte == '!') {
        redisLog(2,"AOF rewrite child asks to stop sending diffs.");
        ...
        if (write(server.aof_pipe_write_ack_to_child,"!",1) != 1) {
            ...
            redisLog(3,"Can't send ACK to AOF child: %s",
                strerror((*__errno_location ())));
        }
    }
...
error:
    redisLog(3,"Error opening /setting AOF rewrite IPC pipes: %s",
        strerror((*__errno_location ())));
...
int rewriteAppendOnlyFileBackground(void) {
    ...
    if (server.aof_child_pid != -1) return -1;
...
void aofUpdateCurrentSize(void) {
    ...
    if (fstat(server.aof_fd,&sb) == -1) {
        redisLog(3,"Unable to obtain the AOF file length. stat: %s",
            strerror((*__errno_location ())));
    } else {
...
void backgroundRewriteDoneHandler(int exitcode, int bysignal) {
    if (!bysignal && exitcode == 0) {
        ...
        if (newfd == -1) {
            redisLog(3,
                "Unable to open the temporary AOF produced by the child: %s", strerror((*__errno_location ())));
            goto cleanup;
        }

        if (aofRewriteBufferWrite(newfd) == -1) {
            redisLog(3,
                "Error trying to flush the parent diff to the rewritten AOF: %s", strerror((*__errno_location ())));
            close(newfd);
            goto cleanup;
        }
        if (server.latency_monitor_threshold) { latency = mstime() - latency; };
...
# 1402 "aof.c"
        ...
        if (rename(tmpfile,server.aof_filename) == -1) {
            redisLog(3,
                "Error trying to rename the temporary AOF file: %s", strerror((*__errno_location ())));
            close(newfd);
        ...
        redisLog(2, "Background AOF rewrite finished successfully");
