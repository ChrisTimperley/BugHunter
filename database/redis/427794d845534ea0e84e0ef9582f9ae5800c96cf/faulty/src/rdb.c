...
void rdbCheckThenExit(char *reason, int where) {
    redisLog(3, "Corrupt RDB detected at rdb.c:%d (%s). "
...
uint32_t rdbLoadLen(rio *rdb, int *isencoded) {
    ...
    if (rioRead(rdb,buf,1) == 0) return (2147483647 * 2U + 1U);
    type = (buf[0]&0xC0)>>6;
    if (type == 3) {
        ...
        if (rioRead(rdb,buf+1,1) == 0) return (2147483647 * 2U + 1U);
        return ((buf[0]&0x3F)<<8)|buf[1];
    } else {

        if (rioRead(rdb,&len,4) == 0) return (2147483647 * 2U + 1U);
        return __bswap_32 (len);
    }
}
...
void *rdbLoadIntegerObject(rio *rdb, int enctype, int flags) {
    ...
    if (enctype == 0) {
        ...
        val = 0;
        rdbCheckThenExit("Unknown RDB integer encoding type", 201);;
    }
    if (plain) {
        char buf[21], *p;
...
void *rdbLoadLzfStringObject(rio *rdb, int flags) {
    ...
    if ((clen = rdbLoadLen(rdb,((void *)0))) == (2147483647 * 2U + 1U)) return ((void *)0);
    if ((len = rdbLoadLen(rdb,((void *)0))) == (2147483647 * 2U + 1U)) return ((void *)0);
...
ssize_t rdbSaveLongLongAsStringObject(rio *rdb, long long value) {
    ...
    if (enclen > 0) {
        ...
        ((enclen < 32)?(void)0 : (_redisAssert("enclen < 32","rdb.c",360),_exit(1)));
...
int rdbSaveStringObject(rio *rdb, robj *obj) {
    ...
    if (obj->encoding == 1) {
        ...
        (((obj->encoding == 0 || obj->encoding == 8))?(void)0 : (_redisAssertWithInfo(((void *)0),obj,"sdsEncodedObject(obj)","rdb.c",376),_exit(1)));
...
void *rdbGenericLoadStringObject(rio *rdb, int flags) {
    ...
    if (isencoded) {
        ...
        default:
            rdbCheckThenExit("Unknown RDB encoding type", 407);;
        }
    }

    if (len == (2147483647 * 2U + 1U)) return ((void *)0);
    if (!plain) {
        robj *o = encode ? createStringObject(((void *)0),len) :
                           createRawStringObject(((void *)0),len);
...
robj *rdbLoadEncodedStringObject(rio *rdb) {
    return rdbGenericLoadStringObject(rdb,(1<<0));
}
# 446 "rdb.c"
int rdbSaveDoubleValue(rio *rdb, double val) {
    ...
    if ((sizeof (val) == sizeof (float) ? __isnanf (val) : sizeof (val) == sizeof (double) ? __isnan (val) : __isnanl (val))) {
        buf[0] = 253;
        len = 1;
    } else if (!(sizeof (val) == sizeof (float) ? __finitef (val) : sizeof (val) == sizeof (double) ? __finite (val) : __finitel (val))) {
        len = 1;
        buf[0] = (val < 0) ? 255 : 254;
    } else {
# 473 "rdb.c"
...
int rdbSaveObjectType(rio *rdb, robj *o) {
    ...
    case 1:
        if (o->encoding == 9)
            ...
            _redisPanic("\"Unknown list encoding\"","rdb.c",507),_exit(1);
    case 2:
        if (o->encoding == 6)
            ...
            _redisPanic("\"Unknown set encoding\"","rdb.c",514),_exit(1);
    case 3:
        if (o->encoding == 5)
            return rdbSaveType(rdb,12);
        else if (o->encoding == 7)
            return rdbSaveType(rdb,3);
        else
            _redisPanic("\"Unknown sorted set encoding\"","rdb.c",521),_exit(1);
    case 4:
        if (o->encoding == 5)
            ...
            _redisPanic("\"Unknown hash encoding\"","rdb.c",528),_exit(1);
    default:
        _redisPanic("\"Unknown object type\"","rdb.c",530),_exit(1);
    }
    return -1;
}
...
int rdbLoadObjectType(rio *rdb) {
    ...
    if (!((type >= 0 && type <= 4) || (type >= 9 && type <= 14))) return -1;
    return type;
}
...
ssize_t rdbSaveObject(rio *rdb, robj *o) {
    ...
    if (o->type == 0) {
        ...
        if (o->encoding == 9) {
            ...
            _redisPanic("\"Unknown list encoding\"","rdb.c",573),_exit(1);
        }
    } else if (o->type == 2) {
        ...
        if (o->encoding == 2) {
            ...
            if ((n = rdbSaveLen(rdb,((set)->ht[0].used+(set)->ht[1].used))) == -1) return -1;
            nwritten += n;
            ...
            _redisPanic("\"Unknown set encoding\"","rdb.c",597),_exit(1);
        }
    } else if (o->type == 3) {
        ...
        if (o->encoding == 5) {
            ...
            if ((n = rdbSaveLen(rdb,((zs->dict)->ht[0].used+(zs->dict)->ht[1].used))) == -1) return -1;
            nwritten += n;
            ...
            _redisPanic("\"Unknown sorted set encoding\"","rdb.c",625),_exit(1);
        }
    } else if (o->type == 4) {
        ...
        if (o->encoding == 5) {
            ...
            if ((n = rdbSaveLen(rdb,(((dict*)o->ptr)->ht[0].used+((dict*)o->ptr)->ht[1].used))) == -1) return -1;
            nwritten += n;
            ...
            _redisPanic("\"Unknown hash encoding\"","rdb.c",654),_exit(1);
        }

    } else {
        _redisPanic("\"Unknown object type\"","rdb.c",658),_exit(1);
    }
    return nwritten;
}
...
size_t rdbSavedObjectLen(robj *o) {
    ssize_t len = rdbSaveObject(((void *)0),o);
    ((len != -1)?(void)0 : (_redisAssertWithInfo(((void *)0),o,"len != -1","rdb.c",669),_exit(1)));
    return len;
}
...
int rdbSaveAuxField(rio *rdb, void *key, size_t keylen, void *val, size_t vallen) {
    if (rdbSaveType(rdb,250) == -1) return -1;
...
int rdbSaveInfoAuxFields(rio *rdb) {
    ...
    if (rdbSaveAuxFieldStrStr(rdb,"redis-ver","3.1.999") == -1) return -1;
...
int rdbSaveRio(rio *rdb, int *error) {
    ...
    snprintf(magic,sizeof(magic),"REDIS%04d",7);
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        db_size = (((db->dict)->ht[0].used+(db->dict)->ht[1].used) <= (4294967295U)) ?
                                ((db->dict)->ht[0].used+(db->dict)->ht[1].used) :
                                (4294967295U);
        expires_size = (((db->dict)->ht[0].used+(db->dict)->ht[1].used) <= (4294967295U)) ?
                                ((db->expires)->ht[0].used+(db->expires)->ht[1].used) :
                                (4294967295U);
...
werr:
    if (error) *error = (*__errno_location ());
...
int rdbSaveRioWithEOFMark(rio *rdb, int *error) {
    ...
    if (rioWrite(rdb,"\r\n",2) == 0) goto werr;
    if (rdbSaveRio(rdb,error) == -1) goto werr;
    if (rioWrite(rdb,eofmark,40) == 0) goto werr;
    return 0;

werr:

    if (error && *error == 0) *error = (*__errno_location ());
    return -1;
}
...
int rdbSave(char *filename) {
    ...
    fp = fopen(tmpfile,"w");
    if (!fp) {
        redisLog(3, "Failed opening .rdb for saving: %s",
            strerror((*__errno_location ())));
        return -1;
    }

    rioInitWithFile(&rdb,fp);
    if (rdbSaveRio(&rdb,&error) == -1) {
    ...
    if (rename(tmpfile,filename) == -1) {
        redisLog(3,"Error moving temp DB file on the final destination: %s", strerror((*__errno_location ())));
        unlink(tmpfile);
        return -1;
    }
...
werr:
    fclose(fp);
    unlink(tmpfile);
    redisLog(3,"Write error saving DB on disk: %s", strerror((*__errno_location ())));
    return -1;
}
...
int rdbSaveBackground(char *filename) {
    ...
    if (server.rdb_child_pid != -1) return -1;
    ...
    if ((childpid = fork()) == 0) {
        ...
        if (childpid == -1) {
            ...
            redisLog(3,"Can't save in background: fork: %s",
                strerror((*__errno_location ())));
            return -1;
        }
    ...
    if (rdbtype == 0) {
        ...
        for (i = 0; i < len; i++) {
            ...
            if (o->encoding == 6) {
                ...
                if (isObjectRepresentableAsLongLong(ele,&llval) == 0) {
        ...
        if ((zsetlen = rdbLoadLen(rdb,((void *)0))) == (2147483647 * 2U + 1U)) return ((void *)0);
        o = createZsetObject();
        zs = o->ptr;
        ...
        while(zsetlen--) {
            ...
            if ((ele->encoding == 0 || ele->encoding == 8) && sdslen(ele->ptr) > maxelelen)
        ...
        while (o->encoding == 5 && len > 0) {
            ...
            (((field->encoding == 0 || field->encoding == 8))?(void)0 : (_redisAssert("sdsEncodedObject(field)","rdb.c",1054),_exit(1)));
            value = rdbLoadStringObject(rdb);
            if (value == ((void *)0)) return ((void *)0);
            (((value->encoding == 0 || value->encoding == 8))?(void)0 : (_redisAssert("sdsEncodedObject(value)","rdb.c",1057),_exit(1)));


            o->ptr = ziplistPush(o->ptr, field->ptr, sdslen(field->ptr), 1);
            o->ptr = ziplistPush(o->ptr, value->ptr, sdslen(value->ptr), 1);
        ...
        while (o->encoding == 2 && len > 0) {
            ...
            if (ret == 1) {
                rdbCheckThenExit("Duplicate keys detected", 1092);;
            }
        }


        ((len == 0)?(void)0 : (_redisAssert("len == 0","rdb.c",1097),_exit(1)));
    } else if (rdbtype == 14) {
        if ((len = rdbLoadLen(rdb,((void *)0))) == (2147483647 * 2U + 1U)) return ((void *)0);
        o = createQuicklistObject();
        quicklistSetOptions(o->ptr, server.list_max_ziplist_size,
        ...
        while (len--) {
            unsigned char *zl = rdbGenericLoadStringObject(rdb,(1<<1));
            ...
            quicklistAppendZiplist(o->ptr, zl);
               ...
               rdbtype == 13)
    {
        unsigned char *encoded = rdbGenericLoadStringObject(rdb,(1<<1));
        ...
        switch(rdbtype) {
            ...
            case 13:
                ...
                break;
            default:
                rdbCheckThenExit("Unknown encoding", 1179);;
                break;
        }
    } else {
        rdbCheckThenExit("Unknown object type", 1183);;
    }
    return o;
}
...
int rdbLoad(char *filename) {
    ...
    while(1) {
        ...
        if (type == 253) {
            ...
            if ((dbid = rdbLoadLen(&rdb,((void *)0))) == (2147483647 * 2U + 1U))
                goto eoferr;
            ...
            uint32_t db_size, expires_size;
            if ((db_size = rdbLoadLen(&rdb,((void *)0))) == (2147483647 * 2U + 1U))
                goto eoferr;
            if ((expires_size = rdbLoadLen(&rdb,((void *)0))) == (2147483647 * 2U + 1U))
                goto eoferr;
            ...
            if (((char*)auxkey->ptr)[0] == '%') {
                ...
                redisLog(0,"Unrecognized RDB AUX field: '%s'",
                    (char*)auxkey->ptr);
            }
        ...
        if ((val = rdbLoadObject(type,&rdb)) == ((void *)0)) goto eoferr;





        if (server.masterhost == ((void *)0) && expiretime != -1 && expiretime < now) {
            decrRefCount(key);
            decrRefCount(val);
            continue;
        }
    ...
    if (rdbver >= 5 && server.rdb_checksum) {
        ...
        ;
        if (cksum == 0) {
            redisLog(3,"RDB file was saved with checksum disabled: no check performed.");
        } else if (cksum != expected) {
            redisLog(3,"Wrong RDB checksum. Aborting now.");
            rdbCheckThenExit("RDB CRC error", 1378);;
        }
    }
...
eoferr:
    ...
    rdbCheckThenExit("Unexpected EOF reading RDB file", 1388);;
    return -1;
}
...
void backgroundSaveDoneHandlerSocket(int exitcode, int bysignal) {
    ...
    if (!bysignal && exitcode == 0) {
        ...
        redisLog(3, "Background transfer error");
    } else {
        redisLog(3,
...
void backgroundSaveDoneHandler(int exitcode, int bysignal) {
    ...
    case 2:
        ...
        break;
    default:
        _redisPanic("\"Unknown RDB child type.\"","rdb.c",1532),_exit(1);
        break;
    }
}
...
int rdbSaveToSlavesSockets(void) {
    ...
    if ((childpid = fork()) == 0) {
        ...
        retval = rdbSaveRioWithEOFMark(&slave_sockets,((void *)0));
...
# 1624 "rdb.c"
        ...
        redisLog(2,"Background RDB transfer started by pid %d",childpid);
        server.rdb_save_time_start = time(((void *)0));
        server.rdb_child_pid = childpid;
...
void bgsaveCommand(redisClient *c) {
    ...
    } else if (rdbSaveBackground(server.rdb_filename) == 0) {
