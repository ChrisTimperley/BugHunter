...
void blockForKeys(redisClient *c, robj **keys, int numkeys, time_t timeout, robj *target) {
    ...
    for (j = 0; j < numkeys; j++) {
        ...
        if (de == ((void *)0)) {
            ...
            incrRefCount(keys[j]);
            ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","t_list.c",746),_exit(1)));
        } else {
            l = ((de)->val);
        }
        listAddNodeTail(l,c);
...
void unblockClientWaitingData(redisClient *c) {
    ...
    ((c->bpop.keys != ((void *)0))?(void)0 : (_redisAssert("c->bpop.keys != NULL","t_list.c",763),_exit(1)));

    for (j = 0; j < c->bpop.count; j++) {

        de = dictFind(c->db->blocking_keys,c->bpop.keys[j]);
        ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","t_list.c",768),_exit(1)));
        l = ((de)->val);
...
# 817 "t_list.c"
    while (numclients--) {
        ln = ((clients)->head);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","t_list.c",819),_exit(1)));
        receiver = ln->value;
...
void brpoplpushCommand(redisClient *c) {
    ...
    if (key == ((void *)0)) {
        ...
        if (key->type != 1) {
            ...
            ((listTypeLength(key) > 0)?(void)0 : (_redisAssert("listTypeLength(key) > 0","t_list.c",960),_exit(1)));
            rpoplpushCommand(c);
        }
    }
}
