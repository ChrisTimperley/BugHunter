...
int setTypeAdd(robj *subject, robj *value) {
    ...
    if (subject->encoding == 2) {
        ...
        if (isObjectRepresentableAsLongLong(value,&llval) == 0) {
            ...
            ((dictAdd(subject->ptr,value,((void *)0)) == 0)?(void)0 : (_redisAssert("dictAdd(subject->ptr,value,NULL) == DICT_OK","t_set.c",40),_exit(1)));
            incrRefCount(value);
            return 1;
        }
    } else {
        _redisPanic("\"Unknown set encoding\"","t_set.c",45),_exit(1);
    }
    return 0;
}
...
int setTypeRemove(robj *setobj, robj *value) {
    ...
    if (setobj->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",64),_exit(1);
    }
    return 0;
}
...
setTypeIterator *setTypeInitIterator(robj *subject) {
    ...
    if (si->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",92),_exit(1);
    }
    return si;
}
...
robj *setTypeNextObject(setTypeIterator *si) {
    ...
    switch(encoding) {
        ...
        case 2:
            ...
            return objele;
        default:
            _redisPanic("\"Unsupported encoding\"","t_set.c",147),_exit(1);
    }
    return ((void *)0);
}
# 165 "t_set.c"
int setTypeRandomElement(robj *setobj, robj **objele, int64_t *llele) {
    if (setobj->encoding == 2) {
        ...
        *llele = intsetRandom(setobj->ptr);
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",172),_exit(1);
    }
...
unsigned long setTypeSize(robj *subject) {
    if (subject->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",183),_exit(1);
    }
}
...
void setTypeConvert(robj *setobj, int enc) {
    setTypeIterator *si;
    ((setobj->type == 2 && setobj->encoding == 6)?(void)0 : (_redisAssert("setobj->type == REDIS_SET && setobj->encoding == REDIS_ENCODING_INTSET",
 "t_set.c"
# 192 "t_set.c"
    ,
 193
# 192 "t_set.c"
    ...
    if (enc == 2) {
        ...
        while (setTypeNext(si,((void *)0),&intele) != -1) {
            ...
            ((dictAdd(d,element,((void *)0)) == 0)?(void)0 : (_redisAssert("dictAdd(d,element,NULL) == DICT_OK","t_set.c",207),_exit(1)));
        }
        ...
        _redisPanic("\"Unsupported set conversion\"","t_set.c",215),_exit(1);
    }
}
...
void spopCommand(redisClient *c) {
    ...
    ((sdslen(c->argv[0]->ptr) == 4)?(void)0 : (_redisAssert("sdslen(c->argv[0]->ptr) == 4","t_set.c",356),_exit(1)));
