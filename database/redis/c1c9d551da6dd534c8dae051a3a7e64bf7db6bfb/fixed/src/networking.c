...
robj *dupLastObjectIfNeeded(list *reply) {
    robj *new, *cur;
    listNode *ln;
    ((((reply)->len) > 0)?(void)0 : (_redisAssert("listLength(reply) > 0","networking.c",76),_exit(1)));
    ln = ((reply)->tail);
    cur = ((ln)->value);
    if (cur->refcount > 1) {
...
void freeClient(redisClient *c) {
    ...
    ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",481),_exit(1)));
    listDelNode(server.clients,ln);


    if (c->flags & 256) {
        ln = listSearchKey(server.unblocked_clients,c);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",487),_exit(1)));
        listDelNode(server.unblocked_clients,ln);
    }


    if (c->flags & 32) {
        ((server.ds_enabled)?(void)0 : (_redisAssert("server.ds_enabled","networking.c",493),_exit(1)));
        if (((c->io_keys)->len) == 0) {
            ln = listSearchKey(server.io_ready_clients,c);



            ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",499),_exit(1)));
    ...
    if (c->flags & 1) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","networking.c",517),_exit(1)));
        listDelNode(l,ln);
    }
...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_redisAssert("c->argc == 0","networking.c",709),_exit(1)));
        ...
        ((c->querybuf[0] == '*')?(void)0 : (_redisAssert("c->querybuf[0] == '*'","networking.c",722),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssert("c->multibulklen > 0","networking.c",743),_exit(1)));
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->flags & 16 || c->flags & 32) return;
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",821),_exit(1);
        }
