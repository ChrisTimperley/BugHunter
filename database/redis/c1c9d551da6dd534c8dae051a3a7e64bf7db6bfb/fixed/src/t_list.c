...
void listTypePush(robj *subject, robj *value, int where) {
    ...
    if (subject->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",37),_exit(1);
    }
}
...
unsigned long listTypeLength(robj *subject) {
    if (subject->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",84),_exit(1);
    }
}
...
listTypeIterator *listTypeInitIterator(robj *subject, int index, unsigned char direction) {
    ...
    if (li->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",99),_exit(1);
    }
    return li;
}
...
int listTypeNext(listTypeIterator *li, listTypeEntry *entry) {
    ...
    ((li->subject->encoding == li->encoding)?(void)0 : (_redisAssert("li->subject->encoding == li->encoding","t_list.c",114),_exit(1)));
    ...
    if (li->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",136),_exit(1);
    }
    return 0;
}
...
robj *listTypeGet(listTypeEntry *entry) {
    ...
    if (li->encoding == 5) {
        ...
        unsigned int vlen;
        long long vlong;
        ((entry->zi != ((void *)0))?(void)0 : (_redisAssert("entry->zi != NULL","t_list.c",149),_exit(1)));
        ...
        ((entry->ln != ((void *)0))?(void)0 : (_redisAssert("entry->ln != NULL","t_list.c",158),_exit(1)));
        value = ((entry->ln)->value);
        incrRefCount(value);
    } else {
        _redisPanic("\"Unknown list encoding\"","t_list.c",162),_exit(1);
    }
    return value;
}
...
void listTypeInsert(listTypeEntry *entry, robj *value, int where) {
    ...
    if (entry->li->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",193),_exit(1);
    }
}
...
int listTypeEqual(listTypeEntry *entry, robj *o) {
    listTypeIterator *li = entry->li;
    if (li->encoding == 5) {
        ((o->encoding == 0)?(void)0 : (_redisAssert("o->encoding == REDIS_ENCODING_RAW","t_list.c",201),_exit(1)));
        return ziplistCompare(entry->zi,o->ptr,sdslen(o->ptr));
    } else if (li->encoding == 4) {
        return equalStringObjects(o,((entry->ln)->value));
    } else {
        _redisPanic("\"Unknown list encoding\"","t_list.c",206),_exit(1);
    }
}
...
void listTypeDelete(listTypeEntry *entry) {
    ...
    if (li->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",231),_exit(1);
    }
}
...
void listTypeConvert(robj *subject, int enc) {
    listTypeIterator *li;
    listTypeEntry entry;
    ((subject->type == 1)?(void)0 : (_redisAssert("subject->type == REDIS_LIST","t_list.c",238),_exit(1)));
    ...
    if (enc == 4) {
        ...
        _redisPanic("\"Unsupported list conversion\"","t_list.c",253),_exit(1);
    }
}
...
void pushxGenericCommand(redisClient *c, robj *refval, robj *val, int where) {
    ...
    if (refval != ((void *)0)) {
        ...
        ((refval->encoding == 0)?(void)0 : (_redisAssert("refval->encoding == REDIS_ENCODING_RAW","t_list.c",313),_exit(1)));
...
void lindexCommand(redisClient *c) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",413),_exit(1);
    }
}
...
void lsetCommand(redisClient *c) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",451),_exit(1);
    }
}
...
void lrangeCommand(redisClient *c) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"List encoding is not LINKEDLIST nor ZIPLIST!\"","t_list.c",529),_exit(1);
    }
}
...
void ltrimCommand(redisClient *c) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","t_list.c",578),_exit(1);
    }
    if (listTypeLength(o) == 0) dbDelete(c->db,c->argv[1]);
    signalModifiedKey(c->db,c->argv[1]);
    server.dirty++;
...
void blockForKeys(redisClient *c, robj **keys, int numkeys, time_t timeout, robj *target) {
    ...
    for (j = 0; j < numkeys; j++) {
        ...
        if (de == ((void *)0)) {
            ...
            incrRefCount(keys[j]);
            ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","t_list.c",770),_exit(1)));
        } else {
            l = ((de)->val);
        }
        listAddNodeTail(l,c);
...
void unblockClientWaitingData(redisClient *c) {
    ...
    ((c->bpop.keys != ((void *)0))?(void)0 : (_redisAssert("c->bpop.keys != NULL","t_list.c",787),_exit(1)));

    for (j = 0; j < c->bpop.count; j++) {

        de = dictFind(c->db->blocking_keys,c->bpop.keys[j]);
        ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","t_list.c",792),_exit(1)));
        l = ((de)->val);
...
int handleClientsWaitingListPush(redisClient *c, robj *key, robj *ele) {
    ...
    clients = ((de)->val);
    numclients = ((clients)->len);
# 842 "t_list.c"
    while (numclients--) {
        ln = ((clients)->head);
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","t_list.c",844),_exit(1)));
        receiver = ln->value;
        ...
        if (dstkey == ((void *)0)) {
            ...
            if (!(dstobj && checkType(receiver,dstobj,1))) {
...
void brpoplpushCommand(redisClient *c) {
    ...
    if (key == ((void *)0)) {
        ...
        if (key->type != 1) {
            ...
            ((listTypeLength(key) > 0)?(void)0 : (_redisAssert("listTypeLength(key) > 0","t_list.c",988),_exit(1)));
            rpoplpushCommand(c);
        }
    }
}
