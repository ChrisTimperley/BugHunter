...
void setGenericCommand(redisClient *c, int nx, robj *key, robj *val, robj *expire, int unit) {
    ...
    if (lookupKeyWrite(c->db,key) != ((void *)0) && nx) {
...
void setCommand(redisClient *c) {
    ...
    setGenericCommand(c,0,c->argv[1],c->argv[2],((void *)0),0);
}

void setnxCommand(redisClient *c) {
    c->argv[2] = tryObjectEncoding(c->argv[2]);
    setGenericCommand(c,1,c->argv[1],c->argv[2],((void *)0),0);
}
...
static int getBitOffsetFromArgument(redisClient *c, robj *o, size_t *offset) {
    ...
    if (getLongLongFromObjectOrReply(c,o,&loffset,err) != 0)
        return -1;
...
void setbitCommand(redisClient *c) {
    ...
    if (getBitOffsetFromArgument(c,c->argv[2],&bitoffset) != 0)
        return;
...
void incrbyfloatCommand(redisClient *c) {
    ...
    value += incr;
    if ((sizeof (value) == sizeof (float) ? __isnanf (value) : sizeof (value) == sizeof (double) ? __isnan (value) : __isnanl (value)) || (sizeof (value) == sizeof (float) ? __isinff (value) : sizeof (value) == sizeof (double) ? __isinf (value) : __isinfl (value))) {
