...
void hashTypeTryConversion(robj *subject, robj **argv, int start, int end) {
    int i;
    if (subject->encoding != 3) return;

    for (i = start; i <= end; i++) {
        if (argv[i]->encoding == 0 &&
...
int hashTypeDelete(robj *o, robj *key) {
    ...
    if (o->encoding == 3) {
        ...
        deleted = dictDelete((dict*)o->ptr,key) == 0;
...
unsigned long hashTypeLength(robj *o) {
    return (o->encoding == 3) ?
        zipmapLen((unsigned char*)o->ptr) : (((dict*)o->ptr)->ht[0].used+((dict*)o->ptr)->ht[1].used);
}
...
hashTypeIterator *hashTypeInitIterator(robj *subject) {
    ...
    if (hi->encoding == 3) {
        ...
        ((0)?(void)0 : (_redisAssertWithInfo(((void *)0),subject,"0","t_hash.c",163),_exit(1)));
    }
    return hi;
}
...
int hashTypeNext(hashTypeIterator *hi) {
    if (hi->encoding == 3) {
        if ((hi->zi = zipmapNext(hi->zi, &hi->zk, &hi->zklen,
            &hi->zv, &hi->zvlen)) == ((void *)0)) return -1;
    } else {
...
void convertToRealHash(robj *o) {
    ...
    dict *dict = dictCreate(&hashDictType,((void *)0));

    ((o->type == 4 && o->encoding != 2)?(void)0 : (_redisAssertWithInfo(((void *)0),o,"o->type == REDIS_HASH && o->encoding != REDIS_ENCODING_HT","t_hash.c",253),_exit(1)));
    p = zipmapRewind(zm);
...
void hincrbyCommand(redisClient *c) {
    ...
    if ((o = hashTypeLookupWriteOrCreate(c,c->argv[1])) == ((void *)0)) return;
    if ((current = hashTypeGetObject(o,c->argv[2])) != ((void *)0)) {
        if (getLongLongFromObjectOrReply(c,current,&value,
            "hash value is not an integer") != 0) {
    ...
    oldvalue = value;
    if ((incr < 0 && oldvalue < 0 && incr < ((-9223372036854775807LL -1)-oldvalue)) ||
    ...
    hashTypeTryObjectEncoding(o,&c->argv[2],((void *)0));
...
void hincrbyfloatCommand(redisClient *c) {
    ...
    if ((current = hashTypeGetObject(o,c->argv[2])) != ((void *)0)) {
        if (getLongDoubleFromObjectOrReply(c,current,&value,
            "hash value is not a valid float") != 0) {
            decrRefCount(current);
            return;
        }
        decrRefCount(current);
