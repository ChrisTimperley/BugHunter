    ...
    if (de) {
        ...
        if (server.ds_enabled && cacheKeyMayExist(db,key)) {
                 ...
                 (2|8))
            {
                ...
                waitEmptyIOJobsQueue();
                processAllPendingIOJobs();
                (((cacheScheduleIOGetFlags(db,key) & (2|8)) == 0)?(void)0 : (_redisAssert("(cacheScheduleIOGetFlags(db,key) & (REDIS_IO_SAVE|REDIS_IO_SAVEINPROG)) == 0","db.c",46),_exit(1)));
            }
            ...
            if (val) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","db.c",54),_exit(1)));
