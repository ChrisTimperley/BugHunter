...
# 42 "db.c"
        if (server.ds_enabled && cacheKeyMayExist(db,key)) {
            ...
            if (val) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","db.c",48),_exit(1)));
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",453),_exit(1)));
...
void setExpire(redisDb *db, robj *key, time_t when) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","db.c",462),_exit(1)));
...
time_t getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",477),_exit(1)));
