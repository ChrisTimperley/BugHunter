...
struct redisCommand redisCommandTable[] = {
    ...
    {"eval",evalCommand,-3,4,zunionInterGetKeys,0,0,0,0,0},
    {"evalsha",evalShaCommand,-3,4,zunionInterGetKeys,0,0,0,0,0},
    {"slowlog",slowlogCommand,-2,0,((void *)0),0,0,0,0,0}
};
...
void beforeSleep(struct aeEventLoop *eventLoop) {
    ...
    while (((server.unblocked_clients)->len)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","redis.c",725),_exit(1)));
        c = ln->value;
...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
        ...
        ((retval == 0) ? (void) (0) : __assert_fail ("retval == 0", "redis.c", 982, __PRETTY_FUNCTION__));
    }
}
...
int processCommand(redisClient *c) {
    ...
    if (server.maxmemory && (c->cmd->flags & 4) &&
...
int prepareForShutdown() {
    ...
    if (server.appendonly) {
        ...
        if (server.bgrewritechildpid != -1) {
            ...
            kill(server.bgrewritechildpid,9);
        }
    ...
    redisLog(3,"Redis is now ready to exit, bye bye...");
    return 0;
}
...
sds genRedisInfoString(char *section) {
    ...
    if (allsections || defsections || !strcasecmp(section,"replication")) {
        ...
        if (server.masterhost) {
            ...
            if (server.replstate != 4) {
                info = sdscatprintf(info,
                    "master_link_down_since_seconds:%ld\r\n",
                    (long)time(((void *)0))-server.repl_down_since);
            }
        }
        info = sdscatprintf(info,
            "connected_slaves:%d\r\n",
