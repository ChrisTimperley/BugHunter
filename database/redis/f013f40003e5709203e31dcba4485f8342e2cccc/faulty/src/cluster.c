...
void migrateCommand(redisClient *c) {
    ...
    ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",1552),_exit(1)));
    ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",1553),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,dbid)","cluster.c",1554),_exit(1)));

    ttl = getExpire(c->db,c->argv[3]);
    ((rioWriteBulkCount(&cmd,'*',4))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',4)","cluster.c",1557),_exit(1)));
    ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",1558),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argv[3]->encoding == REDIS_ENCODING_RAW","cluster.c",1559),_exit(1)));
    ((rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr))","cluster.c",1560),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,(ttl == -1) ? 0 : ttl)","cluster.c",1561),_exit(1)));



    rioInitWithBuffer(&payload,sdsempty());
    ((rdbSaveObjectType(&payload,o))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObjectType(&payload,o)","cluster.c",1566),_exit(1)));
    ((rdbSaveObject(&payload,o) != -1)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObject(&payload,o) != -1","cluster.c",1567),_exit(1)));
    ((rioWriteBulkString(&cmd,payload.io.buffer.ptr,sdslen(payload.io.buffer.ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,payload.io.buffer.ptr,sdslen(payload.io.buffer.ptr))","cluster.c",1568),_exit(1)));
...
void dumpCommand(redisClient *c) {
    ...
    rioInitWithBuffer(&payload,sdsempty());
    ((rdbSaveObjectType(&payload,o))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObjectType(&payload,o)","cluster.c",1652),_exit(1)));
    ((rdbSaveObject(&payload,o))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rdbSaveObject(&payload,o)","cluster.c",1653),_exit(1)));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,firstkey,"n != NULL","cluster.c",1736),_exit(1)));
            } else {
