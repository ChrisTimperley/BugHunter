...
unsigned long aofRewriteBufferSize(void) {
    ...
    if (block == ((void *)0)) return 0;
    unsigned long size =
        (((server.aof_rewrite_buf_blocks)->len)-1) * (1024*1024*10);
    size += block->used;
    return size;
}
...
void stopAppendOnly(void) {
    ((server.aof_state != 0)?(void)0 : (_redisAssert("server.aof_state != REDIS_AOF_OFF","aof.c",166),_exit(1)));
...
int startAppendOnly(void) {
    ...
    ((server.aof_state == 0)?(void)0 : (_redisAssert("server.aof_state == REDIS_AOF_OFF","aof.c",195),_exit(1)));
    if (server.aof_fd == -1) {
...
void flushAppendOnlyFile(int force) {
    ...
    if (nwritten != (signed)sdslen(server.aof_buf)) {
        ...
        if (nwritten == -1) {
            redisLog(3,"Exiting on error writing to the append-only file: %s",strerror((*__errno_location ())));
        } else {
            redisLog(3,"Exiting on short write while writing to "
...
int loadAppendOnlyFile(char *filename) {
    ...
    while(1) {
        ...
        for (j = 0; j < argc; j++) {
            if (fgets(buf,sizeof(buf),fp) == ((void *)0)) goto readerr;
            if (buf[0] != '$') goto fmterr;
        ...
        ((fakeClient->bufpos == 0 && ((fakeClient->reply)->len) == 0)?(void)0 : (_redisAssert("fakeClient->bufpos == 0 && listLength(fakeClient->reply) == 0","aof.c",548),_exit(1)));

        (((fakeClient->flags & (1<<4)) == 0)?(void)0 : (_redisAssert("(fakeClient->flags & REDIS_BLOCKED) == 0","aof.c",550),_exit(1)));
    ...
    if (fakeClient->flags & (1<<3)) goto readerr;
...
int rioWriteBulkObject(rio *r, robj *obj) {
    ...
    } else if (obj->encoding == 0) {
        return rioWriteBulkString(r,obj->ptr,sdslen(obj->ptr));
    } else {
        _redisPanic("\"Unknown string encoding\"","aof.c",597),_exit(1);
    }
}
...
int rewriteListObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 5) {
        ...
        _redisPanic("\"Unknown list encoding\"","aof.c",653),_exit(1);
    }
    return 1;
}
...
int rewriteSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 6) {
        ...
        _redisPanic("\"Unknown set encoding\"","aof.c",700),_exit(1);
    }
    return 1;
}
...
int rewriteSortedSetObject(rio *r, robj *key, robj *o) {
    ...
    if (o->encoding == 5) {
        ...
        ((eptr != ((void *)0))?(void)0 : (_redisAssert("eptr != NULL","aof.c",719),_exit(1)));
        sptr = ziplistNext(zl,eptr);
        ((sptr != ((void *)0))?(void)0 : (_redisAssert("sptr != NULL","aof.c",721),_exit(1)));

        while (eptr != ((void *)0)) {
            ((ziplistGet(eptr,&vstr,&vlen,&vll))?(void)0 : (_redisAssert("ziplistGet(eptr,&vstr,&vlen,&vll)","aof.c",724),_exit(1)));
        ...
        _redisPanic("\"Unknown sorted zset encoding\"","aof.c",769),_exit(1);
    }
    return 1;
}
...
static int rioWriteHashIteratorCursor(rio *r, hashTypeIterator *hi, int what) {
    ...
    _redisPanic("\"Unknown hash encoding\"","aof.c",800),_exit(1);
    return 0;
}
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (o->type == 0) {
                ...
                _redisPanic("\"Unknown object type\"","aof.c",907),_exit(1);
            }
