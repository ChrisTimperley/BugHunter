...
void loadServerConfigFromString(char *config) {
    ...
    for (i = 0; i < totlines; i++) {
                   ...
                   argc == 2)
        {
            ...
            ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","config.c",371),_exit(1)));
...
void configSetCommand(redisClient *c) {
    robj *o;
    long long ll;
    ((c->argv[2]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,c->argv[2],"c->argv[2]->encoding == REDIS_ENCODING_RAW","config.c",511),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,c->argv[2],"c->argv[3]->encoding == REDIS_ENCODING_RAW","config.c",512),_exit(1)));
    o = c->argv[3];
...
void configGetCommand(redisClient *c) {
    ...
    char buf[128];
    int matches = 0;
    ((o->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,o,"o->encoding == REDIS_ENCODING_RAW","config.c",827),_exit(1)));
    ...
    do { if (stringmatch(pattern,"masterauth",0)) { addReplyBulkCString(c,"masterauth"); addReplyBulkCString(c,server.requirepass ? server.requirepass : ""); matches++; } } while(0);;
