...
# 72 "cluster.c"
        ...
        *p = '\0';
        ...
        while(p) {
            ...
            if (!strcasecmp(s,"myself")) {
                ((server.cluster->myself == ((void *)0))?(void)0 : (_redisAssert("server.cluster->myself == NULL","cluster.c",100),_exit(1)));
                ...
                _redisPanic("\"Unknown flag in redis cluster config file\"","cluster.c",119),_exit(1);
            }
            if (p) s = p+1;
        }
        ...
        for (j = 7; j < argc; j++) {
            ...
            if (argv[j][0] == '[') {
                ...
                ((p != ((void *)0))?(void)0 : (_redisAssert("p != NULL","cluster.c",151),_exit(1)));
                *p = '\0';
                direction = p[1];
                slot = atoi(argv[j]+1);
                p += 3;
    ...
    ((server.cluster->myself != ((void *)0))?(void)0 : (_redisAssert("server.cluster->myself != NULL","cluster.c",183),_exit(1)));
...
void clusterInit(void) {
    ...
    if (aeCreateFileEvent(server.el, server.cfd, 1,
        clusterAcceptHandler, ((void *)0)) == -1) _redisPanic("\"Unrecoverable error creating Redis Cluster file event.\"","cluster.c",258),_exit(1);
...
void freeClusterNode(clusterNode *n) {
    ...
    ((dictDelete(server.cluster->nodes,nodename) == 0)?(void)0 : (_redisAssert("dictDelete(server.cluster->nodes,nodename) == DICT_OK","cluster.c",489),_exit(1)));
    sdsfree(nodename);
...
void clusterRenameNode(clusterNode *node, char *newname) {
    ...
    sdsfree(s);
    ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","cluster.c",564),_exit(1)));
...
void clearNodeFailureIfNeeded(clusterNode *node) {
    ...
    ((node->flags & 8)?(void)0 : (_redisAssert("node->flags & REDIS_NODE_FAIL","cluster.c",620),_exit(1)));
...
void nodeIp2String(char *buf, clusterLink *link) {
    ...
    if (getpeername(link->fd, (struct sockaddr*) &sa, &salen) == -1)
        _redisPanic("\"getpeername() failed.\"","cluster.c",744),_exit(1);
...
int clusterDelSlot(int slot) {
    ...
    if (!n) return -1;
    ((clusterNodeClearSlotBit(n,slot) == 1)?(void)0 : (_redisAssert("clusterNodeClearSlotBit(n,slot) == 1","cluster.c",1588),_exit(1)));
...
void clusterSetMaster(clusterNode *n) {
    ...
    ((n != myself)?(void)0 : (_redisAssert("n != myself","cluster.c",1728),_exit(1)));
        ...
        for (j = 0; j < 16384; j++) {
            if (slots[j]) {
                ...
                retval = del ? clusterDelSlot(j) :
                               clusterAddSlot(server.cluster->myself,j);
                ((retval == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"retval == REDIS_OK","cluster.c",1917),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
...
void createDumpPayload(rio *payload, robj *o) {
    ...
    ((rdbSaveObjectType(payload,o))?(void)0 : (_redisAssert("rdbSaveObjectType(payload,o)","cluster.c",2143),_exit(1)));
    ((rdbSaveObject(payload,o))?(void)0 : (_redisAssert("rdbSaveObject(payload,o)","cluster.c",2144),_exit(1)));
# 2154 "cluster.c"
    buf[0] = 6 & 0xff;
    buf[1] = (6 >> 8) & 0xff;
...
try_again:
    ...
    ((rioWriteBulkCount(&cmd,'*',2))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',2)","cluster.c",2430),_exit(1)));
    ((rioWriteBulkString(&cmd,"SELECT",6))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"SELECT\",6)","cluster.c",2431),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,dbid))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,dbid)","cluster.c",2432),_exit(1)));
    ...
    ((rioWriteBulkCount(&cmd,'*',replace ? 5 : 4))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkCount(&cmd,'*',replace ? 5 : 4)","cluster.c",2439),_exit(1)));
...
# 2441 "cluster.c"
        ...
        ((rioWriteBulkString(&cmd,"RESTORE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"RESTORE\",7)","cluster.c",2444),_exit(1)));
    ((c->argv[3]->encoding == 0)?(void)0 : (_redisAssertWithInfo(c,((void *)0),"c->argv[3]->encoding == REDIS_ENCODING_RAW","cluster.c",2445),_exit(1)));
    ((rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr)))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,c->argv[3]->ptr,sdslen(c->argv[3]->ptr))","cluster.c",2446),_exit(1)));
    ((rioWriteBulkLongLong(&cmd,ttl))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkLongLong(&cmd,ttl)","cluster.c",2447),_exit(1)));
...
# 2452 "cluster.c"
    ...
    if (replace)
        ((rioWriteBulkString(&cmd,"REPLACE",7))?(void)0 : (_redisAssertWithInfo(c,((void *)0),"rioWriteBulkString(&cmd,\"REPLACE\",7)","cluster.c",2459),_exit(1)));
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            if (firstkey == ((void *)0)) {
                ...
                ((n != ((void *)0))?(void)0 : (_redisAssertWithInfo(c,firstkey,"n != NULL","cluster.c",2601),_exit(1)));
            } else {
