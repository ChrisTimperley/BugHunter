...
unsigned long setTypeRandomElements(robj *set, unsigned long count, robj *aux_set) {
    ...
    if (set->encoding == 2) {
        ...
        ((elements_copied == elements_to_return)?(void)0 : (_redisAssert("elements_copied == elements_to_return","t_set.c",258),_exit(1)));
        ...
        for (current_element = 0;
            ...
            setTypeAdd(aux_set,
                dupStringObject(((random_elements[current_element])->key)));
        }
        ...
        ((elements_copied == elements_to_return)?(void)0 : (_redisAssert("elements_copied == elements_to_return","t_set.c",281),_exit(1)));
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",296),_exit(1);
    }
...
unsigned long setTypeSize(robj *subject) {
    if (subject->encoding == 2) {
        ...
        _redisPanic("\"Unknown set encoding\"","t_set.c",310),_exit(1);
    }
}
...
# 334 "t_set.c"
        ...
        _redisPanic("\"Unsupported set conversion\"","t_set.c",343),_exit(1);
    }
}
...
void spopWithCountCommand(redisClient *c) {
    ...
    ((elements_returned == count)?(void)0 : (_redisAssert("elements_returned == count","t_set.c",548),_exit(1)));

    {
        ...
        while ((element_encoding = setTypeNext(si, &objele, &llele)) != -1) {
            if (element_encoding == 2) {
                ...
                _redisPanic("\"Unknown set encoding\"","t_set.c",584),_exit(1);
            }
        }
        setTypeReleaseIterator(si);
...
# 723 "t_set.c"
    if (count*3 > size) {
        ...
        while((encoding = setTypeNext(si,&ele,&llele)) != -1) {
            ...
            ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","t_set.c",736),_exit(1)));
        }
        setTypeReleaseIterator(si);
        ((((d)->ht[0].used+(d)->ht[1].used) == size)?(void)0 : (_redisAssert("dictSize(d) == size","t_set.c",739),_exit(1)));
