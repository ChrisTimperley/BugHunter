...
void pfaddCommand(redisClient *c) {
    ...
    if (o == ((void *)0)) {
        ...
        o = createObject(0,sdsnewlen(((void *)0),(((1<<14)*6 +7)/8)+8));
        ...
        if (stringObjectLen(o) != (((1<<14)*6 +7)/8)+8) {
    ...
    if (updated) {
        ...
        notifyKeyspaceEvent((1<<3),"hlladd",c->argv[1],c->db->id);
        server.dirty++;

        registers[(((1<<14)*6 +7)/8)+8 -1] |= (1<<7);
    }
...
void pfcountCommand(redisClient *c) {
    ...
    if (o == ((void *)0)) {
        ...
        registers = o->ptr;
        if ((registers[(((1<<14)*6 +7)/8)+8 -1] & (1<<7)) == 0) {

            card = (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -8];
            card |= (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -7] << 8;
            card |= (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -6] << 16;
            card |= (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -5] << 24;
            card |= (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -4] << 32;
            card |= (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -3] << 40;
            card |= (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -2] << 48;
            card |= (uint64_t)registers[(((1<<14)*6 +7)/8)+8 -1] << 56;
        } else {

            card = hllCount(registers);
            registers[(((1<<14)*6 +7)/8)+8 -8] = card & 0xff;
            registers[(((1<<14)*6 +7)/8)+8 -7] = (card >> 8) & 0xff;
            registers[(((1<<14)*6 +7)/8)+8 -6] = (card >> 16) & 0xff;
            registers[(((1<<14)*6 +7)/8)+8 -5] = (card >> 24) & 0xff;
            registers[(((1<<14)*6 +7)/8)+8 -4] = (card >> 32) & 0xff;
            registers[(((1<<14)*6 +7)/8)+8 -3] = (card >> 40) & 0xff;
            registers[(((1<<14)*6 +7)/8)+8 -2] = (card >> 48) & 0xff;
            registers[(((1<<14)*6 +7)/8)+8 -1] = (card >> 56) & 0xff;
...
void pfmergeCommand(redisClient *c) {
    ...
    for (j = 1; j < c->argc; j++) {
        ...
        registers = o->ptr;
        for (i = 0; i < (1<<14); i++) {
            do { uint8_t *_p = (uint8_t*) registers; unsigned long _byte = i*6/8; unsigned long _fb = i*6&7; unsigned long _fb8 = 8 - _fb; unsigned long b0 = _p[_byte]; unsigned long b1 = _p[_byte+1]; val = ((b0 >> _fb) | (b1 << _fb8)) & ((1<<6)-1); } while(0);
    ...
    registers = o->ptr;
    for (j = 0; j < (1<<14); j++) {
        do { uint8_t *_p = (uint8_t*) registers; unsigned long _byte = j*6/8; unsigned long _fb = j*6&7; unsigned long _fb8 = 8 - _fb; unsigned long _v = max[j]; _p[_byte] &= ~(((1<<6)-1) << _fb); _p[_byte] |= _v << _fb; _p[_byte+1] &= ~(((1<<6)-1) >> _fb8); _p[_byte+1] |= _v >> _fb8; } while(0);
    }
    registers[(((1<<14)*6 +7)/8)+8 -1] |= (1<<7);
...
void pfselftestCommand(redisClient *c) {
    int j, i;
    sds bitcounters = sdsnewlen(((void *)0),(((1<<14)*6 +7)/8)+8);
    ...
    for (j = 0; j < 1000; j++) {
        ...
        for (i = 0; i < (1<<14); i++) {
            ...
            bytecounters[i] = r;
            do { uint8_t *_p = (uint8_t*) bitcounters; unsigned long _byte = i*6/8; unsigned long _fb = i*6&7; unsigned long _fb8 = 8 - _fb; unsigned long _v = r; _p[_byte] &= ~(((1<<6)-1) << _fb); _p[_byte] |= _v << _fb; _p[_byte+1] &= ~(((1<<6)-1) >> _fb8); _p[_byte+1] |= _v >> _fb8; } while(0);
        }

        for (i = 0; i < (1<<14); i++) {
            unsigned int val;

            do { uint8_t *_p = (uint8_t*) bitcounters; unsigned long _byte = i*6/8; unsigned long _fb = i*6&7; unsigned long _fb8 = 8 - _fb; unsigned long b0 = _p[_byte]; unsigned long b1 = _p[_byte+1]; val = ((b0 >> _fb) | (b1 << _fb8)) & ((1<<6)-1); } while(0);
