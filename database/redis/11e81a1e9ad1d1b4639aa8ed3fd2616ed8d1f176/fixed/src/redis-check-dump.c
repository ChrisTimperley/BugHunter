...
# 466 "/usr/include/x86_64-linux-gnu/sys/stat.h" 3 4
extern __inline __attribute__ ((__gnu_inline__)) int
...
# 280 "/usr/include/x86_64-linux-gnu/sys/socket.h" 3 4
...
# 44 "redis-check-dump.c" 2
# 104 "redis-check-dump.c"
typedef struct {
    void *data;
    size_t size;
    size_t offset;
...
int processHeader() {
    ...
    if (memcmp(buf,"REDIS",5) != 0) {
        { printf("Wrong signature in header\n"); exit(1); };
    }

    dump_version = (int)strtol(buf + 5, ((void *)0), 10);
    if (dump_version < 1 || dump_version > 6) {
        { printf("Unknown RDB format version: %d\n", dump_version); exit(1); };
    }
    return dump_version;
}

int loadType(entry *e) {
    uint32_t offset = (positions[level].offset);
    ...
    if (readBytes(&t, 1)) {
        if (checkType(t)) {
            ...
            { sprintf(errors.error[errors.level], "Unknown type (0x%02x)", t); errors.offset[errors.level] = offset; errors.level++; };
        }
    } else {
        { sprintf(errors.error[errors.level], "Could not read type"); errors.offset[errors.level] = offset; errors.level++; };
    }
...
int processTime(int type) {
    uint32_t offset = (positions[level].offset);
    unsigned char t[8];
    int timelen = (type == 252) ? 8 : 4;

    if (readBytes(t,timelen)) {
        return 1;
    } else {
        { sprintf(errors.error[errors.level], "Could not read time"); errors.offset[errors.level] = offset; errors.level++; };
    }
...
uint32_t loadLength(int *isencoded) {
    ...
    if (!readBytes(buf, 1)) return (2147483647 * 2U + 1U);
    type = (buf[0] & 0xC0) >> 6;
    if (type == 0) {
        ...
        if (!readBytes(buf+1,1)) return (2147483647 * 2U + 1U);
        return ((buf[0] & 0x3F) << 8) | buf[1];
    } else {

        if (!readBytes(&len, 4)) return (2147483647 * 2U + 1U);
        return (unsigned int)__bswap_32 (len);
    }
}
...
char *loadIntegerObject(int enctype) {
    ...
    if (enctype == 0) {
        uint8_t v;
        if (!readBytes(enc, 1)) return ((void *)0);
        v = enc[0];
        val = (int8_t)v;
    } else if (enctype == 1) {
        uint16_t v;
        if (!readBytes(enc, 2)) return ((void *)0);
        v = enc[0]|(enc[1]<<8);
        val = (int16_t)v;
    } else if (enctype == 2) {
        uint32_t v;
        if (!readBytes(enc, 4)) return ((void *)0);
        v = enc[0]|(enc[1]<<8)|(enc[2]<<16)|(enc[3]<<24);
        val = (int32_t)v;
    } else {
        { sprintf(errors.error[errors.level], "Unknown integer encoding (0x%02x)", enctype); errors.offset[errors.level] = offset; errors.level++; };
        return ((void *)0);
    }
...
char* loadLzfStringObject() {
    ...
    if ((clen = loadLength(((void *)0))) == (2147483647 * 2U + 1U)) return ((void *)0);
    if ((slen = loadLength(((void *)0))) == (2147483647 * 2U + 1U)) return ((void *)0);
...
char* loadStringObject() {
    ...
    if (isencoded) {
        ...
        default:
            ...
            { sprintf(errors.error[errors.level], "Unknown string encoding (0x%02x)", len); errors.offset[errors.level] = offset; errors.level++; };
            return ((void *)0);
        }
    }
...
int processStringObject(char** store) {
    unsigned long offset = (positions[level].offset);
    char *key = loadStringObject();
    if (key == ((void *)0)) {
        { sprintf(errors.error[errors.level], "Error reading string object"); errors.offset[errors.level] = offset; errors.level++; };
        free(key);
        return 0;
    }
...
double* loadDoubleValue() {
    ...
    if (!readBytes(&len,1)) return ((void *)0);
...
int processDoubleValue(double** store) {
    ...
    if (val == ((void *)0)) {
        { sprintf(errors.error[errors.level], "Error reading double value"); errors.offset[errors.level] = offset; errors.level++; };
        free(val);
        return 0;
    }
...
int loadPair(entry *e) {
    ...
    if (processStringObject(&key)) {
        ...
        { sprintf(errors.error[errors.level], "Error reading entry key"); errors.offset[errors.level] = offset; errors.level++; };
        return 0;
    }
    ...
    if (e->type == 1 ||
        ...
        e->type == 3 ||
        e->type == 4) {
        if ((length = loadLength(((void *)0))) == (2147483647 * 2U + 1U)) {
            { sprintf(errors.error[errors.level], "Error reading %s length", types[e->type]); errors.offset[errors.level] = offset; errors.level++; };
            return 0;
        }
    }
    ...
    case 12:
    case 13:
        if (!processStringObject(((void *)0))) {
            { sprintf(errors.error[errors.level], "Error reading entry value"); errors.offset[errors.level] = offset; errors.level++; };
            return 0;
        }
    break;
    case 1:
    case 2:
        for (i = 0; i < length; i++) {
            offset = (positions[level].offset);
            if (!processStringObject(((void *)0))) {
                { sprintf(errors.error[errors.level], "Error reading element at index %d (length: %d)", i, length); errors.offset[errors.level] = offset; errors.level++; };
                return 0;
            }
        }
    break;
    case 3:
        for (i = 0; i < length; i++) {
            offset = (positions[level].offset);
            if (!processStringObject(((void *)0))) {
                { sprintf(errors.error[errors.level], "Error reading element key at index %d (length: %d)", i, length); errors.offset[errors.level] = offset; errors.level++; };
                return 0;
            }
            offset = (positions[level].offset);
            if (!processDoubleValue(((void *)0))) {
                { sprintf(errors.error[errors.level], "Error reading element value at index %d (length: %d)", i, length); errors.offset[errors.level] = offset; errors.level++; };
                return 0;
            }
        }
    break;
    ...
    break;
    default:
        { sprintf(errors.error[errors.level], "Type not implemented"); errors.offset[errors.level] = offset; errors.level++; };
        return 0;
    }
...
entry loadEntry() {
    ...
    offset[0] = (positions[level].offset);
    if (!loadType(&e)) {
        return e;
    }

    offset[1] = (positions[level].offset);
    if (e.type == 254) {
        if ((length = loadLength(((void *)0))) == (2147483647 * 2U + 1U)) {
            { sprintf(errors.error[errors.level], "Error reading database number"); errors.offset[errors.level] = offset[1]; errors.level++; };
            return e;
        }
        if (length > 63) {
            { sprintf(errors.error[errors.level], "Database number out of range (%d)", length); errors.offset[errors.level] = offset[1]; errors.level++; };
            return e;
        }
    } else if (e.type == 255) {
        if (positions[level].offset < positions[level].size) {
            { sprintf(errors.error[errors.level], "Unexpected EOF"); errors.offset[errors.level] = offset[0]; errors.level++; };
        } else {
            e.success = 1;
        }
        return e;
        ...
        offset[1] = (positions[level].offset);
        if (!loadPair(&e)) {
            { sprintf(errors.error[errors.level], "Error for type %s", types[e.type]); errors.offset[errors.level] = offset[1]; errors.level++; };
            return e;
        }
    }
    ...
    offset[2] = (positions[level].offset);
    if (peekType() == -1) {
        { sprintf(errors.error[errors.level], "Followed by invalid type"); errors.offset[errors.level] = offset[2]; errors.level++; };
        { sprintf(errors.error[errors.level], "Error for type %s", types[e.type]); errors.offset[errors.level] = offset[0]; errors.level++; };
        e.success = 0;
    } else {
        e.success = 1;
    }
...
void process() {
    ...
    if (entry.type != 255) {
        ...
        errors.level = 0;
        { sprintf(errors.error[errors.level], "Expected EOF, got %s", types[entry.type]); errors.offset[errors.level] = positions[0].offset; errors.level++; };
    ...
    if (dump_version >= 5) {
        ...
        crc2 = ((uint64_t)p[0] << 0) |
               ...
               ((uint64_t)p[7] << 56);
        if (crc != crc2) {
            { sprintf(errors.error[errors.level], "RDB CRC64 does not match."); errors.offset[errors.level] = positions[0].offset; errors.level++; };
        } else {
...
int main(int argc, char **argv) {
    ...
    if (sizeof(size_t) == sizeof(int32_t) && size >= 2147483647) {
        { printf("Cannot check dump files >2GB on a 32-bit platform\n"); exit(1); };
    }

    data = mmap(((void *)0), size, 0x1, 0x01, fd, 0);
    if (data == ((void *) -1)) {
    ...
    sprintf(types[0], "STRING");
    sprintf(types[1], "LIST");
    sprintf(types[2], "SET");
    sprintf(types[3], "ZSET");
    sprintf(types[4], "HASH");


    sprintf(types[253], "EXPIRETIME");
    sprintf(types[254], "SELECTDB");
    sprintf(types[255], "EOF");
