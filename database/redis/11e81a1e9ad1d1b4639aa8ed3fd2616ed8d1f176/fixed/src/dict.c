...
int dictRehash(dict *d, int n) {
    ...
    while(n--) {
        ...
        ((d->ht[0].size > (unsigned)d->rehashidx)?(void)0 : (_redisAssert("d->ht[0].size > (unsigned)d->rehashidx","dict.c",260),_exit(1)));
...
void dictReleaseIterator(dictIterator *iter)
{
    if (!(iter->index == -1 && iter->table == 0)) {
        if (iter->safe)
            ...
            ((iter->fingerprint == dictFingerprint(iter->d))?(void)0 : (_redisAssert("iter->fingerprint == dictFingerprint(iter->d)","dict.c",607),_exit(1)));
    }
    zfree(iter);
}
