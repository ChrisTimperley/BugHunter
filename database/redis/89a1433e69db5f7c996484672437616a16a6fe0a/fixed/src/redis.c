...
void beforeSleep(struct aeEventLoop *eventLoop) {
    ...
    if (server.ds_enabled && ((server.io_ready_clients)->len)) {
        ...
        while((ln = listNext(&li))) {
            ...
            ((cmd != ((void *)0))?(void)0 : (_redisAssert("cmd != NULL","redis.c",677),_exit(1)));
            call(c,cmd);
            resetClient(c);
    ...
    while (((server.unblocked_clients)->len)) {
        ...
        ((ln != ((void *)0))?(void)0 : (_redisAssert("ln != NULL","redis.c",689),_exit(1)));
        c = ln->value;
...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
        ...
        ((retval == 0) ? (void) (0) : __assert_fail ("retval == 0", "redis.c", 935, __PRETTY_FUNCTION__));
    }
}
