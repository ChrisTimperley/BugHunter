...
void clusterGetRandomName(char *p) {
    ...
    if (!fp) {
        redisLog(3,
            "Unrecovarable error: can't open /dev/urandom:%s" ,strerror((*__errno_location ())));
        exit(1);
    }
    fread(p,40,1,fp);
    for (j = 0; j < 40; j++)
...
int clusterLoadConfig(char *filename) {
        ...
        *p = '\0';
        ...
        while(p) {
            ...
            if (!strcasecmp(s,"myself")) {
                ((server.cluster.myself == ((void *)0))?(void)0 : (_redisAssert("server.cluster.myself == NULL","cluster.c",75),_exit(1)));
                ...
                _redisPanic("\"Unknown flag in redis cluster config file\"","cluster.c",93),_exit(1);
            }
            if (p) s = p+1;
        }
    ...
    ((server.cluster.myself != ((void *)0))?(void)0 : (_redisAssert("server.cluster.myself != NULL","cluster.c",134),_exit(1)));
...
void freeClusterNode(clusterNode *n) {
    ...
    ((dictDelete(server.cluster.nodes,nodename) == 0)?(void)0 : (_redisAssert("dictDelete(server.cluster.nodes,nodename) == DICT_OK","cluster.c",333),_exit(1)));
    sdsfree(nodename);
...
void clusterRenameNode(clusterNode *node, char *newname) {
    ...
    sdsfree(s);
    ((retval == 0)?(void)0 : (_redisAssert("retval == DICT_OK","cluster.c",372),_exit(1)));
...
void nodeIp2String(char *buf, clusterLink *link) {
    ...
    if (getpeername(link->fd, (struct sockaddr*) &sa, &salen) == -1)
        _redisPanic("\"getpeername() failed.\"","cluster.c",465),_exit(1);
...
int clusterAddSlot(clusterNode *n, int slot) {
    ((clusterNodeSetSlotBit(n,slot) == 0)?(void)0 : (_redisAssert("clusterNodeSetSlotBit(n,slot) == 0","cluster.c",992),_exit(1)));
    ...
    if (!strcasecmp(c->argv[1]->ptr,"meet") && c->argc == 4) {
        ...
        for (j = 0; j < 4096; j++) {
            if (slots[j]) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","cluster.c",1161),_exit(1)));
            }
        }
        zfree(slots);
        clusterUpdateState();
...
clusterNode *getNodeByQuery(redisClient *c, struct redisCommand *cmd, robj **argv, int argc, int *hashslot) {
    ...
    for (i = 0; i < ms->count; i++) {
        ...
        for (j = 0; j < numkeys; j++) {
            ...
            if (slotnode == ((void *)0) || (n && slotnode != n)) {
    ...
    return (n == ((void *)0)) ? server.cluster.myself : n;
}
