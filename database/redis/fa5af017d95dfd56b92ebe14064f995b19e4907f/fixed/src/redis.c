...
void populateCommandTable(void) {
    ...
    for (j = 0; j < numcommands; j++) {
        ...
        while(*f != '\0') {
            ...
            default: _redisPanic("\"Unsupported command flag\"","redis.c",1103),_exit(1); break;
            }
            f++;
        }

        retval = dictAdd(server.commands, sdsnew(c->name), c);
        ((retval == 0) ? (void) (0) : __assert_fail ("retval == 0", "redis.c", 1109, __PRETTY_FUNCTION__));
    }
}
...
static void sigsegvHandler(int sig, siginfo_t *info, void *secret) {
    ...
    bugReportStart();
    redisLog(3,
        "    Redis %s crashed by signal: %d", "2.9.0", sig);
    redisLog(3,
    ...
    redisLog(3, "--- STACK TRACE");
    for (i=1; i<trace_size; ++i)
        redisLog(3,"%s", messages[i]);


    redisLog(3, "--- INFO OUTPUT");
    infostring = genRedisInfoString("all");
    redisLogRaw(3, infostring);
    redisLog(3, "--- CLIENT LIST OUTPUT");
    clients = getAllClientsInfoString();
    redisLogRaw(3, clients);
