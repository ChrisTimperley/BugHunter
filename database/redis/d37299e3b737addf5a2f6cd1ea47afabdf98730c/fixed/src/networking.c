...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->querybuf[0] == '*')?(void)0 : (_redisAssert("c->querybuf[0] == '*'","networking.c",718),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssert("c->multibulklen > 0","networking.c",738),_exit(1)));
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",816),_exit(1);
        }
