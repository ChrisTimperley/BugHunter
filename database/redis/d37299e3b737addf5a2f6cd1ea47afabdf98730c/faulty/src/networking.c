...
int processMultibulkBuffer(redisClient *c) {
    ...
    if (c->multibulklen == 0) {
        ...
        ((c->argc == 0)?(void)0 : (_redisAssert("c->argc == 0","networking.c",703),_exit(1)));
        ...
        ((c->querybuf[0] == '*')?(void)0 : (_redisAssert("c->querybuf[0] == '*'","networking.c",712),_exit(1)));
    ...
    ((c->multibulklen > 0)?(void)0 : (_redisAssert("c->multibulklen > 0","networking.c",732),_exit(1)));
    while(c->multibulklen) {
        ...
        if (c->bulklen == -1) {
            ...
            if (newline != ((void *)0)) {
                ...
                tolerr = (eptr[0] != '\r');
                if (tolerr || bulklen == (-9223372036854775807L - 1L) || bulklen == 9223372036854775807L ||
...
void processInputBuffer(redisClient *c) {
    ...
    while(sdslen(c->querybuf)) {
        ...
        if (c->reqtype == 1) {
            ...
            _redisPanic("\"Unknown request type\"","networking.c",810),_exit(1);
        }
