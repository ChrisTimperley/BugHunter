...
int dictExpand(dict *ht, unsigned int size)
{
    ...
    ((ht->used == 0) ? (void) (0) : __assert_fail ("ht->used == 0", "dict.c", 197, __PRETTY_FUNCTION__));
    _dictFree(ht->table);
...
void dictPrintStats(dict *ht) {
    ...
    for (i = 0; i < 50 -1; i++) {
        ...
        printf("   %s%d: %d (%.02f%%)\n",(i == 50 -1)?">= ":"", i, clvector[i], ((float)clvector[i]/ht->size)*100);
    }
}
