...
void hllSelftestCommand(redisClient *c) {
    int j, i;
    sds bitcounters = sdsnewlen(((void *)0),((16384*6 +7)/8));
    uint8_t bytecounters[16384];
    ...
    for (j = 0; j < 1000; j++) {
        ...
        for (i = 0; i < 16384; i++) {
            ...
            bytecounters[i] = r;
            do { uint8_t *_p = (uint8_t*) bitcounters; int _byte = i*6/8; int _leftshift = i*6&7; int _rightshift = 8 - _leftshift; uint8_t m1 = 255, m2 = ((1<<6)-1); _p[_byte] &= m1 << _rightshift; _p[_byte] |= r >> _leftshift; _p[_byte+1] &= ~(m2 << _rightshift); _p[_byte+1] |= r << _rightshift; } while(0);
        }

        for (i = 0; i < 16384; i++) {
            unsigned int val;

            do { uint8_t *_p = (uint8_t*) bitcounters; int _byte = i*6/8; int _leftshift = i*6&7; int _rightshift = 8 - _leftshift; val = ((_p[_byte] << _leftshift) | (_p[_byte+1] >> _rightshift)) & ((1<<6)-1); } while(0);
