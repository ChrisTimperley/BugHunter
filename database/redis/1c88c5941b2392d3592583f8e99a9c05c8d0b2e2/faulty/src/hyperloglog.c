...
void hllSelftestCommand(redisClient *c) {
    ...
    for (j = 0; j < 1000; j++) {
        ...
        for (i = 0; i < 16384; i++) {
            ...
            bytecounters[i] = r;
            do { int _byte = i*6/8; int _leftshift = i*6&7; int _rightshift = 8 - _leftshift; unsigned int m1 = 255, m2 = ((1<<6)-1); bitcounters[_byte] &= m1 << _rightshift; bitcounters[_byte] |= r >> _leftshift; bitcounters[_byte+1] &= ~(m2 << _rightshift); bitcounters[_byte+1] |= r << _rightshift; } while(0);
        }

        for (i = 0; i < 16384; i++) {
            unsigned int val;

            do { int _byte = i*6/8; int _leftshift = i*6&7; int _rightshift = 8 - _leftshift; val = ((bitcounters[_byte] << _leftshift) | (bitcounters[_byte+1] >> _rightshift)) & ((1<<6)-1); } while(0);
