...
void memtest_fill(unsigned long *l, size_t bytes) {
    ...
    (((bytes & 4095) == 0) ? (void) (0) : __assert_fail ("(bytes & 4095) == 0", "memtest.c", 61, __PRETTY_FUNCTION__));
...
void memtest_compare(unsigned long *l, size_t bytes) {
    ...
    (((bytes & 4095) == 0) ? (void) (0) : __assert_fail ("(bytes & 4095) == 0", "memtest.c", 87, __PRETTY_FUNCTION__));
    l1 = l;
    l2 = l1+words;
    for (w = 0; w < words; w++) {
        if (*l1 != *l2) {
