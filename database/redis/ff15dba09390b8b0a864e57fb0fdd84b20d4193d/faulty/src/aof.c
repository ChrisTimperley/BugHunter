...
void stopAppendOnly(void) {
    ...
    if (server.bgrewritechildpid != -1) {
        ...
        if (kill(server.bgrewritechildpid,9) != -1)
...
int startAppendOnly(void) {
    ...
    if (server.appendfd == -1) {
        redisLog(3,"Used tried to switch on AOF via CONFIG, but I can't open the AOF file: %s",strerror((*__errno_location ())));
        return -1;
    }
    if (rewriteAppendOnlyFileBackground() == -1) {
        server.appendonly = 0;
        close(server.appendfd);
        redisLog(3,"Used tried to switch on AOF via CONFIG, I can't trigger a background AOF rewrite operation. Check the above logs for more info about the error.",strerror((*__errno_location ())));
        return -1;
    }
    return 0;
}
...
void flushAppendOnlyFile(int force) {
    ...
    if (server.appendfsync == 2)
        sync_in_progress = bioPendingJobsOfType(1) != 0;

    if (server.appendfsync == 2 && !force) {
    ...
    if (nwritten != (signed)sdslen(server.aofbuf)) {
        ...
        if (nwritten == -1) {
            ...
            redisLog(3,"Exiting on short write while writing to the append-only file: %s",strerror((*__errno_location ())));
        }
        exit(1);
    }
    ...
    if (server.appendfsync == 1) {


        fdatasync(server.appendfd);
        server.lastfsync = server.unixtime;
    } else if ((server.appendfsync == 2 &&
...
sds catAppendOnlyExpireAtCommand(sds buf, robj *key, robj *seconds) {
    ...
    when = time(((void *)0))+strtol(seconds->ptr,((void *)0),10);
    decrRefCount(seconds);

    argv[0] = createStringObject("EXPIREAT",8);
    argv[1] = key;
    argv[2] = createObject(0,
...
int loadAppendOnlyFile(char *filename) {
    ...
    while(1) {
        ...
        ((fakeClient->bufpos == 0 && ((fakeClient->reply)->len) == 0)?(void)0 : (_redisAssert("fakeClient->bufpos == 0 && listLength(fakeClient->reply) == 0","aof.c",355),_exit(1)));

        (((fakeClient->flags & 16) == 0)?(void)0 : (_redisAssert("(fakeClient->flags & REDIS_BLOCKED) == 0","aof.c",357),_exit(1)));
    ...
    if (fakeClient->flags & 8) goto readerr;
...
int rioWriteBulkObject(rio *r, robj *obj) {
    ...
    if (obj->encoding == 1) {
        ...
        _redisPanic("\"Unknown string encoding\"","aof.c",400),_exit(1);
    }
}
...
int rewriteAppendOnlyFile(char *filename) {
    ...
    fp = fopen(tmpfile,"w");
    if (!fp) {
        redisLog(3, "Failed rewriting the append only file: %s", strerror((*__errno_location ())));
        return -1;
    }
    ...
    for (j = 0; j < server.dbnum; j++) {
        ...
        if (((&aof)->write((&aof),(selectcmd),(sizeof(selectcmd)-1))) == 0) goto werr;
        ...
        while((de = dictNext(di)) != ((void *)0)) {
            ...
            if (o->type == 0) {
                ...
                char cmd[]="*3\r\n$3\r\nSET\r\n";
                if (((&aof)->write((&aof),(cmd),(sizeof(cmd)-1))) == 0) goto werr;
                ...
                if (o->encoding == 5) {
                    ...
                    _redisPanic("\"Unknown list encoding\"","aof.c",496),_exit(1);
                }
            } else if (o->type == 2) {
                char cmd[]="*3\r\n$4\r\nSADD\r\n";
                ...
                if (o->encoding == 6) {
                    ...
                    _redisPanic("\"Unknown set encoding\"","aof.c",521),_exit(1);
                }
            } else if (o->type == 3) {
                ...
                if (o->encoding == 5) {
                    ...
                    ((eptr != ((void *)0))?(void)0 : (_redisAssert("eptr != NULL","aof.c",536),_exit(1)));
                    sptr = ziplistNext(zl,eptr);
                    ((sptr != ((void *)0))?(void)0 : (_redisAssert("sptr != NULL","aof.c",538),_exit(1)));

                    while (eptr != ((void *)0)) {
                        ((ziplistGet(eptr,&vstr,&vlen,&vll))?(void)0 : (_redisAssert("ziplistGet(eptr,&vstr,&vlen,&vll)","aof.c",541),_exit(1)));
                    ...
                    _redisPanic("\"Unknown sorted set encoding\"","aof.c",572),_exit(1);
                }
            } else if (o->type == 4) {
                char cmd[]="*4\r\n$4\r\nHSET\r\n";
                ...
                if (o->encoding == 3) {
                    ...
                    while((p = zipmapNext(p,&field,&flen,&val,&vlen)) != ((void *)0)) {
                        if (((&aof)->write((&aof),(cmd),(sizeof(cmd)-1))) == 0) goto werr;
                        if (rioWriteBulkObject(&aof,&key) == 0) goto werr;
                ...
                _redisPanic("\"Unknown object type\"","aof.c",607),_exit(1);
            }
...
int rewriteAppendOnlyFileBackground(void) {
    ...
    if (server.bgrewritechildpid != -1) return -1;
    start = ustime();
    if ((childpid = fork()) == 0) {
        char tmpfile[256];
...
void aofUpdateCurrentSize(void) {
    ...
    if (fstat(server.appendfd,&sb) == -1) {
        redisLog(3,"Unable to check the AOF length: %s",
            strerror((*__errno_location ())));
    } else {
    ...
    if (!bysignal && exitcode == 0) {
        ...
        if (nwritten != (signed)sdslen(server.bgrewritebuf)) {
            if (nwritten == -1) {
                ...
                redisLog(3,
                    "Short write trying to flush the parent diff to the rewritten AOF: %s", strerror((*__errno_location ())));
            }
            close(newfd);
            goto cleanup;
        }
...
# 799 "aof.c"
        if (server.appendfd == -1) {
             ...
             oldfd = open(server.appendfilename,00|04000);
        } else {
        ...
        if (rename(tmpfile,server.appendfilename) == -1) {
            redisLog(3,
                "Error trying to rename the temporary AOF: %s", strerror((*__errno_location ())));
            close(newfd);
        ...
        redisLog(2, "Background AOF rewrite successful");
