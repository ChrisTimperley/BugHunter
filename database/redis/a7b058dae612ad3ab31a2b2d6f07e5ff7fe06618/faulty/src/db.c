...
void lookupWaitBusyKey(redisDb *db, robj *key) {
    ...
    (((cacheScheduleIOGetFlags(db,key) & 8) == 0)?(void)0 : (_redisAssert("(cacheScheduleIOGetFlags(db,key) & REDIS_IO_SAVEINPROG) == 0","db.c",38),_exit(1)));
}
    ...
    if (de) {
        ...
        if (server.ds_enabled && cacheKeyMayExist(db,key)) {
            ...
            if (val) {
                ...
                ((retval == 0)?(void)0 : (_redisAssert("retval == REDIS_OK","db.c",90),_exit(1)));
...
int removeExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",494),_exit(1)));
...
void setExpire(redisDb *db, robj *key, time_t when) {
    ...
    ((de != ((void *)0))?(void)0 : (_redisAssert("de != NULL","db.c",503),_exit(1)));
...
time_t getExpire(redisDb *db, robj *key) {
    ...
    ((dictFind(db->dict,key->ptr) != ((void *)0))?(void)0 : (_redisAssert("dictFind(db->dict,key->ptr) != NULL","db.c",518),_exit(1)));
...
int *getKeysUsingCommandTable(struct redisCommand *cmd,robj **argv, int argc, int *numkeys) {
    ...
    for (j = cmd->firstkey; j <= last; j += cmd->keystep) {
        ((j < argc)?(void)0 : (_redisAssert("j < argc","db.c",655),_exit(1)));
        keys[i++] = j;
    }
    *numkeys = i;
    return keys;
