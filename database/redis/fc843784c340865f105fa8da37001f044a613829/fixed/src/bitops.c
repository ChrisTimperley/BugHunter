...
# 118 "bitops.c"
    ...
    skipval = bit ? 0 : (9223372036854775807L * 2UL + 1UL);
    l = (unsigned long*) c;
...
# 144 "bitops.c"
    ...
    one = (9223372036854775807L * 2UL + 1UL);
    one >>= 1;
    one = ~one;
    ...
    _serverPanic("\"End of redisBitpos() reached.\"","bitops.c",177),_exit(1);
    return 0;
}
# 202 "bitops.c"
...
void setSignedBitfield(unsigned char *p, uint64_t offset, uint64_t bits, int64_t value) {
    ...
    if (value >= 0)
        ...
        uv = (18446744073709551615UL) + value + 1;
    ...
    for (j = 0; j < numops; j++) {
            ...
            thisop->opcode == 2)
        {
            ...
            if ((o = lookupStringForBitCommand(c,
                thisop->offset + (thisop->bits-1))) == ((void *)0)) return;
            ...
            size_t olen = (o == ((void *)0)) ? 0 : sdslen(o->ptr);
            unsigned char buf[9];
            ...
            memset(buf,0,9);
            unsigned char *src = o ? o->ptr : ((void *)0);
            int i;
            size_t byte = thisop->offset >> 3;
            for (i = 0; i < 9; i++) {
                if (src == ((void *)0) || i+byte >= olen) break;
                buf[i] = src[i+byte];
            }
